<li class="clearfix" id="commonFriend_{{ID}}" onmouseover="Sizzle('a.shut-new',this)[0].style.display='block'" onmouseout="Sizzle('a.shut-new',this)[0].style.display='none'">
	...
	<span class="headpichold">
		<a href="{{APP_URL}}?origin=50121" target="_blank" onclick="logRcd({action:'RecMixed_imgToPrf',guest_id:{{ID}}, type:'{{TYPE_VALUE}}'})">
			<img src="{{TINY_URL}}" width="50" height="50" />
		</a>
	</span>
	<span>
		<a class="name" href="{{APP_URL}}?origin=50121" target="_blank" onclick="logRcd({action:'RecMixed_nameToPrf',guest_id:{{ID}}, type:'{{TYPE_VALUE}}'})">{{NAME}} <img src="http://a.xnimg.cn/imgpro/icons/rcdapp.png" /></a>
		<p>
			<a class="addfriend_action" href="{{APP_URL}}?origin=50122" onclick="logRcd({action:'RecMixed_addFriend',guest_id:{{ID}}, type:'{{TYPE_VALUE}}'});" target="_blank"><img src="http://a.xnimg.cn/imgpro/icons/plus-green.png" /> 添加应用</a>
		</p>
		<p style="color:#888;">
			{{#FIELDS}}
