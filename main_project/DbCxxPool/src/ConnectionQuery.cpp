...
//#endif
		...
		MCE_INFO("ConnectionQuery::doQuery -> " << sql << ", Count: " << rs->size());
		return rs;
	} catch (mysqlpp::Exception& e) {
		MCE_WARN("ConnectionQuery::doQuery -> " << sql << ", Exception: " << e.what());
		throw e;
	} catch (XceException& e) {
		MCE_WARN("ConnectionQuery::doQuery -> " << sql << ", Exception: " << e);
		throw e;
	} catch (Ice::Exception& e) {
		MCE_WARN("ConnectionQuery::doQuery -> " << sql << ", Exception: " << e);
		throw e;
	} catch (...) {
		MCE_WARN("ConnectionQuery::doQuery -> " << sql << ", UnknownException");
...
Ice::Long ConnectionQuery::doExecute(const string& sql) {
	try {
		MCE_INFO("ConnectionQuery::doExecute -> " << sql);
		mysqlpp::Query query = ConnectionHolder::query();
		query << sql;
//#ifndef NEWARCH
...
//#endif
		...
		MCE_WARN("ConnectionQuery::doExecute -> " << sql << ", Exception: " << e.what());
		throw e;
	} catch (Ice::Exception& e) {
		MCE_WARN("ConnectionQuery::doExecute -> " << sql << ", Exception: " << e);
		throw e;
	} catch (...) {
		MCE_WARN("ConnectionQuery::doExecute -> " << sql << ", UnknownException");
		throw "UnkownException";
	}
}
