...
//		MCE_WARN("ResultHandler::handle mysqlpp::Exception -> " << e.what() );
//	} catch (std::exception& e) {
//		MCE_WARN("ResultHandler::handle std::exception -> " << e.what() );
//	} catch (...) {
//		MCE_WARN("ResultHandler::handle -> Unknown exception ");
//	}
//	finish();
//	return count;
//}
...
//#else
		MCE_DEBUG("QueryRunner::query -> " + query.str());
		mysqlpp::UseQueryResult res = query.use();
//#endif
		if (!res) {
			return 0;
		}
		count = handler.handle(res);
		// Check for error: can't distinguish "end of results" and
		// error cases in return from fetch_row() otherwise.
		if (conn.errnum()) {
			MCE_WARN("ResultHandler::handle -> exception when fetching: " << conn.error());
			return 0;
		}
	} catch(const NewConnectFailException& e) {
		ConnectionPoolPtr pool = ConnectionPoolManager::instance().locate(_name);
		if (pool) {
			pool->tryToDelBadDb(e.server(), _pattern, e.readable());
		}
		throw e;
        }
...
//#endif
	...
	try
	{
		...
		statement.query(query);
		if (log.empty()) {
			MCE_INFO("QueryRunner::execute "<<__LINE__<<" -> " << query.str());
		} else {
			MCE_INFO("QueryRunner::execute -> " << log);
		}
...
void QueryHandler::handle() {
	...
	try {
		...
		if (_log.empty()) {
			MCE_INFO("QueryScheduler::run -> " << query.str());
		} else {
			MCE_INFO("QueryScheduler::run -> " << _log);
		}
		query.execute();
	} catch (const NewConnectFailException& e) {
		MCE_WARN("QueryScheduler::run exception -> " << e);
		ConnectionPoolPtr pool = ConnectionPoolManager::instance().locate(_name);
		if (pool) {
			pool->tryToDelBadDb(e.server(), _pattern, e.readable());
		}
		...
		MCE_WARN("QueryScheduler::run exception -> " << e.what());
		throw e;
	} catch (std::exception& e) {
		MCE_WARN("QueryScheduler::run exception -> " << e.what());
		throw e;
	} catch (...) {
		MCE_WARN("QueryScheduler::run exception ");
	}
}
