...
try:
    ...
    for i in range(0, 10):
        ...
        users.append( mop.hi.svc.model.UserConfigManagerPrx.checkedCast(ic.stringToProxy("UCM@UserConfig" + str(i))) )
    #uid=int(200353733)
    #users[uid % 10].reloadUser(200353733)
    f = file("preloadUserConfig.txt")
    while True:
        line = f.readline()
        if len(line) == 0:
            break
        ...
        for i in str:
            ...
            try:
                props=users[eid].getUserConfig(uid)
            except:
                print "excep"
