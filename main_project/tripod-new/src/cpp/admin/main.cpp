...
class Node {
public:
  Node(const string& host, int port) : hashid_(""), host_(host), port_(port) {}
  Node(const string& hashid, const string& host, int port) : hashid_(hashid), host_(host), port_(port) {}
  string toString() {
      if(hashid_ != "") {
          return hashid_ + ":" + host_ + ":" + lexical_cast<string>(port_);
      }else {
  ...
  bool operator<(const Node &m) const {
    ...
    if(hashid_ < m.hashid_) {
        return true;
    }else if(hashid_ > m.hashid_) {
        return false;
    }else {
        if (host_ < m.host_) {
            return true;
        } else if(host_ > m.host_) {
            return false;
        }else {
...
static void start(void *data, const char *el, const char **attr) {
  ...
  if ("node" == string(el) && "tripod" == xmlpath.at(0)) {
    ...
    if (xmlpath.at(xmlpath.size() - 2) == "namespace") {
      string hashid = attrmap["hashid"];
      xmlconfig.caches()[xmlnsname].insert(Node(hashid, host, port));
...
Node GetZooKeeperNode(const string& node) {
  ...
  if (parts.size() == 2) {
    ...
    return Node(hashid, parts[parts.size() - 2], lexical_cast<int>(parts[parts.size() - 1]));
  }
}
...
void GetZooKeeperConfig() {
  ...
  int rc = zoo_get_children(zk, "/Manager", 0, &managernodes);
  if (rc != ZOK) {
  ...
  rc = zoo_get_children(zk, "/Producer", 0, &producernodes);
  if (rc != ZOK) {
  ...
  rc = zoo_get_children(zk, "/Cache", 0, &namespaces);
  if (rc != ZOK) {
...
void Delete() {
  ...
  if(!managers.empty()) {
      for(set<Node>::iterator it = managers.begin(); it != managers.end(); ++it) {
          rc = zoo_delete(zk, ("/Manager/" + const_cast<Node&>(*it).toString()).c_str(), -1);
          if (rc != ZOK) { 
      ...
      for(map<string, set<Node> >::iterator it = producers.begin(); it != producers.end(); ++it) {
          if(!(it->second.empty())) {
              for(set<Node>::iterator iIt = it->second.begin(); iIt != it->second.end(); ++iIt) {
                  rc = zoo_delete(zk, ("/Producer/" + it->first + "/" + const_cast<Node&>(*iIt).toString()).c_str(), -1);
                  if (rc != ZOK) { 
  ...
  if(!caches.empty()) {
      for(map<string, set<Node> >::iterator it = caches.begin(); it != caches.end(); ++it) {
          if(!(it->second.empty())) {
              for(set<Node>::iterator iIt = it->second.begin(); iIt != it->second.end(); ++iIt) {
                  rc = zoo_delete(zk, ("/Cache/" + it->first + "/" + const_cast<Node&>(*iIt).toString()).c_str(), -1);
                  if (rc != ZOK) { 
          ...
          struct String_vector nodes;
          rc = zoo_get_children(zk, ("/Cache/" + it->first).c_str(), 0, &nodes);
          if (rc != ZOK) {
              std::cout<<"Can not get children path Cache "<<it->first<<std::endl;
              exit(-1);
          }
          if(nodes.count == 0) {
              rc = zoo_delete(zk, ("/Cache/" + it->first).c_str(), -1);
              if (rc != ZOK) { 
...
void Insert() {
  ...
  if(!managers.empty()) {
      for(set<Node>::iterator it = managers.begin(); it != managers.end(); ++it) {
          rc = zoo_create(zk, ("/Manager/" + const_cast<Node&>(*it).toString()).c_str(), "", 0, &ZOO_OPEN_ACL_UNSAFE, 0, 0, 0);
          if (rc != ZOK) { 
  ...
  if(!producers.empty()) {
      for(map<string, set<Node> >::iterator it = producers.begin(); it != producers.end(); ++it) {
          if(!(it->second.empty())) {
              for(set<Node>::iterator iIt = it->second.begin(); iIt != it->second.end(); ++iIt) {
                  rc = zoo_create(zk, ("/Producer/" + it->first + "/" + const_cast<Node&>(*iIt).toString()).c_str(), "", 0, &ZOO_OPEN_ACL_UNSAFE, 0, 0, 0);
                  if (rc != ZOK) { 
  ...
  if(!caches.empty()) {
      for(map<string, set<Node> >::iterator it = caches.begin(); it != caches.end(); ++it) {
          rc = zoo_exists(zk, ("/Cache/" + it->first).c_str(), 0, 0);
          if (rc != ZOK) { 
              rc = zoo_create(zk, ("/Cache/" + it->first).c_str(), "0", 1, &ZOO_OPEN_ACL_UNSAFE, 0, 0, 0);
              if (rc != ZOK) { 
          ...
          if(!(it->second.empty())) {
              for(set<Node>::iterator iIt = it->second.begin(); iIt != it->second.end(); ++iIt) {
                  rc = zoo_create(zk, ("/Cache/" + it->first + "/" + const_cast<Node&>(*iIt).toString()).c_str(), "", 0, &ZOO_OPEN_ACL_UNSAFE, 0, 0, 0);
                  if (rc != ZOK) { 
...
int main(int argc, char *argv[]) {
  ...
  int rc = zoo_create(zk, "/mutex", NULL, 0, &ZOO_OPEN_ACL_UNSAFE, ZOO_EPHEMERAL, NULL, 0);
  if (rc != ZOK) {
  ...
  Insert();
  NotifyNode("/Manager");
  NotifyNode("/Producer/cc");
  NotifyNode("/Producer/java");
  NotifyChildren("/Cache");
