    ...
    class HashHolder {
        ...
        public:
            void renew(const std::map<std::string, T>& config) {
                ...
                originalConfigSize_ = originalConfig_.size();
            }
            ...
            std::map<T, DataMap> get(const DataMap& data) {
                std::map<T, DataMap> res;
                ...
                for(DataMap::const_iterator it = data.begin(); it != data.end(); ++it) {
                    T t = get(it->first, configIndex);
                    if(t == NULL) {
                        continue;
                    }
                    typename std::map<T, DataMap>::iterator findIt = res.find(t);
                    if(findIt == res.end()) {
                        DataMap newData;
                        newData[it->first] = it->second;
                        res.insert(make_pair(t, newData));
                    }else{
                        findIt->second[it->first] = it->second;
                    }
                }
                return res;
            }
            ...
            T getRandom() {
                boost::shared_lock<boost::shared_mutex> lock(mutex_);
                return configIndex_.at(rand()%configIndex_.size()).second;
            }
        ...
        private:
            ...
            std::pair<uint32_t, T> getPair(const std::string& key, const std::vector<std::pair<uint32_t,T> >& configIndex) {
                ...
                if(right == end && configIndex[right].first < hash) {
                    right = 0;
                }
            ...
            std::map<uint32_t, T> hashConfig(const std::map<std::string, T>& config, std::map<uint32_t, std::string>& virtualNodesToRealNodesIndex) {
                ...
                for(typename std::map<std::string, T>::const_iterator it = config.begin(); it != config.end(); ++it) {
                    for(int i = 0; i < virtualNodes_ / 4; ++i) {
                        ...
                        for(int h = 0; h < 4; ++h) {
                            uint32_t hashKey = ((uint32_t) (results[3 + h*4] & 0xFF) << 24)
                                | ((uint32_t) (results[2 + h*4] & 0xFF) << 16)
                                | ((uint32_t) (results[1 + h*4] & 0xFF) << 8)
                                | ((uint32_t) (results[0 + h*4] & 0xFF));
