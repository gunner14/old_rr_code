...
//	MCE_DEBUG("Request::init --> line:"<<__LINE__);
	...
	if(pos != 0){
		string tmp = _path.substr(pos);
		MCE_INFO("Request::init --> transfer path: " << _path << " -> " << tmp << " " << _props["uid"]);
		_path = tmp;
	}
	it = props.find("host");
	if (it != props.end()) {
	...
	//手机取在线好友列表，不管是否有gzip，都认为所需要gzip的
	if(_path.find("/wap_") == 0){
		_encoding = 1;
	}
	//手机特殊需求,因为一些特殊网关的存在，会修改http头里的属性值，所以通过参数来指定
	if(_props["isgzip"] == "0"){
		_encoding = 0;
	}
