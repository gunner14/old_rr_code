...
class LoadXiaozuContentTask : public MyUtil::Task{
        JidPtr _jid;
        Ice::Long _xiaozuid;
        Ice::Long _threadid;
        string _iqId;
public:
        LoadXiaozuContentTask(const JidPtr& jid, Ice::Long xiaozuid, Ice::Long threadid, string iqId):
                        Task(2011), _jid(jid), _xiaozuid(xiaozuid), _threadid(threadid), _iqId(iqId){};
        virtual void handle();
        string XMLEncode(const string& body);//小组中取到的内容很乱，xml转义后丢进xml发给客户端
};
...
class loadXiaozuOfflineFeedReplyTask : public MyUtil::Task{
        JidPtr _jid;
        Ice::Long _xiaozuid;
        Ice::Long _threadid;
        int _page;
        ...
        int _stype;
public:
        loadXiaozuOfflineFeedReplyTask(const JidPtr& jid, int stype, Ice::Long xiaozuid, Ice::Long threadid, int page, string iqid): _jid(jid), _stype(stype), _xiaozuid(xiaozuid), _threadid(threadid), _page(page), _iqId(iqid){}
        void handle();
...
public:
	...
	virtual void loadXiaozuContent(const JidPtr& jid, Ice::Long xiaozuid, Ice::Long threadid, const string& iqId, const Ice::Current& = Ice::Current()); // guoqing.liu  use Ice::Long 
