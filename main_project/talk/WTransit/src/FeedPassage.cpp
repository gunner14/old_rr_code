...
void FeedPipe::handle(){
	...
	WTransitManagerI::instance().GSwapFeedSeq(Gobjs);
  if(!Gobjs.empty()) {
    try {
  ...
  if(objs.empty()){
	...
	for (size_t x = 0; x < objs.size(); ++x) {
		...
		MCE_INFO("WTransit-->FeedWithTargetPtr feedactor " << feed->actor);
      ...
      // 获取target中用户的在线副本
			...
			jmap = TalkFunStateAdapter::instance().getUsersJids(target, 22);
		} catch (Ice::Exception& e) {
			MCE_WARN("FeedPipe::handle-->TalkFunStateAdapter::getUsersJids-->" << target.size() << " " <<e);
		} catch (std::exception& e) {
			MCE_WARN("FeedPipe::handle --> online center err: " << target.size() << " "<<e.what());
		}
		MCE_DEBUG("FeedPipe::handle --> jmap.size = "<<jmap.size());
		for (size_t i = 0; i < target.size(); ++i) {
			MCE_DEBUG("target.at" << target.at(i) << "target.size" << target.size());
			JidSeqMap::const_iterator it = jmap.find(target.at(i));
			/*
			if (it == jmap.end()) {   //对不在线的用户发送给3g storm
				MCE_DEBUG("come in" << target.at(i));
				JidPtr jid = new Jid();
				string tailNum;
				try{
					tailNum = boost::lexical_cast<string>(target.at(i)%IMStormAdapter::instance().getCluster());
				}
				catch (Ice::Exception& e) {
					MCE_WARN("FeedPipe::handle-->WTransitAdapter::getCluster-->" << tailNum << " " <<e);
				} catch (boost::bad_lexical_cast & e) {
					MCE_WARN("FeedPipe::handle -->WTransitAdapter::getCluster--> " << "bad_lexical_cast" << " "<<e.what());
				}
				//MCE_INFO("FeedPipe::handle--> target.i:" << target.at(i) << " tailNum:" << tailNum);
				jid->userId = target.at(i);
				jid->index = 2;
				jid->endpoint = "PIMStorm" + tailNum;
				MessagePtr msg;
				msg = converter.getToPIMStormMessage(jid);
				if (msg) {
					msgstorms.push_back(msg);
				}
				MCE_DEBUG("FeedPipe::handle --> send msg to PIMStorm after converter: "<< msg->msg);
				continue;
			}
			*/
      // 为每一个用户副本都推送新鲜事
...
//					if(it->second.at(j)->endpoint.length() >= 8 && "PIMStorm" == it->second.at(j)->endpoint.substr(0, 8)){//3G通过PIMStorm实时收新鲜事 
//						msg = converter.getToPIMStormMessage(it->second.at(j));
//						if (msg) {
//							msgstorms.push_back(msg);
//						}
//					}
					//if( it->second.at(j)->endpoint[0] == 'P' ){//新鲜事 to phone
          if(string::npos != it->second.at(j)->endpoint.find("PTalkProxy")){
            ...
            // modified by jian.wang3 2012-08-03
           	msg = converter.getToWebpagerMessage(it->second.at(j));
						if (msg) {
							msgs.push_back(msg);
						}
	  ...
	  if (!msgs.empty()) {
		...
		if (!msgstorms.empty()) {
			try{
				IMStormAdapter::instance().deliver(msgstorms);
			}catch(Ice::Exception& e){
				MCE_WARN("FeedPipe::handle-->IMStorm::deliver" << "size:" << msgstorms.size() << " " << e);
			}
		}
...
FeedWithTarget::FeedWithTarget(const FeedSeedPtr& feed, const MyUtil::IntSeq& targetSeq, const MyUtil::IntSeq& focusfriendSeq){
	this->feed = feed->feed;//应该叫feedid吧，取名太歧义了
	source = feed->source;
	smallType = feed->type & 0xFFFF;
	bigType = int((feed->type & 0xFFFF) / 100);
	actor = feed->actor;
	...
	replyCount = 0;
	this->target.swap(const_cast< MyUtil::IntSeq&>(targetSeq));
	this->focusfriend.swap(const_cast< MyUtil::IntSeq&>(focusfriendSeq));
	//this->target.push_back(actor);
}
...
// 	return _jfrom;
// }
MessagePtr FeedToMessagConverter::getToPIMStormMessage(const JidPtr& to){
	MCE_DEBUG("FeedToMessagConverter::getToPIMStormMessage");
	if( !_jfrom ){
		MCE_DEBUG("FeedToMessagConverter::getToPIMStormMessage --> FeedToMessagConverter::setData should be called first");
		return 0;
	}
	//---for 3g need specify---
	ostringstream os;
	...
	bool isfocus;
	set<int> focusfriend(_feed->focusfriend.begin(), _feed->focusfriend.end());
	if(focusfriend.find(to->userId) != focusfriend.end())
		isfocus = true;//发向的用户是否特别关注
	else
		isfocus = false;
	//set<int>::iterator iter = focusfriend.begin();
	
	MCE_DEBUG("focus friend size is " << focusfriend.size() );
	os << "<message>" << "<xfeed>" << "<feedid>" << _feed->feed
	   << "</feedid>" << "<type>" << _feed->smallType << "</type>"  
	   << "<fromid>"  << _feed->actor << "</fromid>"
	   << "<toid>"    << to->userId << "</toid>"
	   << "<isfocus>" << (isfocus? "1" : "0") << "</isfocus>"
	   << "</xfeed>"  << "</message>";
	...
	msg->to = to;
	msg->type = PIMSTORM_MESSAGE;
	msg->msg = _toPhoneMsg;
	return msg;
}

MessagePtr FeedToMessagConverter::getToWebpagerMessage(const JidPtr& to) {
	if (to->userId != _feed->actor) {
    MessagePtr tmp_msg = new Message;
    // I don't think _jfrom is right
	  // msg->from = _jfrom;
    tmp_msg->from = new Jid();
		tmp_msg->from->userId = _feed->actor;
    tmp_msg->to = to;
    tmp_msg->type = FEED_STATUS;

    ostringstream os;
    os << "<message type=\"common\" from=\"xfeed@talk.renren.com\"><body>{\"feed_id\":\"" 
      << _feed->feed << "\",\"feed_actor\":\"" << _feed->actor << "\",\"feed_source\":\""
      << _feed->source << "\",\"feed_stype\":\"" << _feed->smallType << "\",\"isfocus\":\"";
    // if the "to" is in focus friends, add label "isfocus" for tiny tips 
	  // MCE_DEBUG("getToWebpagerMessage:" << _feed->focusfriend.size());
	  if (find(_feed->focusfriend.begin(), _feed->focusfriend.end(), 
      to->userId) != _feed->focusfriend.end())
      os << "1\"";
	  else
      os << "0\"";
    os << "}</body></message>";
...
MessagePtr FeedToMessagConverter::getToPhoneMessage( const JidPtr& to ){
	...
	if ( _feed->smallType == 102 ||
		...
		if ( _feed->smallType == 102 ||
			...
			if (_feed->smallType == 102) {
				xml_node f_share = xml.child("f").child("share");
				os << "<id>" << f_share.child("resourceId").first_child().value() << "</id>"
				   << "<title>" << f_share.child("title").first_child().value() << "</title>"
				   << "<friend>" 
           << "<id>" << f_share.child("resourceUserId").first_child().value()<< "</id>" 
           << "<name>" << f_share.child("resourceUserName").first_child().value()<< "</name>" 
           << "</friend>";
        MCE_DEBUG("FeedToMessagConverter::getToPhoneMessage  share_blog to:"<<to->userId<<"   from:"<<_jfrom->userId<<"   xml:"<<os.str());
			} else if (_feed->smallType == 103) {
				xml_node f_share = xml.child("f").child("share");
				os << "<id>" <<f_share.child("meta").child("albumid").first_child().value()<< "</id>"
				   << "<picid>" <<f_share.child("resourceId").first_child().value() << "</picid>"
				   << "<albumname>" << f_share.child("meta").child("albumname").first_child().value() << "</albumname>"
				   << "<friend>"
				   << "<id>"<< f_share.child("resourceUserId").first_child().value()<< "</id>"
				   << "<name>"<< f_share.child("resourceUserName").first_child().value()<< "</name>"
				   << "</friend>";
        MCE_DEBUG("FeedToMessagConverter::getToPhoneMessage  share_photo to:"<<to->userId<<"   from:"<<_jfrom->userId<<"   xml:"<<os.str());
			} else if (_feed->smallType == 104) {
				xml_node f_share = xml.child("f").child("share");
				os << "<id>" << f_share.child("resourceId").first_child().value() << "</id>"
				   << "<albumname>" << f_share.child("title").first_child().value() << "</albumname>"
				   << "<friend>" 
           << "<id>" <<f_share.child("resourceUserId").first_child().value() << "</id>" 
           << "<name>"<<f_share.child("resourceUserName").first_child().value() << "</name>" 
           << "</friend>";
        MCE_DEBUG("FeedToMessagConverter::getToPhoneMessage  share_album to:"<<to->userId<<"   from:"<<_jfrom->userId<<"   xml:"<<os.str());
			}
