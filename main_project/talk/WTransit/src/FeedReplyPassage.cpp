...
MessagePtr FeedReplyToMessagConverter::getToTalkMessage(  const JidPtr& to  ){
  ...
  if(_replyData->stype == 502){
    ...
    //MCE_DEBUG("replyString is ->" << replyString);
    if (replyString.size()>=6) {//所有业务前后都有转义以后的双引号&quot;
      if (replyString.substr(0, 6)=="&quot;" && replyString.substr(replyString.size()-6, 6)=="&quot;") {
        ...
        MCE_WARN("FeedReplyToMessagConverter::getToTalkMessage-->error quot:" << replyString
                 << " stype:" <<_replyData->stype << " data:" << _replyData->xml);
      }
	  }
