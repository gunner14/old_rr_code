	...
	if (doc.load(msg.c_str())) {
		...
		for (; n; n = n.next_sibling("message")) {
			...
			if( s.find("ugc_content") != string::npos){
				MCE_INFO("ConnectionStat::deliver -->deliver ugc_content ok = " << ok << " jid:" << jidToString(session->jid()));
			}
		}
	}else{
		MCE_WARN("CometResponseStat::deliver -->deliver ugc_content document.load error" << " jid:" << jidToString(session->jid()));
	}
}
