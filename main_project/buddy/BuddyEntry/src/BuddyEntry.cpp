...
BuddyDesc translateDesc(uint32_t desc) {
	//BuddyManager contains a copy of these code
...
void EntryListHolder::remove(const Entry& key, const Entry& mask) {
	...
	while (itBegin != _list.end()) {
		MCE_TRACE("pending erase entry " << *itBegin);
		if (equals(key, *itBegin, mask) ) {
			MCE_TRACE("erase from segment begin");
			itBegin = _list.erase(itBegin);
			MCE_TRACE("erase from segment end");
			continue;
		}
		++itBegin;
	}
...
void EntryListHolder::set(const Entry& key){
	IceUtil::Mutex::Lock lock(*this);
	EntryList::iterator it = lower_bound(_list.begin(), _list.end(), key, less_entry());

	if( it!=_list.end() && it->to == key.to){
		MCE_TRACE(" Segment:: set update " << *it );
		it->desc = key.desc;
	}else{
		MCE_TRACE(" Segment:: set insert " << *it );
		_list.insert(it, key);
	}
}
