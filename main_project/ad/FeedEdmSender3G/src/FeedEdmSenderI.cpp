...
#include "FeedEdmCache3gAdapter.h"
...
void FeedEdmSenderI::NoCheckReceive(int user_id, const MyUtil::Str2StrMap& params, const Ice::Current& current){
	...
	if(kind > 0){
      ...
      try{
        ...
        MCE_WARN("call FeedAssistantAdapter error:" << e);
      } catch(...){
         MCE_WARN("call FeedAssistantAdapter error");
      }
		} else {
...
void FeedEdmSenderI::ReceiveForPhone(int user_id, const MyUtil::Str2StrMap& params, const Ice::Current& current){
  ...
  if (_ValidateUser(user_id)) {
    ...
    if (limit_check != 0) {
      ...
      try {
        ...
        MCE_WARN("FeedEdmSenderI::ReceiveForPhone --> call FeedEdmCache server error");
      }

      if (selected_edm != 0) { 
        FeedSeedPtr feed_seed = _BuildSeed(selected_edm,user_id);        
        //MCE_INFO("FeedEdmSenderI::ReceiveForPhone --> until get user info spend time:" << (end_cache.tv_sec - start_cache.tv_sec)* 1000000 + end_cache.tv_usec - start_cache.tv_usec);
        ...
        //MCE_INFO("FeedEdmSenderI::ReceiveForPhone --> send edm spend time:" << (end_cache.tv_sec - start_cache.tv_sec)* 1000000 + end_cache.tv_usec - start_cache.tv_usec);
  ...
  try {
    ...
    MCE_WARN("FeedEdmSenderI::ReceiveForPhone --> call AdNotifyAdapter failed, err->" << e);
      ...
      MCE_WARN("FeedEdmSenderI::ReceiveForPhone --> call AdNotifyAdapter failed");
    }
...
void FeedEdmSenderI::ProcessReceive(int user_id, const MyUtil::Str2StrMap& params, bool ischeck) {
  timeval start_cache,end_cache;
  {
    MyUtil::Str2StrMap::const_iterator ite = params.find("ip");
    if (ite == params.end()) {
      MCE_DEBUG("The user: " << user_id << " has no ip, return.");
      return;
    }
  ...
  if (_ValidateUser(user_id)) {
    ...
    if (limit_check != 0) {
      ...
      //从params里面取到平台和坐标信息存在userInfo
      ite = params.find("platform");
      if (ite != params.end()) {
        user_info->platform = ite->second;
      } else {
        user_info->platform = "-1";
      }
      ite = params.find("coordinate");
      if (ite != params.end()) {
        user_info->coordinate = ite->second;
        MCE_DEBUG("user_id : "<<user_id<<" coordinate : "<<ite->second);
      }
      FeedSeedInfoPtr selected_edm = 0;
     /* try {
      ...
      try {
        ...
        MCE_WARN("call FeedEdmCache server error");
      }
      if (selected_edm != 0) { 
        FeedSeedPtr feed_seed = _BuildSeed(selected_edm,user_id); 
        //MCE_INFO("until get user info spend time:" << (end_cache.tv_sec - start_cache.tv_sec)* 1000000 + end_cache.tv_usec - start_cache.tv_usec);
				ostringstream logos;
				
				logos << "FeedEdmSenderI::ProcessReceive --> send edm :" << selected_edm->configId << " user_id:" << user_id << " limit_type:" << selected_edm->limitType << " ip:" << user_ip << " city:" << user_info->city << " age:" << user_info->age << " stage:" << user_info->stage << " gender:" << user_info->gender << " ";
        ...
        try{
          ...
          MCE_WARN("call FeedAssistantAdapter failed, err->" << e);
        } catch (...) {
          MCE_WARN("call FeedAssistantAdapter failed");
        }
        gettimeofday(&end_cache,NULL);
        MCE_INFO("send edm spend time:" << (end_cache.tv_sec - start_cache.tv_sec)* 1000000 + end_cache.tv_usec - start_cache.tv_usec);
        if (selected_edm->limitType == 1) { 
          FrequencyCache::instance().Add(user_id, kWeekly);//增加到已发送用户cache
        } else {
          FrequencyCache::instance().Add(user_id, kDaily);
  ...
  try {
    ...
    MCE_WARN("call AdNotifyAdapter failed");
  }
}
...
FeedSeedPtr FeedEdmSenderI::_BuildSeed(const FeedSeedInfoPtr& info, int userid) {
  ...
  MCE_DEBUG("feed_type: " << feed_type);
  if(feed_type == 8015 || feed_type == 8016) {
    return _BuildSeedForPhone(info, userid);
  }
  ...
  if ( feed_type == 8012) {
      ...
      std::map<int, UserCachePtr> firends_info;
      try{
        firends_info= UserCacheAdapter::instance().GetUserCache(info->friendId);
			}catch (Ice::Exception& e) {
        MCE_WARN("call UserCacheAdapter failed, err->" << e);
      }catch (...) {
        MCE_WARN("call UserCacheAdapter failed");
      }
...
void FeedEdmSenderI::ProcessUrl(string& str){
...
FeedSeedPtr FeedEdmSenderI::_BuildSeedForPhone(const FeedSeedInfoPtr& info, int userid) {
   FeedSeedPtr feed_seed = new FeedSeed();
   int feed_type = 8015;
   int feed_parent_type = 8000 + 15;
   int type = feed_type&0xffff;
   type |= 1<<28;
   type |= 1<<30;
   type |= 0<<26;
   ...
   /*switch (feed_type) {
    case 8015:
    case 8016:
      f_node.append_attribute("type") = "edm3G";
      break;
    default:break;
   }*/
...
//   body_link_node.set_name("bodyLink");
//   body_link_node.append_child(node_pcdata).set_value(info->bodyUrl.c_str());
   string real_titleLink = info->url.c_str();
   //make sure there is "http://" in url
   string target = "http://";
   size_t idx = real_titleLink.rfind(target);
   if (idx != string::npos)
   {
     real_titleLink.replace(idx, target.length(), "");
   }
   real_titleLink = "http://" + real_titleLink;
   string config_Id = boost::lexical_cast<string>(info->configId).c_str();
   ProcessUrl(real_titleLink);
   string click_titleLink = "http://edm.renren.com/edm3g/click/"+ config_Id +"/2?l=" + real_titleLink;
   title_link_node.append_child(node_pcdata).set_value(click_titleLink.c_str());
   xml_node body_node = edm_node.append_child();
   body_node.set_name("body");
   body_node.append_child(node_pcdata).set_value(info->body.c_str());
   xml_node body_link_node = edm_node.append_child();
   body_link_node.set_name("bodyLink");
   string real_bodyLink = info->bodyUrl.c_str();
   idx = real_bodyLink.rfind(target);
   if (idx != string::npos)
   {
     real_bodyLink.replace(idx, target.length(), "");
   }

   real_bodyLink = "http://" + real_bodyLink;
   ProcessUrl(real_bodyLink);
   string click_bodyLink = "http://edm.renren.com/edm3g/click/"+ config_Id +"/4?l=" + real_bodyLink;
   body_link_node.append_child(node_pcdata).set_value(click_bodyLink.c_str());
   
   xml_node link_type_node = edm_node.append_child();
   link_type_node.set_name("link_type");
   std::string link_type_string = boost::lexical_cast<string>(info->linkType);
   link_type_node.append_child(node_pcdata).set_value(link_type_string.c_str());
   
   xml_node config_node = edm_node.append_child();
   MCE_DEBUG("title_link:" << click_titleLink << " body_link:" << click_bodyLink);
   config_node.set_name("config");
   config_node.append_child(node_pcdata).set_value(boost::lexical_cast<string>(info->configId).c_str());
   xml_node pic_src_node = edm_node.append_child();
   pic_src_node.set_name("pic_src");
   xml_node share_node = edm_node.append_child();
   share_node.set_name("share");
   share_node.append_child(node_pcdata).set_value(boost::lexical_cast<string>(info->sharenum).c_str());
   xml_node member_name_node = edm_node.append_child();
   member_name_node.set_name("member_name");
   member_name_node.append_child(node_pcdata).set_value(boost::lexical_cast<string>(info->memberName).c_str());
   Json::Reader reader;
   Json::Value json_value;
   bool parsingSuccessful;
    /*switch (feed_type) {
      case 8015:
        MCE_DEBUG("3g edm txt");
        //break;
      case 8016:
        {
          std::string media_pic = info->mediaSrc;
          ...
          parsingSuccessful = reader.parse(media_pic, json_value);
          if ( !parsingSuccessful ){
            MCE_WARN("Parse 3g media info failed,edm_id:" << info->configId << ",media:" << media_pic);
          } else {
            pic_src_node.append_child(node_pcdata).set_value(json_value["picSrc"].asString().c_str());
          }
        }
        break;
      default:
        MCE_WARN("error_edm_type,edm_id:" <<info->configId);
    }*/
...
FeedUserInfoPtr FeedEdmSenderI::_GetUserInfo(Ice::Int user_id, int limit_check) {
  ...
  try {
    ...
    MCE_WARN("UserBaseAdapter error,can't get user base common view,user id = "<<user_id<<" error: "<<e);
  } catch(...){
    MCE_WARN("call UserBaseAdapter error,can't get user base common view,user id = "<< user_id);
  }
  ...
  try {
    ...
    MCE_WARN("call UserNetworkAdapter failed");
    return user;
  }
  //try {
