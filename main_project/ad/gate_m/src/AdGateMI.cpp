...
string xce::ad::db_instance_ads;
string xce::ad::db_instance_main;
string xce::ad::db_instance_adwr;
...
void MyUtil::initialize() {
  ...
  xce::ad::db_instance_ads = service.getCommunicator()->getProperties()->getPropertyWithDefault("DbInstance_Ad", "jebe_ads_load");
  xce::ad::db_instance_main = service.getCommunicator()->getProperties()->getPropertyWithDefault("DbInstance_Main", "jebe_main_load");
  xce::ad::db_instance_adwr = service.getCommunicator()->getProperties()->getPropertyWithDefault("DbInstance_Wr", "db_adwr");
  MCE_INFO("db_instance_ads = " << db_instance_ads << ", db_instance_main = " << db_instance_main << ", db_instance_adwr = " << db_instance_adwr);
...
bool AdTestRequest::response() {
  ...
  MCE_INFO("AdEngineBRecplicaAdapter::GetAds return size : " << ads.size() << " uid="
...
string AdGateMI::GetAds(int uid, Ice::Long zoneid, const string& ip_str, const Ice::Current&) {
  ...
  if (uid > 0) {
    ...
    if (!b) {
      ...
      try {
        StatFunc stat("memc AdUserCacheLoader");
        serialized = AdUserCacheLoaderReplicaAdapter::instance().GetById(uid);
      } catch (Ice::Exception& e) {
        MCE_WARN("AdUserCacheLoaderReplicaAdapter GetById error : " << e << " uid="
            << uid);
      } catch (std::exception& e) {
  ...
  bool hide = false;
  try {
    ads = AdEngineBReplicaAdapter::instance().GetAds(serialized, log_url.str(), rotate_slot, hide);
  } catch (Ice::Exception& e) {
    MCE_WARN("AdEngineBReplicaAdapter::GetAds error : " << e << " uid=" << uid);
    err_flag = true;
  } catch (std::exception& e) {
  ...
  for(AdResultMap::const_iterator it = ads.begin(); it != ads.end(); ++it) {
    MCE_INFO("AdEngineBReplicaAdapter::GetAds add default ad : " << it->first << '/' << it->second.size());
  }
