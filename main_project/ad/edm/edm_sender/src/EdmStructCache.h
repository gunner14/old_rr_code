...
class AdLeftBlackList : public MyUtil::Singleton<AdLeftBlackList> {
public:
  void Init();
  void GetBlackGroups(set<Ice::Long>& groups);
  bool IsBlack(AdGroupPtr& group);
  bool IsBlack(Ice::Long group_id);
  class UpdateCheckTimer : public MyUtil::Timer {
  public:
    UpdateCheckTimer() :
      Timer(1 * 60 * 1000) {
