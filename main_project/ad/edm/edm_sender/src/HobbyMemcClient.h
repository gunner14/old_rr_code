...
class  HobbyMemcClient{
	public:
		...
		bool  GetData(int  uid, std::set<uint64_t>& hobbies){
			pair<int, EdmMemcachedClient*> cli = pool_.GetClient(uid); 
			if(!cli.second){ 
				MCE_WARN("GetData:Get Client Error");
				return false; 
			}  
      ...
      map<string, string> res;
			...
			if(bret == false){
        MCE_INFO("GetData:Get Data failed"); 
				return false;
			}   
		...
		bool PutData(int uid, const std::set<uint64_t>& hobbies){
			pair<int, EdmMemcachedClient*> cli = pool_.GetClient(uid);
			if(!cli.second){
				MCE_WARN("PutData:Get Client Error");
				return false;
			}
