...
void MyUtil::initialize() {
  ...
  service.getAdapter()->add(&xce::ad::AdEdmSenderI::instance(), service.createIdentity("ADEDM", ""));
	...
	DegradeManager::instance().Register("edm_interest");
  DegradeManager::instance().Register("left_ad_edm_sender");
  ...
  TaskManager::instance().scheduleRepeated(new EdmDiscountCacheTimer());

  UserDemographicCache::instance().Load();
  TaskManager::instance().scheduleRepeated(new UserDemographicCacheTimer());

  //每天定时删除过期的推左EDM 
  TaskManager::instance().scheduleRepeated(new DailyRemoveOverDueEdmTimer());
  ...
  */
  ...
  xce::serverstate::ServerStateSubscriber::instance().initialize("ControllerAdEdmSender",&AdEdmSenderI::instance(),mod,interval,new EdmChannel(),"10", "3000", "131072", "udp");
...
void AdEdmSenderI::SendEdmTest(int user_id, const MyUtil::Str2StrMap& params, const Ice::Current&){
 ...
 // expression << "u(" << user_id << ")";
  ...
  for (; it != params.end(); ++it){
    ss << it->first << COLON << it->second << SPLIT;
  }
  MCE_DEBUG(ss.str());
...
void AdEdmSenderI::Expose(Ice::Int user_id, Ice::Long adgroup_id, Ice::Int index, const Ice::Current&){
  string str_user_id = lexical_cast<string>(user_id);
  string str_adgroup_id = lexical_cast<string>(adgroup_id);
  string key = str_user_id + LINK + str_adgroup_id;

  try {
    EdmLogCacheAdapter::instance().Expose(key, index);
  } catch (Ice::Exception& e) {
    MCE_WARN("AdEdmSenderI::Expose --> call EdmLogCacheAdapter error -> "<< e);
    return;
  } catch (std::exception& ex) {
    MCE_WARN("AdEdmSenderI::Expose --> call EdmLogCacheAdapter exception error -> "<< ex.what());
    return;
  } catch (...) {
    MCE_WARN("AdEdmSenderI::Expose --> call EdmLogCacheAdapter failed");
    return;
  }
}
...
void AdEdmSenderI::SendEdm(int user_id, const MyUtil::Str2StrMap& params, const Ice::Current&){
  ...
  int deal_ratio = 0;
  string ratio = EngineParametersCache::instance().GetParameterByName("switch_deal_ratio");
//  MCE_DEBUG("deal ratio --> key = switch_deal_ratio," << " value = " << ratio);
  if (!ratio.empty()) {
    try{
      deal_ratio = boost::lexical_cast<int>(ratio);
    } catch (...){
      MCE_WARN("deal_ratio --> deal_ratio error.");
    }
  }
  if(rand() % 100 < deal_ratio) {
    MCE_INFO("Reserved to control edm pool.");
    return ;
  }
  timeval start;
  ...
  if (user_id > 0) {
    ...
    if (!get_serialized_success) {
      ...
      try {
        ...
        serialized = AdUserCacheLoaderReplicaAdapter::instance().GetById(user_id);
      } catch (Ice::Exception& e) {
        MCE_WARN("AdUserCacheLoaderReplicaAdapter GetById error : " << e << " uid = " << user_id);
      } catch (std::exception& e) {
        MCE_WARN("AdUserCacheLoaderReplicaAdapter GetById error : " << e.what() << " uid = " << user_id);
      } catch (...) {
        MCE_WARN("AdUserCacheLoaderReplicaAdapter GetById error : " << " uid = " << user_id);  
      }
    }
  }
  if (serialized.empty()) {
...
groups AdEdmSenderI::GetCandidateGroups(Ice::Int userid,const Ice::Current&){
	MCE_INFO("AdEdmSenderI::GetCandidateGroups Test ,uid:"<< userid);
	UserProfile profile;
	groups vecgroupid;
	MyUtil::Str2StrMap params;
	GetUserInfo(userid, profile, params);
	MatchHandlerParamter para(profile, 3);

	PubType pub_type = Brand_ad;
	for(int i=1;i<=3;i++)
	{
		...
		EdmInvertedIndex::instance().GetBidGroups(profile,pub_type,bid_groups);
		MCE_DEBUG("AdEdmSenderI::GetCandidateGroups uid:"<< userid << " type:"<< pub_type << " size:"<< bid_groups.size());
		set<AdGroupPtr>::iterator sIter = bid_groups.begin();
		for (; sIter != bid_groups.end(); sIter++){
			vecgroupid.push_back((*sIter)->group_id());
		}
	}
	MCE_INFO("AdEdmSenderI::GetCandidateGroups Test ,get size:"<< vecgroupid.size());
	return vecgroupid;
}
/*
...
void AdEdmSenderI::Init(){
  ...
  try {
    ...
    MCE_INFO("AdEdmSenderI::Init --> AdLeftBlackList::instance().Init(); DONE");
...
bool EdmClickRequest::response() {
  ...
  //获取adgroupID
  string adgroup_id = getProperty("aid");
  string key = uid + LINK + adgroup_id;
  MCE_INFO("EdmClickRequest::response --> Key: " << key);
  try {
    EdmLogCacheAdapter::instance().Click(key);
  } catch (Ice::Exception& e) { 
    MCE_WARN("EdmClickRequest::response --> call EdmLogCacheAdapter error -> "<< e);
  } catch (std::exception& ex) {
    MCE_WARN("EdmClickRequest::response --> call EdmLogCacheAdapter exception error -> "<< ex.what());
  } catch (...) {
    MCE_WARN("EdmClickRequest::response --> call EdmLogCacheAdapter failed");
  }
