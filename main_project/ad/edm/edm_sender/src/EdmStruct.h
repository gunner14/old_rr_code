 ...
 * 客户类型：品牌、自助、产品广告
 * ******************************8*****************************************************/
enum PubType { Brand_ad = 1, Self_ad = 2, Product_ad = 3, Left_ad = 4 };
...
public:
  enum Type { EDM_UNKNOWN = 1, EDM_NORMAL = 2, EDM_UID_TARGET = 3, EDM_LEFT = 4 };
  Type edm_type() {
    if ( 8 == display_type() && 2 != edm_cast_type() ) {  //常规Edm 2
      return EDM_NORMAL;
    } else if ( 8 == display_type() && 2 == edm_cast_type() ) {  //Uid Edm 3
      return EDM_UID_TARGET;
    } else if ( 4 == display_type() && 10 == trans_type() && product_type() & 5 == 5) { //推左 Edm 4
      return EDM_LEFT;
    } else {
      return EDM_UNKNOWN;
    }
 ...
 // void UpdateCampainStatus();
  ...
  void add_widget_version(Ice::Long creative_id, std::string& widget_version) {
    creative2widget_version_[creative_id] = widget_version; 
  }
  ...
  std::string widget_version() {
    if (!creative2widget_version_.empty()) {
      return creative2widget_version_.begin()->second;
    }
    return "";
  }
  ...
  int product_type() const {
    return product_type_;
  }
  void set_product_type(int product_type) {
...
private:
  ...
  map<Ice::Long, std::string> creative2widget_version_;
