#ifndef EDM_MEMCACHED_CLIENT
#define EDM_MEMCACHED_CLIENT
...
namespace xce{
namespace ad{
class EdmMemcachedClient{
public:
  //vector<string> Get(vector<keys>);
  EdmMemcachedClient(const string& ip, int port, int timeout, int recv_count_limit);
  ~EdmMemcachedClient();
...
protected:
  bool    Request(const char* buf, int len, EdmCmdType& t, map<string, string>& resdata, map<string, int>& flags);
  bool    HandleRead(EdmCmdType& t, map<string, string>& resdata, map<string, int>& flags);
  bool    CheckConnect();
  bool    Send(const char* buf, int len);
  void    SetSocketOpt();
  bool    FetchData(map<string, string>& resdata, map<string, int>& flags, EdmCmdType& type);
private:
  string  ip_;
  int     port_;
  int     timeout_;
