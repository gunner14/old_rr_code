...
#ifndef __AD_LOG_MAKER_H__
#define __AD__LOG_MAKER_H__
...
class AdLogMaker : public IceUtil::Shared , public IceUtil::Monitor<IceUtil::Mutex> {
public:
  AdLogMaker( const string& name, const string& path, const string& loglevel, const string pattern = "%m%n", const size_t buffer_size = 1): 
...
private:
  ...
  public:
    FlushLogTask(AdLogMaker* parent) {
      parent_ = parent;
    }
    virtual void handle();
  private:
...
typedef IceUtil::Handle<AdLogMaker> AdLogMakerPtr;
