...
//  void set_ad_count(int count);
  ...
  set<AdGroupPtr> GetGlobalGroups(const int count);
  int GetGlobalSize() const {
    return global_list_.size();
  }
  void ResizeGlobalList(int size) {
    global_list_.resize(size);
  }
  bool AddGroupToGlobalList(int rotate_index, const AdGroupPtr& obj);
private:
...
public:
  ...
  map<Ice::Long, vector<AdGroupPtr> > GetGlobalAds() {
    IceUtil::RWRecMutex::RLock lock(mutex_);
    return zone2global_;
  }
  void UpdateCampainStatus();
...
private:
  ...
  map<Ice::Long, vector<AdGroupPtr> > zone2global_;
  IceUtil::RWRecMutex mutex_;
