...
bool BidUnit::Match(const UserProfile& userProfile) {
  ...
  try {
    ...
    if(user_stage == 2) {
      ...
      if(stage_ == 2) {
        //MCE_DEBUG("BidUnit::Match() --> ad_stage == 2");
        /*oss.str("");
        oss << "BidUnit::Match() --> (uint64_t)userProfile.school() = " << (uint64_t)userProfile.school() << ", school_ = ";
        for(int i = 0; i < school_.size(); ++i) {
          oss << " " << school_[i];
        }
        MCE_DEBUG(oss.str());
        oss.str("");
        oss << "BidUnit::Match() --> (boost::lexical_cast<uint64_t>(userProfile.current_area()) = " << boost::lexical_cast<uint64_t>(userProfile.current_area()) << ", school_area_ = ";
        for(int i = 0; i < school_area_.size(); ++i) {
          oss << " " << school_area_[i];
        }
        MCE_DEBUG(oss.str());*/
        ...
        //MCE_DEBUG("BidUnit::Match() --> school not matched");
        return false;
      } else if(stage_ & 2) {
        //MCE_DEBUG("BidUnit::Match() --> ad_stage = " << stage_);
        return AreaMatch(boost::lexical_cast<uint64_t>(userProfile.current_area()), area_);
      } else {
        return false;
      }
...
bool BidUnit::SchoolAreaMatch(uint64_t key, const vector<uint64_t>& area) {
  if(area.empty()) {
    return false;
  }
  /*ostringstream oss;
  ...
  uint64_t country = (key/1000000000000)*1000000000000; 
  //MCE_DEBUG("key = " << key);
