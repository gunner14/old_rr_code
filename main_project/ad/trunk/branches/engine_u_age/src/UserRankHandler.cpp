...
AdGroupPtr UserIndex::GetZonePosFreshGroup(int userid, FreshAdQualityMap & fresh_groups, set<AdGroupPtr>& bid_ids, set<AdGroupPtr>& selected_ids, long adzone, int pos) {
  ...
  if(itf != fresh_groups.end()) {
    ...
    if(total > 0.0) {
      ...
      MCE_INFO("[Fresh:]:userid" << userid << ",total score" << total << ",random:" << randnum <<",adzone:" << adzone<< ",pos:" << pos<<",test ratio:" << test_ratio <<",ratio:" << ratio);
      double sum = 0.0;
...
SelectedAdGroupSeq UserIndex::GetZoneGroups3(const UserProfile & user_profile, long adzone, int count) {
  SelectedAdGroupSeq result;
  ...
  MCE_INFO("UserIndex::GetZoneGroups3new-->user " << user_profile.id() << " zone " << adzone << " matched ad count " << groups.size()<<",debug_flag" << debug_flag);
  if(debug_flag) {
...
/////////////////////////////select fresh group////////////////////////////////////
    m_ptr = GetZonePosFreshGroup(user_profile.id(), fresh_groups, groups, selected_ids, adzone, i+1); 
    if(m_ptr) {
      ...
      selected->demo_state_ = (adzone % 100) * 10 + selected->pos_;
    ...
    for(set<AdGroupPtr>::const_iterator it = groups.begin(); it != groups.end(); ++it) {
      ...
      if(its == selected_ids.end()) { //是否已经选到，不在列表中进行下边操作
        if((*it) && !IsCompetedGroup((*it), selected_ids)) { //是否与已选存在竞争，不存在进行下边操作,即：找最大的ecpm
          //AdQualityU quality = AdQualityCache::instance().GetQuality(adzone, (*it)->group_id(), i+1);
          //double ctr = UserDemographicCache::instance().GetUserAdGroupsCtr(user_profile, adzone ,i+1 ,const_cast<AdGroupPtr&>(*it));
          pair<double,int> ctr_state = UserDemographicCache::instance().GetUserAdGroupsCtr(user_profile, adzone ,i+1 ,(*it)->group_id(),(*it)->member_id());
          ...
          if(!AdBlindness::instance().IsDisplay(user_profile.id(),(*it)->member_id())){//如果呈现屏蔽，则降低权重
            ecpm *= 0.001;
          }
          if(ecpm > max_ecpm) {
            max_ecpm = ecpm;
...
SelectedAdGroupSeq UserIndex::GetZoneGroups2(const UserProfile & user_profile, long adzone, int count) {
  ...
  if(!qualitys.empty()) {
    /*for(set<AdGroupPtr>::const_iterator it = groups.begin(); it != groups.end(); ++it) {
      AdQualityU quality = AdQualityCache::instance().GetQuality(adzone, (*it)->group_id());
      MCE_DEBUG("zoneid = " << adzone << ", quality.isFresh = " << quality.isFresh << ", quality.ecpm = " << quality.ecpm);
      if(debug_flag) {
        MCE_INFO("uid=" << user_profile.id() << "'debug info:" << "zone=" << adzone << " quality.isFresh = " << quality.isFresh);
      }
      if(!AdBlindness::instance().IsDisplay(user_profile.id(),(*it)->member_id())) {
        quality.quality *= 0.001;//decrease the probability of group user can see
        if(debug_flag) {
          MCE_INFO("uid=" <<user_profile.id() <<",adblind debug info,gid:" <<(*it)->group_id()<<",sponsor id:" <<(*it)->member_id()<<",ctr:" <<quality.quality);
        }
      }
      qualitys[*it] = quality;
    }*/
...
bool UserRankHandler::handle(MatchHandlerParamter& para, AdResultMap& result) {
  ...
  SelectedAdGroupSeq selected = user_index->GetZoneGroups3(para.userProfile_, zone_id, pick_count);
  ...
  if(!selected.empty()) {
    for(SelectedAdGroupSeq::const_iterator it = selected.begin(); it != selected.end(); ++it) {
      ...
      AdBlindness::instance().AddPv(uid,(*it)->group_->member_id());
    }
...
void MemberItemList::InitScore(const UserProfile & user_profile, const set<AdGroupPtr> & groups) {
  ...
  for(set<AdGroupPtr>::const_iterator it = groups.begin(); it != groups.end(); ++it) {
    AdGroupPtr group = *it;
    AdQualityU quality = AdQualityCache::instance().GetQuality(zone_id_, group->group_id());
    MCE_DEBUG("MemberItemList::InitScore() --> uid = " << uid << ", quality.isFresh = " << quality.isFresh << ", quality.ecpm = " << quality.ecpm << ",quality.quality:"<<quality.quality<< ", zone_id = " << zone_id_);
    if (quality.isFresh) {
      quality.quality = 0.00001;
      quality.ecpm = 0.00001;
