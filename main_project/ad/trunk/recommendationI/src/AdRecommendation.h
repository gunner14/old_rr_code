...
public:
  ...
  set<int> users;
  map<int, double> category; //广告类别及所属属性值
};
...
private:
//  typedef boost::multi_index_container<ItemUsersPtr, indexed_by<sequenced<> ,
//      ordered_unique<BOOST_MULTI_INDEX_MEMBER(ItemUsers, Ice::Long, gid)> > >
//      ItemUsersEvictor;
//
//  typedef ItemUsersEvictor::nth_index<0>::type SeqIndex;
//  typedef ItemUsersEvictor::nth_index<1>::type GidIndex;
//  ItemUsersEvictor container_;
  typedef map<Ice::Long, ItemUsersPtr> ItemUsersEvictor;
  ItemUsersEvictor gidmap_;
  map<Ice::Long, map<Ice::Long, double> > knncache_;

  map<Ice::Long, map<Ice::Long, double> > knn_prop_; //广告属性的邻居
...
//    MCE_DEBUG("AdRecommend::--> GetKNNCount = " << value);
 ...
 //   MCE_DEBUG("AdRecommend::--> GetMaxReCount = " << value);
  ...
  void InitFromCategory(ItemUsersEvictor &gidmap);
  void InitFromDB(ItemUsersEvictor &container, int type);
