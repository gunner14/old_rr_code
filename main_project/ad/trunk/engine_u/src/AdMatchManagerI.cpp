  ...
  for(AdZoneDict::iterator it = zone_dict.begin(); it != zone_dict.end(); ++it){
    ...
    for(size_t i = 0; i < ad_match_handler_list_.size(); ++i) {
      if (!ad_match_handler_list_[i]->handle(para, ans)) {
        ...
        if((100000000072 == para.adzone_->id()) && (!ans[100000000072].empty())) { //TODO 传说中的控制7273的逻辑？
...
// construct ClickAction  for AntiFraudServer
      ...
      try{
        ...
        MCE_INFO("AdMatchManagerI::Click --> antifraud response : " << " uid = " << uid << " zone_id = "<< para.zone_id_  << " ip = " << ip 
...
// antifauder call end!
      ...
      chargelog << para.creative_id_ << ',' << group_id << ','
        ...
        << "," <<  group_ptr->bid_unit_id() << ',' << ip << ',' << uid << ',' << fraud_value;//在chargelog加入作弊标志
      ...
      clicklog << para.creative_id_ << "," << date.toDateTime() << "," << group_ptr->bid_unit_id()<< "," << cost << ","
          ...
          << group_ptr->bid_unit_id() << ","<< fraud_value <<",1," << group_ptr->display_type();//在clicklog加入作弊标志
