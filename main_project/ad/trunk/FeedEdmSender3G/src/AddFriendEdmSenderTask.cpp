...
void AddFriendEdmSenderTask::Send() {
  ...
  try {
    ...
    MCE_WARN("read data list of add friend edm sender task from SocialFeedCacheAdapter failed");
  }
  ...
  try {
    ...
    MCE_WARN("send add friend edm by FeedAssistantAdapter failed");
  }
  try {
    SocialFeedCacheAdapter::instance().remove(user_id_, friend_id_list, common_id_list);
  } catch (Ice::Exception& e) {
    MCE_WARN("call SocialFeedCacheAdapter failed, err->" << e);
  } catch(...){
    MCE_WARN("call SocialFeedCacheAdapter failed");
  }
