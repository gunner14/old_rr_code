...
//          max_price = info.max_price;
          max_price = info.get_max_price(adzone_id);//add 2012.4.23 by chaojie liu
          MCE_INFO("AdUrlUtil::Get adzone_id = " << adzone_id << ", max_price =  " << max_price);
          
          if ((fabs(max_price-0)<1e-6)|| bid_unit_id == 0) {
            //not active
            creative_id = 0;
            //break;
          } else {
      ...
      //如果价格大于出价，取出价作为价格
      if((fabs(max_price-0)>1e-6) && price > max_price) {
        price = max_price;
      }
  ...
  try {
    AdLogReplicaAdapter::instance().PvBatch(log_seq);
  } catch(Ice::Exception& e) {
    MCE_WARN("AdLogReplicaAdapter::Pv error : " << e << " uid=" << uid);
  } catch(std::exception& e) {
    MCE_WARN("AdLogReplicaAdapter::Pv error : " << e.what() << " uid=" << uid);
  }
  return result_map;
} //end Get
...
void AdUrlUtil::Click(int uid,
  ...
  for(tokenizer::iterator tok_iter = tok.begin(); tok_iter != tok.end(); ++tok_iter, ++num) {
    if (num == 2) {//creative_id
      ...
      max_price = boost::lexical_cast<double>(*tok_iter);
    } else if (num == 7) {
...
// antifauder call end!
  ...
  try{
    ...
    MCE_WARN("AdUrlUtil::Click --> call AdLogReplicaAdapter.ClickForBp ERR  line:"<<__LINE__<<" err:"<<e);
  } catch(std::exception& e){
    MCE_WARN("AdUrlUtil::Click --> call AdLogReplicaAdapter.ClickForBp ERR  line:"<<__LINE__<<" err:"<<e.what());
  }
  ...
  try{
    ...
    MCE_WARN("AdUrlUtil::Click --> call AdLogReplicaAdapter.ClickForCharge ERR  line:"<<__LINE__<<" err:"<<e);
  } catch(std::exception& e){
    MCE_WARN("AdUrlUtil::Click --> call AdLogReplicaAdapter.ClickForCharge ERR  line:"<<__LINE__<<" err:"<<e.what());
  }
  }
...
//          max_price = info.max_price;
      ...
      log << AdTwConfig::instance().server_name() << "||" << ip_oss.str() << "||" << date.toDateTime()
            ...
            << "^B" << tw_pos << "^B1^B2||4||" << append_info;
      /*try {
        AdLogReplicaAdapter::instance().Pv(log.str());
        MCE_INFO("PVLOG");
      } catch(Ice::Exception& e) {
        MCE_WARN("AdLogReplicaAdapter::Pv error : " << e << " uid=" << uid);
      } catch(std::exception& e) {
  ...
  try {
    ...
    if (config) {
      while (config >> line) {
        ...
        if (2 == n) {
          if (key.compare("DB_INSTANCE") == 0) {
            DB_ = value;
            MCE_INFO("[LOAD_CONFIG] DB_INSTANCE = " << DB_);
          } else if (key.compare("sys_min_price") == 0) {
