...
#define LOOKUP_NO_EXCEPTIONS(P, T, V)  												\
        ...
        friend ostream& operator <<(ostream& os, const std::pair<T1, T2>& sp);

        template<typename KEY, typename VALUE>
        friend ostream& operator <<(ostream& os, const map<KEY, VALUE>& mp);
        
        string PrintZoneAdNum();
        string PrintTriggerInfo();
        string PrintGameAdNum();
        string PrintGameMemNum();
			private:
				int load();
