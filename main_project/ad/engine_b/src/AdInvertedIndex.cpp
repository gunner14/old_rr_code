...
bool AdInvertedIndex::TotalLoad(hash_map<Ice::Long, AdGroupPtr>& groups) {
  ...
  for(size_t i = 0; i < res.size(); ++i) {
    ...
    if (EngineConfig::instance().IsForBrand()? (group->display_type() != 1 && group->display_type() != 4 && group->display_type() != 6 && group->display_type() != 7): (group->display_type() == 4)) {
    ...
    } else if(group->display_type() == 1 || group->display_type() == 6 || group->display_type() == 7) {
