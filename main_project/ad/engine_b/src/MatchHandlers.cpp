  ...
  if (para.PickCount() > 0) {
    ...
    if(para.adzone_->GetRotateGroups(para.rotate_index_, para.userProfile_, rotate_set) && !rotate_set.empty()){
      ...
      //if ((para.adzone_->member_type() == 0) && ((para.adzone_->GetRotateSize() - para.rotate_index_) <= para.adzone_->self_rotation_number())) {
			  ...
			  int rotate_index = 0;
			  if(para.adzone_->GetRotateSize() != 0){
					rotate_index = para.rotate_index_ % para.adzone_->GetRotateSize();
				}
			
				if ((para.adzone_->member_type() == 0) && para.adzone_->IsSelfSlot(rotate_index)) {
...
bool LogHandler::handle(MatchHandlerParamter& para, AdResultMap& result) {
  ...
  for(vector<SelectedAdGroupPtr>::iterator sit = zone_selected.begin();
    ...
    if(!group_ptr->creative_ids().empty()){
      ...
      ad_info.creativeid = group_ptr->GetCreative(para.userProfile_.id(),para.adzone_->id(), next_creative_index);
    } else{
    ...
    if(100000000072 == para.adzone_->id() && para.remain_state_ == DEFAULT) {
      ...
      MCE_DEBUG("[REMAIN] LogHandler::handle() --> normal cpm log: " << cpm_log.str());
...
/*
  try {
    subts.reset();
    AdLogReplicaAdapter::instance().PvBySeq(pv_log_seq);
  } catch (Ice::Exception& e) {
    MCE_WARN("LogHandler::handle AdLogReplicaAdapter.PvBySeq ERR line:"
            << __LINE__ << " err:" << e);
