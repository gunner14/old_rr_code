...
public:
  AdZone():self_rotation_number_(0) {
    member_online_status_ = 0;
    am_online_status_ = 0;
    audit_status_ = 0;
    delete_flag_ = 0;
  }
  void  OutputDebugRotate(ostream& out);
	AdGroupPtr GetRotateGroups(int rotate_index, const UserProfile& user_profile);
  AdGroupPtr GetRotateGroup(int rotate_index);
	bool GetRotateGroups(int rotate_index, const UserProfile& user_profile, set<AdGroupPtr>& groups);
  ...
  AdGroupPtr GetGoogleGroups();
  set<AdGroupPtr> GetGoogleGroups(int count);
  bool AddGroupToGoogleList(int rotate_index, const AdGroupPtr& obj);
  int GetGoogleSize();
  void ResizeGoogleList(int size);

  bool FindStage(int stage);
  void ClearRotateList(){
    rotate_list_.clear();
  }
	...
	void AddSelfPos(int pos){
		self_pos_.insert(pos);
	}
	...
	bool IsSelfSlot(int pos){
		return self_pos_.find(pos) != self_pos_.end();
	}
  ...
  AdGroupPtr only_first(const UserProfile& user);
  void add_only_first(AdGroupPtr only_first);
...
private:
  ...
  vector<AdGroupPtr> google_list_;

  vector<AdGroupPtr> global_list_; // 投放给所有用户的广告
  vector<AdGroupPtr> only_first_;

	set<int>   self_pos_; 		//中小广告展示的位置
...
public:
  ...
  void Init();
  AdZoneDict GetAdZoneOrdered(const string& referer, const UserProfile& userprofile);
  AdZoneDict GetAdZoneOrdered(const string& referer, const int stage);
  AdZoneDict GetAdZoneOrdered(const string& referer, const UserProfile& userprofile, bool hide);
...
private:
  ...
  void LoadUrlAdzoneMaping(map<Ice::Long, AdZonePtr> & zones, DictTreePtr & trie, multimap<string, AdZonePtr> & ref_zone_map);
	...
	void SortOneGroup(map<Ice::Long, AdZonePtr>& zones,set<Ice::Long>  zoneGroup);
	void AISortRotates(map<Ice::Long, AdZonePtr>& zones);
	int  CheckNeedSort(const map<Ice::Long, AdZonePtr>& zones,const set<Ice::Long>&  zoneGroup);
};
...
public:
  ...
  Ice::Long GetNextCreative2(Ice::Long zone_id, int rotate_index);
  Ice::Long GetCreative(int uid, Ice::Long zone_id, int rotate_index);
  bool Available();
  bool HasCreatives();
  ...
  void add_creative_id(Ice::Long cretive_id, int ad_type) {
    if(2 == ad_type) {
      saa_creative_ids_.insert(cretive_id);
    } else if(1 == ad_type) {
      sa_creative_ids_.insert(cretive_id);
    }
  }
  ...
  AdGroup(Ice::Long group_id) : group_id_(group_id), member_id_(0), campaign_id_(0), ad_type_(0) {
    am_status_ = 0;
    audit_status_ = 0;
    delete_flag_ = 0;
    campaign_status_ = 0;
    ...
    rotation_number_ = 1;
    creative_index_ = 0;
    sa_creative_index_ = 0;
    saa_creative_index_ = 0;
  }
...
private:
  ...
  set<Ice::Long> sa_creative_ids_;
  set<Ice::Long> saa_creative_ids_;
  int creative_index_;
  int sa_creative_index_;
  int saa_creative_index_;
  int ad_type_;
...
public:
  ...
  map<Ice::Long, vector<AdGroupPtr> > GetGoogleAds();
...
private:
  ...
  map<Ice::Long, vector<AdGroupPtr> > zone2google_;
  IceUtil::RWRecMutex mutex_;
...
/***********************************************************************************************************************/
class UserBind : public IceUtil::Shared {
public:
  ...
  Ice::Long creative_id() const {
    return creative_id_;
  }
  void set_creative_id(Ice::Long creative_id) {
    this->creative_id_ = creative_id;
  }
...
typedef IceUtil::Handle<UserBind> UserBindPtr;

class UserBindPool : public MyUtil::Singleton<UserBindPool> {
public:
  void Init();
  UserBindPtr GetUserBind(const int user_id, const Ice::Long zone_id);
private:
  set<UserBindPtr> pool_;
  IceUtil::RWRecMutex mutex_;
};
