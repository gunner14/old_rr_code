...
void AdUrlMaping::GetAdZone(const string& Url,  set<Ice::Long>& AdZones) {
  MCE_DEBUG("AdUrlMaping::GetAdZone");
  vector<string> refers;
  PreProcess(Url, refers);
  IceUtil::RWRecMutex::RLock lock(mutex_);
  for(vector<string>::iterator it = refers.begin(); it != refers.end(); ++it) {
    map<string, set<Ice::Long> >::iterator mit = url_adzone_map_.find(*it);
    if(mit != url_adzone_map_.end()) {
      AdZones = mit->second;
     break;//找到，结束
    }
  }
...
bool AdUrlMaping::Load(const string& db) {
  ...
  sql << "select distinct url,adzone_id from url_adzone_maping";
  res = QueryRunner(db, CDbRServer).store(sql);
  map< string, set<Ice::Long> > tmp_url_adzone_map;
  ...
  for (size_t i = 0; i < res.num_rows(); i++) {
    ...
    map< string, set<Ice::Long> >::iterator it = tmp_url_adzone_map.find(url);
    if(it != tmp_url_adzone_map.end()) {
      it->second.insert(adzone_id);
    } else {
      set<Ice::Long> s;
      s.insert(adzone_id);
      tmp_url_adzone_map[url] = s;
    }
  }
  {
    IceUtil::RWRecMutex::WLock lock(mutex_);
    //TODO
    if(!tmp_url_adzone_map.empty()) {
      url_adzone_map_.swap(tmp_url_adzone_map);
    }
  }
  return true;
}
