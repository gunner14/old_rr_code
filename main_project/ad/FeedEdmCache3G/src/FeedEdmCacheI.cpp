...
FeedSeedInfoPtr FeedEdmCacheI::SelectEdmById(int edmid, const Ice::Current& current){
  ...
  FeedEdmModelPtr fit_edm_ptr;
  {
    ...
    feed_edm_selected->memberName = fit_edm_ptr->configure_model_ptr()->member_name(); //广告主名称
...
ConfigureModelPtr FeedEdmCacheI::_FormatEdmConfigure(const mysqlpp::Row& row) {
  ...
  configure_ptr->set_member_name((std::string)row["member_name"]);
  if (configure_ptr->member_name() == "NULL") {
    configure_ptr->set_member_name("");
  }
