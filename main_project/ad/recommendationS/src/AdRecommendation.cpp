...
const string UserItemPool::TableName = "ad_uid_gid_inpage";

void UserItem::FillUserData() {
  string serialized;
  if (userid > 0) {
  ...
  MCE_INFO("UserItem::FillUserDate-->userid:" << userid << ",age:" <<age << ",gender:" << gender);
}
void UserItemPool::Init() {//
  ...
  sql << "SELECT * FROM "<< GetTableName() <<" limit 400000" ;
...
double AdRecommend::GetPropertyDistance(UserItemPtr& uptx, UserItemPtr& upty
    ,double age_percent, double gender_percent) {
  double result = 0;
  result += gender_percent * (1.0 - abs(uptx->gender - upty->gender));
  result += age_percent * (1.0 - (double)(abs(uptx->age - upty->age)) / 65.0);
  return result;
...
multimap<double, UserItemPtr> AdRecommend::GetKNNeighbour(UserItemPtr& upt) {//可以考虑设置门限，不一定要是指定的k
  ...
  if ((!upt)) {
    MCE_INFO("AdRecommend::GetKNNeighbour-->user is null");
    return result;
  }
  int flag = 1;
  if(upt->gids.empty()) {
    ...
    MCE_INFO("AdRecommend::GetKNNeighbour-->uid:" << upt->userid << "has click");
  }
  UserItemListPtr c = list_;
  vector<UserItemPtr>::iterator otherit = c->list.begin();
  int excellentcount = 0;
  double sim_threshold = GetSimThreshold();
  double gender_percent = GetGenderPercent();
  double age_percent = GetAgePercent();
  double prop_threshold = GetPropThreshold();
...
//    if((upt->userid == 2602) && ((*otherit)->userid == 2802)){ 
//      MCE_DEBUG("AdRecommend::GetKNNeighbour test:distance:" << distance);
//    }
    if(flag == 1) {
      double pvalue = GetPropertyDistance(upt, (*otherit), age_percent, gender_percent);

 //     MCE_DEBUG("AdRecommend::GetKNNeighbour--> userid:" << upt->userid <<",click distance:"
 //         << distance << ",prop distance:" << pvalue);
      distance = distance * (1.0 - gender_percent - age_percent) + pvalue;
    } else if(strategy == 1) {
      double pvalue = GetPropertyDistance(upt, (*otherit), age_percent, gender_percent);
  ...
  //    MCE_DEBUG("AdRecommend::GetKNNeighbour--> userid:" << upt->userid <<",click distance:"
   //       << distance << ",prop distance:" << pvalue);
    ...
    if ((distance != 1) && (distance > 0)) { //和自己行为一样的用户可以不考虑？just for testing
  ...
  for (multimap<double, UserItemPtr>::reverse_iterator mit = tmp.rbegin(); (mit
    ...
    if((flag == 1) && ( i > knn_count)) { //有点击行为，需要控制邻居个数， 如果没有点击行为，不需要控制邻居个数
      break;
    }
    double distance = mit->first;
    if((flag == 1) && (distance < sim_threshold)) {
      break;
    }
    if((flag == 0) && (distance < prop_threshold)) {
      break;
    }
  ...
  MCE_INFO("AdRecommend::GetKNNeighbour-->uid:" << upt->userid <<"Neighbour num:"
      << result.size() << ",IshasClick:" << flag <<",parameters: KNNCount:" << knn_count
      << ",SimThreshold:" << sim_threshold << ",PropThreshold:" << prop_threshold <<
      ",Strategy:" << strategy);
...
//  }
 ///////////////////////////////////////////////////////////////
  ...
  double rec_threshold_empty = GetReThresholdEmpty();
  ...
  for (multimap<double, Ice::Long>::reverse_iterator itm = result_tmp.rbegin(); (itm
    ...
    if ((AdGroupPool::instance().HasAdGroup(groupid))) {//找到有效广告// 还没有对已看广告进行处理
      ...
      if((!flag)) {
        if (value < rec_threshold_empty) {
          MCE_INFO("AdRecommend::GetRecommendation--> less than rec_threshold_empty:" << rec_threshold_empty)
          break;
        }
      } else if(value < rec_threshold) {
