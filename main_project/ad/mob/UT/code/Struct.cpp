...
void AdGroup::add_creative_id(Ice::Long creative_id) {
	creative_id_set_.insert(creative_id);
}

Ice::Long AdGroup::creative_id() {
	if ( false == creative_id_set_.empty() ) {
		return *(creative_id_set_.begin());
	} else {
		return 0;
	}
}
...
bool AdGroup::have_creative() {
	return false == creative_id_set_.empty();
}

string AdGroup::widget() {
	if( false == creative_seq_.empty() ){
		if ( ROTATE3G == display_type() ) { //品牌广告随机出
			int random = rand() % creative_seq_.size();
			if (creative_seq_[random]) {
				return creative_seq_[random]->widget();
			}
		} else { //自助广告出第一个
...
string AdGroup::widget(Ice::Long creative_id) {
	if( false == creative_seq_.empty() ){
...
void AdGroup::add_widget_id(Ice::Long creative_id, Ice::Long widget_id) {
	creative2widget_[creative_id]=widget_id;
}

Ice::Long AdGroup::widget_id() {
...
bool AdTargetUnit::Match(const UserProfile& userProfile, const AdMobReqPara& request) {
	try {
		...
		if(user_stage == STAGE_COLLEGE) {
			...
			if(school_.empty() || school_.front() == ALL ||
				...
				if( !grade_.empty() && grade_.front() != ALL && grade_.front() != -1 && find(grade_.begin(), grade_.end(), userProfile.grade()) == grade_.end() ) {
					return false;
				}
			}
		...
		if( !platform_.empty() && platform_.front() != ALL ) {
			...
			bool flag = false;
			if( find( platform_.begin(), platform_.end(), platform_code) != platform_.end() ) {
				flag = true;
			} else if ( find( platform_.begin(), platform_.end(), platform_codex) != platform_.end() ) {
				flag = true;
			}
			if ( false == flag) {
				return false;
