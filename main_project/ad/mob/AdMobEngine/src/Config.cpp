...
void EngineConfig::InitializeServer(int server_index) {
	set_server_index( server_index );
	ostringstream oss;
	oss << "AdMob" << server_index << "@" << GetIp();
	set_server_name( oss.str() );
}
void EngineConfig::TotalLoad(){
	MCE_INFO("AdMobConfig::TotalLoad --> start");
	Statement sql;
	map<string, string> local_property_dict;
	map<string, int> local_property_dict_int;
	map<string, double> local_property_dict_double;
	sql << " SELECT  config_key, config_value "
	    << " FROM    admob_config ";
	mysqlpp::StoreQueryResult res;
	try {
		Util::ExecuteQuery(sql, GetDB(), "admob_config", res);
		if ( res && res.num_rows() > 0 ) {
			for( size_t i = 0; i < res.num_rows(); i++){
				mysqlpp::Row row = res.at(i);
				string key = mysql_string(row["config_key"]);
				string value = mysql_string(row["config_value"]);
				local_property_dict[key] = value;
				local_property_dict_int[key] = atoi(value.c_str());
				local_property_dict_double[key] = atof(value.c_str());
			}

			MCE_INFO("AdMobConfig::TotalLoad --> end <DB Table engineconfig> size: " << res.size() 
					  << " property_dict_ size:" << local_property_dict.size() );
			{
				IceUtil::RWRecMutex::WLock lock(mutex_);
				property_dict_.swap(local_property_dict);
				property_dict_int_.swap(local_property_dict_int);
				property_dict_double_.swap(local_property_dict_double);
			}
			Print();
		} else {
			MCE_WARN("AdMobConfig::TotalLoad --> end <DB> engineconfig have no data");
		}
	} catch (Ice::Exception& e) {
		MCE_WARN("AdMobConfig::TotalLoad : Ice Exception: " << e);
	} catch (std::exception& e) {
		MCE_WARN("AdMobConfig::TotalLoad : std exception: " << e.what());
	}
}

string EngineConfig::GetProperty(const string& key) {
...
string EngineConfig::GetPropertyWithDefault(const string& key, string& value) {
	IceUtil::RWRecMutex::RLock lock(mutex_);
	string result = GetProperty(key);
	return ( true == result.empty() ) ? value : result;
}

int EngineConfig::GetPropertyAsIntWithDefault(const string& key, int value) {
	IceUtil::RWRecMutex::RLock lock(mutex_);
	map<string, int>::const_iterator it = property_dict_int_.find(key);
	if (it != property_dict_int_.end()) {
		return it->second;
	}
	return value;
}

double EngineConfig::GetPropertyAsDoubleWithDefault(const string& key, double value) {
	IceUtil::RWRecMutex::RLock lock(mutex_);
	map<string, double>::const_iterator it = property_dict_double_.find(key);
	if (it != property_dict_double_.end()) {
		return it->second;
	}
	return value;
}
...
void EngineConfig::Print() {
	...
	for (; it != property_dict_.end(); ++it) {
		MCE_INFO("Engine Property --> [" << it->first << "]:" << it->second);
	}
}
