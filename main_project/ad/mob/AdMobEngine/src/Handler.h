...
struct HandlerPara {
  ...
  enum DeviceType {DEV_DEFAULT=0, DEV_UNKNOWN=1, IOS_IPAD=2, IOS_IPHONE=3, IOS_ITOUCH=4, IOS_UNKNOWN=5 };
  ...
  string cpc_charge_log_;  //cpc charge
	...
	//其他变量
  DeviceType dev_type_;  //201-ipad,202-iphone,203-ipod touch
...
private:
  ...
  inline void PrintUserProfile(UserProfile& profile);
	inline void PrintReqLog(const HandlerPara& para);
...
public:
  ...
  virtual void FilterGroups(set<AdGroupPtr>& selected, const set<Ice::Long>& last_selected);
	virtual double ComputeWeight(const AdGroupPtr& adgroupPtr);
};
...
public:
  ...
  void UrlEncode(const string& input, string& output);
	string ClickUrlHead;
private:
	void BuildClickUrlInfo(HandlerPara& para, string& redirect_info);
  void GetPicInfo(HandlerPara& para, string& pic_url, string& pic_width);
	void GetDateTime(string& date_time);
};
