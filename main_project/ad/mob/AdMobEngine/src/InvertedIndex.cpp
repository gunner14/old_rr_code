...
void InvertedIndex::Get(const HandlerPara& para, const Ice::Long zone_id, const InvertedIndexType type, set<AdGroupPtr>& result_set) {
	DimIndexPtr dim_index;
	{
		...
		if ( BRAND_INDEX == type ) {
			...
			MCE_INFO("<" << para.request_.uid << "> [YELLOW] InvertedIndex::Get --> undefined index, type:" << type << " zone:" << zone_id);
			return;
		}
	}
	if (!dim_index) {
		MCE_INFO("<" << para.request_.uid << "> [YELLOW] InvertedIndex::Get --> index not found, type:" << type << " zone:" << zone_id);
		return;
	}
	...
	MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> gender:" << profile.gender() << ", gender.size:" << gender.size());
	...
	MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> & stage:" << profile.stage() 
			<<", stage.size:" << stage.size() <<", result.size:" << result_set.size());

	it = set_intersection(result_set.begin(), result_set.end(), age.begin(), age.end(), result.begin());
	result_set.clear();
	result_set.insert(result.begin(), it);
	MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> & age:" << profile.age() 
			<< ", age.size:" << age.size() <<", result.size:" << result_set.size());
	...
	if ( (int)profile.stage() == STAGE_STAFF ) {
		...
		result_set.clear();
		result_set.insert(result.begin(), it);
		MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> worker, & area:" << profile.ip_area() 
				<< ", area.size:" << area.size() << ", result.size:" << result_set.size());
	} else {
		...
		result_set.clear();
		result_set.insert(result.begin(), it);
		MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> student, & grade:" << profile.grade() 
				<<", grade.size:" << grade.size() << ", result.size:" << result_set.size());

		MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> student, school:" << profile.school() << ", school.size:" << school.size());
		MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> student, school_area(current_area):" << profile.current_area()
		              << ", school_area.size:" << school_area.size());
		MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> student, ip_area:" << profile.ip_area() << ", ip_area.size:" << area.size());
		school.insert(school_area.begin(), school_area.end());
		school.insert(area.begin(), area.end());
    MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> student, U school, school_area, ip_area. union.size:" << school.size());

		it = set_intersection(result_set.begin(), result_set.end(), school.begin(), school.end(), result.begin());
		result_set.clear();
		result_set.insert(result.begin(), it);

		MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> student, & union.size:" << school.size() 
				<< ", result.size:" << result_set.size());
	}
	//3G定向
	...
	result_set.clear();
	result_set.insert(result.begin(), it);
	MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> & platform:" << client.osVersion 
			<< ", platform.size:" << platform_set.size() << ", result.size:" << result_set.size());
	...
	result_set.clear();
	result_set.insert(result.begin(), it);
	MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> & network:" << network_code 
			<< ", network.size:" << network_set.size() <<", result.size:" << result_set.size());
	...
	result_set.clear();
	result_set.insert(result.begin(), it);
	MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> & resolution:" << floor_width 
			<<", resolution.size:" << resolution_set.size() <<", result.size:" << result_set.size());
	...
	if ( BRAND_INDEX == type ) {  //品牌包含LBS定向功能
		...
		result_set.clear();
		result_set.insert(result.begin(), it);
		MCE_DEBUG( "<" << para.request_.uid << "> InvertedIndex::Get --> & lbs, result.size:" << result_set.size());
	}
	
	//设备类型定向
	FilterGroupsByDeviceType(result_set, para.dev_type_);
	MCE_DEBUG("<" << para.request_.uid << "> InvertedIndex::Get --> after dev filter, result.size:" << result_set.size());
}
...
/*
 * Function: FilterGroupsByDeviceType
 * Description: 根据设备类型（iphon,ipad,itouch等）去除不能显示的广告
 */
void InvertedIndex::FilterGroupsByDeviceType(set<AdGroupPtr>& selected, const HandlerPara::DeviceType device_type) {
  set<AdGroupPtr>::iterator it = selected.begin();
  MCE_DEBUG("InvertedIndex::FilterGroupsByDeviceType --> device_type:"<<device_type);
  for ( ; it != selected.end();) {
    bool is_valid = CheckGroupByDeviceType(*it, device_type);
    if (false == is_valid) {
			int print_flag = EngineConfig::instance().GetPropertyAsIntWithDefault("PRINT_FDEVICE_FLAG", 0);
			if (print_flag == 1){
				MCE_INFO("InvertedIndex::FilterGroupsByDeviceType --> device_type:" << device_type 
				         << " invalid adgroupId:"<<(*it)->group_id());
      }
			selected.erase(it++);
		} else {
		  ++it;
	...
	MCE_DEBUG("InvertedIndex::FilterGroupsByDeviceType --> end");
}

inline bool InvertedIndex::CheckGroupByDeviceType(AdGroupPtr ad, const HandlerPara::DeviceType device_type) {
	string cast_type_3g = ad->creative()->GetValue("cast_type_3G");
	int print_flag = EngineConfig::instance().GetPropertyAsIntWithDefault("PRINT_FDEVICE_FLAG", 0);
	if (print_flag == 1){
		MCE_INFO("InvertedIndex::CheckGroupByDeviceType --> ad:" << ad->group_id()
	           << " creative:" << ad->creative()->creative_id() << " cast_type_3g:" << cast_type_3g);
	}
	if (true == cast_type_3g.empty())
		return true;
  //ipad,itouch去掉打电话广告. cast_type_3g:1跳转,2打电话,3下载.
	if ((device_type == HandlerPara::IOS_IPAD || device_type == HandlerPara::IOS_ITOUCH) && 0 == cast_type_3g.compare("2")){
		return false;
	}
	return true;
}
