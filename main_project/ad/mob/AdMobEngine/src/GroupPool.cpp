...
void GroupPool::TotalLoad() {
	...
	TotalLoadAdGroup(local_pool);
	TotalLoadAdCreative(local_pool);
	FilterNoCreativeAdGroups(local_pool);
	{
...
void GroupPool::TotalLoadAdGroup(map<Ice::Long, AdGroupPtr>& local_pool) {
...
void GroupPool::TotalLoadAdCreative(map<Ice::Long, AdGroupPtr>& local_pool) {
	...
	try {
		...
		for( size_t i = 0; i < res.num_rows(); i++){
			...
			map<Ice::Long, AdGroupPtr>::iterator it = local_pool.find( group_id );
			if ( it != local_pool.end() ) {
...
void GroupPool::FilterNoCreativeAdGroups(map<Ice::Long, AdGroupPtr>& local_pool) {
  map<Ice::Long, AdGroupPtr>::iterator it = local_pool.begin();
  for(; it != local_pool.end(); ) {
