...
void MyUtil::initialize() {
  ...
  TaskManager::instance().config(TASK_LEVEL_CALCULATE, MyUtil::ThreadPoolConfig(1, 3));
...
void AdRecommendLoader::execute(int userid) {
 // MCE_INFO("[LOADER] AdRecommendLoader::execute-->userid" << userid);
...
//    }
  ...
  }
  {
   ...
   // PrintList("after AdRecommendLoader::execute:");
    MCE_INFO("[LOADER] AdRecommendLoader::after execute-->userid:" << userid <<",listH size:" <<listH_.size() << ",listL size:" << listL_.size() <<",set size:" << set_.size());
