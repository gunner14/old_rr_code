...
void FreshAdCache::LoadAuditedOnlineFreshAd(){ //加载审核过的在线新广告
  ...
  try {
    ...
    mysqlpp::StoreQueryResult res = QueryRunner(db.c_str(), CDbRServer, "audit_status").store(sql);
    if (!res) {
      MCE_WARN("FreshAdCache::LoadAuditedOnlineFreshAd Error ad_audit_his select  fail!");
      return;
    }
    MCE_INFO("FreshAdCache::LoadAuditedOnlineFreshAd,audit gids Size=: " << res.num_rows());
    for (size_t i = 0; i < res.num_rows(); ++i) {
...
void FreshAdCache::LoadFreshAdData(){
  ...
  try {
    ...
    //mysqlpp::StoreQueryResult res = q.store();
    res = QueryRunner(db.c_str(), CDbRServer, "rt_DZPMGGS").store(sql);
    if (!res || res.num_rows() <= 0) {
