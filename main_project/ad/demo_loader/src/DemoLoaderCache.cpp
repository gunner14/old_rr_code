...
void ::MyUtil::initialize() {
  ...
  service.getAdapter()->add(&xce::ad::DemoLoaderCache::instance(), service.createIdentity("M",""));
	...
	string st_DbInstance = service.getCommunicator()->getProperties()->getPropertyWithDefault("DbInstance_Config_St","ad_st");
	string q_DbInstance = service.getCommunicator()->getProperties()->getPropertyWithDefault("DbInstance_Config_Quality","ad_quality");
	string stat_DbInstance = service.getCommunicator()->getProperties()->getPropertyWithDefault("DbInstance_Stat","ad_stat");
	string statbigt_DbInstance = service.getCommunicator()->getProperties()->getPropertyWithDefault("DbInstance_StatBigt","ad_statbigt");
	string ads_DbInstance = service.getCommunicator()->getProperties()->getPropertyWithDefault("DbInstance_Ads","jebe_ads_load");

  xce::ad::AdDemoConfig::instance().SetDBST(st_DbInstance);
	xce::ad::AdDemoConfig::instance().SetDBQuality(q_DbInstance);
  xce::ad::AdDemoConfig::instance().SetDBSource_Stat(stat_DbInstance);
  xce::ad::AdDemoConfig::instance().SetDBSource_StatBigt(statbigt_DbInstance);
  xce::ad::AdDemoConfig::instance().SetDBSource_Ads(ads_DbInstance);
  ...
  xce::ad::AdZoneLoader::instance().Load();
  TaskManager::instance().schedule(new xce::ad::AdZoneLoaderTimer());
...
AdZoneQualityMap DemoLoaderCache::GetAdZoneQualityPool(const ::Ice::Current& current){
  MCE_INFO("DemoLoaderCache::GetAdZoneQualityPool--> call from " << current.con->toString());
  AdZoneQualityMap result;
  result = AdZoneLoader::instance().GetAdZoneQualityPool();
  MCE_INFO("DemoLoaderCache::GetAdZoneQualityPool, AdZoneQualityMap size:"<<result.size());
  return result;
}
...
void DemoLoaderCache::LoadSystemParameter(){
  ...
  try{
    ...
    sql << " select * from system_parameter";
		string db = AdDemoConfig::instance().GetDBST();
		string pattern = "system_parameter";
		mysqlpp::StoreQueryResult res = QueryRunner(db.c_str(), CDbRServer, pattern.c_str()).store(sql);
    if (!res || res.num_rows() <= 0) {
      MCE_WARN("DemoLoaderCache Load Error " << pattern << ".  select  fail!");
    }
...
void DemoLoaderCache::LoadLimitedAdPool(){
  ...
  try{
    ...
    sql << "select adzone_id, adgroup_id as group_id, abs_pos, sum(pv_count) as sum_impr, sum(sum_click_price) as sum_cost from "
        << " rt_DZPMGGS where time >= " << end_time_pos 
        << " and adzone_id in (100000000001,100000000063,100000000070,100000000072) and member_id <> 999"
...
void DemoLoaderCache::LoadCpmPool() {
  ...
  try {
    string db = AdDemoConfig::instance().GetDBSource_Test();
    mysqlpp::StoreQueryResult cpm_res = QueryRunner(db.c_str(), CDbRServer, "cpm_adgroup").store(cpm_sql);
    if (!cpm_res || cpm_res.num_rows() <= 0) {
      MCE_INFO("DemoLoaderCache::LoadCpmPool Error cpm_adgroup select 0 rows!");
    }
    ...
    MCE_INFO("DemoLoaderCache::LoadCpmPool error: " << e.what());
  } catch (...) {
  ...
  try {
    const string DB_INSTANCE = AdDemoConfig::instance().GetDBSource_Stat(); 
    mysqlpp::StoreQueryResult ctr_res = QueryRunner(DB_INSTANCE, CDbRServer, "Log_DCZY").store(ctr_sql);
    if (!ctr_res || ctr_res.num_rows() <= 0) {
      MCE_INFO("DemoLoaderCache::LoadCpmPool Error select Log_DCZY fail!");
    }
  ...
  tctr_sql << " SELECT FLOOR(creative_id/100000) as adgroup_id,adzone_id,SUM(pv_count) as sum_pv_count,SUM(click_count) as sum_click_count,SUM(sum_click_price) as sum_click_price"
           ...
           << " GROUP BY FLOOR(creative_id/100000),adzone_id";
  try {
    string db_instance = AdDemoConfig::instance().GetDBSource_Stat();
    mysqlpp::StoreQueryResult ctr_res = QueryRunner(db_instance, CDbRServer, "Log_DCZY").store(tctr_sql);;
    if (!ctr_res || ctr_res.num_rows() <= 0) {
      MCE_INFO("DemoLoaderCache::LoadCpmPool Error select Log_DCZY fail!");
    }
    MCE_INFO("DemoLoaderCache::LoadCpmPool res size : " << ctr_res.num_rows());
  ...
  try {
    string db_instance = AdDemoConfig::instance().GetDBSource_Stat();
    mysqlpp::StoreQueryResult ctr_res = QueryRunner(db_instance, CDbRServer, "Log_HGZ").store(hctr_sql);
    if (!ctr_res || ctr_res.num_rows() <= 0) {
      MCE_INFO("DemoLoaderCache::LoadCpmPool Error select Log_HGZ fail!");
    }
    MCE_INFO("DemoLoaderCache::LoadCpmPool res size : " << ctr_res.num_rows());
...
void DemoLoaderCache::LoadCpmMemberPool(CpmAdgroupMap &cpm_member_groups){
  ...
  */
  ...
  try {
    string db_instance = AdDemoConfig::instance().GetDBSource_Stat();
    mysqlpp::StoreQueryResult ctr_res = QueryRunner(db_instance, CDbRServer, "Log_DMZD").store(hctr_sql);;
    if (!ctr_res || ctr_res.num_rows() <= 0) {
      MCE_INFO("DemoLoaderCache::LoadCpmMemberPool Error select Log_DMZD fail!");
    }
...
void DemoLoaderCache::LoadAverageEcpm(){
  ...
  try {
    string db = AdDemoConfig::instance().GetDBST();
    string pattern = "average_ecpm";
    mysqlpp::StoreQueryResult ctr_res = QueryRunner(db.c_str(), CDbRServer, pattern.c_str()).store(ctr_sql);
    if (!ctr_res || ctr_res.num_rows() <= 0) {
      MCE_WARN("DemoLoaderCache::LoadAverageEcpm Error " << pattern << ".  select  fail!");
    }
...
void DemoLoaderCache::LoadDisplayLessAd(){
  ...
  try {
    ...
    string db = AdDemoConfig::instance().GetDBST();
    string pattern = "rt_HZPMGGS";
    mysqlpp::StoreQueryResult ctr_res = QueryRunner(db.c_str(), CDbRServer, pattern.c_str()).store(ctr_sql);
    if (!ctr_res || ctr_res.num_rows() <= 0) {
      MCE_WARN("DemoLoaderCache::LoadDisplayLessAd Error " << pattern << ".  select  fail!");
    }
...
void DemoLoaderCache::LoadUpdatedAd(set<Ice::Long>& update_ads){ //只加载修改过的广告，不加载新建广告
   ...
   /////////////////////////////////////Load fresh gids////////////////////////////////////
   Statement sql;//加载两天之内的新广告
   sql << "select adgroup_id as group_id from adgroup where create_time >='"
   ...
   try {
     string db_instance = AdDemoConfig::instance().GetDBSource_Ads();
     mysqlpp::StoreQueryResult res = QueryRunner(db_instance, CDbRServer, "adgroup").store(sql);
     if (!res || res.num_rows() < 0){
 ...
 try {
   ...
   string db = AdDemoConfig::instance().GetDBSource_Audit();
   mysqlpp::StoreQueryResult res = QueryRunner(db, CDbRServer, "ad_audit_his").store(sql);
   if (!res) {
     MCE_WARN("DemoLoaderCache Load Error ad_audit_his select  fail!");
     return;
   }
...
void DemoLoaderCache::InsertAmplifyGrouptoPool(AdQualityMap& res_pool){
  ...
  map<Ice::Long,pair<Ice::Long,double> > ecpm_table_tmp;
  {
    ...
    string db_instance = AdDemoConfig::instance().GetDBSource_Ads();
    mysqlpp::StoreQueryResult qr_res = QueryRunner(db_instance, CDbRServer, "adgroup").store(fresh_qr_sql);
    ...
    mysqlpp::StoreQueryResult ctr_res = QueryRunner(db,CDbRServer,"average_ecpm").store(ctr_sql);
    if (!ctr_res || ctr_res.num_rows() <= 0) {
      MCE_INFO("DemoLoaderCache::InsertAmplifyGrouptoPool Error " << "average_ecpm" << ".  select  fail!");
    }
...

  {
    ...
    mysqlpp::StoreQueryResult res = QueryRunner(db,CDbRServer,"rt_HZPMGGS").store(sql);
    if (!res || res.num_rows() <= 0) {
      MCE_WARN("DemoLoaderCache::InsertAmplifyGrouptoPool Load Error " << "rt_HZPMGGS" << ".  select  fail!");
    }
  ...
  { ////加载最新更新的广告
    ...
    if(!update_ads.empty()){
      ...
      //////////////////////////////////////////////////////////////////////////////////////////
      {
        ...
        string db_instance = AdDemoConfig::instance().GetDBSource_Ads();
        mysqlpp::StoreQueryResult qr_res = QueryRunner(db_instance, CDbRServer, "adgroup").store(update_gid_price_sql);
    ...
    << " rt_HZPMGGS  where time >= " << fresh_begin_time <<"  and adgroup_id in " << update_ads_list
            << " group by adzone_id, adgroup_id ,abs_pos ";
        mysqlpp::StoreQueryResult res = QueryRunner(db.c_str(), CDbRServer, "rt_HZPMGGS").store(sql);
        if (!res || res.num_rows() <= 0) {
          MCE_INFO(" DemoLoaderCache::InsertUpdateGrouptoPool Load Error " << "rt_HZPMGGS" << ".  select  fail!");
        }
...
void DemoLoaderCache::LoadPool() {
  ...
  try {
    ...
    {/****************no_gender*****************************/
      sql << "select adzone_id, member_id, adgroup_id as group_id, abs_pos, stage, sum(pv_count) as sum_impr, sum(click_count) as sum_click from "
      ...
      mysqlpp::StoreQueryResult res = QueryRunner(db.c_str(), CDbRServer, "rt_DZPMGGS").store(sql);
      if (!res || res.num_rows() <= 0) {
        MCE_WARN("DemoLoaderCache Load Error " << "rt_DZPMGGS" << ".  select  fail!");
      }
      MCE_INFO("[DemoLoaderCache]::Load   res size : " << res.num_rows());
      for (size_t i = 0; i < res.num_rows(); ++i) {
        mysqlpp::Row row = res.at(i);
    ...
    {
      sql.clear();
      sql << "select adzone_id,member_id, adgroup_id as group_id, abs_pos, gender, sum(pv_count) as sum_impr, "
          <<" sum(click_count) as sum_click from rt_DZPMGGS  where time <= " << begin_time_pos  << " and time >=" << end_time_pos << " " 
          <<" group by adzone_id, adgroup_id, abs_pos, gender ";
      mysqlpp::StoreQueryResult res = QueryRunner(db.c_str(), CDbRServer, "rt_DZPMGGS").store(sql);
      if (!res || res.num_rows() <= 0) {
        MCE_WARN("DemoLoaderCache Load Error " << "rt_DZPMGGS" << ".  select  fail!");
      }
  ...
  try {
   ...
   // Connection app_conn("test","10.3.16.119","ad","rrad");
    map<string,pair<Ice::Long,Ice::Long> > app_spid_pool;
    {
      ...
      mysqlpp::StoreQueryResult res = QueryRunner(db.c_str(), CDbRServer, "app_demographic_ctr").store(sql);
      if (!res || res.num_rows() <= 0) {
        MCE_WARN("DemoLoaderCache Load Error " << "app_demographic_ctr" << ".  select  fail!");
      }
      MCE_INFO("[DemoLoaderCache]::Load   res size : " << res.num_rows());
      for (size_t i = 0; i < res.num_rows(); ++i) {
        mysqlpp::Row row = res.at(i);
    ...
    /*****************************app spid_type***********************************************/
    {
     ...
     sql << "select app as app_name, member_id as sponsor_id,abs_pos as pos_id, gender, stage, sum(pv_count) as sum_impr, "
      ...
      mysqlpp::StoreQueryResult res = QueryRunner(db.c_str(), CDbRServer, "Log_DABGGSD").store(sql);
      if (!res || res.num_rows() <= 0) {
        MCE_WARN("DemoLoaderCache Load Error " << "Log_DABGGSD" << ".  select  fail!");
      }
      MCE_INFO("[DemoLoaderCache]::Load   res size : " << res.num_rows());
      for (size_t i = 0; i < res.num_rows(); ++i) {
        mysqlpp::Row row = res.at(i);
...
void DemoLoaderCache::LoadAppSortPos(){
  ...
  try{
    ...
    Statement sql;
    {
      ...
      mysqlpp::StoreQueryResult res = QueryRunner(db.c_str(), CDbRServer, "Log_DABGGSD").store(sql);
      if (!res || res.num_rows() <= 0) {
        MCE_INFO("DemoLoaderCache Load AppSortPos Error " << "Log_DABGGSD" << ".  select  fail!");
      }
