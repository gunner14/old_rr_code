...
void ScoreAdCache::LoadAvgCpc() {   //获取 avg_cpc
  ...
  try{
    ...
    string db_q = AdDemoConfig::instance().GetDBQuality(); 
		string pattern = "adgroup_zone_factor";
    mysqlpp::StoreQueryResult res = QueryRunner(db_q.c_str(), CDbRServer, pattern.c_str()).store(sql); 
    if (!res) {
      MCE_WARN("LoadAvgCpcCache Error " << pattern << ".  select  fail!");
      return;
    }
...
void ScoreAdCache::LoadMap_group_cam() {
  ...
  try{
    ...
    sql << "select adgroup_id,campaign_id from adgroup_factor";
		string pattern = "adgroup_factor";
		string db_q = AdDemoConfig::instance().GetDBQuality();
    mysqlpp::StoreQueryResult res = QueryRunner(db_q.c_str(), CDbRServer, pattern.c_str()).store(sql);
    if (!res) {
      MCE_WARN("ScoreAdCache::Load LoadLink_group_camCache Error " << pattern << ".  select  fail!");
      return;
    }
...
void ScoreAdCache::LoadMap_cam_spon() { 
  ...
  try{
    ...
    string db_q = AdDemoConfig::instance().GetDBQuality();
		string pattern = "campaign_factor";
    mysqlpp::StoreQueryResult res = QueryRunner(db_q.c_str(), CDbRServer, pattern.c_str()).store(sql);
    if (!res) {
      MCE_WARN("ScoreAdCache::Link_cam_sponCache Error " << pattern << ".  select  fail!");
      return;
    }
...
void ScoreAdCache::LoadSponsor_score() {   //获取 sponsor_id 和 质量分
  ...
  try{
    ...
    sql << "select sponsor_id,total_paid from sponsor_factor";
		string db_q = AdDemoConfig::instance().GetDBQuality();
		string pattern = "sponsor_factor";
		mysqlpp::StoreQueryResult res = QueryRunner(db_q.c_str(), CDbRServer, pattern.c_str()).store(sql);
    if (!res) {
      MCE_WARN("ScoreAdCache::LoadSponsor_scoreCache Error " << pattern << ".  select  fail!");
      return;
    }
...
void ScoreAdCache::UpdateScoreAdPool() {    // 更新 quality_score
  ...
  try{
    ...
    string db_q = AdDemoConfig::instance().GetDBQuality();
    string pattern = "quality_score_factor";
    mysqlpp::StoreQueryResult res = QueryRunner(db_q.c_str(), CDbRServer, pattern.c_str()).store(sql);
    if (!res) {
      MCE_WARN("ScoreAdCache::Load UpdateScoreCache Error " << pattern << ".  select  fail!");
      return;
    }
...
void ScoreAdCache::LoadScore_status() {   //获取 sponsor_id 和 状态
  ...
  try{
    ...
    string db_q = AdDemoConfig::instance().GetDBQuality();
    string pattern = "sponsor_score_status";
    mysqlpp::StoreQueryResult res = QueryRunner(db_q.c_str(), CDbRServer, pattern.c_str()).store(sql);
    if (!res) {
      MCE_WARN("ScoreAdCache::LoadScore_status Error " << pattern << ".  select  fail!");
      return;
    }
...
void ScoreAdCache::LoadScoreAdPool() {    // 计算 quality_score
  ...
  try{
    ...
    mysqlpp::StoreQueryResult res = QueryRunner(db_q.c_str(), CDbRServer, pattern.c_str()).store(sql);
    if (!res) {
      MCE_WARN("ScoreAdCache::Load TmpCache Error " << pattern << ".  select  fail!");
      return;
    }
    MCE_INFO("ScoreAdCache::Load Rows Size=: " << res.num_rows());
    for (size_t i = 0; i < res.num_rows(); ++i) {
