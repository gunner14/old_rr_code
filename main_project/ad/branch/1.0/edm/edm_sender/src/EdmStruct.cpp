...
bool AdGroup::Available() {
  MCE_DEBUG("AdGroup::Available -->group_id:"<< group_id_<<"member_table_status_: " << member_table_status_ <<",campaign_status_:" << campaign_status_ << ",member_status_: "
  << member_status_ << ",am_status_:" << am_status_ << ",audit_status_: " << audit_status_ << ",delete_flag_: " << delete_flag_ 
  << ",daily_sys_status_:" << daily_sys_status_ << ",total_sys_status_: " << total_sys_status_ << ",point_valid_: " << point_valid_);
...
int AdGroup::edm_member_type(){//类型转换，根据member_category_推断客户类型。1是品牌 ；7是产品； 其他是自助
  if(1 == member_category_){
    return Brand_ad;
  } else if(7 == member_category_) {
    return Product_ad;
  } else {
    return Self_ad;
  }
