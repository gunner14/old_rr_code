...
ThriftAppender::ConnectorThread::ConnectorThread (
    ThriftAppender & socket_appender)
    : ta (socket_appender)
    , exit_flag (false)
{ }
...
void
ThriftAppender::ConnectorThread::run ()
{
    while (true)
    {
        ...
        getLogLog().debug (
            LOG4CPLUS_TEXT("ThriftAppender::ConnectorThread::run()")
        ...
        {
            thread::Guard guard (ta.access_mutex);
            if (ta.socket.isOpen ())
                continue;
        }
        ...
        helpers::Socket socket (ta.host, ta.port);
        if (! socket.isOpen ())
        {
            getLogLog().error(
                LOG4CPLUS_TEXT("ThriftAppender::ConnectorThread::run()")
        ...
        // Connection was successful, move the socket into ThriftAppender.
...
void
ThriftAppender::ConnectorThread::terminate ()
{
    {
        thread::Guard guard (access_mutex);
        exit_flag = true;
...
void
ThriftAppender::ConnectorThread::trigger ()
{
    trigger_ev.signal ();
}
...
//////////////////////////////////////////////////////////////////////////////
// ThriftAppender ctors and dtor
//////////////////////////////////////////////////////////////////////////////

ThriftAppender::ThriftAppender(const tstring& host_, int port_,
...
ThriftAppender::ThriftAppender(const helpers::Properties & properties)
 : Appender(properties),
   port(9998)
{
...
//////////////////////////////////////////////////////////////////////////////
// ThriftAppender public methods
//////////////////////////////////////////////////////////////////////////////

void 
ThriftAppender::close()
{
    getLogLog().debug(LOG4CPLUS_TEXT("Entering ThriftAppender::close()..."));
...
//////////////////////////////////////////////////////////////////////////////
// ThriftAppender protected methods
//////////////////////////////////////////////////////////////////////////////

void
ThriftAppender::openSocket()
{
    if(!socket.isOpen()) {
...
void
ThriftAppender::append(const spi::InternalLoggingEvent& event)
{
/*
#if ! defined (LOG4CPLUS_SINGLE_THREADED)
    if (! connected)
...
*/
    if(!socket.isOpen()) {
        openSocket();
        if(!socket.isOpen()) {
            getLogLog().error(LOG4CPLUS_TEXT("ThriftAppender::append()- Cannot connect to server"));
            return;
        }
    }
