...
bool DefaultHandler::handle(MatchHandlerParamter& para, AdResultMap& result) {
  ...
  if((100000000070 == para.adzone_->id() || 100000000072 == para.adzone_->id() || 100000000065 == para.adzone_->id()
      || 100000000093 == para.adzone_->id() ) && para.is_selected_vip_) {
  ...
  if (para.PickCount() > 0) {
   ...
   //   MCE_DEBUG("[MAIN_PROCESS] DefaultHandler::handle ==>NO DEFAULT  line:"<<__LINE__<<",adzone:" << adzone->id())
    ...
    if(100000000093 == para.adzone_->id() || 100000000065 == para.adzone_->id() ||
       100000000072 == para.adzone_->id() || 100000000070 == para.adzone_->id()){
      default_groups = para.adzone_->GetDefaultGroupsForSA(para.PickCount());
    } else {
      default_groups = para.adzone_->GetDefaultGroups(para.PickCount());
    }
...
bool StyleHandler::handle(MatchHandlerParamter& para, AdResultMap& result) {
  if( para.is_selected_vip_ && (100000000070 == para.adzone_->id() || 100000000072 == para.adzone_->id() ||
