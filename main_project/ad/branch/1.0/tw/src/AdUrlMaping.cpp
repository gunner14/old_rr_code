...
bool AdUrlMaping::Load() {
  string db = AdTwConfig::instance().DB();
  MCE_INFO("AdUrlMaping load ..., db = " << db);
  Statement sql;
  mysqlpp::StoreQueryResult res;
  sql << "select url,stage,adzone_id from url_adzone_maping where adzone_id in(100000000001, 100000000063, 100000000070, 100000000072)";
...
void AdUrlMaping::PreProcess(const string& Url, vector<string>& UrlSeq) {
  ...
  if(!ref_with_star.empty() && ref_with_star.compare(Url) != 0) {
  ...
  size_t found = ref_with_star.find_first_of("?");
  string ref;
  if(found != string::npos) {
    ref = ref_with_star.substr(0, found);
