...
void ::MyUtil::initialize() {
  ...
  TaskManager::instance().scheduleRepeated(&AdClickCache::instance());
}
...
AdQualityMap AdQualityManagerI::GetQualitysByZone(Ice::Long zone_id, const ::Ice::Current&) {
  ...
  MCE_INFO("GetQualitysByZone(" << zone_id << ") aqm.size=" << aqm.size() << "  used_time=" << ts.getTime());
  return aqm;
}
...
void AdQualityManagerI::Init() {
  ...
  MCE_INFO("AdQualityManagerI::Init --> AdMemberPool::Init() DONE");

  AdCampaignPool::instance().Init();
  MCE_INFO("AdQualityManagerI::Init-->  AdCampaignPool::Init() DONE");

  AdGroupPool::instance().Init();
  MCE_INFO("AdQualityManagerI::Init -->  AdGroupPool::Init() DONE");

  AdClickCache::instance().Reload();
  MCE_INFO("AdQualityManagerI::Init -->  AdClickCache::Init() DONE");
