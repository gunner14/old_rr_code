...
void AdSendSpeedLower::UpdateData(){
  ...
  if (!DBQuotaLoader::instance().Load(temp_ratio, CAMPAIGN_VEC, sql1, new CampaignComputeRatio(), campaign_last_update_time_, AD_CAMPAIGN_BALANCE_TABLE_NAME)){
    ...
    HttpQuotaLoader::instance().Load(temp_ratio, CAMPAIGN_VEC, new CampaignComputeRatio(), CAMPAIGN_URL);
  }
  ...
  if (!DBQuotaLoader::instance().Load(temp_ratio, MEMBER_VEC, sql2, new MemberComputeRatio(), member_last_update_time_, AD_MEMBER_BALANCE_TABLE_NAME)){
    ...
    HttpQuotaLoader::instance().Load(temp_ratio, MEMBER_VEC, new MemberComputeRatio(), MEMBER_URL);
  }
...
void AdSendSpeedLowerTimer::process()
{
  AdSendSpeedLower *adLower = (AdSendSpeedLower*)instance_;
  if(adLower == NULL)
  {
    MCE_WARN("AdSendSpeedLower::process get a NULL adLower!");
    return ;
  }
  adLower->UpdateData();
}

int AdSendSpeedLowerTimer::interval() const
{
  return ONE_MINIUTE;
}
