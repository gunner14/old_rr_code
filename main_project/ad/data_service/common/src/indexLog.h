...
namespace xce{
	namespace ad{
        ...
        void setAdNumAfterSelect(int an){
          ...
          for(std::vector<int>::iterator it = small_widget_num_.begin(); it != small_widget_num_.end(); ++it)
          {
            ostringstream s;
            s << *it << "xxx,";
            msg_ += s.str();
          ...
          for(std::vector<int>::iterator it = after_select_small_widget_num_.begin(); 
              it != after_select_small_widget_num_.end(); ++it)
          {
            ostringstream s;
            s << *it << "yyy,";
            msg_ += s.str();
        ...
        void setSmallWidgetNum(int num)
        {
          small_widget_num_.push_back(num);
        }

        void setSmallWidgetNumAfterSelect(int num){

          if(num == 0){
            after_select_small_widget_num_.push_back(small_widget_num_.back());
            return;
          }

          after_select_small_widget_num_.push_back(num);
        }
        ...
        void reset()
        {
          ...
          time_use_.clear();
          adv_num_.clear();
          after_select_adv_num_.clear();
          small_widget_num_.clear();
          after_select_small_widget_num_.clear();
        }
        ...
        std::vector<int> small_widget_num_;
        std::vector<int> after_select_small_widget_num_;
		};
