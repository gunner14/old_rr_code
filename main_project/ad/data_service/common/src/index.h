		...
		class AdPool : public MyUtil::Singleton<AdPool>
    {
      public:
        ...
        AdMemberPtr getAdMember(long id);
				...
				void UpdatePool(__gnu_cxx::hash_map<long, AdGroupPtr>& adgroup,
						__gnu_cxx::hash_map<long, AdCampaignPtr>& campaign_pool,
						__gnu_cxx::hash_map<long, AdMemberPtr>& member_pool);

        void AddAdGroup(AdGroupPtr);
        void AddAdCampaign(AdCampaignPtr);
        void AddAdMember(AdMemberPtr);

        void RemoveAdGroup(long id);
        void RemoveAdCampaign(long id);
        void RemoveAdMember(long id);

        std::vector<Ice::Long> getAdMemberIDList();
        std::vector<Ice::Long> getAdCampaignIDList();

        int LoadIndex(const std::string &level, std::vector<std::vector<std::string> >&items);
      ...
      private:
        ...
        int loadAdMembers(std::vector<std::vector<std::string> >&items);
        int loadAdCampaigns(std::map<long, AdCampaignPtr> &campaign_pool);
        int loadAdGroups(std::vector<std::vector<std::string> >&items);
        int loadAdCreatives(std::vector<std::vector<std::string> >&items);
        int loadAdZoneRef(std::vector<std::vector<std::string> >&items);
        int loadBidUnits(std::vector<std::vector<std::string> >&items);
