...
void DBSelector::outputDBConfig()
{
	...
	MCE_INFO("AD SOURCE: " << ad_db_source_ );
...
int DBSelector::loadAdMembers(__gnu_cxx::hash_map<long, AdMemberPtr> &member_pool)
{

	com::xiaonei::xce::Statement sql;
	sql << MEMBER_QUERY.c_str();
	mysqlpp::StoreQueryResult res;
	int ret = excuteQuery(sql, "admember", res, ad_db_source_);
	if(ret < 0)
	{
    ...
    if (res.num_rows() > 0) 
    {
      for (size_t i = 0; i < res.num_rows(); ++i) 
      {
        ...
        member_pool[member->member_id_] = member;
				MCE_DEBUG("DBSelector::loadMembers Valid member_id: " << member->member_id_ );
      }
...
int DBSelector::loadAdCampaigns(__gnu_cxx::hash_map<long, AdCampaignPtr> &campaign_pool)
{
  ...
  int ret = excuteQuery(sql, "adcampaign", res, ad_db_source_);
  if(ret < 0)
  {
    ...
    if (res.num_rows() > 0) 
    {
      for (size_t i = 0; i < res.num_rows(); ++i) 
      {
        ...
        campaign_pool[campaign->campaign_id_] = campaign;
				MCE_DEBUG("DBSelector::loadAdCampaigns Valid campagin_id: " << campaign->campaign_id_);
      }
...
int DBSelector::loadAdGroups(__gnu_cxx::hash_map<long, AdGroupPtr> &group_pool)
{
	...
	int ret = excuteQuery(sql, "adgroup", res, ad_db_source_);
	if(ret < 0)
	{
      ...
      for (size_t i = 0; i < res.num_rows(); ++i) 
      {
        ...
        if(point_valid)
        {
          group_pool[group->group_id_] = group;
					MCE_DEBUG("DBSelector::loadAdGroups: valid groupId: " << group->group_id_);
        }
...
int DBSelector::loadAdCreatives(__gnu_cxx::hash_map<long, AdGroupPtr> &group_pool)
{
  ...
  int cnt = 0;
	...
	int ret = excuteQuery(sql, "creative", res, ad_db_source_);
	if(ret < 0)
	{
	...
	MCE_DEBUG("DBSelector::loadAdCreatives row.size:" << res.num_rows());
...
int DBSelector::loadAdZoneRef(const __gnu_cxx::hash_map<long, AdGroupPtr> &group_pool)
{
  ...
  int cnt = 0;
	...
	int ret = excuteQuery(sql, "adgroup_adzone_ref", res, ad_db_source_);
	if(ret < 0)
	{
		MCE_WARN("DBSelector::LoadAdZone excuteQuery failed");
		return -1;
	}
	MCE_DEBUG("DBSelector::LoadAdZone From DB ,size:" << res.num_rows());
    ...
    if (res.num_rows() > 0) 
    {
      ...
      for (; i < res.num_rows(); ++i) 
      {
        ...
        if(it != group_pool.end())
        {
          ...
          ++cnt;
					MCE_DEBUG("DBSelector::LoadAdZone Valid groupid:"<< group_id << " ,zone_id:" << zone_id);
        }
      }
...
int DBSelector::loadBidUnits(std::map<long, TargetingIndex::FeatureIndexPtr> &zone_index)
{
  int cnt = 0;
	...
	mysqlpp::StoreQueryResult res;
	int ret = excuteQuery(sql, "bid_unit", res, ad_db_source_);
	if(ret < 0)
	{
...
*/
	...
	MCE_DEBUG("DBSelector::loadBidUnit size:" << res.num_rows());
  try{
    size_t i = 0;
    for (; i < res.num_rows(); ++i) 
    {
