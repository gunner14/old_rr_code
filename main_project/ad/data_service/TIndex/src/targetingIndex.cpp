...
//TODO 更新目前的倒排索引 
void TargetingIndex::UpdateZoneIndex(std::map<long, FeatureIndexPtr>& zone_index){
	MCE_INFO("TargetingIndex::ZoneIndex from Server size:" << zone_index.size());
...
int TargetingIndex::loadBidUnits()
{
	...
	MCE_DEBUG("TargetingIndex::ZoneIndex from DB size:" << zone_index.size());
	if(ret < 0)
	{
