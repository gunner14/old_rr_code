...
TagParser::TagParser(TagHandler* handler) :
	_handler(handler), _current(xml_node()), _root(xml_node()), _doc(0),
...
bool TagParser::feed(const SharedPtr& channel, const std::string& data) {
...
void TagParser::addTag(const SharedPtr& channel) {
	if (_root.empty()) {
...
bool TagParser::closeTag(const SharedPtr& channel) {
...
void TagParser::streamEvent(const SharedPtr& channel, xml_document_ptr& tag) {
	//_handler->handleTag(tag);
	_handler->messaged(channel, tag);
}
