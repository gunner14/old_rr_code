#include "UserNetworkReaderPreloader.h"
...
void MyUtil::initialize() {
	...
	string ctlEndpoint = ServiceI::instance().getCommunicator()->getProperties()->getPropertyWithDefault("UserNetworkReader.Controller", "ControllerUserNetworkReader");
        ...
        int timeout = ServiceI::instance().getCommunicator()->getProperties()->getPropertyAsIntWithDefault("UserNetworkReader.Proxy.Timeout", 300);
		
        int cluster = 10;
		
		GeneralPreloader<mop::hi::svc::model::NetworkReaderPrx>::instance().initialize(
