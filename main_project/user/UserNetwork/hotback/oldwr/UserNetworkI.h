...
protected:
	virtual void handle() {
		...
		if (minusTime > MAX_TIMEOUT) {
			...
			if (!dataI) {
				dataI = ServiceI::instance().locateObject<NetworkCacheDataPtr> (NETWORK_DATA, _userId);
				UserNetworkReaderAdapter::instance().setData(_userId, dataI->getData());
			}
		} else {
			ostringstream inMsg;
			...
			if (!_data) {
				...
				UserNetworkReaderAdapter::instance().setData(_userId, _data->getData());
			}
			_mutex.notify();
		}
	}
