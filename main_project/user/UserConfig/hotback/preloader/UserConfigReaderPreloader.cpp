#include "UserConfigReaderPreloader.h"
...
void MyUtil::initialize() {
	...
	string ctlEndpoint = ServiceI::instance().getCommunicator()->getProperties()->getPropertyWithDefault("UserConfigReader.Controller", "ControllerUserConfigReader");
        ...
        int timeout = ServiceI::instance().getCommunicator()->getProperties()->getPropertyAsIntWithDefault("UserConfigReader.Proxy.Timeout", 300);
		
        int cluster= 100;
		
		GeneralPreloader<mop::hi::svc::model::UserConfigReaderPrx>::instance().initialize(
