...
void MyUtil::initialize() {
	...
	UserApiCacheManagerI::instance().setValid(false);
	UserApiCacheManagerI::instance().set_mod(mod);
}

UserApiCacheManagerI::UserApiCacheManagerI() :_valid(false) {
}

UserApiCacheDataPtr UserApiCacheManagerI::getUserApiCache(::Ice::Int userId, const Ice::Current&) {
	MCE_INFO("UserApiCacheManagerI::getUserApiCache, userId=" << userId);
	int cluster = 4;
	if (userId % cluster == mod_) {
        	return UserApiCacheEntryPtr::dynamicCast(retention_->locate(userId))->GetUserApiCacheData();
	}
	return new UserApiCacheData;
}

Int2UserApiCacheData1Map UserApiCacheManagerI::getUserApiCache1List(const IntSeq& userIds, const Ice::Current&) {
        ObjectResultPtr objres = getUserApiCacheData(userIds);
        Int2UserApiCacheData1Map result;
        for (MyUtil::IntSeq::const_iterator it = userIds.begin(); it != userIds.end(); ++it) {
                ...
                if (hasRes != objres->data.end()) {
                        result[*it]=UserApiCacheEntryPtr::dynamicCast(hasRes->second)->GetUserApiCacheData1();
                } else {
