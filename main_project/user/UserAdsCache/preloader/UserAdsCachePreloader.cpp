#include "UserAdsCachePreloader.h"
...
using namespace xce::useradscachepreloader;

using namespace IceUtil;

void MyUtil::initialize() {
	ServiceI& service = ServiceI::instance();
	service.getAdapter()->add(&UserAdsCachePreloader::instance(), service.createIdentity("M", ""));
	string ctlEndpoint = ServiceI::instance().getCommunicator()->getProperties()->getPropertyWithDefault("UserAdsCache.Controller", "ControllerUserAdsCache");
		...
		GeneralPreloader<xce::useradscache::UserAdsCacheManagerPrx>::instance().initialize(
                new BatchUsersIdProducer,
                new UserAdsCachePreloaderFactory,
                ctlEndpoint,
				cluster,
				subInterval,
                timeout,
...
//--------------------------UserAdsCachePreloaderFactory::create-----------------------------------------------------
MyUtil::ObjectResultPtr UserAdsCachePreloaderFactory::create(Ice::Int minId,Ice::Int maxId) {
	...
	/*Statement sql;
	sql << "SELECT id, status, stage, gender, birth_year, birth_month, birth_day, home_province, home_city FROM user_basic WHERE id >= "<<minId<<" AND id <= "<<maxId;
	BatchUserBasicResultHandlerI handler(results);
	QueryRunner(DB_USER_BASIC, CDbRServer).query(sql,handler);*/
...
MyUtil::ObjectResultPtr UserAdsCachePreloaderFactory::create(const MyUtil::IntSeq& ids) {
	ObjectResultPtr results = new ObjectResult;
