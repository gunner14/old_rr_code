...
ObjectResultPtr UserUrlPreloaderFactory::create(Ice::Int minId,Ice::Int maxId) {
	...
	Statement sql;
	sql << "SELECT id, largeurl, mainurl, headurl, tinyurl, flexurl FROM " << TABLE_USER_URL << " WHERE id >= "<< minId <<" AND id <= "<< maxId;
...
ObjectResultPtr UserUrlPreloaderFactory::create(const MyUtil::IntSeq& ids) {
	...
	Statement sql;
	sql << "SELECT id, largeurl, mainurl, headurl, tinyurl, flexurl FROM " << TABLE_USER_URL << "WHERE id IN (  ";
