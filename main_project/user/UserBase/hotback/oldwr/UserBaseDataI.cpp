...
void UserStateDataI::setProperty(const string& strKey, const string& iVal)
{
    if (!strcasecmp(strKey.c_str(), CUSID.c_str())){
        ...
        state = boost::lexical_cast<Ice::Long>(iVal);
    }
}
...
{
  if(obj->ice_isA("::xce::userbase::UserStateDataN")){
	  UserStateDataNPtr data = UserStateDataNPtr::dynamicCast(obj);
    id = data->id;
    level = data->level;
    star = data->star;
    state = data->state;
  } else if(obj->ice_isA("::xce::userbase::UserStateData")) {
    ...
    MCE_WARN("UserStateDataI::parse Unknown type : " + obj->ice_id());
  }
}
...
void UserStateDataI::parse(mysqlpp::Row& row )
{
    ...
    state = (long)(row[CUSSTATE.c_str()]);
}

UserStateDataPtr UserStateDataI::getOldData()
{
	IceUtil::Mutex::Lock lock(m_mutex);
	UserStateDataPtr res = new UserStateData();
	res->id = id;
	res->star= star; 
	res->state = (int)state;
	res->level = level;
...
{
  if(obj->ice_isA("::xce::userbase::UserUrlDataN")) {
    UserUrlDataNPtr data = UserUrlDataNPtr::dynamicCast(obj);
    id = data->id ;
  ...
  } else if (obj->ice_isA("::xce::userbase::UserUrlData")) {
    ...
    MCE_WARN("UserUrlDataI::parse UnKnown type : " + obj->ice_id());
  }
...
void UserUrlDataI::parse(mysqlpp::Row& row )
{
  ...
  flexUrl = (string) row["flexurl"];
}

UserUrlDataPtr UserUrlDataI::getOldData() {
	IceUtil::RWRecMutex::RLock lock(_mutex);
	UserUrlDataPtr newData = new UserUrlData;
	newData->id = id;
...
bool UserUrlResultHandlerI::handle(mysqlpp::Row& row) const {
	...
	_result->flexUrl = (string) row["flexurl"];
	return true;
}
//---------------------------------------------------------------------------
...
UserBaseViewDataPtr UserBaseViewDataI::getOldData() {
  UserBaseViewDataPtr result = new UserBaseViewData;
  result->upd = updi_;
  result->und = undi_;
  result->urd = urdi_;
  result->ulcd = ulcdi_;
  result->usd = usdi_->getOldData();
  result->userurl = uurldi_->getOldData();
  result->userbasic = ubdi_;
  return result;
}

UserBaseViewData3Ptr UserBaseViewDataI::getData() {
  UserBaseViewData3Ptr result = new UserBaseViewData3;
  result->upd = updi_;
  result->und = undi_;
  result->urd = urdi_;
  result->ulcd = ulcdi_;
  result->usd = usdi_;
  result->userurl = uurldi_;
  result->userbasic = ubdi_;
  return result;
}

UserCommonViewDataPtr UserCommonViewDataI::getOldData() {
  UserCommonViewDataPtr result = new UserCommonViewData;
  result->userpassport = updi_;
  result->username = undi_;
  result->userright = urdi_;
  result->userlogincount = ulcdi_;
  result->userstate = usdi_->getOldData();
  result->userurl = uurldi_->getOldData();
  result->stage = usgd_->stage;
  result->univ = usgd_->univ;
  result->gender = ubod_->gender;
  return result;
}

UserCommonViewDataNPtr UserCommonViewDataI::getData() {
  UserCommonViewDataNPtr result = new UserCommonViewDataN;
  result->userpassport = updi_;
  result->username = undi_;
  result->userright = urdi_;
  result->userlogincount = ulcdi_;
  result->userstate = usdi_->getData();
  result->userurl = uurldi_->getData();
  result->stage = usgd_->stage;
  result->univ = usgd_->univ;
  result->gender = ubod_->gender;
  return result;
