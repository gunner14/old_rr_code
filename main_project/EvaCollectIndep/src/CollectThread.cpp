...
void EvaCollector::initialize(string loggerEndpoints) {	
	...
	logger_ = ::com::xiaonei::service::XceLoggerAdapter::instance().getLogger(loggerEndpoints);
...
void Stat::merge() {
	...
	//cout<<"add Log size is "<<tails_.size()<<std::endl;
		...
		if(statMsg.find("record_99") != string::npos){	//统计99％的stat，并且到达指定之间则打印log
			...
			//	os << " " <<recordMap.size() << " " << statMsg;		
				...
				try {
                	         	...
                	         	cout << "[IceThread]:  Ice::Exception " << e.what() << std::endl;
		                 }catch(std::exception& e){
                	         	cout << "[IceThread]:  td::exception " << std::endl;
		                 }catch(...){
                	         	cout << "[IceThread]:  Exception " << std::endl;
		                 }				
			}
		 }else {
...
/* void StatThreadPool::output(const string& serviceName, const std::vector<LogCount>& logVector, ::xce::eva::log::XceLoggerPtr logger, const std::vector<std::string>& loggerCategories){
        string statMsg;
...
void LogHolder::OutputThread::run() {
	...
	while (true) {		
		...
		Stat stat(t);
		stat.merge();
	//	std::cout<<"printLog"<<time(NULL)<<std::endl;
