...
#define XAR_IMPL(name) \
  size_t PIDX_##name = xce::pcount::instance().push_add(#name, 0)

// 一般用在构造列表
#define XAR_INIT(name) \
  PIDX_##name(xce::pcount::instance().push_add(#name, 0))

#define XAR_INC(name) \
  xce::pcount::instance().push_inc(PIDX_##name)

#define XAR_DEC(name) \
  xce::pcount::instance().push_dec(PIDX_##name)

// 注册function, xar::output 时调用
#define XAR_POLL(name, func) \
  xce::pcount::instance().poll_add(name, func)
