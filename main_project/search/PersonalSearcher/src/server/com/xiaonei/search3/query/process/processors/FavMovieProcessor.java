 ...
 **/
public class FavMovieProcessor {
	...
	private static List<Query> parseBasic(Map<String, Map<String, String>> map) {
		...
		for (Entry<String, Map<String, String>> outerEntry : map.entrySet()) {
			// [favMovie-->[favMovie-->篮球]]
			String typeOuter = outerEntry.getKey();
			if (typeOuter.equals("favMovie")) {
				// [favMovie-->0]
				for (Entry<String, String> innerEntry : outerEntry.getValue()
						.entrySet()) {
					String typeInner = innerEntry.getKey();
					if (typeInner.equals("favMovie")) {
						Query query = new TermQuery(new Term(UserFieldNames.FAV_MOVIE,
								innerEntry.getValue()));
						queryList.add(query);
					}
					break;// 很重要
	...
	public static void main(String args[]) {
		...
		innerMap.put("favMovie", "You Raise Me Up");
		innerMap.put("favMovie", "You Raise Me Up");
		outerMap.put("favMovie", innerMap);
		List<Query> queryAll = FavMovieProcessor.process(outerMap, 2.0f);
