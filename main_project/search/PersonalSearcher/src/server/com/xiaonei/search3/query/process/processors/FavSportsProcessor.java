 ...
 **/
public class FavSportsProcessor {
	...
	private static List<Query> parseBasic(Map<String, Map<String, String>> map) {
		...
		for (Entry<String, Map<String, String>> outerEntry : map.entrySet()) {
			// [favSports-->[favSports-->篮球]]
			String typeOuter = outerEntry.getKey();
			if (typeOuter.equals("favSports")) {
				// [favSports-->0]
				for (Entry<String, String> innerEntry : outerEntry.getValue()
						.entrySet()) {
					String typeInner = innerEntry.getKey();
					if (typeInner.equals("favSports")) {
						Query query = new TermQuery(new Term(UserFieldNames.FAV_SPORT,
								innerEntry.getValue()));
						queryList.add(query);
					}
					break;// 很重要
	...
	public static void main(String args[]) {
		...
		innerMap.put("favSports", "You Raise Me Up");
		innerMap.put("favSports", "You Raise Me Up");
		outerMap.put("favSports", innerMap);
		List<Query> queryAll = FavSportsProcessor.process(outerMap, 2.0f);
