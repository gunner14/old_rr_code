 ...
 **/
public class FavMusicProcessor {
	...
	private static List<Query> parseBasic(Map<String, Map<String, String>> map) {
		...
		for (Entry<String, Map<String, String>> outerEntry : map.entrySet()) {
			// [favMusic-->[favMusic-->篮球]]
			String typeOuter = outerEntry.getKey();
			if (typeOuter.equals("favMusic")) {
				// [favMusic-->0]
				for (Entry<String, String> innerEntry : outerEntry.getValue()
						.entrySet()) {
					String typeInner = innerEntry.getKey();
					if (typeInner.equals("favMusic")) {
						Query query = new TermQuery(new Term(UserFieldNames.FAV_MUSIC,
								innerEntry.getValue()));
						queryList.add(query);
					}
					break;// 很重要
	...
	public static void main(String args[]) {
		...
		List<Query> queryAll = FavMusicProcessor.process(outerMap, 2.0f);
