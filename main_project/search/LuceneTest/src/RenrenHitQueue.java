...
final class RenrenHitQueue extends PriorityQueue<ScoreDoc> {

	private boolean prePopulate;

	/**
	 * Creates a new instance with <code>size</code> elements. If
	 * <code>prePopulate</code> is set to true, the queue will pre-populate
	 * itself with sentinel objects and set its {@link #size()} to
	 * <code>size</code>. In that case, you should not rely on {@link #size()}
	 * to get the number of actual elements that were added to the queue, but
	 * keep track yourself.<br>
	 * <b>NOTE:</b> in case <code>prePopulate</code> is true, you should pop
	 * elements from the queue using the following code example:
	 * 
	 * <pre>
	 * PriorityQueue pq = new RenrenHitQueue(10, true); // pre-populate.
	 * ScoreDoc top = pq.top();
	 * 
	 ...
	 * 
	 * <p>
	 * <b>NOTE</b>: This class pre-allocate a full array of length
	 * <code>size</code>.
	 * 
	 * @param size
	 *            the requested size of this queue.
	 * @param prePopulate
	 *            specifies whether to pre-populate the queue with sentinel
	 *            values.
	 * @see #getSentinelObject()
	 */
	RenrenHitQueue(int size, boolean prePopulate) {
