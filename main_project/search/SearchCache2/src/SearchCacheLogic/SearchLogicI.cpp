...
void initialize() {
	...
	int cluster = service.getCommunicator()->getProperties()->getPropertyAsInt("SearchLogic.Cluster");
	int mod = service.getCommunicator()->getProperties()->getPropertyAsInt("SearchLogic.Mod");
	string dsn  = service.getCommunicator()->getProperties()->getPropertyWithDefault("SearchLogic.Dsn", TOKYOTYRANT_DSN_PREFIX"127.0.0.1:1982");//1981原来
	MCE_DEBUG("SearchLogic data source name: " << dsn );
  //initialize=1，读取全部数据库，初始化SearchLogic
  //注意，在配置search_searchpersistentcache.xml时，只能指定一个服务进行初始化工作 
  int initialize = service.getCommunicator()->getProperties()->getPropertyAsInt("SearchLogic.Initialize");
  ...
  ServerStateSubscriber::instance().initialize("ControllerSearchLogic", &xce::searchcache::SearchLogicManagerI::instance(), mod);
  ...
  xce::searchcache::SearchLogicManagerI::instance().bootstrap(cluster, mod, dsn, initialize);

	service.registerXceObserver(new xce::searchcache::ServiceSchemaI);

  //配置日志TT
  ostringstream update_dsn("SearchLogic.LogTTDsn");
  string strDsn = service.getCommunicator()->getProperties()->getProperty(update_dsn.str());
  MCE_INFO("SearchLogic " << update_dsn.str()  << " data source name: " << strDsn);
...
const int kMessageQueueThreadNum = 500;
...
void SearchLogicManagerI::bootstrap(int cluster, int mod, string dsn, int initialize) {
  _cluster = cluster;
  _mod = mod;
  _dsn = dsn;
  ...
  for (int i=0; i<_cluster; ++i) {
    ostringstream dsn; 
    dsn << "SearchLogic.InitDsn" << i;
    //获得需要初始化的数据源
    //数据源格式：SearchLogic.InitDsn0=tokyotyrant://127.0.0.1:1981
    //有多少个cluster就有多少个数据源，数据源的其实id=0
    string strDSN = service.getCommunicator()->getProperties()->getProperty(dsn.str());
    MCE_DEBUG("SearchLogic " << dsn.str()  << " data source name: " << strDSN);
...
void SearchLogicManagerI::load(const ::MyUtil::IntSeq& ids, const Ice::Current& ic) {
  const Ice::ConnectionPtr con = ic.con;
  if (con.get() != NULL) {
    MCE_DEBUG("SearchLogicManagerI::load " << ids.size() << " objects. invoker is :" << con->toString() );
  }
...
void LoadUpdateDataToDbCacheTask::handle() {
  ...
  for (MyUtil::LongSeq::const_iterator it = ids.begin(); it != ids.end(); ++it) {
    ...
    if (NULL != obj.get()) {
      boost::shared_ptr<IDbCache> db_handler = SearchLogicManagerI::instance().getDbHandler(userId); 
      if (db_handler) {
        if (obj->_status < 3) {
          ...
          MCE_WARN("the user "<<userId<<" status is "<<obj->_status);
...
void UpdateViewCountTask::handle() {
  ...
  MCE_DEBUG("UpdateViewCountStartTime is "<<string_start_time);
  ...
  MCE_DEBUG("UpdateViewCountTimeInterval: " << dwTimeInterval<<" hours");

  while(true) {

    current_time = time(NULL);
    MCE_DEBUG("[TEMPLE] current_time="<<current_time<<", start_time="<<start_time);
    if (current_time >= start_time) {
      MCE_DEBUG("[TEMPLE] current_time>=start_time");
      break;
    }
...
void FillTask::handle() {
        ...
        if (db_handler) {
          if (obj->_status < 3) {
            ...
            if (!db_handler->Remove(userId)) {
              ...
              MCE_WARN(userId<<" status is "<<obj->_status<<", remove form db");
              try {
  ...
  //SearchLogicManagerI::instance().setValid(true);
  ...
  try{
...

    {
      for (MyUtil::LongSeq::const_iterator it = ids.begin(); it != ids.end(); ++it) {
        long userId = (*it);
        SearchCacheDataPtr obj = ServiceI::instance().findObject<SearchCacheDataPtr>(SEARCH_CACHE,userId); 
        MCE_DEBUG("[UpdatePCAccordingIds] "<<" Id: "<< userId<<" Network size: "<< obj->_networks.size());
        if (NULL != obj.get()) {
          boost::shared_ptr<IDbCache> db_handler = SearchLogicManagerI::instance().getDbHandler(userId); 
          if (db_handler) {
        ...
        //先遍历看是否有关心的字段
        {
          ...
          for(map<string, string>::const_iterator fieldIter=updateUnitIter->fieldPairMap.begin(); fieldIter!=updateUnitIter->fieldPairMap.end(); fieldIter++) {
            ...
            if (0 == tableName.compare("user_names") && 0 == (fieldIter->first).compare("NAME") && 0 == (fieldIter->second).compare("")) {
              MCE_WARN("NAME is empty, so dump it");
              break;
            }
            ...
            MCE_DEBUG("[UpdataPCAccordingFieldMap]"<<" Id: "<<updateUnitIter->id<<" Table_Field: "<<table_name_and_field); 
...
void UpdateTask::UpdateSpecialTable(MyUtil::LongSeq& uids, Statement& sql, void (SearchCacheData::*pFunc)(vector<mysqlpp::Row>& rows), MyUtil::LongSeq& queryFailedIds, MyUtil::LongSeq& updateLogTTIds, MyUtil::LongSeq& deleteIds, Int2ByteSeq& id2BytesMap) {
  ...
  while (it!=ids.end()) {
    boost::shared_ptr<IDbCache> db_handler = SearchLogicManagerI::instance().getDbHandler(*it); 
    if (db_handler) {
      SearchCacheDataPtr obj = new (std::nothrow) SearchCacheData;
