...
namespace xce {
  namespace searchcache {
    ...
    class SearchPersistentCacheManagerI: virtual public SearchCacheManager,
    public MyUtil::Singleton<SearchPersistentCacheManagerI> {
     public:
       SearchPersistentCacheManagerI();
     ...
     public:
      ...
      void run() {
        ...
        while(true) {
          ...
          if (!batch.empty())  {
            MCE_INFO("befor handle, mod=" << _mod << ", _cache.size()=" << _cache.size() << ", batch.size()=" << batch.size());
            update_task.handle(batch);
            MCE_INFO("after handle, mod=" << _mod << ", _cache.size()=" << _cache.size());
          }
        }
      }
     private:
