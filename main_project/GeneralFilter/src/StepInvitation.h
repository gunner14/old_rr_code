#ifndef __STEPINVITAION_H__
#define __STEPINVITAION_H__
...
namespace gf
{
	...
	static const int MAX_COUNT_INVITATION = 20;
	static int OVER_COUNT_INVITATION = 12;
	
    static int MAX_MARK_INVITATION = 10000;
    static int DEC_MARK_TIMESLOT_INVITATION = 60*5;
    static int MAX_DEC_MARK_INVITATION = 15;
    
    static double ALPHA_INVITATION = 0.5;//加倍系数
    static double BETA_INVITATION = 15; //指数分母
    
	class StepInvitation :public Step
	{
	protected:
		IceUtil::Mutex _lock;
	public:
		
		int steps[MAX_COUNT_INVITATION];
		int currentTime;
		int index;
		int avg;
		int likeAvgCount;
	...
	/*		if (++dayCount >= 300) {
			...
			MCE_DEBUG("call add");
			index = (index+1)% MAX_COUNT_INVITATION;
			total -= steps[index];
			steps[index] = time - currentTime;
			total += steps[index];
			currentTime = time;
			avg = total / MAX_COUNT_INVITATION;
		...
		/*	if (dayCount >= 300)	
			...
			if(likeAvgCount >= OVER_COUNT_INVITATION)
			{
				likeAvgCount = 0;
				for(int i = 0; i< MAX_COUNT_INVITATION; ++i)
				{
					if(abs(avg - steps[i]) <= 1) likeAvgCount ++;
				}
			}
			MCE_DEBUG("call getAvgCount:"<<likeAvgCount);
			if((likeAvgCount >= OVER_COUNT_INVITATION) || (mark > 0)) 
			{
		...
		void incMark()
		{
		    ...
		    if(mark > MAX_MARK_INVITATION) return;
		    mark += (int)(ALPHA_INVITATION * pow(2,(double)mistakeCount/(double)BETA_INVITATION));
		    mistakeCount++;
		    decMarkCount = 0;
		...
		void decMark()
		{
		    ...
		    mark = mark - (steps[index]/DEC_MARK_TIMESLOT_INVITATION)*MAX_DEC_MARK_INVITATION -1;
		    if(mark < 0) mark = 0;
