#ifndef __STEPJOINAGROUP_H__
#define __STEPJOINAGROUP_H__
...
namespace gf
{
	...
	static const int MAX_COUNT_JOINAGROUP = 20;
	static int OVER_COUNT_JOINAGROUP = 12;
	
    static int MAX_MARK_JOINAGROUP = 10000;
    static int DEC_MARK_TIMESLOT_JOINAGROUP = 60*5;
    static int MAX_DEC_MARK_JOINAGROUP = 15;
    
    static double ALPHA_JOINAGROUP = 0.5;//加倍系数
    static double BETA_JOINAGROUP = 15; //指数分母
    
	class StepJoinAGroup :public Step
	{
	protected:
		IceUtil::Mutex _lock;
	public:
		
		int steps[MAX_COUNT_JOINAGROUP];
		int currentTime;
		int index;
		int avg;
		int likeAvgCount;
	...
	/*		if (++dayCount >= 300) {
			...
			MCE_DEBUG("call add");
			index = (index+1)% MAX_COUNT_JOINAGROUP;
			total -= steps[index];
			steps[index] = time - currentTime;
			total += steps[index];
			currentTime = time;
			avg = total / MAX_COUNT_JOINAGROUP;
		...
		/*	if (dayCount >= 300)	
			...
			if(likeAvgCount >= OVER_COUNT_JOINAGROUP)
			{
				likeAvgCount = 0;
				for(int i = 0; i< MAX_COUNT_JOINAGROUP; ++i)
				{
					if(abs(avg - steps[i]) <= 1) likeAvgCount ++;
				}
			}
			MCE_DEBUG("call getAvgCount:"<<likeAvgCount);
			if((likeAvgCount >= OVER_COUNT_JOINAGROUP) || (mark > 0)) 
			{
		...
		void incMark()
		{
		    ...
		    if(mark > MAX_MARK_JOINAGROUP) return;
		    mark += (int)(ALPHA_JOINAGROUP * pow(2,(double)mistakeCount/(double)BETA_JOINAGROUP));
		    mistakeCount++;
		    decMarkCount = 0;
		...
		void decMark()
		{
		    ...
		    mark = mark - (steps[index]/DEC_MARK_TIMESLOT_JOINAGROUP)*MAX_DEC_MARK_JOINAGROUP -1;
		    if(mark < 0) mark = 0;
