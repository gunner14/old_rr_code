...
#ifndef _DECAF_UTIL_CONCURRENTCOMPUTINGMAP_H_
#define _DECAF_UTIL_CONCURRENTCOMPUTINGMAP_H_
...
namespace concurrent{
	...
	//copy from ConcurrentStlMap.h
    template <typename K, typename V, typename COMPARATOR = std::less<K> >
    class ConcurrentComputingMap : public ConcurrentMap<K, V, COMPARATOR> {
    private:
        std::map<K,V,COMPARATOR> valueMap;
     //   mutable concurrent::Mutex mutex;
        boost::function<bool (const K& ,V&)> createFunction;
    ...
    public:
         ...
         */
        // ConcurrentComputingMap() : ConcurrentMap<K,V,COMPARATOR>(), valueMap(), mutex(), createFunction(NULL){}


    
        ConcurrentComputingMap(boost::function<bool (const K& ,V&)> createFunc) 
		: ConcurrentMap<K,V,COMPARATOR>(), valueMap(),  createFunction(createFunc){}
         ...
         */
        ConcurrentComputingMap( const ConcurrentComputingMap& source ) :
            ConcurrentMap<K,V,COMPARATOR>(), valueMap()  {
         ...
         * @param source The source map.
         */
        ConcurrentComputingMap( const Map<K,V,COMPARATOR>& source ) :
            ConcurrentMap<K,V,COMPARATOR>(), valueMap() {

            copy( source );
        }

        virtual ~ConcurrentComputingMap() {}

        /**
         * {@inheritDoc}
         */
	virtual bool equals( const ConcurrentComputingMap& source ) const {
            {
         ...
         * {@inheritDoc}
         */
        virtual void copy( const ConcurrentComputingMap& source ) {
            {
                IceUtil::Mutex::Lock lock(mutex_);
            //synchronized( &mutex ) {
                this->valueMap.clear();
        ...
        virtual V& get( const K& key ) {
            ...
            //synchronized( &mutex ) {
                ...
                if( iter != valueMap.end() ){
                    ...
                    V value;
                    if(createFunction!=NULL && createFunction(key,value))
                        return (valueMap.insert(std::make_pair<K,V>(key,value)).first)->second;
                }
            }
         ...
         * {@inheritDoc}
         */
        virtual void putAll( const ConcurrentComputingMap<K,V,COMPARATOR>& other ) {
            { 
                IceUtil::Mutex::Lock lock(mutex_);
