...
//typedef set<AdGroupPtr> AdGroupSet;

class SubGroup: public IceUtil::Shared {
public:

  pair<int, int> Score() {
    Ice::Long gid = group->group_id_;

    int score = sqrt((double)click * 1000000 / pv * group->max_price_);
    int quality = sqrt((double)click * 1000000 / pv);
//    float score_f = sqrt((double)click * 1000000 / pv * group->max_price_);
//    MCE_DEBUG("SubGroup::Score --> gid = " << group->group_id_
//        <<",key = "<< key
//        << ",max_price_ = " << group->max_price_ <<
//        ",click = " << click << ",pv = " << pv <<
//        ",score = " << score << ",score_f = " << score_f <<
//        ",quality = " << quality <<
//        ",key = " << key);

    return make_pair(score, quality);
  }
  int click;
  int pv;
  AdGroupPtr group;
...
typedef IceUtil::Handle<SubGroup> SubGroupPtr;
...
//      MCE_DEBUG("AdSet::Click --> can not find group in this set, group = " << gid );
  ...
  AdSetPtr operator &(const AdSet& p2) {
    ...
    while (it1 != set_.end() && it2 != p2.set_.end()) {
      ...
      //MCE_DEBUG( __FILE__ << ":" << __LINE__ << ", it1:" << it1->first << ", it2:" << it2->first);
    }
    return res;
  }
