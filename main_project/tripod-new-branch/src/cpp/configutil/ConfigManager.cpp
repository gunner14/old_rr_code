...
bool ConfigManager::updateCacheConfig(const ConfigDataPtr& config, zhandle_t* zk, bool checkVersion, bool& updateFlag) {
    ...
    try
    {
        ...
        if(checkVersion) {
            ...
            for(std::map<std::string, ConfigDataPtr>::const_iterator it = config->childrenDatas_.begin(); it != config->childrenDatas_.end(); ++it) {
                std::map<std::string, ConfigDataPtr>::const_iterator findIt = childrenConfig->childrenDatas_.find(it->first);
                if(findIt != childrenConfig->childrenDatas_.end()) {
                    if(it->second->version_ != findIt->second->version_) {
                        it->second->version_ = findIt->second->version_;
                        std::ostringstream oss1;
                        oss1<<oss.str()<<"/"<<it->first;
                        bool sucChildrenFlag = getChildrenDatas(it->second, zk, oss1, true); 
                        if(sucChildrenFlag == false) {
            ...
            for(std::map<std::string, ConfigDataPtr>::const_iterator it = childrenConfig->childrenDatas_.begin(); it != childrenConfig->childrenDatas_.end(); ++it) {
                std::ostringstream oss1;
                oss1<<oss.str()<<"/"<<it->first;
                bool sucGrandsonFlag = getChildrenDatas(it->second, zk, oss1);
                if(sucGrandsonFlag == false) {
                    MCE_WARN("ConfigManager::updateCacheConfig get namespace " << it->first << " children failed");
                    return false;
                }
                config->childrenDatas_.insert(make_pair(it->first, it->second));
            }
        }
    }catch(...)
    {
