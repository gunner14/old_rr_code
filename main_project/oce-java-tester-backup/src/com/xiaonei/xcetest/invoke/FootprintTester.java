...
public class FootprintTester extends TestCase {
	...
	public void testSingle() throws Exception {
		...
		List<FootprintView> fs = AdapterFactory.getFootprintAdapter().getAll(
				host);
	...
	public void testGet() throws FootprintException {
		int hostId = 700000027;
		List<FootprintView> fs = AdapterFactory.getFootprintAdapter().getAll(
				hostId);
		for (FootprintView fv : fs) {
			System.out.println(":" + fv.toString());
		}
//		Channel channel = Channel.newOceChannel("Footprint");
//		FootprintManagerPrx manager = FootprintManagerPrxHelper
//				.checkedCast(channel.getCommunicator().stringToProxy(
//						"M@Footprint7"));
//		Footprint[] res = manager.getAll(hostId, 0, -1);
//		System.out.println(res.length);
	}

	public void testTribe() throws FootprintException {
		int tribeId = 200190004;
		List<FootprintView> fs = AdapterFactory.getTribeFootprintAdapter()
				.getAll(tribeId);
		for (FootprintView fv : fs) {
			System.out.println(fv.toString());
		}
