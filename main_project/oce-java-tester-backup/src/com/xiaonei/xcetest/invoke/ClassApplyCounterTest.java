...
public class ClassApplyCounterTest extends TestCase {
	...
	public void tearDown() {
		...
		for (ClassApply apply : result) {
			System.out.println(apply.getClassId() + ", " + apply.getClassName()
					+ ", " + apply.getCount());
		}
	}
	...
	public void testLiXu() throws Exception {
		adapter.sendApply(applicant, 4266);
		ClassApply[] result = adapter.getCounterSeq(admin);
		if (result.length > 0) {
			ClassApply ca = result[0];
			System.out.println("Orig: " + ca.getClassId() + ", " + ca.getClassName() + ", " + ca.getCount());
		} else {
	...
	public void testClassApplyCounter() throws Exception {
		...
		ClassApply ca = result[0];
		System.out.println(ca.getClassId() + ", " + ca.getClassName() + ", "
				+ ca.getCount());
