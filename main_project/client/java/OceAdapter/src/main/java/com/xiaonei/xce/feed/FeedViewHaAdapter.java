...
public class FeedViewHaAdapter extends ReplicatedClusterAdapter {
	...
	public List<FeedData> getUnlimitedDataByStype(int uid, int begin, int limit, int[] types, int render, int view) {
        ...
        public List<String> getTimelineDataWithAdminByStypes(int ownerid, int uid, 
            String name, int year, int month, int render, int begin, int limit, boolean isAdmin, int[] stypes) {
                String[] html = null;
                FeedViewPrx prx = null;
                try {
                        prx = getManager(uid);
                        html = prx.GetTimelineDataWithAdminByStypes(ownerid, uid, name, year, month, render, begin, limit, isAdmin, stypes);
                } catch (Exception e) {
                        String err = this.getClass().getName() + ".getTimelineDataWithAdminByStypes [" + prx + "] uid=" + uid;
                        Oce.getLogger().error(err + " " + e);
                        throw new RuntimeException(err + " " + e, e);
                }
                List<String> res = new ArrayList<String>();
