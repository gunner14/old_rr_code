...
public class BuddyByAddTimeCacheAdapter<T extends BuddyByAddTime> extends
     ...
     * @return
     */
    public List<T> getFriendListAndTimeWithBegin(int userId, int begin, int limit) {
        TimeCost stat = TimeCost.begin(XceStat.getLogger(), "BuddyByAddTimeCache " + userId);
        String prxstr = "NULL Prx";
        try {
            List<T> result = new ArrayList<T>();
            BuddyByAddTimeCacheManagerPrx prx = getBuddyByAddTimeCacheManagerTwoway(userId);
            prxstr = prx.ice_toString();
            BuddyByAddTimeNode[] ret = prx.getFriendListAndTimeWithBegin(userId, begin, limit);
            for (BuddyByAddTimeNode one : ret) {
                T obj = _factory.create();
                obj.setBase(one.userId, one.time);
                result.add(obj);
            ...
            System.err.println("BuddyByAddTimeFriends getFriendListAndTimeWithBegin(" + userId
                    + ") Ice.ConnectionTimeOutException " + prxstr);
            stat.endException(e);
        } catch (Exception e) {
            e.printStackTrace();
            stat.endException(e);
	...
	public static void main(String[] args) {
        ...
        Map<Integer, List<Integer>> listResBatch = BuddyByAddTimeCacheAdapter.getInstance()
                .getFriendLists(batch);
        int countRes1 = BuddyByAddTimeCacheAdapter.getInstance().getFriendCount(userId1);
        int countRes2 = BuddyByAddTimeCacheAdapter.getInstance().getFriendCount(userId2);
        Map<Integer, Integer> countResBatch = BuddyByAddTimeCacheAdapter.getInstance()
                .getFriendCounts(batch);
        List<DefaultBuddyByAddTime> ret = BuddyByAddTimeCacheAdapter.getInstance()
                .getFriendListAndTimeWithBegin(userId2, 10, 8);
        StringBuilder sb = new StringBuilder();
        sb.append(userId2 + " [ ");
        for (DefaultBuddyByAddTime item : ret) {
            sb.append(item.getUserId() + "---" + item.getTime() + " ");
        }
        sb.append("]");
        System.out.println("--getFriendListAndTimeWithBegin() " + sb.toString());
