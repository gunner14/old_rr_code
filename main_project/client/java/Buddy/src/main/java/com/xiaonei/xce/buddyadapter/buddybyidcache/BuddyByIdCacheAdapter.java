...
public class BuddyByIdCacheAdapter extends ReplicatedClusterAdapter {
    ...
    public int getFriendCount(int userId) {
        TimeCost stat = TimeCost.begin(XceStat.getLogger(), "BuddyByIdCache " + userId);
    ...
    public Map<Integer, Integer> getFriendCounts(List<Integer> userIds) {
        ...
        TimeCost cost = TimeCost.begin(XceStat.getLogger(), "BuddyByIdCache getFriendCounts start:"
                + userIds.get(0) + " size:" + userIds.size());
        for (int i = 0; i < cluster; ++i) {
    ...
    public Map<Integer, List<Integer>> getFriendListsAsc(List<Integer> userIds) {
        ...
        for (int i = 0; i < cluster; ++i) {
            ...
            try {
                Map<Integer, int[]> subTargets = getBuddyByIdCacheManagerTwoway(i).getFriendLists(
                        MathUtil.list2array(subUserIds));
    ...
    protected BuddyByIdCacheManagerPrx getBuddyByIdCacheManagerTwoway(int userId) {
        ...
        if (prx0 != null) {
            BuddyByIdCacheManagerPrx prx = BuddyByIdCacheManagerPrxHelper.uncheckedCast(prx0
...
class BuddyByIdLoaderAdapter extends ReplicatedClusterAdapter {
    ...
    public int filter(int userId, List<Integer> buddys) {
        try {
            return getBuddyByIdLoaderTwoway(userId).filter(userId, MathUtil.list2array(buddys));
        } catch (Ice.TimeoutException e) {
            Oce.getLogger().error(this.getClass().getName() + ".filter " + userId + " ");
        }
        return 0;
    }
    ...
    protected BuddyByIdLoaderPrx getBuddyByIdLoaderOneway(int userId) {
        Ice.ObjectPrx prx0 = getValidProxy(userId, 300);
        if (prx0 != null) {
            BuddyByIdLoaderPrx prx = BuddyByIdLoaderPrxHelper.uncheckedCast(prx0.ice_oneway());
            return prx;
        } else {
            return null;
        }
    ...
    protected BuddyByIdLoaderPrx getBuddyByIdLoaderTwoway(int userId) {
        Ice.ObjectPrx prx0 = getValidProxy(userId, 300);
        if (prx0 != null) {
            BuddyByIdLoaderPrx prx = BuddyByIdLoaderPrxHelper.uncheckedCast(prx0.ice_twoway());
            return prx;
        } else {
            return null;
        }
