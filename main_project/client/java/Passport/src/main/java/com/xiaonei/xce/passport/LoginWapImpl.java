...
public class LoginWapImpl implements Login {

	private static AccountType[] defaultTypes = new AccountType[] { AccountType.Phone, AccountType.Account };
	private static LoginWapImpl _instance = new LoginWapImpl();

	public static LoginWapImpl getInstance() {
		return _instance;
	}
	...
	protected LoginWapImpl() {
		init();
		// prevent outside new.
	}

	private class LoginLogicHolder extends ReplicatedClusterMCAdapter {
		private final static String ctr_endpoints = "ControllerWapLoginLogic";
	...
	private class LoginCacheHolderById extends ReplicatedClusterMCAdapter {
		private final static String ctr_endpoints = "ControllerWapLoginCache";
		private static final int _interval = 120;
	...
	public Account loginVirtual(List<Type> types, String user, String password,
		...
		try {
			prx = getLoginLogic();
			accs = prx.loginVirtual(defaultTypes, user, password, site, expirePeriod, true);
		} catch(ConnectTimeoutException e) {
			Oce.getLogger().error(
					this.getClass().getName() + ".loginVirtual [" + prx
					+ "] user=" + user + " password= " + password + " site=" + site
					+ " expirePeriod=" + expirePeriod + " Ice.ConnectTimeoutException");
			throw e;
	     } catch (TimeoutException e) {
	...
	public Account switchAccountIncVirtualId(int sourceId, int desId, String pwd,
		...
		try {
			prx = getLoginLogic();
			AccountInfo acinfo = prx.switchAccountIncVirtualId(sourceId, desId, pwd, reuse_ticket);
			return new Account(acinfo, lTicket2STicket(acinfo.ticket));
		} catch(ConnectTimeoutException e) {
			Oce.getLogger().error(
					this.getClass().getName() + ".switchAccountIncVirtual [" + prx
	...
	public List<Account> getById(int id) {
		...
		try {
			...
			Oce.getLogger().error(
					this.getClass().getName() + ".getById [" + prx
	...
	private boolean shortTicketFlag;
	...
	private void init() {
	    	...
	    	//此处应该写成: shortTicketFlag = (System.getProperty("WAPUSESHORTTICKET") != null)
	    
	    	String shortflag = System.getProperty("WapUseShortTicket", "false");
	    	if(shortflag.equals("true")){
	    	    shortTicketFlag=true;
	    	}else {
	    	    shortTicketFlag=false;
	    	}
	...
	public static void main(String[] args) {
		...
		LoginWapImpl wapImpl = LoginWapImpl.getInstance();
