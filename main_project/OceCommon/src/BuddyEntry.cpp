...
EntryListHolder::EntryListHolder(uint32_t from) :
	_from(from) {
...
uint32_t EntryListHolder::getFrom() {
	return _from;
}
...
void EntryListHolder::remove(const Entry& key) {
	MCE_TRACE("Segment::del " << key);
	//	dump();

	IceUtil::Mutex::Lock lock(*this);
	EntryList::iterator itBegin = lower_bound(_list.begin(), _list.end(), key, less_entry());
	
	while (itBegin != _list.end() && equals(key, *itBegin) ) {
                itBegin = _list.erase(itBegin);
        }
}
...
void EntryListHolder::set(const Entry& key, const Entry& value, const Entry& valueMask) {
	MCE_TRACE("Segment::set " << key  << " " << value << " / " << valueMask);
	if ( valueMask.to != MAX_INT) {
		MCE_WARN("Segment::set not ready.");
		return;
	}
	//	dump();
	...
	IceUtil::Mutex::Lock lock(*this);
	EntryList::iterator itBegin = lower_bound(_list.begin(), _list.end(), key, less_entry());
	EntryList::iterator itEnd = lower_bound(itBegin, _list.end(), key, less_equal_entry());
	for (; itBegin != itEnd; ++itBegin) {
		MCE_TRACE("set segment " << *itBegin )
		if (equals(key, *itBegin)) {
			itBegin->desc = (itBegin->desc & ~DESC_MASK) | (value.desc
					& valueMask.desc);
			MCE_TRACE("seted entry " << *itBegin );
		}
	}
...
bool EntryListHolder::equals(const Entry& l, const Entry& r) {
	return (l.to == r.to);
}
