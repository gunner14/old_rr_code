...
void FeedMiniAdapter::addFeed(const FeedItem& item,
...
void FeedMiniAdapter::setRead(int uid, int stype, Ice::Long source) {
  getManagerOneway(uid)->setRead(uid, stype, source);
}
void FeedMiniAdapter::setReadById(int uid, Ice::Long feedId) {
  getManagerOneway(uid)->setReadById(uid, feedId);
}
void FeedMiniAdapter::setReadAll(int uid) {
  getManagerOneway(uid)->setReadAll(uid);
}

void FeedMiniAdapter::PressureTest(int uid, int begin, int limit)
{
...
void CollectTaskN::Process(FeedItemSeqSeq & t) {
  ...
  try {
    ...
    t = FeedMiniAdapter::instance().getManager(firstuid)->GetFeedItemSeqSeqByUsers(ids_);
  } catch (Ice::Exception& e) {
    MCE_WARN("CollectTaskN::Handle --> FeedMiniAdapter.GetFeedItemSeqSeqByUsers, index = "<<index_<<" err:"<<e);
  }
}
