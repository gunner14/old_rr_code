...
#ifndef SEARCHLOGICADAPTER_H_
#define SEARCHLOGICADAPTER_H_
...
class SearchLogicAdapter: public MyUtil::Singleton<SearchLogicAdapter>, public MyUtil::ReplicatedClusterAdapterI<SearchCacheManagerPrx> {
			...
			void invoke(const int id, const std::string tableName, const std::map<string, string>& fieldPairMap) {
                ...
                if(0 < fieldPairMap.size())
                {
                    ...
                    for(iter=fieldPairMap.begin(); iter!=fieldPairMap.end(); iter++)
                    {
                        ...
                        //std::cout<<"invoke right!!"<<std::endl;  //add for test
                    }
