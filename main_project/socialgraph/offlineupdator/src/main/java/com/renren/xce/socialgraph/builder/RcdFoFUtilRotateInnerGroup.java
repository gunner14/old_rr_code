...
public class RcdFoFUtilRotateInnerGroup {
	...
	public final static byte[] QUALIFIER = Bytes.toBytes(OfflineUpdator.kFoFSingleGroupIndex * OfflineUpdator.kFoFGroupSize);
		...
		try {
			...
			if (!result.isEmpty()) {
				...
				if (value.length > 0) {
				    ...
				    int leftPos = kGroupIndex * kGroupSize + OfflineUpdator.kFoFMinPosition;
				    int rightPos = kGroupIndex * kGroupSize + OfflineUpdator.kFoFMaxPosition;	
				    if (leftPos > rightPos) {
				...
				Clogging.Error("getRcdFoFData: Fecthing result fof data from hbase failed, result size: " + result.size() + " group index: " + OfflineUpdator.kFoFSingleGroupIndex + 
						" FAMILILY length "	+ FAMILILY.length + "  " + " QUALIFIER length " + QUALIFIER.length);
			}
		} catch (IOException e) {
			e.printStackTrace();
			return fofList;
