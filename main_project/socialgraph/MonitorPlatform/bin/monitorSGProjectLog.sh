...
#服务部署路径
moduleLogPath="/data/xce/SGProject/logs"
...
function Excute()
{
	...
	for logdir in ${ModuleNameList[@]}
	do
		logfilelist=(`ls ${moduleLogPath}"/"$logdir`)
		for file in ${logfilelist[@]}
		do
			if [ ! -f ${moduleLogPath}"/"$logdir"/"$file ]
			then
				continue
			else
				resultlog=$LOG_DIR"/"$logdir
				nowTime=`date "+%Y-%m-%d %H:%M"`

				cat ${moduleLogPath}"/"$logdir"/"$file | grep -E "$nowTime" | grep -E "ERROR" > $resultlog
			fi
		done
...
function SendBackLog()
{
	...
	do
		...
		then
			...
			Print LOG_INFO "`hostname` $modulename has WARN or ERROR LOG."
		fi
	done
}
...
Main()
{
	...
	do
		...
		Print LOG_INFO "monitorSGProjectLog is still running."
