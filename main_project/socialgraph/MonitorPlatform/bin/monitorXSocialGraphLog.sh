...
#/**
#*	@file:			monitorXSocialGraphLog.sh
...
function Excute()
{
	...
	do
		...
		then
			continue
			#Print LOG_ERROR "[`hostname`] No Any Log in $moduleLogPath/${ModuleNameList[$i]}"
		else
			#结果日志
			resultlog=$LOG_DIR"/"${ModuleNameList[$i]}

			nowTime=`date "+%H:%M:%S"`
			cat $modulelog | grep -E "$nowTime" | grep -E "ERROR" > $resultlog 
		fi
	done
...
{	
	...
	do
		...
		then
			...
			mv $LOG_DIR"/"${modulename} $LOG_DIR"/"${modulename}"_""`date "+%Y-%m-%d_%H-%M-%S"`"
...
Main()
{
	...
	do
		...
		Print LOG_INFO "monitorXSocialGraphLog is still running."
