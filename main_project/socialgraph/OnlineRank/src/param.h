...
#define COPYRIGHT "zhang yeyin"
#define VERSION	"1.0"
#define PACKAGE	"Common utility"
...
struct Option {
  const char *name;			//TYPE arg = get<TYPE>(name)
  char        short_name;		//CMD -short_name value
  const char *default_value;		//default value
  const char *arg_description;		//TYPE description
  const char *description;		//info for help
};
