...
template<class T>
void swap(size_t i, size_t j, T* elemSeq){
	T tmp=elemSeq[i];
	elemSeq[i]=elemSeq[j];
	elemSeq[j]=tmp;
}
...
template<class T>
void qsort(size_t left, size_t right, T* elemSeq){
	if(left>=right)return;
	size_t mid = (left+right)/2;
	T midItem = elemSeq[mid];
	swap<T>(left,mid, elemSeq);
	size_t i,j;
	for(i=left+1,j=right; i<=j;){
		while(elemSeq[i]>midItem && i<=right)i++;
		while(elemSeq[j]<midItem && j>=left)j--;
		if(i>=j)break;
		swap<T>(i,j, elemSeq);
		if(elemSeq[i]==midItem)i++;
		if(elemSeq[j]==midItem)j--;
	}
	swap<T>(left,j, elemSeq);

	if(j>1)
		qsort<T>(left,j-1,elemSeq);
	qsort<T>(j+1,right,elemSeq);
}
...
inline size_t pow(size_t base, size_t index){
	size_t n=1;
...
inline size_t atoi(char* src, size_t len){
	size_t n=0;
	for(size_t i=0; i<len; i++){
		n+=(size_t)(src[len-1-i]-'0')*pow(10, i);
	}
	return n;
}
