...
public:
	...
	std::vector<int > GetRecommedList() const { 
		return recommend_list_;
	}

	bool IsVisit(long ugc_id) const {
		std::set<long>::iterator fit = block_ugc_list_.find(ugc_id);	
		if (fit != block_ugc_list_.end()) {
			return true;
		} 
	...
	bool IsInterested(const std::string& interest) const {
		std::set<std::string>::iterator fit = interest_list_.find(interest);
		if (fit != interest_list_.end()) {
			return true;
		}
