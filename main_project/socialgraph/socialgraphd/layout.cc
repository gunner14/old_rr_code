...
std::string Rcd3NewFriendsLayout::DoLayout(UserCacheMap* usercaches) {
  ...
  if (host_ % 10 == 1 ||
      host_ % 10 == 8) { 
    tpl_name  += rcd_type + std::string("18");
  } else {
    tpl_name += rcd_type;
...
//-----------------------------------------------------------------------------------------------------------
std::string RcdShoppingMasterLayout::DoLayout(UserCacheMap* usercaches) {
  std::string out;
  if (!usercaches)
    return out;
  if (host_ == item_data_.id()) {
  ...
  if(0 != sharescount) {
	...
	dict.SetValue("TYPE_VALUE", rcd_type);
        ...
        dict.SetValue("NAME", iter->second->name());
        dict.SetValue("TINY_URL", xce::HeadUrl::FromPart(iter->second->tinyurl()));
        stringstream ss_shares_count;
        if(sharescount > 100)
                ss_shares_count<<"99+";
        else
                ss_shares_count<<sharescount;
        dict.SetValue("COMMONS", ss_shares_count.str().c_str());
	...
	stringstream sslikes;
        ...
        if(cnt[1] > 99999){
                ...
                sslikes<<cnt[1];
        dict.SetValue("FANS", ssfans.str().c_str());
        dict.SetValue("LIKES", sslikes.str().c_str());
        tpl->Expand(&out, &dict);
  }
  return out;
}
...
//-----------------------------------------------------------------------------------------------------------
std::string PhotoFriendLayout::DoLayout(UserCacheMap* usercaches) {
        std::string out;
        if (!usercaches)
                return out;
        ...
        ctemplate::TemplateDictionary dict(rcd_type);
        dict.SetValue("TYPE_VALUE", rcd_type + "_" + "PO");          //example: FFW_E1
	dict.SetIntValue("ID", iter->second->id());
        dict.SetValue("NAME", iter->second->name());
        dict.SetValue("TINY_URL", xce::HeadUrl::FromPart(iter->second->tinyurl()));
        dict.SetValue("MESSAGE", rcd_type);
...
LayoutPtr LayoutFactory::Create(int uid, const Item& item_data) {
	if (COMMON_FRIENDS == item_data.message()) {			//common_friends use RcdLayout 
		...
		return LayoutPtr(new RcdLayout(uid, item_data));
		//return LayoutPtr(new Rcd4FriendsLayout(uid, item_data));
	} else if (PAGE == item_data.message() || PAGECOLLEGE == item_data.message() ) {						//page recommend use PageLayout
	...
	} else if (RCDSM == item_data.message()) {             //
                return LayoutPtr(new RcdShoppingMasterLayout(uid, item_data));
        }
