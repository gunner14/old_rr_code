...
import xce.socialgraph.recommend.page.PageCollegeRecommendUtil;
 ...
 */
public class PageCollegeBuilder implements DataBuilder {
	...
	public List<DataItem> getData(int hostId) {
		...
		List<PageRecommendData> pageList = PageCollegeRecommendUtil.getInstance().getPageRecommend(hostId, -1);
		...
		for (PageRecommendData page : pageList) {
			...
			item.setMessage(CreateRecommendDataThread.PAGECOLLEGE);
	...
	public String getBuilderType() {
		return CreateRecommendDataThread.PAGECOLLEGE;
	}
}
