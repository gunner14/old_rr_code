...
import org.apache.mahout.cf.taste.hadoop.item.RecommenderJobConstants;
...
public final class HadoopUtil {
    ...
    /**
     * Create a map-only Hadoop Job out of the passed in parameters. Does
     * not set the Job name.
     * 
     * @see #getCustomJobName(String,
     *      org.apache.hadoop.mapreduce.JobContext, Class, Class)
     */
    ...
    /**
     * Create a map and reduce Hadoop job. Does not set the name on the
     * job.
     * 
     * @param inputPath The input {@link org.apache.hadoop.fs.Path}
     * @param outputPath The output {@link org.apache.hadoop.fs.Path}
     * @param inputFormat The
     *        {@link org.apache.hadoop.mapreduce.InputFormat}
     * @param mapper The {@link org.apache.hadoop.mapreduce.Mapper} class
     *        to use
     * @param mapperKey The {@link org.apache.hadoop.io.Writable} key
     *        class. If the Mapper is a no-op, this value may be null
     * @param mapperValue The {@link org.apache.hadoop.io.Writable} value
     *        class. If the Mapper is a no-op, this value may be null
     * @param reducer The {@link org.apache.hadoop.mapreduce.Reducer} to
     *        use
     * @param reducerKey The reducer key class.
     * @param reducerValue The reducer value class.
     * @param outputFormat The
     *        {@link org.apache.hadoop.mapreduce.OutputFormat}.
     ...
     * @see #getCustomJobName(String,
     *      org.apache.hadoop.mapreduce.JobContext, Class, Class)
     * @see #prepareJob(org.apache.hadoop.fs.Path,
     *      org.apache.hadoop.fs.Path, Class, Class, Class, Class, Class,
    ...
    public static Job prepareJob(String jobName, Path inputPath, Path outputPath,
            ...
            Class<? extends Writable> reducerValue, Class<? extends OutputFormat> outputFormat, Configuration conf)
            throws IOException {
        Job job = prepareJob(inputPath, outputPath, inputFormat, mapper, mapperKey, mapperValue, reducer, reducerKey,
                reducerValue, outputFormat, conf);
    ...
    public static void optimizeJob(Configuration jobConf) {
        ...
        jobConf.set("mapreduce.map.java.opts", RecommenderJobConstants.MAPRED_MAP_CHILD_JAVA_OPTS);
        jobConf.set("mapreduce.reduce.java.opts", RecommenderJobConstants.MAPRED_REDUCE_CHILD_JAVA_OPTS);
        /*jobConf.set("mapred.child.java.opts",
        		RecommenderJobConstants.MAPRED_CHILD_JAVA_OPTS);*/
        jobConf.setInt("mapreduce.job.jvm.numtasks", RecommenderJobConstants.MAPRED_JOB_JVM_NUMTASKS);

        jobConf.setInt("mapred.max.split.size", RecommenderJobConstants.MAPRED_MAX_SPLIT_SIZE);
        /*jobConf.set("mapred.output.compression.codec",
        		RecommenderJobConstants.MAPRED_OUTPUT_COMPRESSION_CODEC);*/

        jobConf
                .setInt("mapreduce.reduce.shuffle.parallelcopies",
                        RecommenderJobConstants.MAPRED_REDUCE_PARALLEL_COPIES);
        jobConf.setInt("mapreduce.tasktracker.http.threads", RecommenderJobConstants.MAPRED_TT_HTTP_THREADS);
        jobConf.setInt("mapreduce.task.timeout", RecommenderJobConstants.MAPRED_TASK_TIMEOUT);
        jobConf.setInt("mapreduce.task.io.sort.mb", RecommenderJobConstants.IO_SORT_MB);
        jobConf.setInt("mapreduce.task.io.sort.factor", RecommenderJobConstants.IO_SORT_FACTOR);
        jobConf.setInt("io.file.buffer.size", RecommenderJobConstants.IO_FILE_BUFFER_SIZE);

        jobConf.setFloat("mapred.job.shuffle.input.buffer.percent",
                RecommenderJobConstants.MAPRED_JOB_SHUFFLE_INPUT_BUFFER_PERCENT);
        jobConf.setFloat("mapred.job.shuffle.merge.percent", RecommenderJobConstants.MAPRED_JOB_SHUFFLE_MERGE_PERCENT);
    }

    public static String getCustomJobName(String className, /*JobContext*/Job job, Class<? extends Mapper> mapper,
     ...
     * @param pt The
     *        {@link org.apache.mahout.common.iterator.sequencefile.PathType}
     * @param filter Apply the {@link org.apache.hadoop.fs.PathFilter}. May
     *        be null
