...
public abstract class AbstractJob extends Configured implements Tool {
  ...
  public static boolean shouldRunNextPhase(Map<String, List<String>> args, AtomicInteger currentPhase) {
  ...
  protected Job prepareJob(String jobName, Path inputPath, Path outputPath, Class<? extends Mapper> mapper,
	      Class<? extends Writable> mapperKey, Class<? extends Writable> mapperValue, Class<? extends Reducer> reducer,
	      Class<? extends Writable> reducerKey, Class<? extends Writable> reducerValue) throws IOException {
	  Job job = prepareJob(inputPath, outputPath, mapper, mapperKey, mapperValue, reducer, reducerKey, reducerValue);
  ...
  protected Job prepareJob(Path inputPath,
    ...
    job.setJobName(HadoopUtil.getCustomJobName(getClass().getSimpleName(), job, mapper, reducer/*Reducer.class*/));
    return job;
  }
  
  protected Job prepareJob(String jobName,
		  Path inputPath,
          Path outputPath,
          Class<? extends InputFormat> inputFormat,
          Class<? extends Mapper> mapper,
          ...
          Class<? extends Writable> reducerValue,
          Class<? extends OutputFormat> outputFormat) throws IOException {
	  Job job = prepareJob(inputPath, outputPath, inputFormat, mapper, mapperKey, mapperValue, 
			  reducer, reducerKey, reducerValue, outputFormat);
	  job.setJobName(jobName);
	  return job;
  }
