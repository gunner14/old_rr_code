...
public final class RecommenderJob extends AbstractJob {
  ...
  public int run(String[] args) throws Exception {
    ...
    if (shouldRunNextPhase(parsedArgs, currentPhase)) {
      Job prePartialMultiply1 = prepareJob("6/9 prePartialMultiply1Job",
      ...
      prePartialMultiply1.setNumReduceTasks(RecommenderJobConstants.NUM_REDUCE_TASKS_AGGREGATE_AND_RECOMMEND);
      boolean succeeded = prePartialMultiply1.waitForCompletion(true);
      if (!succeeded) 
        return -1;
      //continue the multiplication
      Job prePartialMultiply2 = prepareJob("7/9 prePartialMultiply2Job", new Path(prepPath, PreparePreferenceMatrixJob.USER_VECTORS),
      ...
      prePartialMultiply2.getConfiguration().setInt(UserVectorSplitterMapper.MAX_PREFS_PER_USER_CONSIDERED,
              maxPrefsPerUser);
      prePartialMultiply2.setNumReduceTasks(RecommenderJobConstants.NUM_REDUCE_TASKS_AGGREGATE_AND_RECOMMEND);
      succeeded = prePartialMultiply2.waitForCompletion(true);
      if (!succeeded) 
        return -1;
      //finish the job
      Job partialMultiply = prepareJob("8/9 partialMultiplyJob",
      ...
      partialMultiply.setNumReduceTasks(RecommenderJobConstants.NUM_REDUCE_TASKS_AGGREGATE_AND_RECOMMEND);
    ...
    if (shouldRunNextPhase(parsedArgs, currentPhase)) {
      ...
      Job aggregateAndRecommend = prepareJob("9/9 aggregateAndRecommendJob", 
      ...
      aggregateAndRecommend.setNumReduceTasks(RecommenderJobConstants.NUM_REDUCE_TASKS_AGGREGATE_AND_RECOMMEND);
      HadoopUtil.delete(aggregateAndRecommend.getConfiguration(), outputPath);
  ...
  private static void setIOSort(/*JobContext*/Job job) {
