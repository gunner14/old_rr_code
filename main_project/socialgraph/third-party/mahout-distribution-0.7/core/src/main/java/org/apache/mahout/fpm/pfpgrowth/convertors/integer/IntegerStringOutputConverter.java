...
/**
 * Collects the Patterns with Integer id and Long support and converts them
 * to Pattern of Strings based on a reverse feature lookup map.
 */
public final class IntegerStringOutputConverter implements OutputCollector<Integer, List<Pair<List<Integer>, Long>>> {
    ...
    public void collect(Integer key, List<Pair<List<Integer>, Long>> value) throws IOException {
        String stringKey = featureReverseMap[key]+""/*.get(key)*/;
        ...
        for (Pair<List<Integer>, Long> e : value) {
            ...
            for (Integer i : e.getFirst()) {
                pattern.add(featureReverseMap[i]+""/*.get(i)*/);
            }
