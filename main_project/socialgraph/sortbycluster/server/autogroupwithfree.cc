...
void AutoGroupWithFree::CreateGroup() {
	MCE_INFO("[AutoGroupWithFree] Grouping userid(" << userid_ << ")");
...
void AutoGroupWithFree::Display() {
	MCE_INFO("[AutoGroupWithFree] Display");
...
std::vector<int> AutoGroupWithFree::GetBestOffer(int limit) {
	std::vector<int> resultlist;
	if (grouped_stats_.groupednum_ > limit) {
		GetOfferFromGroup(resultlist, limit);
	} else {
		GetOfferFromRetain(resultlist, limit);
	}

	if ((int)resultlist.size() > limit) {
  		resultlist.resize(limit);
  }

	SortUtil::FilterUserPhoto(resultlist);

	return resultlist;
}
// add the JS rank strategy
std::vector<int> AutoGroupWithFree::GetFinalBestOffer(int limit) {
	std::vector<int> resultlist;
	if (grouped_stats_.groupednum_ > limit) {
		GetOfferFromGroup(resultlist, limit);
	} else {
	...
	SortUtil::RankByUserValue(resultlist);

	return resultlist;
}

void AutoGroupWithFree::GetOfferFromGroup(std::vector<int>& resultlist, int limit) {
	std::set<int> tmp_recommend;

	//get key node from each group
	for (std::map<int, std::vector<int> >::iterator iter = group_members_.begin();
			iter != group_members_.end(); ++iter) {
		float rate = (float)iter->second.size() / (float)grouped_stats_.groupednum_;
		int tmp_size = (int)(limit * rate + 1);		//min size is 1

		if ((int)iter->second.size() >= tmp_size) {
			tmp_recommend.insert(iter->second.begin(), iter->second.begin() + tmp_size);
		}
	}
	resultlist.insert(resultlist.begin(), tmp_recommend.begin(), tmp_recommend.end());		//sort by id

	SortByOnlineStatus(resultlist);				//sort member by their online status
}

void AutoGroupWithFree::GetOfferFromRetain(std::vector<int>& resultlist, int limit) {
	std::set<int> tmp_recommend;

	//get key node from each group
	for (std::map<int, std::vector<int> >::iterator iter = group_members_.begin();
			iter != group_members_.end(); ++iter) {
		tmp_recommend.insert(iter->second.begin(), iter->second.end());
	}
	resultlist.insert(resultlist.begin(), tmp_recommend.begin(), tmp_recommend.end());

	int left_num = limit - (int)resultlist.size();

	BOOST_FOREACH(int id, friendlist_) {
		if (--left_num < 0)
			break;

		if (tmp_recommend.find(id) == tmp_recommend.end()) {
	...
	sort(resultlist.begin(), resultlist.end());		//sort by id
	SortByOnlineStatus(resultlist);				//sort member by their online status
}

void AutoGroupWithFree::SortByOnlineStatus(std::vector<int>& resultlist) {
	using namespace xce::onlinestatefilter::adapter;

	MyUtil::IntSeq onlinelist = OnlineStateFilterAdapter::instance().getOnlineIdsFrom(resultlist);	//get online list

	//get retain
	std::vector<int> retainlist;
	std::vector<int>::iterator iter1 = resultlist.begin();
	std::vector<int>::iterator iter2 = onlinelist.begin();
	while (iter1 != resultlist.end() 
		|| iter2 != onlinelist.end()) {
		if (iter2 == onlinelist.end()) {
			retainlist.push_back(*iter1);
			++iter1;
			continue;
		}
	...
	//using random strategy to assign result
	random_shuffle(onlinelist.begin(), onlinelist.end());
	random_shuffle(retainlist.begin(), retainlist.end());
	resultlist.clear();
	resultlist.insert(resultlist.end(), onlinelist.begin(), onlinelist.end());
	resultlist.insert(resultlist.end(), retainlist.begin(), retainlist.end());
}
