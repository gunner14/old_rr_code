...
int  FriendFetcher::Read(int uid, vector<USERID>& friends){
	...
	// query database
	try {
	     QueryRunner(dbInstance.c_str(), CDbRServer, pattern.str()).query(sql,FriendReadHandler(friends));
	} catch (Ice::Exception& e) {
	     CLOG(ERROR) << time(NULL) << "FriendFetcher::Read() QueryRunner Exception: " << e.what() ;
        }
