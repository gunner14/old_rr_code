...
void NewAutoGroupWithLabel::CreateGroup() {
...
void NewAutoGroupWithLabel::CreateStats() {
...
void NewAutoGroupWithLabel::Display() {
	std::cout << std::endl;
	std::cout << "[NewAutoGroupWithLabel] Display" << std::endl;
...
void NewAutoGroupWithLabel::CreateClique() {
	//一个人在一个组
	int groupindex = 0;
...
void NewAutoGroupWithLabel::SortMembersOfIndexGroup() {
	for (std::map<int, std::vector<int> >::iterator iter = indexgroup_.begin();
			iter != indexgroup_.end(); ++iter) {
...
void NewAutoGroupWithLabel::AddFriendsIntoGroup() {
...
void NewAutoGroupWithLabel::DeleteMembersFromGroup(Strategy strategy, const std::map<int, std::vector<int> >& backupmap) {
	for (std::map<int, std::vector<int> >::iterator iter = indexgroup_.begin();
			iter != indexgroup_.end(); ++iter) {
...
void NewAutoGroupWithLabel::UniteSimilarGroup() {
...
void NewAutoGroupWithLabel::RefineIndexGroup() {
	int newindex = 0;
...
void NewAutoGroupWithLabel::CreateRealGroup() {
	for (std::map<int, std::vector<int> >::iterator iter = indexgroup_.begin();
			iter != indexgroup_.end(); ++iter) {
		for (std::vector<int>::iterator it = iter->second.begin();
				it != iter->second.end(); ++it) {
...
GroupStats NewAutoGroupWithLabel::GetGroupStats() {
	return grouped_stats_;
}

void NewAutoGroupWithLabel::GetGroupResult(Items& result) {
...
void NewAutoGroupWithLabel::CreateGroupTags() {
//	PTIME(pt, "PTIME AutoGroupWithLabel CreateGroupTags", true, 0);
	...
	std::vector<GroupLabel> label_list;		//获取该组最适当的标签
	for (std::map<int, std::vector<int> >::iterator iter = group_members_.begin();
			iter != group_members_.end(); ++iter) {
		...
		std::sort(label_list.begin(), label_list.end(), GroupLabelComp());	//找到出现次数最多的组标签
	...
	MergeSameLabel();		//相同标签的组进行合并
}

//并行取用户资料
void NewAutoGroupWithLabel::GetFriendsNetwork() {
	//LOG(INFO) << "[NewAutoGroupWithLabel] GetFriendsNetwork friendlist_ size:" << friendlist_.size();
	...
	std::map<int, UnivInfoList> univ_info_map;
	IceUtil::Handle<LoadUserDataThread<University> > load_univdata_ptr = 
		new LoadUserDataThread<University>(UniversityType, "university_info", terms_list, 
																			 friendlist_, univ_info_map);			//获取大学资料
	thread_ctrl.push_back(load_univdata_ptr->start());

	terms_list.clear();
	terms_list.push_back("userid"); terms_list.push_back("high_school_name"); terms_list.push_back("enroll_year");
	std::map<int, HighSchoolList> highschool_info_map;
	IceUtil::Handle<LoadUserDataThread<HighSchool> > load_highschooldata_ptr =
		new LoadUserDataThread<HighSchool>(HighSchoolType, "high_school_info", terms_list, 
																			 friendlist_, highschool_info_map);				//获取高中资料
	thread_ctrl.push_back(load_highschooldata_ptr->start());

	terms_list.clear();
	terms_list.push_back("userid"); terms_list.push_back("junior_high_school_name"); terms_list.push_back("junior_high_school_year");
	std::map<int, JuniorSchoolList> juniorschool_info_map;
	IceUtil::Handle<LoadUserDataThread<JuniorSchool> > load_juniorschooldata_ptr =
		new LoadUserDataThread<JuniorSchool>(JuniorSchoolType, "junior_high_school_info", terms_list, 
													 							 friendlist_, juniorschool_info_map);			//获取初中资料
	thread_ctrl.push_back(load_juniorschooldata_ptr->start());

	terms_list.clear();
	terms_list.push_back("userid"); terms_list.push_back("elementary_school_name"); terms_list.push_back("elementary_school_year");
	std::map<int, ElementarySchoolList> elementaryschool_info_map;
	IceUtil::Handle<LoadUserDataThread<ElementarySchool> > load_elementaryschooldata_ptr =
		new LoadUserDataThread<ElementarySchool>(ElementarySchoolType, "elementary_school_info", terms_list, 
																						 friendlist_, elementaryschool_info_map); 		//获取小学资料
	thread_ctrl.push_back(load_elementaryschooldata_ptr->start());

	terms_list.clear();
	terms_list.push_back("userid"); terms_list.push_back("college_name"); terms_list.push_back("enroll_year");
	std::map<int, CollegeList> college_info_map;
	IceUtil::Handle<LoadUserDataThread<College> > load_college_ptr =
			new LoadUserDataThread<College>(CollegeType, "college_info", terms_list, 
																			friendlist_, college_info_map);			//获取中专资料
	thread_ctrl.push_back(load_college_ptr->start());

	for (std::vector<IceUtil::ThreadControl>::iterator iter = thread_ctrl.begin();
			iter != thread_ctrl.end(); ++iter) {
		iter->join();				//等待数据都取成功再继续运行
	}

	//拼接好本地的usermap
	for (std::map<int, UnivInfoList>::iterator iter = univ_info_map.begin();
			iter != univ_info_map.end(); ++iter) {
		user_network_map_[iter->first].univ_list_.swap(iter->second);

		std::map<int, HighSchoolList>::iterator hfit = highschool_info_map.find(iter->first);
		if (hfit != highschool_info_map.end()) {
			user_network_map_[iter->first].highschool_list_.swap(hfit->second);
		}

		std::map<int, JuniorSchoolList>::iterator jfit = juniorschool_info_map.find(iter->first);
		if (jfit != juniorschool_info_map.end()) {
			user_network_map_[iter->first].juniorschool_list_.swap(jfit->second);
		}

		std::map<int, ElementarySchoolList>::iterator efit = elementaryschool_info_map.find(iter->first);
		if (efit != elementaryschool_info_map.end()) {
			user_network_map_[iter->first].elementaryschool_list_.swap(efit->second);
		}
		
		std::map<int, CollegeList>::iterator cfit = college_info_map.find(iter->first);
		if (cfit != college_info_map.end()) {
			user_network_map_[iter->first].college_list_.swap(cfit->second);
		}
	}
...
void NewAutoGroupWithLabel::ShowUserNetwork() {
...
GroupLabel NewAutoGroupWithLabel::GetGroupLabel(LabelType type, const std::vector<int>& friendlist) {
...
template <typename T>
std::pair<T, int> NewAutoGroupWithLabel::FetchBestItem(const std::vector<T>& item_list) {
...
void NewAutoGroupWithLabel::MergeSameLabel() {
	//PTIME(pt, "PTIME NewAutoGroupWithLabel MergeSameLabel", true, 0);
...
void LoadUserDataThread<T>::run() {
	...
	std::string sql;
	sql = "SELECT ";
	BOOST_FOREACH(const std::string& term, terms_) {		//拼接要获取的数据项
		sql += term + ", ";
	}
	...
	sql += " FROM " + table_ + " WHERE userid in (";
	...
	try {
		if (JuniorSchoolType == type_) {
			//初中信息的enroll_year字段为junior_high_school_year
			QueryRunner("search2_userbasic", CDbRServer).query(exe_sql, InfoHandler<T>(type_, data_, "junior_high_school_year"));
		} else if (ElementarySchoolType == type_) {
			//小学信息的enroll_year字段为elementary_school_year
			QueryRunner("search2_userbasic", CDbRServer).query(exe_sql, InfoHandler<T>(type_, data_, "elementary_school_year"));
		} else {
			//使用默认的enroll_year选项
			QueryRunner("search2_userbasic", CDbRServer).query(exe_sql, InfoHandler<T>(type_, data_));
		}
	} catch (Ice::Exception& e) {
		LOG(INFO) << "[LoadUserDataThread] run Ice Exception : " << e.what();
	} catch (...) {
		LOG(INFO) << "[LoadUserDataThread] run Unknown Exception";
	}
}
...
template <typename T>
bool InfoHandler<T>::handle(mysqlpp::Row& row) const {
	int enroll_year = (int)row[enroll_term_.c_str()];
	int userid = (int)row[userid_term_.c_str()];

	switch (type_) {	  //对不同type进行不同的处理，拼接成返回数据
		case UniversityType:
			{
				...
				T item(univ, enroll_year, academy);
				data_[userid].push_back(item);
				break;
			}
		case HighSchoolType:
			{
		...
		case JuniorSchoolType:
			{
				...
				T item(juniorschool, enroll_year);
				data_[userid].push_back(item);
				break;
			}
		...
		case ElementarySchoolType:
			{
				...
				T item(elementaryschool, enroll_year);
				data_[userid].push_back(item);
				break;
			}
