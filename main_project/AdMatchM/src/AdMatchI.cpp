...
AdInfoSeq AdMatchI::GetAds(const string& userProfile, ::Ice::Int count,
  ...
  return Matcher::instance().GetAds(userProfile, count, referer);
}
...
AdInfoSeq AdMatchI::GetAdsTest(::Ice::Int uid, ::Ice::Int stage,
  ...
  return Matcher::instance().GetAdsTest(uid, stage, gender, age, currentArea,
      ipArea, school, grade, count, referer);
}
...
void AdMatchI::Click(int uid, const string & ip, const string& text,
  ...
  MCE_DEBUG("AdMatchI::Click --> uid = " << uid);
...
//AdInfoSeq Matcher::GetAds(const string & pro_str, int count,
//    const string & referer) {
//  if (!ready_) {
//    MCE_WARN("Matcher::GetAds --> matcher is not ready, uid = " << profile.id());
//    return out;
//  }
//  AdInfoSeq out;
//  UserProfile profile;
...
//  int uid = profile.id();
//  Ice::Long current_area, ip_area, school;
//  Params params;
...
//        == "0000000000000000") {
//      MCE_DEBUG("Matcher::GetAds --> uid = "<< uid <<",current_area is not provided ,use 86000000000000");
//      current_area = 86000000000000;
//    } else {
//      current_area = lexical_cast<Ice::Long> (profile.current_area());
//    }
//  }
//  if (params.stage == 4) {
//    if (profile.ip_area().empty() || profile.ip_area() == "0000000000000000") {
//      MCE_DEBUG("Matcher::GetAds --> uid = "<< uid <<",ip_area = "<< profile.ip_area()<< ",use 86000000000000");
//      ip_area = 86000000000000;
//    } else {
//      ip_area = lexical_cast<Ice::Long> (profile.ip_area());
//    }
//  }
//  params.current_province = current_area / 10000000000 % 100;
//  params.ip_province = ip_area / 10000000000 % 100;
//
//  int m_key = AdUtil::GetUserKey(params);
//  MCE_DEBUG("Matcher::GetAds --> uid = "<< uid <<", m_key = " << m_key);
...
//    if (current_area == 86000000000000) { //只提供全国
...
//      Ice::Long school_province = -(current_area / 10000000000 * 10000000000);
//      AdSetPtr setProvince = s_idx_.Get(school_province);
//
//      Ice::Long school_city = -(current_area / 100000000 * 100000000);
//      AdSetPtr setCity = s_idx_.Get(school_city);
//      MCE_DEBUG("current_area = "<< current_area << ",school_province = " << school_province << ",school_city = " << school_city);
//
//      AdSetPtr setSchool = s_idx_.Get(school);
//
//      setB = setChina;
//
//      if (setProvince) {
...
//    }
//  } else {//白领
//    if (ip_area == 86000000000000) { //只提供全国
...
//        setChina = new AdSet;
//      }
//      AdSetPtr setProvince = a_idx_.Get(ip_area / 10000000000 * 10000000000);
//      AdSetPtr setCity = a_idx_.Get(ip_area / 100000000 * 100000000);
//      MCE_DEBUG("Matcher::GetAdsTest --> setChina size = " << setChina->Size());
//      setB = setChina;
//
//      if (setProvince) {
...
//  }
//
//  vector<pair<AdGroupPtr, int> > groups = FilterAdGroupB(setA, setB, count);
//  int ip = profile.ip();
//
//  return Format(groups, m_key, count, profile, referer);
//}
...
AdInfoSeq Matcher::GetAds(const string & pro_str, int count,
  ...
  if (params.stage == 2) {//学生
    if (params.current_area == 86000000000000) { //只提供全国
      ...
      MCE_DEBUG("Matcher::GetAdsTest --> setChina size = " << setChina->Size());
      Ice::Long school_province = -(params.current_area / 10000000000 * 10000000000);
      AdSetPtr setProvince = s_idx_.Get(school_province);
    ...
    if (params.ip_area == 86000000000000) { //只提供全国
      ...
      AdSetPtr setProvince = a_idx_.Get(params.ip_area / 10000000000 * 10000000000);
...
AdInfoSeq Matcher::GetAdsTest(::Ice::Int uid, ::Ice::Int stage,
  ...
  return Matcher::GetAds(tmp, count, referer);
...
void Matcher::Load(bool is_first, const Ice::LongSeq& group_ids) {
  MCE_DEBUG("Matcher::Load --> is_first = " << is_first << ", group_ids size = " << group_ids.size() << ",group_ids = " << PrintHelper(group_ids));
  ...
  for (map<Ice::Long, AdGroupPtr>::iterator it = ad_group_map.begin(); it
    ...
    if (part_ids.size() >= 7000 || it == ad_group_map.end()) {
      ...
      //      MCE_DEBUG("Matcher::Load --> bseq size = " << bseq.size() << ",creative size = " << creative.size());
      ...
      for (size_t i = 0; i < bseq.size(); ++i) {
        ...
        MCE_DEBUG("Matcher::Load --> gid = " << cit->first
            << ",keys = " << PrintHelper(git->second->keys_)
            << ",schools = " << PrintHelper(bid_unit.schools_)
            << ",areas = " << PrintHelper(bid_unit.areas_));
...
//采用新策略的选择，加大前三个广告的值
vector<pair<AdGroupPtr, int> > Matcher::FilterAdGroupB(const AdSetPtr& s1,
  ...
  if (score_map.size() > 3 && sumB * 3 > sumA) {
    MCE_DEBUG(__FILE__ << ":" << __LINE__ << ", 3 * sumB / sumA = " << 3 * sumB / sumA);
  ...
  for (multimap<int, SubGroupPtr>::iterator mit = score_map.begin(); mit
      != score_map.end(); ++mit) {
    float rate = (-1.0 * mit->first) / sum;
    MCE_DEBUG("Matcher::FilterAdGroup --> gid = " << mit->second->group->group_id_ << ",score = " << mit->first << ",sum = " << sum <<",probility = " << rate);
  }
    ...
    for (multimap<int, SubGroupPtr>::iterator sit = score_map.begin(); sit
      ...
      if (left <= rd && rd < right) {
        ...
        if (next->gid != 0) {//该广告不是最后一个
          ...
          price = max(min_price, min(this_price, (int) (sqrt(this_price
              * next_price) + 1)));
          MCE_DEBUG("Matcher::FilterAdGroup --> gid = " << g->group->group_id_ <<
              ",this_price = " << this_price <<
              ",next gid = " << next->gid <<
              ",next_price = " << next_price <<
              ",(int) (sqrt(this_price * next_price) + 1))" << (int) (sqrt(this_price* next_price) + 1)
          );
        } else {
          price = min(this_price, (int) (sqrt(this_price * 50)) + 1);
        }

        ++(g->pv);
        out.push_back(make_pair(g->group, price));
        MCE_DEBUG("Matcher::FilterAdGroup --> rd = " << rd << ",left = " << left << ",right = " << right);

        //删除相同member的group
        map<Ice::Long, set<Ice::Long> >::iterator mit = member_map.find(
            g->group->member_id_);
        MCE_DEBUG(__FILE__ << ":" << __LINE__ << ", " << g->group->member_id_ );
...
//text格式：  index_key_creative_bidid_cost_pvtime
AdInfoSeq Matcher::Format(vector<pair<AdGroupPtr, int> > & group_price,
    int m_key, int ad_count, UserProfile & profile, const string & referer) {
  MCE_DEBUG("Matcher::Format --> referer = " << referer);
  time_t pv_time;
  pv_time = time(0);
  ...
  MCE_DEBUG("Matcher::Format --> group_price size = " << group_price.size());

  AdInfoSeq ads;
  set<Ice::Long> tmp;
  for (vector<pair<AdGroupPtr, int> >::iterator git = group_price.begin(); git
      != group_price.end(); ++git) {
    ...
    text_ios << gServiceIndex << "_" << m_key << "_" << mit->first << "_"
        << bid_id << "_" << git->second << "_" << pv_time;


    AdPvParameterPtr pv = new AdPvParameter;
    pv->groupid = git->first->group_id_;
    pv->referer = referer;//TODO
    pv->creativeid = ad.creative;

    int a, b, c, d; //将整数ip转化成string
    a = ip / 16777216;
    b = (ip - a * 16777216) / 65536;
    c = (ip - a * 16777216 - b * 65536) / 256;
    ...
    http_ios << "/show?userid=" << uid << "&tt=0&type=.js&adzoneid="
        << adzone_id << "&count=" << ad_count << "&rowoffset=0&ip=" << pv->ip
        << "&jbc=cookie|a|b&ubc=" << ubc;

    pv->http = http_ios.str();
    pv->bitunitid = bid_id;
    pv_seq.push_back(pv);
    ...
    MCE_DEBUG("Matcher::Format --> text = " << ad.text);
    MCE_DEBUG("Matcher::Format --> http = " << pv->http);
  }
  MCE_DEBUG("Matcher::Format --> creatives are " << PrintHelper(tmp));
  AdAnalysisAdapter::instance().Pv(pv_seq);
  return ads;
}

//index_key_creative_bidid_cost_pvtime_ubc  ubc中可能有空格
void Matcher::Click(int uid, const string & ip, const string& text,
    const string & referer, const string & click_url) {
  MCE_INFO("Matcher::Click --> uid = " << uid << ",ip = " << ip << ",text = " << text << ",referer = " << referer << ",click_url");
  ...
  if (key != 0) { // 非默认广告
    ...
    MCE_DEBUG("Matcher::Click --> groupid = " << gid << ",creativeid = " << creative << ",bidid = " << bid_id);
...
void Matcher::ScanChangedPrices() {
  ...
  if (res.empty()) {
    MCE_WARN("Matcher::ScanChangedPrices --> price :empty ");
    return;
  }
...
void Matcher::ScanChangedBidUnits() {
  ...
  if (res.empty()) {
    MCE_WARN("Matcher::ScanChangedBidUnits --> empty ");
    return;
  }
...
void Matcher::ScanChangedCreatives() {
  ...
  if (res.empty()) {
    MCE_WARN("Matcher::ScanChangedCreatives --> res empty ");
    return;
  }
