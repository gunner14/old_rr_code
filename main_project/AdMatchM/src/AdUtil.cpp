...
void AdUtil::LoadSchools() {
  ...
  if (res.empty()) {
    MCE_WARN("AdUtil::LoadSchools --> no ads in bid_unit ");
    return;
  }
...
void BidUnit::Init(BidParamsPtr & params, unsigned index) {
  ...
  MCE_DEBUG("BidUnit::Init --> age = " << params->ages);
...
//bool BidUnit::Match(const Params & params){
//  int u_stage = params.stage;//用户的stage
//  if(!stages_.count(u_stage) || !ages_.count(params.age)){
//    return false;
//  }
//  if(u_stage == 2){
//    if(!provinces_.count(params.current_province)){
//      return false;
//    }
//    if(!grades_.count(params.grade)){
//      return false;
//    }
//  }
//}
