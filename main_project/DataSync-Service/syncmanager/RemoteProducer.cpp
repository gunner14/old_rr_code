...
xce::messagepipe::MessageDataSeq RemoteProducer::send(const SyncMessageISeq& datas) {
    ...
    while(it != datas.end()) {
        ...
        xce::messagepipe::MessageDataPtr msg(new xce::messagepipe::MessageData(Cluster_, cData.dbtable(), strVec));
        if(!xce::messagepipe::Producer::getInstance()->send(ZkAddress_, msg)) {
