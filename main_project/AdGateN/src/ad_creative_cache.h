...
//  void GetJson(const string & referer, int refresh_source, int refresh_idx, const string & emtag, const AdResultMap & ads, string * html) const;
  ...
  void SetAdDBSource(const string& db) {
    ad_db_instance = db;
  }
  void SetMainDBSource(const string& db) {
    main_db_instance = db;
  }
private:
  string ad_db_instance;
  string main_db_instance;
  void GetZonesByReferer(const string & referer, set<long> * zones) const;
  bool TranslateWidget(const string &widget, string &ad_widget);
