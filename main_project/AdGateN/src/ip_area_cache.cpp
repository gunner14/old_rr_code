...
IpAreaCache::IpAreaCache() : default_area_("0086000000000000"), ready_(false), db_instance_("db_adwr") {
}
...
bool IpAreaCache::Reload() {
  ...
  // 加载 data_schemas 表信息
  MCE_INFO("IpAreaCache::Reload() --> db_instance = " << db_instance_);
  {
    Statement sql;
    ...
    try {
      mysqlpp::StoreQueryResult res = QueryRunner(db_instance_, CDbRServer).store(sql);
