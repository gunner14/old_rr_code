...
var FeedKeyView = function(stype, version, using_seq_id, test_seq_id, status, readonly) {
  ...
  this.dom_ = $('<div><div id="panel" style="margin-left:0px; width:998px; line-height:24px; border:1px solid #CAD9EA;border-top:0;">'
                 ...
                 + ' &nbsp; 预发布模板序号:<span id="test-seq-id" style="color:red;">' + test_seq_id + '</span></span>' 
      ...
      '<div><table width="1000" id="mapping-table" border="0" class="t1">' +
          '<tbody><tr>' +
              '<th colspan="3">Feed必填字段设置&nbsp;<a id="reload-test" style="float:right;" href="#nogo">重载该类型模板</a> </th>' +
          '</tr>' +
          '<tr>' +
            '<th>KEY</th>' +
  ...
  $('#reload-test', this.dom_).click(
    function() {
      if(!confirm('确定加载类型' + T.stype_ + '模板吗?')) {
        return;
      }
      // T.KeyListAdd('', '', '');
      $.ajax('/reloadtest?stype=' + T.stype_ + '&version=' + T.version_, {
        success : function(text) {
          alert(text);
        },
        error : function() {
          alert('重新加载 ' + T.stype_ + '模板 失败');
        }
      });
    }
  );
...
FeedKeyView.prototype.ValidateKeys = function() {
  ...
  for(var i = 0; i < nodes.length; ++i) {
    var k = $(nodes[i]).val();
    if (k && k.length > 0 && (k != '(stype)') && !FindKey($(nodes[i]).val())) {
      alert('映射字段不合法:' + k);
      return false;
    }
  }
...
FeedKeyView.prototype.UpdateTopKeys = function() {
  ...
  $('#MiniMergeBy', this.dom_).prepend($('<option value="(stype)">(按类型去重)</option>'));
  $('#NewsMergeBy', this.dom_).prepend($('<option value="">(不去重)</option>'));
  $('#NewsMergeBy', this.dom_).prepend($('<option value="(stype)">(按类型去重)</option>'));
}
...
var TplListItem = function(view, status, view_desc, tpl_content) {
  ...
  this.tpl_ = $('<tr class="expanded-row" style="display:none;"><td colspan="2"><textarea width="80%" rows="30">' + decodeURIComponent(tpl_content) + '</textarea><ul style="margin-left:80%;padding-left:2px;list-style:circle;"><a href="http://www.clearsilver.net/docs/man_templates.hdf" target="_blank">clearsilver模板语言文档</a><br/> <br/><b>默认变量</b><li>新鲜事id : &lt?cs var:id ?&gt </li>' 
              ...
              + '<li>统计信息: &lt?cs var:vType ?&gt </li>'
              + '<li>新鲜事触发者VIP图标id: &lt?cs var:actorVipIconId ?&gt </li>'
              + '<li>新鲜事触发者VIP帽子id: &lt?cs var:actorVipHatId ?&gt </li>'
              + '<li>新鲜事触发者VIP等级: &lt?cs var:actorVipLevel ?&gt </li></ul>'
              ...
              + '<li><a href="/include-ACTOR.txt" target="_blank">ACTOR</a> : 标题中新鲜事触发者的信息</li>' 
    ...
    function() {
      ...
      return function() {
        ...
        $.ajax(url, {
          ...
          context: document.body,
          success: function(msg){
            if(decodeURIComponent(msg).indexOf('code=0') > 0){
              url = '/save-tpl?tpl_id=' + g_tpl_id
                  + '&view=' + view_type
                  + '&stype=' + g_stype
                  + '&version=' + g_version;
              $.ajax(url, {
  ...
  $('#remove-template', this.dom_).click(
    function() {
      ...
      return function() {
        ...
        var stype_id = $('#edit-tpl span.stype_id').text();
        var version = $('#edit-tpl span.stype_version').text();
        var url = '/remove-tpl?tpl_id=' + g_tpl_id + '&view=' + view_type + '&stype=' + g_stype + '&version=' + g_version;
        $.ajax(url, {
