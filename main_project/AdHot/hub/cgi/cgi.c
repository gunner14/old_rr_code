...
static char * Utf8StringTrunc(const char * in, const int in_len, const int max_width) {
  char * out =  (char *)malloc (in_len + 3); // 该长度考虑了'\0'和省略号
  int out_len = 0;

  size_t cur_width = 0;
	size_t last_size = 0; // 最后一个字符的字节数
	size_t i = 0;

	while(cur_width < max_width && i < in_len)
	{
		if ((in[i] & 0x80) == 0) { // 英文字母
			out[out_len++] = in[i];
      last_size = 1;
			cur_width += 1; // 英文字母作1个宽度
			++i;
		} else if ((in[i] & 0xE0) == 0xC0) { // 110, 宽度是2 ?
			if (in[i+1] == 0) {
				break;
      }
			out[out_len++] = in[i];
...
NEOERR * MaxLength(CSPARSE *parse, CS_FUNCTION *csf, CSARG *args, CSARG *result) {
  NEOERR *err;
  char * text;
  long int max_len = 0;
  ...
  char * out_buf = Utf8StringTrunc(text, strlen(text), max_len);
  result->s = out_buf;
  result->alloc = 1;
...
NEOERR *RFeedTime(const char *in, char **ret) {
  ...
  if (diff_minutes <= 0) {
    ...
    snprintf(buf, buf_len - 1, "%d小时前", diff_minutes / 60);
  } else if (diff_minutes >= 24*60 && diff_minutes < 3*24*60) {
    snprintf(buf, buf_len - 1, "%d天前", diff_minutes / (24 * 60));
  } else {
    struct tm timeinfo;
...
NEOERR *cgi_register_strfuncs(CSPARSE *cs)
{
  ...
  if (err != STATUS_OK) return nerr_pass(err);
  err = cs_register_esc_strfunc(cs, "x_rfeedtime", RFeedTime);
  if (err != STATUS_OK) return nerr_pass(err);
  err = cs_register_function(cs, "x_maxlen", 2, MaxLength);
  if (err != STATUS_OK) return nerr_pass(err);
  return STATUS_OK;
}
