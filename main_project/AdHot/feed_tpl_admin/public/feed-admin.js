  ...
  child.each(
    function() {
      ...
      if (kids && kids.length) {
        ...
        if (t.children('data_type').text() == 'number') {
          var n = t.children('name').text();
          T.top_key_list_[T.top_key_list_.length] = n;
        }
      }
    }
  );
...
FeedKeyView.prototype.KeyListAddChild = function(root, key, note, data_type, kids, is_loop) {
  if (root.hasClass('multi')) {
    ...
    root.addClass('multi');
    $('span.child-loop', root).css('visibility', 'visible');
    root.click( function() {
        var t = $(this);
        if (t.hasClass('plus')) {
          t.removeClass('plus');
