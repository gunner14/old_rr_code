...
void MyUtil::initialize() {
    ...
    int mod = service.getCommunicator()->getProperties()->getPropertyAsInt("LocalProducerManager.Mod");
    int interval = service.getCommunicator()->getProperties()->getPropertyAsIntWithDefault("LocalProducerManager.Interval",5);
    xce::serverstate::ServerStateSubscriber::instance().initialize("ControllerLocalProducerManager",&com::renren::datasync::LocalProducerManagerI::instance(),mod,interval);
    
    MyUtil::ServiceMonitorManager::instance().start();
    
    std::string zkAddress = service.getCommunicator()->getProperties()->getPropertyWithDefault("LocalProducerManager.ZkAddress", com::renren::datasync::DEFAULT_ZKADDRESS);
    std::string cluster = service.getCommunicator()->getProperties()->getPropertyWithDefault("LocalProducerManager.Cluster", com::renren::datasync::DEFAULT_CLUSTER);
