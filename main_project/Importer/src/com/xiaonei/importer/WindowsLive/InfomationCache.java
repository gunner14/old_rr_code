...
public class InfomationCache {
	...
	public InfomationCache() {
		_uuid = new ConcurrentLinkedQueue<UUID>();
		_uuuserId = new ConcurrentHashMap<UUID, Integer>();
		_uuidCache = new ConcurrentHashMap<Integer, Map<String, UUID>>();
		_tokenCache = new ConcurrentHashMap<UUID, WindowsLiveAuthToken>();
	...
	private Map<UUID, WindowsLiveAuthToken> _tokenCache;
	...
	public void putToken(UUID id, WindowsLiveAuthToken token) {
		if (_logger.isTraceEnabled()) {
			_logger.trace("putToken UUID[" + id + "] => Token[" + token.token + "," + token.validateFrom + ","
					+ token.validateUntil + "]");
		}
		_tokenCache.put(id, token);
	}

	public WindowsLiveAuthToken getToken(UUID id) {
		if (_logger.isTraceEnabled()) {
			_logger.trace("getToken UUID[" + id + "]");
		}
