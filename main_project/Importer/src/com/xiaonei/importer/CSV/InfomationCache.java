...
public class InfomationCache {
	...
	public void putUUID(OwnerInfo owner, UUID id) {
		...
		if (_uuidCache.containsKey(owner.userId)) {
			...
			if (accounts.containsKey(owner.email)) {
				...
				if (_logger.isTraceEnabled()) {
					_logger
							.trace("remove exist request from cache. owner.userId="
									+ owner.userId
									+ " owner.email="
									+ owner.email);
				}
			...
			if (_logger.isTraceEnabled()) {
				_logger.trace("put new request into cache. owner.userId="
						+ owner.userId + " owner.email=" + owner.email);
			}
			accounts.put(owner.email, id);
		} else {
			if (_logger.isTraceEnabled()) {
				_logger.trace("put this into cache: owner.userId="
						+ owner.userId + " owner.email=" + owner.email);
			}
	...
	public UUID getUUID(OwnerInfo owner) {
		if (_logger.isTraceEnabled()) {
			_logger.trace("getUUID owner.userId=" + owner.userId
		...
		throw new NoSuchElementException(
				"No such request for OwnerInfo[userId=" + owner.userId
	...
	public void putContacts(UUID id, BuddyList contacts) {
		if (_logger.isTraceEnabled()) {
			_logger.trace("putContacts UUID[" + id + "] => BuddyList["
					+ contacts.status.code + ", " + contacts.status.message
					+ "]");
		}
			...
			try {
				if (_uuid.size() > 500) {
					...
					if (_logger.isInfoEnabled()) {
						_logger.info("Reducing cache done. size="
								+ _uuid.size() + "|" + _uuuserId.size() + "|"
								+ _uuidCache.size() + "|"
								+ _contactsCache.size());
					}
				}
