...
public class FillUserMailHandler implements ResultSetHandler {
	...
	public FillUserMailHandler(BuddyList buddy, Map<Integer, String> id2msnMap) {
		this(buddy);
		...
		while (rs.next()) {
			...
			try {
				...
				if (_logger.isDebugEnabled()) {
					_logger.debug("email = " + email + " id=" + id);
				}
...
//							-1, "SELF");
					...
					_buddy.buddies[_key2pos.get(key)].status = new BuddyInfoStatus(
							1, "USER");
				_buddy.users.put(key, new BuddyUserInfo(email, id, "", null, 0, 0, null));
			} catch (Exception e) {
				e.printStackTrace();
