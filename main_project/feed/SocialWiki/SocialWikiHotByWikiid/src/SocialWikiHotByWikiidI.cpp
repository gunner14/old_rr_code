...
Ice::ObjectPtr SocialWikiFactoryI::create(Ice::Int wikiid) {
    ...
    Statement sql;
    sql << "SELECT " << SW_COL_FEEDID << "," << SW_COL_POSITION << "," << SW_COL_TYPE << ", UNIX_TIMESTAMP(" << SW_COL_UPDATETIME  << ") as " << SW_COL_UPDATETIME << " FROM " << table.str()
        << " WHERE " << SW_COL_WIKIID << "= " << wikiid
        << " AND UNIX_TIMESTAMP(" << SW_COL_UPDATETIME << ")>(UNIX_TIMESTAMP()-" <<  DURATION_TIME << ")"
        << " ORDER BY " << SW_COL_POSITION << " DESC LIMIT " << LOAD_LIMIT;
