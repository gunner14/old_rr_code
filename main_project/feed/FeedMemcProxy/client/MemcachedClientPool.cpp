...
void PoolManager::CheckValid(){
	...
	if("im" == business_){
  	cluster = IMMemcProxyAdapter::instance().GetClusterSize();
	}else{
  	cluster = FeedMemcProxyAdapter::instance().GetClusterSize();
	}
	MCE_DEBUG("PoolManager::CheckValid--> business_:" << business_ << " cluster:" << cluster);
  if(cluster == 0){
    cluster = 1;
  }
  ...
  for(int i=0; i<cluster; i++){
    ...
    try{
			if("im" == business_){
				info = IMMemcProxyAdapter::instance().GetProxyInfo(i);
			}else{
