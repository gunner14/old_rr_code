...
void CompressWrap::Decompress(const char * data, string * output) {
  ...
  //MCE_INFO("CompressWrap::Decompress --> decompress " << QLZ_SCRATCH_DECOMPRESS << " " << decompressed_size+2);
  char decompressed[16*1024] = {0};
  //char scratch[QLZ_SCRATCH_DECOMPRESS] = {0};
  if(decompressed_size+2 > 16*1024){
    MCE_INFO("CompressWrap::Decompress --> buffer not enough " << decompressed_size+2);
    return;
  }
  //MCE_INFO("CompressWrap::Decompress --> decompress beg");
  size_t sz = qlz_decompress(data, decompressed, scratch);
  //MCE_INFO("CompressWrap::Decompress --> decompress end");
