...
const static int DB_ZONE_SIZE=1000000000;
const static int DB_ZONE_SPLIT=18;

class FeedContentDB {
public:
  static const string GetTableName(int mod, int zone) {
    ostringstream table;

	if (zone >= DB_ZONE_SPLIT) {
    	table << "feed_content_" << mod << "_" << zone;
	} else if (zone < 0) {
    	table << "feed_content_bc_" << abs(mod) << "_" << abs(zone);
	} else {
    	table << "feed_content_" << mod;
	}
  ...
  static const string GetTableName(Ice::Long id) {
    ostringstream table;
	int zone = id/DB_ZONE_SIZE;

	if (zone >= DB_ZONE_SPLIT) {
    	table << "feed_content_" << id % 100 << "_" << id/DB_ZONE_SIZE;
	} else if (zone < 0) {
    	table << "feed_content_bc_" << abs(id) % 100 << "_" << abs(id)/DB_ZONE_SIZE;
	} else {
  ...
  static map<int, map<int, vector<Ice::Long> > > GetIdMap(const vector<Ice::Long> ids) {
    map<int, map<int, vector<Ice::Long> > > id_map;
    for (unsigned i = 0; i != ids.size(); ++i) {
      id_map[ids.at(i) % 100][ids.at(i)/DB_ZONE_SIZE].push_back(ids.at(i));
    }
    return id_map;
  }
  ...
  static const string GetInstance(int zone) {
	return (zone < 0 || zone >= DB_ZONE_SPLIT) ? "feed_content_db":"feed_db"; 
  }
