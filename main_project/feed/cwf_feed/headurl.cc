...
struct Storage {
  ...
  std::string LocateNormal(const std::string & pattern) {
    ...
    MCE_INFO("group not exist " << pattern);
  ...
  static bool LoadFromDB(MapType* map) {
    ...
    for (size_t i = 0; i<result.num_rows(); ++i) {
      ...
      MCE_INFO(row["group_name"] << " " << row["domain"]);
      Node node;
      // node.id = row["id"];
    ...
    for (MapType::const_iterator i=map->begin(); i!=map->end(); ++i) {
      MCE_INFO(i->first << " " << i->second.size());
    }
  ...
  static bool LoadFromCode(MapType * map) {
    Node arr[] = {
//      {"", "", 0, 0},
#include "feed/feedview/storage.inc"
    };
...
#endif
    if (LoadFromCode(&map)) {
      MCE_INFO("Storage LoadFromDB failed. LoadFromCode succeeded.");
      boost::mutex::scoped_lock guard(lock_);
      map_.swap(map);
    }
  }
