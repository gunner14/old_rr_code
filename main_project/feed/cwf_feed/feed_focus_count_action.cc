#include "feed/cwf_feed/feed_focus_count_action.h"
...
cwf::HttpStatusCode FeedFocusCountAction::Process(cwf::Request * req, cwf::Response * res) {
  int cwf_pid = getpid();
  MCE_INFO("FeedFocusCountAction::Process --> pid:" << cwf_pid);
...
// Expires	Thu, 01 Jan 1970 00:00:00 GMT
  ...
  // LOG(INFO) << "uid:" << id;
  MCE_INFO("FeedFocusCountAction::Process --> pid:" << cwf_pid <<  "uid:" << u->id());

  //feed::FocusManagerPrx fprx = CachedFocusAdapter(u->id());
  //if (!fprx) {
    //MCE_INFO("FeedFocusCountAction::Process --> pid:" << cwf_pid << " fprx is NULL!");
  ...
  try {
    ...
    focus_count = feed::FeedFocusAdapter::instance().GetFocusFeedIncCount(u->id());
  } catch (Ice::Exception& e) {
    MCE_WARN("FeedFocusCountAction::Process --> pid:" << cwf_pid <<  "uid:" << u->id() << " exception:" << e);

  } catch (std::exception& e) {
    MCE_WARN("FeedFocusCountAction::Process --> pid:" << cwf_pid <<  "uid:" << u->id() << " exception:" << e.what());
