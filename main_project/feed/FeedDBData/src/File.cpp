...
LogFile::LogFile(int fileId) : File(sizeof(FeedItem)+sizeof(BlockInfo)){
...
bool LogFile::Restore(char* buf){
	...
	if(buf){
		...
		for(size_t i=0; i<log_count_; i++){
			BlockInfo* blk = (BlockInfo*)(log_buf_ + i*block_size_);
	...
	for(size_t i=0; i<log_count_; i++){
		...
		*blk = *(BlockInfo*)(log_buf_ + i*block_size_);
		FeedItem* item = (FeedItem*)(log_buf_ + i*block_size_ + sizeof(BlockInfo));
		if(item->feed < 0){
		...
		if(blk->curCount == -3){
			CacheManager::instance().removeAll(blk->userId, blk->fileId, blk->blockIndex, false);
		}
		if(i>0 && i%10000==0){
...
void LogFile::Log(const FeedItemBlockInfoPtr& data){
	...
	//TimeStat t;
	char buf[128] = {0};
	memcpy(buf, data->blk.get(), sizeof(BlockInfo));
	memcpy(buf + sizeof(BlockInfo), &(data->item), sizeof(FeedItem));
	Write(buf);
	//PerformanceStat::instance().incAsyncPut(t.getTime());
}

void LogFile::Log(const FeedItemBlockInfoSeq& data){
	IceUtil::Monitor<IceUtil::Mutex>::Lock lock(_monitor);
	if(changing_file_){
		_monitor.wait();
	}
	...
	for(int i=0; i<(int)data.size(); i++){
		memcpy(buf+off, data.at(i)->blk.get(), sizeof(BlockInfo));
		memcpy(buf+off+sizeof(BlockInfo), &(data.at(i)->item), sizeof(FeedItem));
		count++;
