...
inline bool RedisClient::GetList<std::string>(const std::string& key, std::vector<std::string>& ret_vec, int begin, int limit) {
	...
	oss_log << "RedisClient::GetList<std::string> --> address_:" << address_ << " port_:" << port_ << " key:" << key << " begin:" << begin << " limit:" << limit; 
  ...
  if(limit < 1) {
    MCE_WARN("RedisClient::GetList<std::string> --> address_:" << address_ << " port_:" << port_ << " key:" << key << " begin:" << begin << " limit:" << limit);
    return false; 
  }
  ...
  while(start <= stop) {
    ...
    loop_log << "RedisClient::GetList<std::string> --> address_:" << address_ << " port_:" << port_ << " key:" << key << " start:" << start << " batch_end:" << batch_end << " fetch size:" << fetch_size; 
