...
bool FeedTestAction::Match(const std::string& url) const {
  // http://www.renren.com/feedretrieve.do
  return boost::starts_with(url, "/ebpt/show") ||  boost::starts_with(url, "/ebpt");
}

cwf::HttpStatusCode FeedTestAction::Process(cwf::Request * req, cwf::Response * res) {
...
// Expires	Thu, 01 Jan 1970 00:00:00 GMT
  ...
  //if (u->id() % 10 == 2 || u->id() % 10 == 1 || u->id() % 10 == 9 || u->id() % 10 == 3|| u->id() % 10 == 4|| u->id() % 10 == 0) 
  {
    ...
    try {
      ...
      MCE_INFO("FeedTestAction::Process --> type :" << type << " uid :" << u->id());
      ...
      MCE_WARN("FeedTestAction::Process --> uid :" << u->id() << "  : " << e);
 ...
 /* else {
...

  {
    ...
    try {
      ...
      //LOG(INFO) << "prx:" << prx;
      if (!prx) {
        //LOG(INFO) << "feedtypeaction.cc::Process --> FeedStatLog prx is NULL !";
        MCE_WARN("FeedTestAction::Process --> FeedStatLog prx is NULL !");
      } else {
      ...
      //LOG_F(WARNING) << e;
      MCE_WARN("FeedTestAction::Process --> " << e);
    }
  ...
  //LOG(VERBOSE) << "p: " << page << " seq: " << feed_seq.size()
   // << " expand: " << feed_count;
  MCE_INFO("FeedTestAction::Process --> p: " << page << " " << feed_count << "/" << feed_seq.size());
...
static void InitTestAction() {
  // LOG(INFO) << "Register FeedTestAction";
  cwf::FrameWork::RegisterAction(new FeedTestAction);
