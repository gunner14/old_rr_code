...
void CacheManager::put(const FeedItemBlockInfo64Ptr& data, int flag, bool overwrite){
	BlockInfo64Ptr blk = data->blk;
	if(blk->key <= 0){
		MCE_WARN("CacheManager::put--> invalid userid f:" << blk->fileId << " b:" << blk->blockIndex << " userid:" << blk->key << " cc:" << blk->curCount
                << " preF:" << blk->preFileId << " preB:" << blk->preBlockIndex); 
	}
	if(blk->key==261947900 || blk->key==227805680){
		MCE_INFO("@put@ userid:"<<blk->key << " node:" << blk->serverId <<" file:"<<blk->fileId<<" index:"<<blk->blockIndex
...

	{
		...
		if(cache->GetHead()->user_id!=0 && (cache->GetHead()->user_id!=(UINT32)blk->key)){
			MCE_WARN("CacheManager::put --> userid err, " << cache->GetHead()->user_id << " " << blk->key << " f:" << blk->fileId << " b:" << blk->blockIndex << " flag:" << flag);
			return;
		}
...
void CacheManager::syncPut(const FeedItemBlockInfo64Ptr& data, bool log, int flag){
	BlockInfo64Ptr blk = data->blk;
	...
	if(blk->curCount == 1){
		//更新上一块的next指针
		changeNextPtr(blk->key, blk->preFileId, blk->preBlockIndex, blk->fileId, blk->blockIndex, true);
	}
}

void CacheManager::asyncPut(const FeedItemBlockInfo64Ptr& data, bool log, int flag, bool overwrite){
	//TimeStat t;
	BlockInfo64Ptr blk = data->blk;
	if(log && locateDataFile(blk->fileId)){
	...
	if(blk->curCount == 1){
		...
		changeNextPtr(blk->key, blk->preFileId, blk->preBlockIndex, blk->fileId, blk->blockIndex, false);
	}
}

void CacheManager::asyncPut(const FeedItemBlockInfo64Seq& data, bool log, int flag, bool overwrite){
	TimeStat t;
	...
	for(int i=0; i<data.size(); i++){
		BlockInfo64Ptr blk = data.at(i)->blk;
...
void CacheManager::changeNextPtr(Ice::Long key, int fileId, int blockIndex, int nextFileId, int nextBlockIndex, bool sync){
	if(fileId==0 || blockIndex==-1){
		return;
	}
	BlockInfo64Ptr tmp = new BlockInfo64;
	tmp->key = key;
	tmp->totalCount = -2;
	tmp->curCount = -2;
...
void CacheManager::changePrePtr(Ice::Long key, int fileId, int blockIndex, int preFileId, int preBlockIndex, bool sync){
	if(fileId==0 || blockIndex==-1){
		return;
	}
	BlockInfo64Ptr tmp = new BlockInfo64;
...
int CacheManager::get(const BlockInfo64Ptr& blk, FeedItem* data, int count, bool log){

	if(log){
		MCE_INFO("CacheManager::get --> userid:"<<blk->key<<" file:"<<blk->fileId<<" index:"<<blk->blockIndex
	...
	for(int i=0; c<count; i++){
		...
		if(!file){
			MCE_DEBUG("#get# can not find file " << blk->key << " " << file_id << " " << block_index);
			break;
		}
		BlockCachePtr cache = file->Locate(block_index);
		//BlockCachePtr cache = locateBlockCache(fileId, blockIndex);
		if(!cache){
			MCE_INFO("#get# can not find block "  << blk->key << " " << block_index << " " << file_id);
			break;
		}
		//blk里的key不为0时，不检查用户合法性，测试用
		if(blk->key!=0 && (cache->GetHead()->user_id!=(UINT32)blk->key)){
			//MCE_WARN("CacheManager::get --> userid err, " << cache->GetHead()->user_id << " " << blk->key << " f:" << blk->fileId << " b:" << blk->blockIndex);
			break;
		}
		...
		if(blk->key==0 || log){
			...
			/*MCE_INFO("CacheManager::get --> userid:" << blk->key << " f:" << file_id << " b:" << block_index << " cc:" 
...
void CacheManager::removeAll(Ice::Long key, int fileId, int blockIndex, bool log){
...
void CacheManager::remove(const BlockInfo64Ptr& blk, long long feed, bool depth, bool log){
	...
	for(;;){
		...
		if(blk->key!=0 && cache->GetHead()->user_id != (UINT32)blk->key){
			MCE_WARN("CacheManager::remove --> userid err, " << cache->GetHead()->user_id << " " << blk->key 
	...
	MCE_INFO("CacheManager::remove --> user:" << blk->key << " -- feed:" << feed << " deleted:" << deleted);
	if(deleted && log){
		DataFilePtr file = locateDataFile(file_id);
		if(!file){
			return;
...
/*void CacheManager::syncRemove(const BlockInfo64Ptr& blk, long long feedId, bool log){
	if(log){
		FeedItem item;
		item.feed = -feedId;
...
void CacheManager::asyncRemove(const BlockInfo64Ptr& blk, long long feedId, bool log){
	//FeedItem item;
	//memset(&item, 0, sizeof(FeedItem));
	//item.feed = -feedId;
	/*if(log){
...
bool CacheManager::moveBlock(const BlockInfo64Ptr& blkInfoFrom, const BlockInfo64Ptr& blkInfoTo){
	return true;
}
...
void CacheManager::decTotalCount(const UserFeedCount64Seq& userFeedCounts){
	for(int trycount=0; trycount<10; trycount++){
		try{
			MCE_INFO("CacheManager::decTotalCount --> dec total count " << userFeedCounts.size());
			FeedDBMasterAdapter::instance().decTotalCount(userFeedCounts);
			break;
		}catch(Ice::Exception& e){
...
BlockInfo64Seq CacheManager::getBlockInfoBatch(const LongSeq& ids){
	BlockInfo64Seq blkInfos;
	for(int trycount=0; trycount<10; trycount++){
		try{
			blkInfos = FeedDBMasterAdapter::instance().getBlockBatch(ids);
			break;
		}catch(Ice::Exception& e){
			MCE_WARN("CacheManager::getBlockInfo64Batch --> " << trycount << " ids:" << ids.size() << " " << e);
			sleep(5);
		}
	}
	MCE_INFO("CacheManager::getBlockInfo64Batch --> getBlocks " << ids.size() << " " << blkInfos.size());
	return blkInfos;
}

void CacheManager::checkBlocks(const BlockInfo64Seq& blkInfos, const vector<BlockCache*>& caches,
			vector<BlockCache*>& blksToDelete, vector<Ice::Long>& userIdsToNew, vector<BlockCache*>& blksToNew, vector<BlockCache*>& blksToMove)
{
	for(size_t i=0; i<blkInfos.size(); i++){
		BlockInfo64Ptr blkInfo = blkInfos.at(i);
		...
		if(blkInfo->key != h->user_id){
			MCE_WARN("CacheManager::checkBlocks --> userid err " << blkInfo->key << " " << h->user_id);
			continue;
		}
		...
		//分配新块
			userIdsToNew.push_back(blkInfo->key);
...
void CacheManager::deleteBlocks(const vector<BlockCache*>& blksToDelete){
	UserFeedCount64Seq userFeedCounts;
	for(size_t i=0; i<blksToDelete.size(); i++){
		BlockHead* head = blksToDelete.at(i)->GetHead();
		UserFeedCount64Ptr feedCount = new UserFeedCount64;
		feedCount->key = head->user_id; 
...
void CacheManager::newBlocks(const vector<BlockCache*>& blksToNew, const vector<Ice::Long>& userIdsToNew){
	...
	BlockInfo64Seq newBlkInfos;
	for(int trycount=0; trycount<10; trycount++){
		try{
			newBlkInfos = FeedDBMasterAdapter::instance().newBlockBatch(userIdsToNew);
			break;
		}catch(Ice::Exception& e){
	...
	for(size_t i=0; i<newBlkInfos.size(); i++){
		BlockInfo64Ptr newBlkInfo = newBlkInfos.at(i);
		BlockCachePtr c = blksToNew.at(i);
		if(c->GetHead()->user_id != newBlkInfo->key){
			MCE_WARN("CacheManager::newBlocks --> userid err when new blk " << newBlkInfo->key << " " << c->GetHead()->user_id);
			continue;
		}
		
		BlockInfo64Ptr tmp = new BlockInfo64;
		tmp->key = c->GetHead()->user_id;
		tmp->totalCount = 0;
		tmp->curCount = 0;
		...
		changeNextPtr(tmp->key, tmp->preFileId, tmp->preBlockIndex, newBlkInfo->fileId, newBlkInfo->blockIndex, true);
		MCE_INFO("CacheManager::newBlocks --> userid:" << tmp->key << " preF:" << c->file_id() << " preB:" << c->index()
...
/*void CacheManager::moveBlocks(int fileId, const vector<BlockCache*>& blksToMove, vector<BlockCache*>& need2Rectify){
	for(size_t i=0; i<blksToMove.size(); i++){
		...
		if(head->cur_count != 0){
			...
			BlockInfo64Ptr tmp = new BlockInfo64;
			tmp->key = blksToMove.at(i)->GetHead()->user_id;
			tmp->totalCount = 0;
			tmp->curCount = 0;
...
set<int> CacheManager::deleteFile(int fileId){
	...
	for(; index<count; index++){
		...
		if((caches.size()==10000) || (index == count-1)){
			...
			BlockInfo64Seq blkInfos = getBlockInfo64Batch(ids);
			vector<BlockCache*> blksToMove;
			vector<int>	keysToNew;
			vector<BlockCache*> blksToDelete;
			vector<BlockCache*> blksToNew;
			UserFeedCountSeq userFeedCounts;
			checkBlocks(blkInfos, caches, blksToDelete, keysToNew, blksToNew, blksToMove);
...
void CacheManager::rectifyBlocks(int fileId, const vector<BlockCache*>& need2Rectify){
	...
	for(size_t i=0; i<need2Rectify.size(); i++){
		...
		if((tmpSeq.size()==10000) || (i==need2Rectify.size()-1)){
			BlockInfo64Seq tmpBlkInfos = getBlockInfoBatch(tmpSeq);
	...
	for(size_t i=0; i<need2Rectify.size(); i++){
		...
		if(c->GetHead()->user_id != blkInfo->key){
			MCE_WARN("CacheManager::retifyBlocks --> rectify id err " << blkInfo->key << " " << c->GetHead()->user_id);
			continue;
		}
		if(blkInfo->preFileId!=fileId){
			MCE_WARN("CacheManager::retifyBlocks --> rectify something err " << blkInfo->key << " preF:" << blkInfo->preFileId);
                        continue;
		}
		changePrePtr(blkInfo->key, blkInfo->fileId, blkInfo->blockIndex, c->file_id(), c->index(), true);
	}
...
void CacheManager::deleteFileKeepMin(int fileId){
	...
	for(; index<=count; index++){
		...
		if((caches.size()==10000) || (index==count && !caches.empty())){
			MCE_INFO("CacheManager::deleteFile --> " << fileId << " " << index);
			BlockInfo64Seq blkInfos = getBlockInfoBatch(ids);
			vector<BlockCache*> blksToMove;
			vector<BlockCache*> blksToMove1;
			vector<Ice::Long>	userIdsToMove;	

			vector<BlockCache*> blksToNew;
			vector<Ice::Long>	userIdsToNew;
			...
			for(size_t i=0; i<blkInfos.size(); i++){
				...
				if(blkInfo->key != h->user_id){
					MCE_WARN("CacheManager::deleteFileKeepMin --> userid err " << blkInfo->key << " " << h->user_id);
					continue;
				}
				...
				//保证每个用户至少30条新鲜事，其实已经超过30了
					...
					userIdsToMove.push_back(blkInfo->key);
			...
			*/
			...
			for(size_t i=0; i<blksToMove.size(); i++){
				...
				if(c1 && c2 && (c1->GetHead()->user_id == c2->GetHead()->user_id)){
					...
					FeedItemBlockInfo64Ptr data = new FeedItemBlockInfo64;
					MCE_INFO("CacheManager::deleteFile --> move " << tmp->key <<" size:" << items.size() 
