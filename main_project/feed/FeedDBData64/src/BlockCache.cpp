...
bool BlockCache::PutData(const BlockInfo64Ptr& blk, const FeedItem* data, bool overwrite){
	BlockHead* head = GetHead();
	if(!head){
		return false;
	}
	if(head->user_id > 0 && (UINT32)blk->key != head->user_id){
		MCE_WARN("BlockCache::putData --> user id err,  " << head->user_id << " " << blk->key);
		return false;
	}
	...
	for(; pos!=End(); pos++){
		if(pos->feed < data->feed){
			...
			if(overwrite){
			  MCE_WARN("BlockCache::PutData --> feed exist overwrite" << blk->key << " f:" << blk->fileId << " b:" << blk->blockIndex << " feed:" << data->feed);
        *pos = *data;
        return true;
      }
			MCE_WARN("BlockCache::PutData --> feed exist return " << blk->key << " f:" << blk->fileId << " b:" << blk->blockIndex << " feed:" << data->feed);
      return false;
		}
	}
