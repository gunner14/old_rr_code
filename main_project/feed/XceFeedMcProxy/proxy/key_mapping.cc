...
static string online_cache_backup_memcached_nodes = "10.3.22.119:11219 800;"\
...
static string talk_group_view_nodes = "10.3.22.43:11219 800;";
static string talk_group_message_nodes = "10.3.22.119:11219 800;";
static string talk_group_info_nodes = "10.3.22.43:11219 800;";
*/
static string memcached_config_path = "memcachedconfig";

void LoadCluster(std::string ns,std::string nodes) {
	
	map<string, Continuum *>::const_iterator it = g_clusters.find(ns);
	if (it == g_clusters.end()) {	
		Continuum * c = new Continuum;
		if (c->SetCacheNodes(nodes)) {
			MCE_INFO("加载 Continuum " << ns << ":" << nodes);
			g_clusters.insert(make_pair(ns, c));
		} else {
			MCE_WARN("加载 Continuum 失败 : " << ns << "-" << nodes);
			delete c;
		} 
	} else { 
		if (it->second->SetCacheNodes(nodes)) {
			MCE_INFO("加载 Continuum " << ns << ":" << nodes);
		} else {
			MCE_WARN("加载 Continuum 失败 : " << ns << "-" << nodes);
		}
...
bool InitCacheClusters() {
	if(!ProxyServer::instance().InitService(memcached_config_path))
		return false;

{
	std::map<std::string, std::string> memc_ips = ProxyServer::instance().GetMemcIps();
	for (std::map<std::string, std::string>::iterator iter = memc_ips.begin();
	iter!=memc_ips.end();++iter) {
		LoadCluster(iter->first, iter->second);
  }
}
	return true;
/*
