...
void ClientConnection::Start() {
  try{ 
    ...
    if(ProxyServer::instance().ClientSize() > 100000){
      Close("limited");
      return;
    }
...
bool ClientConnection::ParserSetCmd(MemCmdHead* head){
  ...
  cli->Init(head->type_, recv_buf_, cur_recv_len_, this, 1);
  cli->Request();
...
bool ClientConnection::ParserGetCmd(MemCmdHead* head) {
  ...
  map<ip::tcp::endpoint, string> cmd_lines;
	map<ip::tcp::endpoint, int> cmd_keys;
  for (size_t i = 0; i < keys.size(); ++i) {
    ...
    if (it == cmd_lines.end()) {
      it = cmd_lines.insert(make_pair(ep, string("get"))).first;
		...
		map<ip::tcp::endpoint, int>::iterator iter = cmd_keys.find(ep);
		if (iter == cmd_keys.end()) {
			iter = cmd_keys.insert(make_pair(ep,0)).first;
		}
    it->second += ' ';
    it->second += keys[i];
		iter->second += 1;
  }
	cmds_ = cmd_lines.size();	
  map<ip::tcp::endpoint, string>::iterator it = cmd_lines.begin();
	map<ip::tcp::endpoint, int>::iterator iter; 
  for(; it!=cmd_lines.end(); ++it){
    ...
    up_clis_.insert(make_pair<MemcachedClient*, string>(cli, it->second));
		iter = cmd_keys.find(it->first);
		if(iter != cmd_keys.end()) {
    	cli->Init(head->type_, it->second.c_str(), it->second.size(), this, iter->second);
		} else {
			cli->Init(head->type_, it->second.c_str(), it->second.size(), this, 0);
		}
...
void ClientConnection::HandleRead(int index, const boost::system::error_code& error, size_t bytes_transferred)
{
  ...
  if(!res){
	...
	//MCE_DEBUG("ClientConnection::HandleRead-->data_len_:" << head.data_len_ <<  " body_len_:" << head.body_len_ << " head.key_:" << head.key_ << " key_len_:" << head.key_len_ << " flag:" << head.flag_);

	size_t first_timeout = first_timeout_;
	if(head.key_){
		char* pos = (char *)memchr(head.key_, '#', strlen(head.key_));
		if(pos){
			string keytype = string(head.key_).substr(0, pos-head.key_);
			if("IMONLINE" == keytype || "IMONLINEBACKUP" == keytype){
				first_timeout = 250;
			}
			//else{
			//	first_timeout = 50;
			//}
			//MCE_DEBUG("ClientConnection::HandleRead--> pos:" << (pos - head.key_) << " key_type:" << keytype << " first_timeout:" << first_timeout);
		}
	}
  gettimeofday(&begin_time_, NULL); 
  if (first_timeout > 0) {
    timer_.expires_from_now(boost::posix_time::millisec(first_timeout));
...
void ClientConnection::HandleMemcCommandTimeout(int index, const boost::system::error_code& error) {
  ...
  MCE_INFO("ClientConnection::HandleMemcCommandTimeout --> write end to client, " << index_ << " reqtype:" << req_type_ << " " 
            ...
            << " write:" << write_bytes_ << " cost:" << DiffTime(begin_time_, end) << " up:" << up_clis_.size() << " ip:" << cli_ip_ << " port:" << cli_port_ << " cmds:" << cmds_ << " clis:" << up_clis_.size() << " " << this);
  up_clis_.clear();
  //missed_keys_.clear(); 
  CheckRead(true);
}
