...
void MemcachedClient::Reset(){
  ...
  MCE_INFO("MemcachedClient::Reset --> " << this << " type:" << cmd_type_ << " cost:" << CaculateCost() << " " << business_type_ << " recv:"
            ...
            << " " << recv_values_ <<" " << " "  << keys_ << " dur:" << (time(NULL)-start_time_) << " ip:" << endpoint_.address().to_string() << " port:"<< endpoint_.port() << " " << handle_);
  reseting_ = true;
  handle_ = NULL;
  socket_.close();
...
void MemcachedClient::Init(CmdType cmd_type, const char* cmd_buf, int len, MemcDataHandle* handle, int keys){
  recv_values_ = 0;
  write_len_ = 0;
  cur_recv_len_ = 0; 
  total_recv_len_ = 0; 
  ...
  //MCE_INFO("MemcachedClient::Init --> " << cmd_buf);
	char *p = (char*)memchr(cmd_buf,'#',len);
  business_type_ = string(cmd_buf+4, p-cmd_buf-4);
	keys_ = keys;
}
