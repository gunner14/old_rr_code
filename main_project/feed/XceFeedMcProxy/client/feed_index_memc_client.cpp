...
bool FIndexMemcClient::AddFIndex(Ice::Long source, Ice::Int stype, Ice::Int actor, Ice::Long feedid, Ice::Long miniMerge) {
  ...
  ts.reset();
	if (cost_mem < 160) {
  	MCE_INFO("FIndexMemcClient::AddFIndex -->  key:" << key_str << " r:" << r << " cost_total:" << cost_mem << " ip:" << ip); 
	} else {
  	MCE_WARN("FIndexMemcClient::AddFIndex -->  key:" << key_str << " r:" << r << " cost_total:" << cost_mem << " ip:" << ip); 
	}
...
bool FIndexMemcClient::AppendFIndex(Ice::Long source, Ice::Int stype, Ice::Int actor, Ice::Long feedid, Ice::Long miniMerge) {
  ...
  cost_mem = ts.getTime();
	string ip = cli.second->GetIp();
	if(cost_mem < 160) {
  	MCE_INFO("FIndexMemcClient::AppendFIndex -->  ret:" << r << " key_str:" << key_str << " cost_total:" << cost_mem << " ip:" << ip); 
	} else {
  	MCE_WARN("FIndexMemcClient::AppendFIndex -->  ret:" << r << " key_str:" << key_str << " cost_total:" << cost_mem << " ip:" << ip); 
	}
...
bool FIndexMemcClient::RemoveFIndex(Ice::Long source, Ice::Int stype) {
  ...
  if(!cli.second->Delete(key_str)) {
    MCE_WARN("FIndexMemcClient::RemoveFIndex -->  remove failed!  key:" << key_str << " ip:" << ip);
  ...
  ts.reset();
	if(cost_mem < 160) {
  	MCE_INFO("FIndexMemcClient::RemoveFIndex --> key:" << key_str << " cost_total:" << cost_mem << " ip:" << ip);
	} else {
  	MCE_WARN("FIndexMemcClient::RemoveFIndex --> key:" << key_str << " cost_total:" << cost_mem << " ip:" << ip);
	}
