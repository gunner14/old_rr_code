...
FeedContentDict FeedMemcClient::GetFeedDict(const MyUtil::LongSeq& ids) {
  ...
  pool_.ReleaseClient(cli.first, cli.second);
	float cost_total = cost_mem+cost_decom;
	if(cost_total < 160) {
  	MCE_INFO("FeedMemcClient::GetFeedDict --> ids:" << ids.size() << " res:" << dict.size() << " decompress:" << decompress_count
          << " cost_mem:" << cost_mem << " cost_par:" << cost_decom << " cost_total:" << cost_total
          << " ip:" << ip);
	} else {
  	MCE_WARN("FeedMemcClient::GetFeedDict --> ids:" << ids.size() << " res:" << dict.size() << " decompress:" << decompress_count
          << " cost_mem:" << cost_mem << " cost_par:" << cost_decom << " cost_total:" << cost_total
          << " ip:" << ip);
	}
  return dict;
...
bool FeedMemcClient::SetFeed(const FeedContentPtr& content) {
  ...
  pool_.ReleaseClient(cli.first, cli.second);
	cost_mem = ts.getTime();
	float cost_total = cost_comp + cost_mem;
	if(cost_total < 160) {
  	MCE_INFO("FeedMemcClient::SetFeed --> feedid:" << key << " cost_mem:" << cost_mem << " cost_par:" << cost_comp << " cost_total:" << cost_total << " ip:" << ip);
	} else {
  	MCE_WARN("FeedMemcClient::SetFeed --> feedid:" << key << " cost_mem:" << cost_mem << " cost_par:" << cost_comp << " cost_total:" << cost_total << " ip:" << ip);
	}
