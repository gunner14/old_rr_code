#ifndef __FEED_SERVER_STATE_CLIENT__
#define __FEED_SERVER_STATE_CLIENT__
...
class FeedServerStateSubscriber;

class FeedServerStateBackDoorI : virtual public ServerStateBackDoor {
public:
    FeedServerStateBackDoorI(const Ice::ObjectAdapterPtr& adapter,
            FeedServerStateSubscriber& sss) : subscriber_(sss) {
        if (adapter) {
            ostringstream oss;
            oss << "SSBD" << index_;
            index_++;
            id_ = oss.str();
            adapter->add(this, adapter->getCommunicator()->stringToIdentity(id_));
        }
    }
    virtual MyUtil::Str2StrMap control(const Str2StrSeqMap& data, const Ice::Current& c);

private:
    FeedServerStateSubscriber& subscriber_;
    std::string id_;
    static int index_;
};
typedef IceUtil::Handle<FeedServerStateBackDoorI> FeedServerStateBackDoorIPtr;

class FeedServerStateSubscriber : public MyUtil::SbjSubscriber,
			public MyUtil::Singleton<FeedServerStateSubscriber>,
			public IceUtil::Mutex {

public:
    FeedServerStateSubscriber():
        _hasTopic(false),
        _inited(false) 	{		
    };
...
private:
    ...
    bool _hasTopic;
    bool _inited;
    FeedServerStateBackDoorIPtr _door;

    void init_ServerStateBackDoor();
};

typedef IceUtil::Handle<FeedServerStateSubscriber> FeedServerStateSubscriberPtr;
