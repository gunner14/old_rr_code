...
public:
	...
	void addEDMFeed(const FeedItem& item, int userid){
		try{
			...
			MCE_WARN("EDMFeedManager::addEDMFeed. uid:" << userid << " fid:" << item.feed << " insert db fail. error:"<< e);
			return;
		}catch (std::exception& e) {
			MCE_WARN("EDMFeedManager::addEDMFeed. uid:" << userid << " fid:" << item.feed << " insert db fail. error:" << e.what());
			return;
	  }

		IceUtil::Mutex::Lock lock(_mutex[index(userid)]);
		_pool[index(userid)][userid].push_back(item);
		MCE_INFO("EDMFeedManager::addEDMFeed. insert mem. uid:" << userid << " fid:" << item.feed);
	}
