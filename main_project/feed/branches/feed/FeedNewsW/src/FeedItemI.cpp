...
void FeedItemManagerI::ResortFeedItems(int uid, MyUtil::LongSeq feedIdSeq,
    const Ice::Current&) {
  //  int index = GetIndex(uid);
  //  UidIndex& uid_index = pool_[index].get<1> ();
  //  UidIndex::iterator it = uid_index.find(uid);
  //  if (it != uid_index.end()) {
  //    MCE_INFO("FeedItemManagerI::EraseUser. uid in mem. uid:"<< uid);
  //    it->ResortFeedItems(uid,feedIdSeq);
  //  } else {
  //    MCE_INFO("FeedItemManagerI::EraseUser. uid not in mem. uid:" << uid);
  //  }
  ...
  int index = GetIndex(uid);
  {
    ...
    if (user_feed) {
      user_feed->live_feed_.ResortFeedItems(uid, feedIdSeq);
    }
