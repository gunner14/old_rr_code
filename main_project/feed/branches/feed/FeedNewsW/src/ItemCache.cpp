...
FeedPtr FeedPool::Add(const FeedItem& item) {
  ...
  Feed* f = NULL;
  {
    ...
    if (it == feed_map_[i].end()) {
      f = new Feed(item);
      feed_map_[i].insert(pair<Ice::Long, Feed*> (item.feed, f));
    } else {
      f = it->second;
    }
    ++(f->ref_count_);
...
FeedPtr FeedPool::Get(Ice::Long feedid) {
...
void FeedPool::UpdateInteraction(){
  ...
  for(int i=0; i<kPoolSize; i++){
    ...
    {
      IceUtil::Mutex::Lock lock(mutex_[i]);
      hash_map<Ice::Long, Feed*>::iterator it = feed_map_[i].begin();
...
void FeedPool::UpdateInteraction(const LongSeq& feeds, bool load){
  FeedInteractionCountSeq count_seq;
  try{
