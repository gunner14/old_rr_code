...
bool DataNodeManager::init(const char* filename){
	...
	//int recordsize = sizeof(FeedItem);
	int	nNodes = SSDFeedDBDataAdapter::instance().getCluster();
	if(nNodes == 0){
		nNodes = 1;
	}
	//nNodes = 1;
...
BlockInfoPtr DataNodeManager::moveBlock(const BlockInfoPtr& blkInfoOld){
	...
	*newBlkInfo = *blkInfoOld;
	...
	try{
		SSDFeedDBDataAdapter::instance().moveBlock(blkInfoOld, newBlkInfo);
	}catch(Ice::Exception& e){
...
void RetryTimer::handle(){
	...
	while(true){
		...
		try{
			SSDFeedDBDataAdapter::instance().putBatch(p.first, p.second);
		}catch(Ice::Exception& e){
