...
void FeedAssistantI::removeFeedReply(Ice::Long source, Ice::Int stype,
 ...
 ///////////
  if(feedIds.size()> 1) {
    MCE_INFO("TestLog##removeFeedReply--> source:" << source << " stype:" << stype << " actor:" << actor << " feedIds size:" << feedIds.size());
    for (size_t h = 0; h < indexSeq.size(); ++h) {
      MCE_INFO("TestLog## --> source:" << indexSeq.at(h)->source << " psource:" << indexSeq.at(h)->psource  << " actor:" << indexSeq.at(h)->actor << " time:" << indexSeq.at(h)->time << " ptype:" << indexSeq.at(h)->ptype  << " stype:" << indexSeq.at(h)->stype << " newsmerge:" << indexSeq.at(h)->newsMerge << " minimerge:" << indexSeq.at(h)->miniMerge << " feed:" << indexSeq.at(h)->feed); 
    }
  }
/////////////
