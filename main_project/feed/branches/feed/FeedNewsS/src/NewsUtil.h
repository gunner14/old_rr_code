...
public:
  ...
  //TODO 将本体放到队列最后
  bool Sort(vector<SharedFeedPtr> & fv, bool & main_with_comment) {
...
//      long fid = fv.at(i)->feed_id();
    ...
    vector<SharedFeedPtr> mains;
    for (vector<SharedFeedPtr>::iterator i = fv.begin(); i != fv.end();) {
...
class FeedFilter: public IceUtil::Shared {
public:
  FeedFilter(const IntSeq& types,bool by_stype,bool show_edm):by_stype_(by_stype),show_edm_(show_edm){
  	type_set_.insert(types.begin(),types.end());
  }
  bool FilterType(int stype){
  	if(type_set_.empty()){
      return true;
    }
    if(by_stype_){
      return type_set_.count(stype);
    }else{
      return type_set_.count(stype / 100 * 100) ;
    }
  }
  bool show_edm(){
  	return show_edm_;
...
typedef IceUtil::Handle<FeedFilter> FeedFilterPtr;
...
public:
	...
	static pair<int,long> GetStypeMergeKey(const FeedItem & item){
		return make_pair(GetStype(item.typeinfo),item.merge);
	}
	static pair<int,int> GetStypeActorKey(const FeedItem & item){
		return make_pair(GetStype(item.typeinfo),item.actor);
	}
...
//				: 60 * 5;
	...
	static bool IsUpdateTime(const FeedItem & item){
    return item.weight & 0x02;
	}
	static int GetView(const FeedItem & item){
    int v =  (item.weight & 0xFF00) >> 8;
//    MCE_INFO("fid:" << item.feed << " view:" << v);
    return v;
	}
	static bool IsEdm(const FeedItem & item){
		return item.time < 0 || item.time > time(NULL);
	}
	static int GetWeightId(const FeedItem & item){
		return (item.weight) >> 16 & 0xFF;
	}
	static int GetFriendRank(const FeedItem & item){
		return item.weight >> 24 ;
	}
