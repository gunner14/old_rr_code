...
class FeedUnit{
public:
	pair<int,long> GetStypeMerge();
	pair<int,int> GetStypeActor();
	long GetFeedId(){
		return item->feed;
	}
	long GetScore();
...
typedef vector<FeedUnit> FeedUnit1D;
typedef vector<vector<FeedUnit> > FeedUnit2D;
typedef vector<vector<vector<FeedUnit> > > FeedUnit3D;
