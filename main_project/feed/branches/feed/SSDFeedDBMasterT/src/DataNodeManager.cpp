...
bool DataNodeManager::init(const char* filename){
	...
	//int recordsize = sizeof(FeedItem);
	int	nNodes = SSDFeedDBDataTAdapter::instance().getCluster();
	if(nNodes == 0){
		nNodes = 1;
	}
	//nNodes = 1;
...
BlockInfoPtr DataNodeManager::moveBlock(const BlockInfoPtr& blkInfoOld){
	...
	*newBlkInfo = *blkInfoOld;
	...
	try{
		SSDFeedDBDataTAdapter::instance().moveBlock(blkInfoOld, newBlkInfo);
	}catch(Ice::Exception& e){
		...
		for(size_t i=0; i<tmpIt->second.size(); i++){
			...
			//for(int j=0; j<10; j++){
				...
				try{
					...
					SSDFeedDBDataTAdapter::instance().putBatchBinary(feed, tmpIt->second.at(i).second);
				}catch(Ice::Exception& e){
...
void RetryTimer::handle(){
	...
	while(true){
		...
		try{
			SSDFeedDBDataTAdapter::instance().putBatch(p.first, p.second);
		}catch(Ice::Exception& e){
