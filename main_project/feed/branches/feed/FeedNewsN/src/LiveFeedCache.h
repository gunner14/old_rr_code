...
public:
  ...
  string DumpToString();
private:
  static void ResetEdmTime(int & t,int uid,long fid,int type){
  	int old = t;
  	if(t == -1) t = time(NULL) + 480;
  	else if(t < 0) t =  time(NULL) - t;
  	MCE_INFO("LiveFeed::ResetEdmTime. oldtime:" << old << " newtime:" << t
  			<< " uid:" << uid << " fid:" << fid << " type:" << type);
  }
//  static void ResetEdmTime(LiveFeedItem & f){
//  	if(f.time_ >= 0) return;
//
//  	if(f.time_ == -1) f.time_ += 480;
//  	else f.time_ += -(f.time_);
//  }
  int GetIndex(int stype);
  int GetSize(int index);
