...
//            feeds_[pos % COUNT].time_ = time(NULL) + 8 * 60;
            const LiveFeedItem & lf = feeds_[pos % COUNT];
            ResetEdmTime(lf.time_,uid,lf.GetFid(), lf.GetStype());
//            ResetEdmTime(feeds_[pos % COUNT].time_,uid,);
...
Ice::Long LiveFeed::CaculateWeightForSort(int uid,
  ...
  viewed_factor = (viewcount == 0 )? 1200:0;
//  if (uid % 10 == 8) {
...
//weight = ft + pow(0.95, live_item.GetViewedCount())*ft + 40*interaction*weight_id + 10*frank;
	weight = ft +  40*interaction*weight_id + 10*frank;

//  if(uid % 20 == 5){ // Oct 1st version
//		long viewed_factor = (Ice::Long) pow(0.7, viewcount) * 1200;
//		weight = Ice::Long(ft + viewed_factor + pow(0.6, viewcount) * (60
//				* interaction * weight_id + 10 * frank));
//  }else if( uid % 20 == 15){  // August version
//  	weight = Ice::Long(ft + pow(0.6, viewcount) * (60
//        * interaction * weight_id + 10 * frank));
//  }else{ // OCT 18TH VERSION
//		int ftbonus = FtBonus10Minutes(fw.time);
//		weight = float( pow(0.7, viewcount) * (ft + 6 * weight_id + 10 * frank + ftbonus));
//  }
