...
void FeedAsyncI::Init() {
  ...
  qm_->Init(5, 10000000);
  MCE_INFO("FeedAsyncI::Init. init done");
}
...
bool FeedAsyncI::ReplaceXML(const  MyUtil::LongSeq & feeds , const string & xml , const Ice::Current& ){
...
bool FeedAsyncI::UpdateReply(const map<Ice::Long, FeedReplyPtr>& buffer, const Ice::Current& ){
...
bool FeedAsyncI::CopyFeedContent(Ice::Long old_feed_id, Ice::Long new_feed_id,
    int new_type, const string& new_props, const Ice::Current&) {
  return true;
}
//
...
//}
//
void FeedAsyncI::Restore(const string & begintime, const string & endtime,
    int thishour, const Ice::Current&) {
  MCE_INFO("FeedAsyncI::Restore");
