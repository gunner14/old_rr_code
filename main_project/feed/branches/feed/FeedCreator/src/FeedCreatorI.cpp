...
bool FeedCreatorI::CreateWithReply(Ice::Int stype, Ice::Int version,
	...
	MCE_INFO("expr:" << seed->extraProps["expr"] << ", line:" << __LINE__);	
	} else {
...
string FeedCreatorI::Data2XML(int stype, int version,
	...
	for (Str2StrMap::const_iterator it = props.begin(); it != props.end(); ++it) {
		xml_node n;
		ostringstream os;
		MCE_INFO("CS TPL KEY:" << it->first << " -> " << it->second);
...
FeedTypeConfigPtr FeedCreatorI::BuildFeedTypeConfig(mysqlpp::Row& row) {
	...
	try{
		...
		config->expired_day = (int) row["lifttime"];
...
DataConfigPtr FeedCreatorI::BuildDataConfig(mysqlpp::Row& row) {
	...
	try{
		...
		for(set<string>::iterator it = config->keys.begin(); it != config->keys.end(); ++it){
			MCE_INFO("key ->" << *it);
		}
	}catch(std::exception & e){
...
set<string> FeedCreatorI::DataSchema2Map(const string& schema){
	xml_document doc;
	...
	for(node = node.first_child();node;node = node.next_sibling()){
		string key;
		XML2Map(node,key,result);
	}
	return result;
}
void FeedCreatorI::XML2Map(xml_node node, string key, set<string>& result){
	if(!node){
		result.insert(key);
		return;
	}
...
bool FeedCreatorI::DataCheck(const DataConfigPtr & config, const Str2StrMap & keys){
	for(set<string>::iterator it = config->keys.begin(); it != config->keys.end(); ++it){
		Str2StrMap::const_iterator kit = keys.find(*it);
		if(kit == keys.end()){
			MCE_WARN("FeedCreatorI::DataCheck --> key:" << *it << " is not found");
			return false;
		}
	}
	return true;
