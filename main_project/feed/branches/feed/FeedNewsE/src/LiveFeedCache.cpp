...
void UniqueFeedList::GetFeedWrappers(int uid,const FeedFilterPtr & filter, vector<
		...
		if (uniquefeed.time_ < 0 || uniquefeed.time_ > time(NULL)) {
			if (filter->show_edm()) {
				if(uniquefeed.time_ < 0 ){
					...
					MCE_INFO("UniqueFeedList::GetFeedWrappers. uid:"<<uid<<" edmfid:" << fw.feed << " time:" << uniquefeed.time_);
					edm_size++;
				}
			}else{
