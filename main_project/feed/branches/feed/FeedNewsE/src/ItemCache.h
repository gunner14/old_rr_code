...
class SharedFeed;
typedef hash_map<Ice::Long, SharedFeed*> FeedMap;
typedef IceUtil::Handle<SharedFeed> SharedFeedPtr;
typedef vector<SharedFeedPtr> SharedFeedSeq;

class SharedFeedPool : public MyUtil::Singleton<SharedFeedPool>{
public:
...
public:
  ...
  inline pair<int, int> GatherKey(){
    if(SmallType() == 2002 || SmallType() == 3707){
  ...
  //-------------------------------------------------------------------------
  void __incRef() {
    SharedFeedPool::instance().Inc(this);
  }
  void __decRef() {
    SharedFeedPool::instance().Release(this);
  }
  int __getRef() const {
    return 0;
  }
