...
class UniqueFeed {
public:
  UniqueFeed(const SharedFeedPtr& feed, int weight, int time) :
    sharedfeed_(feed), weight_(weight), time_(time) {
...
public:
  ...
  UniqueFeedList() :
    ...
    unviewed_count_ = 0;
    memset(unique_feeds_, 0, sizeof(unique_feeds_));
  ...
  void Add(int uid, const UniqueFeed& item, bool updatetime, bool checkenter);
  //void Add(const FeedPtr& feed);
  //void Add(const vector<FeedItem>& items, bool updatetime);
  multimap<Ice::Long, SharedFeedPtr> GetFeedSeq(int begin, int limit);
  vector<SharedFeedPtr> GetFeed();
  vector<UniqueFeed*> GetUniqueFeeds();
  vector<vector<vector<SharedFeedPtr> > > GetGatheredFeed(int uid, int begin,
      int limit, bool hot, vector<vector<vector<SharedFeedPtr> > >& random,
      int& unviewedcount, int& edm_count);
  vector<vector<vector<SharedFeedPtr> > > GetFeedByType(int uid, const IntSeq& types,
      int begin, int limit, bool hot = false) const;
  vector<vector<vector<SharedFeedPtr> > > GetFeedByStype(int uid,
  ...
  Ice::Long CaculateWeight(int uid, const UniqueFeed& live_item, bool log);
  Ice::Long CaculateWeightForSort(int uid, const UniqueFeed& live_item) const;
  //void                      CheckEnterHot(int uid, bool log);
  //void                      MergeFeed(vector<vector<LiveFeedItem*> >& merge_feeds, vector<bool>& enter_hot_flag, 
  void MergeFeed(int uid, vector<vector<UniqueFeed*> >& merge_feeds,
  ...
  //}
  string DumpToString();
  void GetFeedWrappers(int uid,const FeedFilterPtr & checker,
  		vector<FeedWrapper> & fws,hash_map<long,SharedFeedPtr> & feedmap,int & edm_size);
  void IncView(const set<long> & showfids );
private:
  int GetIndex(int stype);
  int GetSize(int index);
  ...
  void
  Add2(int uid, const UniqueFeed& item, bool updatetime, bool checkenter);
  void RemoveOldest();
  //  void RemoveOldestNormal();
  void Remove(int stype, int merge);
  int Mod(int i) {
  ...
  void ShiftItems(int del_idx, int begin_idx) {
    ...
    for (int i = del_idx; i != begin_idx; i = Mod(i - 1)) {
      unique_feeds_[Mod(i)] = unique_feeds_[Mod(i - 1)];
    }
  }

  //bool          	inited_;
  UniqueFeed unique_feeds_[COUNT];
  int size_;
  int end_;
  int check_entered_;
  //int             hot_size_;
