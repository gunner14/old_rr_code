...
static const int kMaxPoolSize = 64;
...
bool ProxyConnPool::Reset() {
  ...
  string endpoint;
  try {
    endpoint = FeedMemcProxyAdapter::instance().GetEndpoint(proxy_index_);
  } catch(Ice::Exception& e) {
    MCE_WARN("FeedMemcProxyAdapter::GetEndpoint Ice error : " << e);
  } catch (...) {
    MCE_WARN("FeedMemcProxyAdapter::GetEndpoint error.");
  }
  ...
  MCE_INFO("FeedMemcProxy " << proxy_index_ << " endpoint : " << endpoint);
...
memcached_st * ProxyConnPool::Pop() {
  ...
  memcached_st * memc = 0;
  IceUtil::Mutex::Lock lock(mutex_);
  MCE_DEBUG("proxy " << proxy_index_ << " conn count=" << memc_pool_.size()
      << " reseting=" << reseting);
...
void ProxyConnPool::CheckState() {
  MCE_DEBUG("ProxyConnPool check. index=" << proxy_index_);
  if (error_count_ > kMaxErrorCount) {
...
pair<int, memcached_st *> MemcClient::GetMemc() {
  ...
  for(int i = 0; i < kProxyClusterSize; ++i) {
    ...
    MCE_DEBUG("GetMemc from pool " << index);
