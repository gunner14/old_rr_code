...
bool HashDB::flushDataNode(const map<int, DataNode>& nodes){
	...
	Ice::Long key = 0;
	bool suc = tchdbput(_db, &key, sizeof(Ice::Long), data, nodes.size()*sizeof(DataNode));
	PerformanceStat::instance().stat("HashDBPut", t.getTime());
	return suc;
}
...
bool DataNodeManager::init(const char* filename){
	...
	//int recordsize = sizeof(FeedItem);
	int	nNodes = FeedDBDataAdapter::instance().getCluster();
	if(nNodes == 0){
		nNodes = 1;
	}
	//nNodes = 1;
...
BlockInfo64Ptr DataNodeManager::BlockDataToBlockInfo(const BlockData& blk){
...
BlockData DataNodeManager::BlockInfoToBlockData(const BlockInfo64Ptr& blk){
	BlockData data;
...
BlockInfo64Ptr DataNodeManager::get(BlockDBKey key, bool write){
	TimeStat t;
	...
	if(!suc){
		if(!write){
			...
			if(allocFreeBlock(key, newBlk)){
				...
				MCE_INFO("DataNodeManager::get --> new blk node:" << blkInfo->serverId << " u:" << blkInfo->key
...
BlockInfo64Ptr DataNodeManager::newBlock(BlockDBKey key){
	IceUtil::Mutex::Lock lock(_blockMutexs[key%MUTEX_COUNT]);
	if(key == 0){
		return 0;
	}
	...
	if(!suc){
		...
		if(allocFreeBlockInNode(newBlk, blkOld.serverId)){
			...
			blkInfo->preBlockIndex = blkOld.blockIndex;
			_hashDB.put(key, newBlk);
			MCE_INFO("DataNodeManager::newBlock --> new blk " << blkInfo->serverId << " u:" << blkInfo->key << " f:" << blkInfo->fileId << " b:" << blkInfo->blockIndex
				<< " preF:" << blkInfo->preFileId << " preB:" << blkInfo->preBlockIndex);
			return blkInfo;
		}
		MCE_WARN("allocFreeBlock err");
...
BlockInfo64Ptr DataNodeManager::moveBlock(const BlockInfo64Ptr& blkInfoOld){
	BlockData newBlk;
	BlockInfo64Ptr newBlkInfo = new BlockInfo64;
	*newBlkInfo = *blkInfoOld;
	...
	try{
		FeedDBDataAdapter::instance().moveBlock(blkInfoOld, newBlkInfo);
	}catch(Ice::Exception& e){
		MCE_WARN("DataNodeManager::moveBlock --> invoke data node to move err, " << e);
		return 0;
	}
	_hashDB.put(newBlkInfo->key, newBlk);
	return newBlkInfo;
}
...
void DataNodeManager::decTotalCount(const UserFeedCount64Seq& feedCounts){
	for(size_t i=0; i<feedCounts.size(); i++){
		Ice::Long key = feedCounts.at(i)->key;
		if(key==0){
			continue;
		}
		IceUtil::Mutex::Lock lock(_blockMutexs[key%MUTEX_COUNT]);
...
void RetryTimer::handle(){
	...
	while(true){
		pair<FeedItem, BlockInfo64Seq> p;
		{
			IceUtil::Mutex::Lock lock(mutex_);
			if(feeds_.empty()){
				break;
		...
		try{
			FeedDBDataAdapter::instance().putBatch(p.first, p.second);
		}catch(Ice::Exception& e){
