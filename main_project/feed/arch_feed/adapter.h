#ifndef XCE_ARCH_ADAPTER_H__
#define XCE_ARCH_ADAPTER_H__
...
#include "feed/base_feed/network.h"
...
inline 
Ice::CommunicatorPtr CloneCommunicator(const Ice::CommunicatorPtr& c) {
...
// Locator: 封装 communicator 及Prx的创建
// concept:
...
// TODO: Prx失效后，减少重查registery的请求的量
...
public:  
  RegistryLocator() {}
  RegistryLocator(const Ice::CommunicatorPtr& cp) 
    : communicator_(cp) {
    ...
    query_ = IceGrid::QueryPrx::uncheckedCast(cp->stringToProxy(instance_name + "/Query"));
  }
...
//      {"Ice.ThreadPool.Client.SizeMax", "1"},
      ...
      #endif
      {"Ice.Default.Locator", XCE_DEFAULT_LOCATOR}, // "IceGrid/Locator:tcp -h 10.2.18.202 -p 4061"
      // {"Ice.Trace.Network", "1"},
  ...
  static Ice::CommunicatorPtr passport() {
    const stringpair arr [] = {
      ...
      {"Ice.MessageSizeMax", "20480"},
      {"Ice.Default.Host", Network::local_ip().c_str()},
      {"Ice.Default.Locator", "Passport/Locator:default -h PassportMaster -p 4076:default -h PassportReplica1 -p 4076:default -h PassportReplica2 -p 4076"},
      {0, 0} // 标记结束
    };
    return BuildCommunicator(arr);
  }
  ...
  static Ice::CommunicatorPtr search_cache() {
    const stringpair arr [] = {
      ...
      {"Ice.MessageSizeMax", "20480"},
      {"Ice.Default.Host", Network::local_ip().c_str()},
      {"Ice.Default.Locator", "SearchCache/Locator:default -h 10.2.17.61 -p 4061"},
      {0, 0} // 标记结束
    };
    return BuildCommunicator(arr);
  }
  ...
  static Ice::CommunicatorPtr talk() {
    const stringpair arr [] = {
      ...
      {"Ice.MessageSizeMax", "20480"},
      {"Ice.Default.Host", Network::local_ip().c_str()},
      {"Ice.Default.Locator", "XNTalk/Locator:default -h XNTalkRegistry -p 12000"},
      {0, 0} // 标记结束
    };
    return BuildCommunicator(arr);
  }
  ...
  static Ice::CommunicatorPtr feed() {
    const stringpair arr [] = {
      ...
      {"Ice.MessageSizeMax", "20480"},
      {"Ice.Default.Host", Network::local_ip().c_str()},
      {"Ice.Default.Locator", "XceFeed/Locator:default -h XceFeedRegistry -p 14300:default -h XceFeedRegistryReplica1 -p 14301:default -h XceFeedRegistryReplica2 -p 14302"},
      {0, 0} // 标记结束
    };
    return BuildCommunicator(arr);
  }
  ...
  // user monitor
  // UserMonitor/Locator:default -h UserMonitorMaster -p 4066:default -h UserMonitorReplica1 -p 4066:default -h UserMonitorReplica2 -p 4066
  

  static RegistryLocator& instance() {
    static RegistryLocator r(Default());
    return r;
  }
  static RegistryLocator& xiaonei_instance() {
    static RegistryLocator r(xiaonei());
    return r;
  }
  static RegistryLocator& talk_instance() {
    static RegistryLocator r(talk());
    return r;
  }
  static RegistryLocator& passport_instance() {
    static RegistryLocator r(passport());
    return r;
  }
  static RegistryLocator& feed_instance() {
    static RegistryLocator r(feed());
    return r;
  }
private:
  Ice::CommunicatorPtr communicator_;
...
*/
template<typename Prx, class Locator = RegistryLocator, InvokeType Invoke = TWO_WAY>
class ClientPtr {
public:
  ...
  ClientPtr(const Locator& locater)
    : locator_(locater) {}
  ...
  template<class Policy>
  bool Create(const Policy& pol, size_t uid) {
    // TODO: not thread safe
    static typename Policy::template CacheHolder<Prx, Locator, Invoke> cache_((locator_), pol);
    t_ = cache_(uid);
    return true;
  }
  ...
  void set_timeout(int millisec) {
    t_ = t_->ice_timeout(millisec);
  }
protected:
  Prx t_;
  Locator locator_;
...
//       return std::vector<Prx>();
    ...
    for (std::vector<Ice::ObjectPrx>::iterator it = proxies.begin(); it != proxies.end(); ++it) {
...
private:
  void EnsureReplicatedProxyInCache( const Ice::ObjectPrx& replicated) {
    ...
    if (it != _replicated_proxies_cache.end())
      return;
    std::vector<Ice::ObjectPrx> x = baseclass::locator_.FindAllReplicas(replicated);
  ...
  Ice::ObjectPrx SelectBestProxy( const Ice::ObjectPrx& replicated) {
    ...
    LOG(WARNING) << "Adapter::select_best_proxy valid: " << valid_size << " invalid: " << invalid_size;
    if (valid_size > 0) {
  ...
  std::vector<Ice::ObjectPrx> SelectAllProxy(const Ice::ObjectPrx& replicated) {
    ...
    LOG(WARNING) <<"Adapter::select_all_proxy valid: " << it->second.first.size();
    LOG(WARNING) <<"Adapter::select_all_proxy invalid: " << it->second.second.size();
    for(std::vector<Ice::ObjectPrx>::iterator itr=it->second.first.begin();itr!=it->second.first.end();++itr){
      results.push_back(*itr);
    }
    for(std::vector<Ice::ObjectPrx>::iterator itr=it->second.second.begin();itr!=it->second.second.end();++itr){
      results.push_back(*itr);
    }
    ...
    try {
      ProxySeq keys;
      {
        ...
        for (ReplicatedProxyCache::const_iterator it =
          _replicated_proxies_cache.begin(); it != _replicated_proxies_cache.end(); ++it) {
      ...
      for (ProxySeq::const_iterator it = keys.begin(); it != keys.end(); ++it) {
        ...
        for (ProxySeq::size_type x = 0; x < all_replicas.size(); ++x) {
          try {
            ...
            LOG(WARNING)<<"Adapter::ValidateThread::run() Ice::Ex while validating " << e;
          } catch (...) {
            //ensure timeout exception will not break whole thing.
            LOG(WARNING)<<"Adapter::ValidateThread::run() some exception while validating " << *it;
          }
        }
...

      {
        ...
        for (ReplicatedProxyCache::iterator it =
          _replicated_proxies_cache.begin(); it
            != _replicated_proxies_cache.end(); ++it) {
          ...
          //MCE_TRACE("ReplicatedAdapterI::ValidateThread::run " << it->first << " Befor:"<<new_it->second.first.size()<<" After:" << it->second.first.size() );
        }
      }
    } catch (...) {
      //Ensure this thread will not terminate.
      LOG(WARNING)<<"Adapter::ValidateThread::run() some exception in main loop.";
    }
  }
...
public:
  ...
  std::string operator()(size_t uid) const {
    return ProxyName(uid);
  }

  size_t cluster(size_t uid) const {
    return uid % cluster_count_;
  }
  size_t cluster_count() const {
    return cluster_count_;
  }

  template<typename Prx, class Locator, InvokeType Invoke>
  struct CacheHolder {
    ...
    Prx operator()(size_t uid) const {
      return cache_[pol_.cluster(uid)];
    }
