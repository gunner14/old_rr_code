...
//   FeedDBInstance = service.getCommunicator()->getProperties()->getPropertyWithDefault(
  ...
  xce::feed::serverstate::FeedServerStateSubscriber::instance().initialize("ControllerFeedMiniR",
      &FeedItemManagerI::instance(), mod, interval, new XceFeedControllerChannel());
}
...
bool FeedItemManagerI::LoadFeed(int uid, int time, int count){
    if(count < 0 ) {
        MCE_INFO("FeedItemManagerI::LoadFeed -->  count < 0 , return false.");
        return false;
    }	
...
FeedDataSeq FeedItemManagerI::getFeedData(int uid, int begin, int limit,const Ice::Current& current) 
{
...
FeedDataSeq FeedItemManagerI::getFeedDataByStype(int uid,const MyUtil::IntSeq& stypes, int begin, int limit, const Ice::Current&  current) 
{
    InvokeClient tm = InvokeClient::create(current,"", InvokeClient::INVOKE_LEVEL_INFO);
    return GetFeedDataImpl(uid, stypes, true, begin, limit);
}

FeedDataSeq FeedItemManagerI::getFeedDataByType(int uid,const MyUtil::IntSeq& types, int begin, int limit, const Ice::Current& current) 
{
   InvokeClient tm = InvokeClient::create(current,"", InvokeClient::INVOKE_LEVEL_INFO); 
...
FeedDataSeq FeedItemManagerI::getFeedData2(int uid, int begin, int limit,const Ice::Current& current) 
{
   InvokeClient tm = InvokeClient::create(current,"", InvokeClient::INVOKE_LEVEL_INFO);
   return FeedDataSeq();
}
...
void FeedItemManagerI::setReadAll(int uid, const Ice::Current& current) {
  InvokeClient tm = InvokeClient::create(current,"", InvokeClient::INVOKE_LEVEL_INFO);
...
bool FeedItemManagerI::hasFeed(int uid, const Ice::Current& current) {
  InvokeClient tm = InvokeClient::create(current,"", InvokeClient::INVOKE_LEVEL_INFO);
  {
...
FeedDataSeq FeedItemManagerI::GetFeedDataImpl(int uid, const IntSeq& types, bool stype, int begin, int limit)
{
  if(limit <0)
  {
      MCE_INFO("FeedItemManagerI::GetFeedDataImpl -->  limit <0. return false.");
      return FeedDataSeq();
  }  
  int index = GetIndex(uid);
  ...
  if((size_t)limit > live_feeds.size() && old_time>0){
...
FeedItemSeq FeedItemManagerI::GetOriginalFeedData(int uid, const Ice::Current& current){
  InvokeClient tm = InvokeClient::create(current,"", InvokeClient::INVOKE_LEVEL_INFO);
  IceUtil::Mutex::Lock lock(mutexs_[GetIndex(uid)]);
  const UserFeed* user_feed = GetUserFeed(uid);
  if(user_feed){
...
void FeedItemManagerI::Reload(int uid, const Ice::Current& current){
  InvokeClient tm = InvokeClient::create(current,"", InvokeClient::INVOKE_LEVEL_INFO);
  IceUtil::Mutex::Lock lock(mutexs_[GetIndex(uid)]);
  RemoveUserFeed(uid); 
}
