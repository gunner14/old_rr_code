...
//	HFeed1D useless_hfeeds;
	...
	BOOST_FOREACH(HeavyFeedPtr hfeed,hfeed1d){
		...
		if( (hfeed->GetStype()!=8182) && (mergetype == FeedMergeReplace && sfeed1d.size() >= 1 * 2)//适当放宽条件，避免唯一的本体取不到,保留stype = 8182  的所以新鲜事
...
set<long> & GetRequest::GetShowedFids(){
  ...
  for(int i3d = begin_; i3d < begin_+limit_ && i3d < (int)hfeed3d_.size();++i3d ){
    ...
    BOOST_FOREACH(HFeed1D & hfeed1d,hfeed2d){
      ...
      if (0 && ((uid_ / 1000) % 10 == 5 || uid_ == 241610785 || uid_ == 461193456) && FeedBuilder::NeedAppendStype(hfeed1d.front()->GetStype()) && hfeed2d.size() == 2) {
        showed_sfeed3d_.push_back(SFeed2D());
        useless_count_ += GetShowedFidsFromHFeed1D(hfeed2d[1], *(showed_sfeed3d_.rbegin()));
        break;
      }
    }
  }
