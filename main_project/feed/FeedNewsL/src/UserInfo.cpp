...
long UserInfo::DownloadMaxRead(){
	...
	MCE_DEBUG("UserInfo::DownloadMaxRead. uid:" << uid_ << " pre_max_read_fid:" << pre_max_read_fid_
...
void UserInfo::MergeAndSort(GetRequest & req){
  ...
  req.logmap_["MergeFeed_cost"] = -1;
  req.logmap_["GatherFeed_cost"] = -1;
  req.logmap_["GatherFeed_item3d"] = -1;

  if (req.IsSort() && InfoOpt::instance().UseInfoOpt(req.uid_)) {
    InfoOptPara para(GetMaxRead(), recent_cnt_, refresh_time_); 
    InfoOpt::instance().Optimize(req, hbuf_, para);
  } else {
    TimeStat ts;
	  MergeFeed(req);
    req.logmap_["MergeFeed_cost"] = ts.Get();
  	if(req.IsSort())
	  	GatherFeed(req);
	  else
		  GatherFeedSimple(req);
    req.logmap_["GatherFeed_cost"] = ts.Get();
    req.logmap_["GatherFeed_item3d"] = req.hfeed3d_.size();
  }

  TimeStat ts;
 	req.GetShowedFids();

  req.logmap_["GetShowedFids_cost"] = ts.Get();
}
...
void UserInfo::MergeFeed(GetRequest & req){
	...
	MCE_DEBUG("UserInfo::MergeFeed. merge done." << req.ToString());
}
...
void UserInfo::MergeByStypeMergeAndGetScore(GetRequest & req,StypemergeScoreMap & vstypemerge_score){
  ...
  double bayestime = 0;
    ...
    TimeStat tclua;
      ...
      tclua.Get();
			vstypemerge_score[key] = hfeed->GetScoreLua(req.uid_,req.IsSort(),req.GetNowTime(),GetMaxRead(),recent_cnt_,refresh_time_, bayestime);
      totaltime += tclua.Get();
		}
		++it;
	}

  req.logmap_["scorecost"] = totaltime;
  req.logmap_["bayestime"] = bayestime;
}
...
void UserInfo::GatherFeed(GetRequest & req){
  ...
  map<int, int> stype_index;
    ...
    if (UserInfoPool::instance().StypeInFoldSet(hfeed->GetStype())) {
      map<int, int>::iterator stypeit = stype_index.find(hfeed->GetStype());
      if (stypeit != stype_index.end()) {
        req.hfeed3d_[stypeit->second].push_back(item1d);
      } else {
        HFeed2D item2d;
        item2d.push_back(item1d);
        req.hfeed3d_.push_back(item2d);
        stype_index[hfeed->GetStype()] = req.hfeed3d_.size()-1;
      }
      continue;
    }
...
void UserInfo::GatherFeedSimple(GetRequest & req){
	MCE_DEBUG("UserInfo::GatherFeedSimple.begin. scoremap size:" << req.score_iterator_multimap_.size());
...
vector<long> UserInfo::SetRead(int stype,long merge ){
	vector<long> res;
	{
		...
		for(HBuf::iterator it = hbuf_.begin(); it != hbuf_.end();){
			HeavyFeed & hfeed = *it;
      if (stype == 8182 && hfeed.SameStype(stype) ) {
