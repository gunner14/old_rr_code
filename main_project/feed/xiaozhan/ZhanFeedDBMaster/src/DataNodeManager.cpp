...
bool DataNodeManager::init(const char* filename){
	...
	//int recordsize = sizeof(FeedItem);
	int	nNodes = ZhanFeedDBDataAdapter::instance().getCluster();
	if(nNodes == 0){
		nNodes = 1;
	}
	//nNodes = 1;
	for(int i=0; i<nNodes; i++){
		...
		/*try{
			...
			ZhanFeedDBDataAdapter::instance().newFile(node.serverId, node.fileId);
...
BlockInfoPtr DataNodeManager::moveBlock(const BlockInfoPtr& blkInfoOld){
	...
	*newBlkInfo = *blkInfoOld;
	...
	try{
		ZhanFeedDBDataAdapter::instance().moveBlock(blkInfoOld, newBlkInfo);
	}catch(Ice::Exception& e){
		...
		for(size_t i=0; i<tmpIt->second.size(); i++){
			...
			//for(int j=0; j<10; j++){
				...
				try{
					...
					ZhanFeedDBDataAdapter::instance().putBatchBinary(feed, tmpIt->second.at(i).second);
				}catch(Ice::Exception& e){
...
void RetryTimer::handle(){
	...
	while(true){
		...
		try{
			ZhanFeedDBDataAdapter::instance().putBatch(p.first, p.second);
		}catch(Ice::Exception& e){
			MCE_WARN("RetryTimer::handle --> invoke SSDFeedDBData err, " << e);
			IceUtil::Mutex::Lock lock(mutex_);
