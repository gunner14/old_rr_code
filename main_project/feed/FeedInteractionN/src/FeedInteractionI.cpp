...
const string DbHelper::kColDispatchCount = "dispatch_count";
...
void MyUtil::initialize() {
  ...
  service.getAdapter()->add(&FeedInteractionI::instance(), service.createIdentity("M", ""));
    ...
    MCE_WARN("start ice ex " << e);
  }catch(std::exception& e){
    MCE_WARN("start std ex " << e.what());
  }catch(...){
    MCE_WARN("start ice ex ");
  }
}
...
InteractionCacheItem FeedInteractionCache::QueryCacheItemFromDb(Ice::Long feed){
  ...
  if (res.num_rows()>0) {
    ...
    cache_item.range_ = row[DbHelper::kColDispatchCount.c_str()];
...
map<Ice::Long, FeedInteractionDbItem>::iterator
  ...
  sql << "insert into " << table_name << " (" << DbHelper::kColFeedId
      ...
      << DbHelper::kColViewCount << "," << DbHelper::kColDispatchCount <<  "," << DbHelper::kColActor
...
InteractionCacheItem FeedInteractionCache::FindFeedInteraction(Ice::Long feed, bool load) {
...
vector<InteractionCacheItem> FeedInteractionCache::FindFeedInteraction(const MyUtil::LongSeq feeds, bool load) {
  vector<InteractionCacheItem> res;
...
void FeedInteractionI::Inc(int type, Ice::Long feed, int count, const ::Ice::Current&){
  ...
  if(!res){
    if(type == 1){
      ...
      MCE_INFO("FeedInteractionI::Inc --> Inc reply count, " << feed << " count:" << count);
    }
  }
}
...
FeedInteractionCountSeq FeedInteractionI::GetFeedInteractionCountSeq(const ::MyUtil::LongSeq& feeds_seq, bool load, const ::Ice::Current&) {
  ...
  TimeStat ts;
  vector<InteractionCacheItem> res = FeedInteractionCache::instance().FindFeedInteraction(feeds_seq, load);
  ...
  MCE_INFO("FeedInteractionI::GetFeedInteractionCountSeq --> size:" << feeds_seq.size() << " res:" << feed_interaction_count_seq.size()
          << " time:" << ts.getTime() << " load:" << load);
  /*if(load){
    PerformanceStat::instance().stat("GetInteractionLoad", ts.getTime());
  }else{
    PerformanceStat::instance().stat("GetInteractionNotLoad", ts.getTime());
  }*/
  //return FeedInteractionCountSeq();
  return feed_interaction_count_seq;
}

string FeedInteractionI::GetFeedInteractionSeq(const string& feeds, bool load, const ::Ice::Current&) {
  int UNIT = sizeof(Ice::Long);
  int pos = 0;
  if(feeds.size()%UNIT != 0){
    return "";
  }
  TimeStat ts;
  MyUtil::LongSeq feedids;
  const char* p = feeds.c_str();
  const char* end = p + feeds.size();
  for(;p!=end;){
    feedids.push_back(*((Ice::Long*)p));
    p += UNIT;
  }
  MCE_INFO("FeedInteractionI::GetFeedInteractionCountSeq --> decode feedid " << (feeds.size()/UNIT) << " " << feedids.size());
  vector<InteractionCacheItem> res = FeedInteractionCache::instance().FindFeedInteraction(feedids, load);
  string res_str;
  for(size_t i=0; i<res.size(); i++){
    char tmp[128] = {0};
    char* p = tmp;
    *(Ice::Long*)(p) = res.at(i).feed_;
    *(int*)(p+=sizeof(Ice::Long)) = res.at(i).count_ & 0xffff;
    *(int*)(p+=sizeof(int))       = res.at(i).count_ >> 16;
    *(int*)(p+=sizeof(int))       = 0;
    *(int*)(p+=sizeof(int))       = res.at(i).range_;
    res_str.insert(res_str.end(), tmp, p+sizeof(int));
  }
  /*
  if(load){
    PerformanceStat::instance().stat("GetInteractionLoad", ts.getTime());
  ...
  MCE_INFO("FeedInteractionI::GetFeedInteractionCountSeq --> ids: " << feeds.size() << " res:" << res.size() << " " << (res_str.size()/24)
          << " time:" << ts.getTime() << " load:" << load);
  return res_str;
}
...
bool ResultHandlerI::handle(mysqlpp::Row& row) const {
  ...
  obj->item.range_ = row[DbHelper::kColDispatchCount.c_str()];
