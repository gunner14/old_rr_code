...
UserFeedsI::UserFeedsI() {
	...
	build_history_ = props->getPropertyAsIntWithDefault("Service." + service.getName() + ".BuildHistory", 15);//单位为天 15->1
...
FeedMetaSeq UserFeedsI::getByTypesWithFilterSimple(const IntSeq& userids, ::Ice::Int begin, ::Ice::Int limit, const IntSeq& types, const StrSeq& filter, const ::Ice::Current& current) {
	ostringstream oss;
	oss << " userids.size: " << userids.size() << " begin: " << begin << " limit: " << limit;
	InvokeClient tm = InvokeClient::create(current, oss.str(), InvokeClient::INVOKE_LEVEL_INFO);
	...
	try {
		if(filter.empty() && types.empty())
			return getSegmentSimple(fs_.gets(userids, begin + limit), begin, limit);
		return getSegmentSimple(fs_.gets_with_filter(userids, TypesFilter(types), IdsFilter(filter), begin + limit), begin, limit);
	} catch (std::exception& e) {
		MCE_WARN("UserFeedsI::getByTypes Exception! " << e.what());
	} catch (...) {
		MCE_WARN("UserFeedsI::getByTypes Exception!");
...
FeedMetaSeq UserFeedsI::getBiggerThanFeedidByTypesWithFilterSimple(const IntSeq& userids, ::Ice::Long feedid, ::Ice::Int limit, const IntSeq& types, const StrSeq& filter, const ::Ice::Current& current) {
	ostringstream oss;
	oss << " userids.size: " << userids.size() << " limit: " << limit << " feedid: " << feedid;
	InvokeClient tm = InvokeClient::create(current, oss.str(), InvokeClient::INVOKE_LEVEL_INFO);

	try {
		return getSegmentSimple(fs_.gets_bigger_feedid_with_filter(userids, TypesFilter(types), IdsFilter(filter), feedid, limit), 0, limit);
	} catch (std::exception& e) {
		MCE_WARN("UserFeedsI::getBiggerThanFeedidByTypesWithFilterSimple Exception! " << e.what());
	} catch (...) {
		MCE_WARN("UserFeedsI::getBiggerThanFeedidByTypesWithFilterSimple Exception!");
	}

	return FeedMetaSeq();
}

FeedMetaSeq UserFeedsI::getBiggerThanFeedid(const IntSeq& userids, ::Ice::Long feedid, ::Ice::Int begin, ::Ice::Int limit, const IntSeq& types, const StrSeq& filter, const ::Ice::Current& current) {
	ostringstream oss;
	oss << " userids.size: " << userids.size() << " limit: " << limit << " feedid: " << feedid;
	InvokeClient tm = InvokeClient::create(current, oss.str(), InvokeClient::INVOKE_LEVEL_INFO);

	try {
		return getSegment(fs_.gets_bigger_feedid_with_filter(userids, TypesFilter(types), IdsFilter(filter), feedid, begin + limit), userids, begin, limit);
	} catch (std::exception& e) {
		MCE_WARN("UserFeedsI::getBiggerThanFeedidByTypesWithFilter Exception! " << e.what());
	} catch (...) {
		MCE_WARN("UserFeedsI::getBiggerThanFeedidByTypesWithFilter Exception!");
...
FeedMetaSeq UserFeedsI::getBiggerThanFeedidSimple(const IntSeq& userids, ::Ice::Long feedid, ::Ice::Int begin, ::Ice::Int limit, const IntSeq& types, const StrSeq& filter, const ::Ice::Current& current) {
	ostringstream oss;
	oss << " userids.size: " << userids.size() << " limit: " << limit << " feedid: " << feedid;
	InvokeClient tm = InvokeClient::create(current, oss.str(), InvokeClient::INVOKE_LEVEL_INFO);

	try {
		return getSegmentSimple(fs_.gets_bigger_feedid_with_filter(userids, TypesFilter(types), IdsFilter(filter), feedid, begin + limit), begin, limit);
	} catch (std::exception& e) {
		MCE_WARN("UserFeedsI::getBiggerThanFeedidByTypesWithFilterSimple Exception! " << e.what());
	} catch (...) {
		MCE_WARN("UserFeedsI::getBiggerThanFeedidByTypesWithFilterSimple Exception!");
...
bool UserFeedsI::put_without_add_count(const FeedMeta& meta) {
  if (meta.userid < 0) {
    MCE_WARN("UserFeedsI::put_without_add_count fail. userid: " << meta.userid << " feedid: " << meta.feedid);
    return false;
  }
...
bool UserFeedsI::puts_without_add_count(int mod, const std::vector<FeedMetaData>& feeds) {
  if (mod < 0) {
    MCE_WARN("UserFeedsI::puts_without_add_count fail. mod: " << mod);
    return false;
  }
...
FeedMetaSeq UserFeedsI::getSegmentSimple(const std::vector<std::vector<FeedMetaData> > & data, int begin, int limit) {
    ...
    if (begin >= it->size()) {
      continue;
		...
		for(std::vector<FeedMetaData>::const_iterator id = it->begin() + begin;
				id != it->end(); ++id) {
			seq.push_back(id->createFrom(id->userid));
		}
	}
	std::sort(seq.begin(), seq.end(), FeedMetaCompare());
...
void BuildUserMapTask::handle() {
	...
	int cpu_size = (int)(sysconf(_SC_NPROCESSORS_CONF) * 0.8); //1.5->0.1
