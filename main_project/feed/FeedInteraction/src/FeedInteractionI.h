...
public:
  ...
  static string GetExpr(Ice::Long feedid){
    ostringstream os;
    os << "feed_content" << "_" << feedid%kTableCount;
    return os.str();
  }
};
...
public:
	...
	InteractionCacheItem FindFeedInteraction(Ice::Long feed);
  vector<InteractionCacheItem> FindFeedInteraction(const MyUtil::LongSeq feeds); 
