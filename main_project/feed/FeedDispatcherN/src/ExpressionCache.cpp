...
set<int> ExprCache::Expr2Ids(string & inputExpr, int actor) {
   MCE_INFO("ExprCache::Expr2Ids --> inputExpr:" << inputExpr << " actor:" << actor);
  ...
  bool in_cache = false;
  {
    ...
    if (eit != index.end()) {
      ...
      ++find_count_;
      result = (*eit)->ids();
      MCE_INFO("ExprCache::Expr2Ids --> this expression = " << inputExpr << " is in expression-cache. ids size = " << result.size() );
    } else {
      MCE_INFO("ExprCache::Expr2Ids --> this expression = " << inputExpr << " is Not in expression-cache");
    }
  }
  if(in_cache && result.size()>0){
    if ( DegradeManager::instance().IsEnable("FeedDispatcher::module_FiltFriend")) {
...
FactorEntryPtr ExprCache::FactorFactory(const FactorPtr & inputFactor) {
  ...
  ostringstream ios;
//  if (type != 'J') {
  Ice::Long param_id = inputFactor->param_id();
    ios << type << '(' << param_id << ')';
//  } else {
//    long param_idL = inputFactor->param_idL();
//    ios << type << '(' << param_idL << ')';
//  }
