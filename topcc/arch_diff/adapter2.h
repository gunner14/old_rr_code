...
#define DEFAULT_ARRAY \
  ...
  {"Ice.Default.Host", base::Network::local_ip().c_str()},
...
struct LockedStaticArray {
  ...
  static base::ReadWriteLock _lock_; // get proxy
  static bool _quit_;
};
...
template <typename ElementType, typename Cate>
  base::ReadWriteLock LockedStaticArray<ElementType, Cate>:: _lock_;
template <typename ElementType, typename Cate>
...
struct AdapterCore {
  ...
  static void Run(const std::string& topicname, const MyUtil::SbjSubscriberPrx& subscriber, const MyUtil::SubjectObserverPrx& proxy) {
    ...
    // 定期注册一次, 心跳,默认120ms
    base::Post(boost::bind(Run<Prx>, "ClusterState", subscriber, proxy), 120);
  }
  ...
  static void InitTarget(const std::string& endpoint) {
    ...
    std::string localIP = base::Network::local_ip().c_str();
    ...
    base::Post(boost::bind(Run<Prx>, "ClusterState", subscriber, proxy));
  }
