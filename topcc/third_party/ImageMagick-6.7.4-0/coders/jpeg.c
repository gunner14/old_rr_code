  ...
  ExceptionInfo *exception)
{
  ...
  */
    ...
    int size_of_JSAMPLE = sizeof(JSAMPLE);
      int size_of_JSAMPROW = sizeof(JSAMPROW);
  jpeg_destroy_decompress(&jpeg_info);
  jpeg_pixels=(unsigned char *) RelinquishMagickMemory(jpeg_pixels);
  (void) CloseBlob(image);
  return(GetFirstImageInList(image));
...
{
  #define OUTPUT_BUF_SIZE  MaxBufferExtent
  ...
  if(cinfo->progressive_mode) {
    ...
    bytes_to_write = OUTPUT_BUF_SIZE - dest->manager.free_in_buffer;
  }
                                                                                                    
  if(WriteBlob(dest->image,bytes_to_write,dest->buffer)!= bytes_to_write)
    ERREXIT(cinfo, JERR_FILE_WRITE);
                                                                                                    
  dest->manager.next_output_byte = dest->buffer;
  dest->manager.free_in_buffer   = OUTPUT_BUF_SIZE;
