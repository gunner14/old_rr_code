...
extern void DiskFileBlockAddrRefPtrDecRef(DiskFileBlockAddrRefPtr* ptr);
...
/*@null@*/
item *do_item_alloc(char *key, const size_t nkey, const int flags, const rel_time_t exptime, const int nbytes, char* item_buf, bool* del, bool disk_item) {
    if (del) {
      *del = false;
    }
    uint8_t nsuffix;
    ...
    if (search != NULL && (refcount_incr(&search->refcount) == 2)) {
        if ((search->exptime != 0 && search->exptime < current_time)
            ...
            it->slabs_clsid = 0;
        } else if ((it = slabs_alloc(ntotal, id, disk_item)) == NULL) {
            ...
            if (del) {
              *del = true;
              if (item_buf) {
                memcpy(item_buf, (char*)it, ITEM_ntotal(it));
              }
            } else if (gVerbose){
              fprintf(stderr, "do_item_alloc Remove: ");
              char* tmp = ITEM_key(it);
              for (unsigned i = 0; i < it->nkey; ++i) {
        ...
        it = slabs_alloc(ntotal, id, disk_item);
        if (search != NULL) {
            refcount_decr(&search->refcount);
        }
    }
...
void do_item_remove(item *it) {
    ...
    if (refcount_decr(&it->refcount) == 0) {
      if (it->it_flags & ITEM_SSD) {
        ...
        DiskFileBlockAddrRefPtrDecRef(addr.block_addr_);
      }
        item_free(it);
    }
}
...
static bool DiskItemDataRemoved(item *it) {
  // it is expected as a ITEM_SSD
  DiskDataAddr addr = *(DiskDataAddr*)ITEM_data(it);
...
item *do_item_get(const char *key, const size_t nkey, const uint32_t hv) {
    ...
    if (it != NULL) {
        ...
        } else if ((it->it_flags & ITEM_SSD) && DiskItemDataRemoved(it)) {
          do_item_unlink(it, hv);
          do_item_remove(it);
          it = NULL;
          if (was_found) {
              fprintf(stderr, " -nuked by remove from disk");
          }
        }
        else {
