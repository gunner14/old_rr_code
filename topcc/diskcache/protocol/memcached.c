...
extern void DiskManagerRDLock(size_t level, size_t file);
extern void DiskManagerUnLock(size_t level, size_t file);
extern bool DiskManagerInit(const DiskLevelConf* confs, unsigned num_confs, unsigned num_threads);
extern char* DiskManagerAquireBuf();
extern void DiskManagerReleaseBuf(char* buf);
extern enum DataGetStatus
DiskManagerGet(const DiskDataAddr addr, char* buf, const unsigned buf_size, unsigned* vallen);

bool gVerbose = false;
char gLogFile[256];
const unsigned kBigBufSize = 1024 * 1024;
...
static void settings_init(void) {
    ...
    settings.average_key_len = 32;
    settings.average_value_len = 48 * 1024;
}
...
static void conn_cleanup(conn *c) {
    ...
    if (c->item) {
      item* cur_it = (item*)c->item;
      if (cur_it->it_flags & ITEM_SSD) {
        if (c->disk_item_data_buf) {
          DiskManagerReleaseBuf(c->disk_item_data_buf);
        }
      }
        item_remove(c->item);
        c->item = 0;
        c->disk_item_data_buf = NULL;
    }
...
static bool GetDataFromDisk(const DiskDataAddr addr, char** buf, unsigned* nbytes) {
  char* disk_item_buf = DiskManagerAquireBuf();
  unsigned disk_item_data_len;
  enum DataGetStatus ret = DiskManagerGet(addr, disk_item_buf, kBigBufSize, &disk_item_data_len);
  if (ret == kDataGetOK) {
    if (buf) {
      *buf = disk_item_buf;
    }
    if (nbytes) {
      *nbytes = disk_item_data_len;
    }
    return true;
  } else {
    // failed
    DiskManagerReleaseBuf(disk_item_buf);
    if (buf) {
      *buf = NULL;
    }
    if (nbytes) {
  ...
  if (ret == kCheckCodeError) {
    fprintf(stderr, "DiskGetError: CheckCodeError\n");
    fflush(stderr);
    return false;
  }
  if (ret == kDataGetBufOverflow) {
    fprintf(stderr, "DiskGetError: BufOverflow\n");
    fflush(stderr);
    return false;
  }
  return false;
...
#if 0
int AddIovData(conn *c, item *it, uint32_t hv) {
    unsigned int nbytes = it->nbytes - 2;
    DiskDataAddr* addr = (DiskDataAddr*)ITEM_data(it);
    char* buf = DiskManagerAquireBuf();
    *DISK_ITEM_data_buf_(it) = buf;
    *DISK_ITEM_data_hash_(it) = hv;
    if (DiskManagerGet(*addr, buf, kBigBufSize, &nbytes) != kDataGetOK) {
      fprintf(stderr, "DiskManagerGet Error: addr: (%d, %d, %d, %d, %d)\n",
          addr->level_, addr->file_group_, addr->file_, addr->block_, addr->offset_in_block_);
      fflush(stderr);
    }
    return add_iov(c, buf, it->nbytes - 2);
}
#endif
...
static void complete_incr_bin(conn *c) {
    ...
    case DELTA_ITEM_NOT_FOUND:
        if (req->message.body.expiration != 0xffffffff) {
            ...
            it = item_alloc(c, key, nkey, 0, realtime(req->message.body.expiration),
...
static bool FileAddrValid(const DiskDataAddr* addr) {
  return addr->block_addr_->data_->valid_;
}

static bool DoubleCheckItem(item *it, const DiskDataAddr expected_addr) {
  if (!it) {
    return false;
  }
  if (!FileAddrValid(&expected_addr)) {
    return false;
  }
  if (!(it->it_flags & ITEM_SSD)) {
    return false;
  }
  DiskDataAddr addr = *(DiskDataAddr*)ITEM_data(it);
  if (addr.block_addr_->data_ != expected_addr.block_addr_->data_ ||
      addr.offset_in_block_ != expected_addr.offset_in_block_) {
    return false;
  }
  return true;
}
...
static void process_bin_get(conn *c) {
    ...
    unsigned nbytes = 0;
  if (it) {
    bool is_disk_item = it->it_flags & ITEM_SSD;
    if (!is_disk_item) {
      data_buf = ITEM_data(it);
      nbytes = it->nbytes;
    } else {
      DiskDataAddr addr = *(DiskDataAddr*) ITEM_data(it);
      DiskFileBlockAddr block_addr = *addr.block_addr_->data_;
      DiskManagerRDLock(block_addr.level_, block_addr.file_);
      item_remove(it); // decrease refcount
      // after item_remove, addr.file_addr_->data_ may have been freed,
      // but file_addr is saved
      it = item_get(key, nkey); // get after read lock has been locked
      if (DoubleCheckItem(it, addr)) {
        if (!GetDataFromDisk(addr, &data_buf, &nbytes)) {
          // Get Failed
          item_remove(it);
        }
      }
      DiskManagerUnLock(block_addr.level_, block_addr.file_);
    }
  }
    ...
    if (data_buf) {
        ...
        MEMCACHED_COMMAND_GET(c->sfd, ITEM_key(it), it->nkey, nbytes, ITEM_get_cas(it));
        ...
        /* Add the data minus the CRLF */
        add_iov(c, data_buf, nbytes - 2);
        //add_iov(c, ITEM_data(it), it->nbytes - 2);
        // AddIovData handles retrieving data from disk

        conn_set_state(c, conn_mwrite);
        c->write_and_go = conn_new_cmd;
        /* Remember this command so we can garbage collect it later */
        c->item = it;
        c->disk_item_data_buf = data_buf;
    } else {
        pthread_mutex_lock(&c->thread->stats.mutex);
        c->thread->stats.get_cmds++;
...
static void process_bin_sasl_auth(conn *c) {
    ...
    item *it = item_alloc(c, key, nkey, 0, 0, vlen);
...
static void process_bin_update(conn *c) {
    ...
    it = item_alloc(c, key, nkey, req->message.body.flags,
...
static void process_bin_append_prepend(conn *c) {
    ...
    it = item_alloc(c, key, nkey, 0, 0, vlen+2);
        ...
        || comm == NREAD_APPEND || comm == NREAD_PREPEND))
    {
        ...
        if (comm == NREAD_APPEND || comm == NREAD_PREPEND) {
            ...
            if (stored == NOT_STORED) {
                ...
                new_it = item_alloc(c, key, it->nkey, flags, old_it->exptime, it->nbytes + old_it->nbytes - 2 /* CRLF */);
...
static inline void process_get_command(conn *c, token_t *tokens, size_t ntokens, bool return_cas) {
  printf("Unexpected command: process_get_command\n");
    char *key;
    size_t nkey;
    int i = 0;
    item *it;
...
static void process_update_command(conn *c, token_t *tokens, const size_t ntokens, int comm, bool handle_cas) {
    ...
    it = item_alloc(c, key, nkey, flags, realtime(exptime), vlen);
...
static void process_touch_command(conn *c, token_t *tokens, const size_t ntokens) {
  printf("Unexpected command: process_touch_command\n");
    char *key;
    size_t nkey;
    int32_t exptime_int = 0;
    item *it;
...
enum delta_result_type do_add_delta(conn *c, const char *key, const size_t nkey,
    ...
    if (res + 2 > it->nbytes || it->refcount != 1) { /* need to realloc */
        item *new_it;
        new_it = item_alloc(c, ITEM_key(it), it->nkey, atoi(ITEM_suffix(it) + 1), it->exptime, res + 2 );
        if (new_it == 0) {
            do_item_remove(it);
            return EOM;
        }
        ...
        case conn_mwrite:
          if (IS_UDP(c->transport) && c->msgcurr == 0 && build_udp_headers(c) != 0) {
            ...
            case TRANSMIT_COMPLETE:
              cur_it = (item*)c->item;
              if (cur_it && (cur_it->it_flags & ITEM_SSD)) {
                if (c->disk_item_data_buf) {
                  DiskManagerReleaseBuf(c->disk_item_data_buf);
                  c->disk_item_data_buf = NULL;
                }
...
static void usage(void) {
    ...
    printf("-x            Use disk cache. Specify disk cache root path and space to use.\n"
           "              eg. with option '-x /some/path,25', trcache will use 25 \n"
           "              gigabytes disk space in directory '/some/path'           \n"
           "              while with option '-x /some/path1,25:/another/path2,256', \n"
           "              trcache will use 25G disk space in /some/path1 and 256G in /another/path2\n"
           "              they will be treated as the same cache level\n"
           "              Up to 4 disk cache space can be used. These disk space   \n"
           "              will be used as a multi-level cache, so it's highly recommended \n"
           "              to add high speed device in front of slow counterparts \n"
           );
    printf("-K            Specify the value size / key size ratio, eg. 45000/32. \n"
           "              This option take effect only when disk cache is enabled. \n"
           "              The value size and key size is not required to be accurate, \n"
           "              but they do help the disk cache work more efficient. If you have \n"
           "              no idea how this option should be set, just do not care\n"
        );
    return;
}
...
int main (int argc, char **argv) {
    ...
    const unsigned kMaxDiskLevel = 4;
    unsigned num_disk_level = 0;
    DiskLevelConf disk_conf[kMaxDiskLevel];
    memset(disk_conf, 0, sizeof(DiskLevelConf) * kMaxDiskLevel);
    size_t num_disk_cache_size = 0;
    strcpy(gLogFile, "/data/cache.log");
    ...
    while (-1 != (c = getopt(argc, argv,
          ...
          "x:"  /* Extended disk cache space, size in gigabytes*/
          "V"  /* disk operation verbose */
          "K:"  /* value size/key size ratio */
          "g:" /* log file */
        ))) {
        switch (c) {
        case 'x':
          if (num_disk_level == kMaxDiskLevel) {
            printf("Invalid disk option: too many disk levels\n");
            exit(0);
          }
          char* saveptr;
          ...
          bool level_ok = false;
          while (token) {
            DiskLevelDir* dir = (DiskLevelDir*)malloc(sizeof(DiskLevelDir));
            if (sscanf(token, "%[^,],%lu", dir->dir, &dir->size) != 2) {
              free(dir);
              printf("Invalid disk option: %s\n", token);
              exit(0);
            }
            dir->size *= 1024 * 1024 * 1024;
            num_disk_cache_size += dir->size;

            if (disk_conf[num_disk_level].size >= disk_conf[num_disk_level].capacity) {
              disk_conf[num_disk_level].capacity = disk_conf[num_disk_level].capacity == 0 ?
                  1 : 2*disk_conf[num_disk_level].capacity;
              disk_conf[num_disk_level].dirs = (DiskLevelDir**)realloc(disk_conf[num_disk_level].dirs,
                  disk_conf[num_disk_level].capacity * sizeof(DiskLevelDir*));
            }
            disk_conf[num_disk_level].dirs[disk_conf[num_disk_level].size] = dir;
            ++disk_conf[num_disk_level].size;
            level_ok = true;
          ...
          break;
        case 'K':
          if (sscanf(optarg, "%u/%u", &settings.average_value_len, &settings.average_key_len) != 2) {
            printf("Invalid value size/key size ratio: %s\n", optarg);
            exit(0);
          }
          if (settings.average_value_len == 0 || settings.average_key_len == 0) {
            printf("Invalid value size/key size ratio: %u/%u\n",
    ...
    conn_init();
    slabs_init(settings.maxbytes, num_disk_cache_size, settings.factor, preallocate);
  ...
  /* disk init */
    DiskManagerInit(disk_conf, num_disk_level, settings.num_threads);
...
char* ITEM_key_(item* item) {
  return ITEM_key(item);
}
char* ITEM_data_(item* item) {
