...
static size_t disk_item_mem_limit = 0;
static size_t disk_item_mem_malloced = 0;
...
/*@null@*/
static void *do_slabs_alloc(const size_t size, unsigned int id, bool disk_item) {
    slabclass_t *p;
    void *ret = NULL;
    item *it = NULL;
    ...
    if (disk_item) {
      if (disk_item_mem_limit && disk_item_mem_malloced + size > disk_item_mem_limit) {
          MEMCACHED_SLABS_ALLOCATE_FAILED(size, id);
          return 0;
      }
      disk_item_mem_malloced += size;
    } else {
    ...
    if (! (p->end_page_ptr != 0 || p->sl_curr != 0 ||
           do_slabs_newslab(id, disk_item) != 0)) {
...
static void do_slabs_stats(ADD_STAT add_stats, void *c) {
    ...
    APPEND_STAT("total_malloced", "%llu", (unsigned long long)mem_malloced + (unsigned long long)disk_item_mem_malloced);
...
void *slabs_alloc(size_t size, unsigned int id, bool disk_item) {
    void *ret;

    pthread_mutex_lock(&slabs_lock);
    ret = do_slabs_alloc(size, id, disk_item);
