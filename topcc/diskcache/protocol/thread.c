...
extern bool DiskManagerEnabled();
extern bool DiskManagerAppend(const char *key, KeyLengthType keylen, const char *val, ValueLengthType vallen, rel_time_t exp_time,
    DiskDataAddr* addr);
extern bool gVerbose;
extern const unsigned kBigBufSize;
...
uint32_t refcount_incr32(uint32_t *refcount) {
#ifdef HAVE_GCC_ATOMICS
    return __sync_add_and_fetch(refcount, 1);
#elif defined(__sun)
    return atomic_inc_ushort_nv(refcount);
...
uint32_t refcount_decr32(uint32_t *refcount) {
#ifdef HAVE_GCC_ATOMICS
    return __sync_sub_and_fetch(refcount, 1);
#elif defined(__sun)
    return atomic_dec_ushort_nv(refcount);
 ...
 */
void PrintStr(char* buf, unsigned len) {
  for (unsigned i = 0; i < len; ++i) {
    fprintf(stderr, "%c", buf[i]);
  }
}

item *item_alloc(conn *c, char *key, size_t nkey, int flags, rel_time_t exptime,
    int nbytes) {
  if (!DiskManagerEnabled()) {
    return do_item_alloc(key, nkey, flags, exptime, nbytes, NULL, NULL, false);
  }

  char item_buf[kBigBufSize];
  bool has_item_del = false;
  item *it = do_item_alloc(key, nkey, flags, exptime, nbytes, item_buf,
      &has_item_del, false);
  if (!it || !has_item_del) {
    // failed or success but no item deleted
    return it;
  }
  // allocate success but an old item has to be deleted

  // first try to allocate space for saving deleted item's disk address
  // if there's no space to save disk address, appending deleted item to disk will
  // make no sense.
  item* del = (item*) item_buf;
  int del_item_flags = strtoul(ITEM_suffix(del), NULL, 10);
  const unsigned DISK_ITEM_SIZE = sizeof(DiskDataAddr) + 2;
  // save the user flags in disk_item, it will be used in bin_get
  item* disk_item = do_item_alloc(ITEM_key(del), del->nkey, del_item_flags,
      del->exptime, DISK_ITEM_SIZE, NULL, NULL, true);
  if (!disk_item) {
    return it;
  }

  DiskDataAddr addr;
  // if disk manager is enabled, appending will never fail
  DiskManagerAppend(ITEM_key(del), del->nkey, ITEM_data(del), del->nbytes,
      del->exptime, &addr);
  disk_item->it_flags |= ITEM_SSD;
  memcpy(ITEM_data(disk_item), (char*) &addr, sizeof(DiskDataAddr));
  store_item(disk_item, NREAD_SET, c);
  // Decrease reference count
  item_remove(disk_item);
  assert(disk_item->refcount == 1);
  return it;
}
