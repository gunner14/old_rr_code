...
CacheItem _cache[10] = {
#ifndef kHeadVersion
  {"/fmn046", "10.3.22.50", 11211},
  {"/fmn047", "10.3.22.50", 11311},
  {"/fmn048", "10.3.22.50", 11311},
  {"/fmn049", "10.3.22.50", 11411},
  {"/fmn050", "10.3.22.50", 11411},

  {"/fmn051", "10.3.22.51", 11211},
  {"/fmn052", "10.3.22.51", 11311},
  {"/fmn053", "10.3.22.51", 11311},
  {"/fmn054", "10.3.22.51", 11411},
  {"/fmn055", "10.3.22.51", 11411},
#else
  {"/hdn", "10.3.17.85", 11211},
#endif
};
...
bool NginxCache::Init(const string& strConfFile) {
  ...
  memcached_behavior_set(client_, MEMCACHED_BEHAVIOR_DISTRIBUTION, MEMCACHED_DISTRIBUTION_MODULA);  //重要,必须调用此语句使"一致性hash生效"

  hashkit_st new_kit;
  const hashkit_st *kit = memcached_get_hashkit(client_);
  hashkit_clone(&new_kit, kit);
  hashkit_return_t hash_rc = hashkit_set_custom_function(&new_kit, my_hash_function, NULL);
  memcached_set_hashkit(client_, &new_kit);
  
  return true;
}

bool NginxCache::Set(const string& strKey, const string& strValue, time_t expiration_secs, uint32_t flags)
{
  rc_ = memcached_set(client_, strKey.c_str(), strKey.size(), strValue.c_str(), strValue.size(),  expiration_secs, flags);
