...
AsmLevSearch (SHAPE &Shape,     // io
    ...
    int iLev,                   // in
    const LANDMARK LandTab[], float *fB)   // in
{
...
while ((iter < Model.AsmLevs[iLev].nMaxSearchIters) &&
	...
	if(fB!=0)
	{
		int dim = Model.EigVecs.nrows();
		for(int i=0; i<dim; i++) fB[i] = b(i);
	//	printf("dim = %d\n", dim);
	//	b.print();
	}
}
...
if (fGetStartShape(StartShape, DetParams,
    ...
    for (int iModel = 0; iModel < nModels; iModel++)
        ...
        for (int iLev = nStartLev; iLev >= 0; iLev--)   // for each lev in image pyr
            ...
            AsmLevSearch(Shape1, SearchImgs, Models[iModel], iLev, gLandTab, 0);
		...
		CombinedShape.print("stasm Landmarks:\n");
        }
