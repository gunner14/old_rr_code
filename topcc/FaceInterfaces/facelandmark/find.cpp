...
extern Image gFaceMask;
extern Image gEyeMask;
...
static void LoadMask (Image &Mask,          // out
                      const char sFile[])   // in
{
	char sMask[SLEN];
	...
	int i;
	for(i=0; sFile[i]&&sFile[i]>='A'&&sFile[i]<='z'; i++) sMask[i] = sFile[i];
	sMask[i] = 0;
...
void InitEye(const char sDataDir[])
{
...
{
//InitEye(sDataDir);                     // init gFaceMask etc.
