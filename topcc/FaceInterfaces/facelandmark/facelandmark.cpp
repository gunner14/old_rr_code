...
Image gFaceMask;
Image gEyeMask;
Mat   gLightingCorrectionMat;
int   MAX_PROF_WIDTH_1D = 50;
Vec   gProf(MAX_PROF_WIDTH_1D, MAX_PROF_WIDTH_1D);

//-------------------------------------------------------------------------------------------------
bool FaceAsmCreate(void **pp, char *pathname)
{
	static const int NMODELS_MAX = 2;  // max number of stacked models
	ASM_MODEL *pModels = new ASM_MODEL[NMODELS_MAX];

	char sConfFile0[SLEN], sConfFile1[SLEN];
	//strcpy(sConfFile0, "landmarks/mu-68-1d.conf");
    //strcpy(sConfFile1, "landmarks/mu-76-2d.conf");
	sprintf(sConfFile0, "%s/mu-68-1d.conf", pathname);
	sprintf(sConfFile1, "%s/mu-76-2d.conf", pathname);
 
	const int nInitModels = nInitAsmModels(pModels, sConfFile0, sConfFile1);
...
int FaceAsmLandmarks(void *p, int w, int s, int h, byte *data, int x, int y, int d, 
					 float *point, float *b)
{
	// crop the face image
	...
	if(Shape.nrows())  // successfully located landmarks?
	{
        ...
        for(int iPoint=0; iPoint<npoint; iPoint++)
		{
			...
			point[2*iPoint]   = x0+Shape(iPoint, VX);
			point[2*iPoint+1] = y0+Shape(iPoint, VY);
		}
    }
...
//-------------------------------------------------------------------------------------------------
void FaceAsmRelease(void **pp)
{
	if(*pp!=0)
	{
		ASM_MODEL *pModels = (ASM_MODEL*)(*pp);
		delete []pModels;
		*pp = 0;
	}
