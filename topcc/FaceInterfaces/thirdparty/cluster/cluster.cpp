...
****/
void cuttreed( int elements, Node* tree, double distance, int clusterid[] ){

	int i;
	int ncluster = -1;

	for(i=0; i<elements-1;i++){
		if(tree[i].distance > distance) break;
	}
	if(i >= elements -1) ncluster = 1;
	else ncluster = elements-i;
	cuttree(elements, tree, ncluster, clusterid);
