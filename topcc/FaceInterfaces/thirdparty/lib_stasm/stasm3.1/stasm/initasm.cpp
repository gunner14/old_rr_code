...
double CONF_NormalizedProfLen = 1;      // root mean square length of profiles: added by Zheng
...
//-----------------------------------------------------------------------------
static bool InitAsm (ASM_MODEL &Asm,                // out
                     int &nLevs,                    // out
                     char sAsmFile[],               // out
                     ...
                     const char sConfFile[])        // in
{
	if(!ReadConfFile(ConfTab, nConfTab, sConfFile, "STASM")) return false;
	if(!CONF_sAsmFile[0]) Err("no sAsmFile entry in %s", sConfFile);
	ExpandAsmFileName(sAsmFile, sConfFile, CONF_sAsmFile);  // init sAsmFile
	ReadAsmFile(Asm, nLevs, sAsmFile);
	if(CONF_fXm2vts) InitLandTab(Asm.nPoints);
                ...
                const char sConfFile1[]) // in: 2nd config filename, "" if none
{
	...
	if(nModels==0)                       // Models not yet initialized?
    {
		int nLevs0;
		char sAsmFile0[SLEN];
		if(!InitAsm(Models[0], nLevs0, sAsmFile0, StasmConfTab, NELEMS(StasmConfTab), sConfFile0)) return 0;
		nModels++;
		...
		if(sConfFile1 && sConfFile1[0])    // stacked models?
        {
			int nLevs1;
			char sAsmFile1[SLEN];
			if(!InitAsm(Models[1], nLevs1, sAsmFile1, StasmConfTab, NELEMS(StasmConfTab), sConfFile1)) return 0;
			CheckModelConsistency(Models[0], Models[1], nLevs0, nLevs1, sAsmFile0, sAsmFile1);
