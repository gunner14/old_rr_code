                          ...
                          const bool fEyes)        // in: find eyes too?
{
	...
	bool fSuccess = true;
	if (fSuccess && fEyes) FindEyesGivenVjFace(DetParams, Img, sDataDir);
	return fSuccess;
}
...
bool fFindDetParams (DET_PARAMS &DetParams, // out
                     Image &Img, //const char sPath[],    // in: image path
                     unsigned DetAttr,      // in: which face detector
                     ...
                     bool fIssueWarnings)   // in: true to issue warnings if needed
{
	...
	//sLoadImage(Img, "test-image.jpg");//sPath);
	switch (DetAttr)
    {
	case FA_ViolaJones:
		fFoundFace = fFindViolaJonesFace(DetParams, Img, "", sDataDir, CONF_nVjMethod!=1);
        break;
    case FA_Rowley:
        fFoundFace = fRowleyFindFace(DetParams, Img, "", sDataDir);
        break;
    default:
        Err("fFindDetParams: bad DetAttr 0x%x", DetAttr);
    }
	if(fIssueWarnings) PossiblyIssueDetWarning(fFoundFace, DetAttr, DetParams);
	return fFoundFace;
}
...
int nGetDetParams (DET_PARAMS &DetParams,   // out
            Image &Img, //const char sImage[],        // in: image name
            ...
            bool fIssueWarnings)        // in: true to issue warnings if needed
{
	...
	if(!sShapeFile)
    {
		if(fFindDetParams(DetParams, Img, DetAttr, sDataDir, fIssueWarnings))
			nGenDet = 1;    // successful
    }
	else    // use shape file
    {
		const char sImage[256] = "";
...
fGetStartShape (
        ...
        Image &Img, //const char sImage[],            // in: image file name
        ...
        bool fIssueWarnings)            // in: true to issue warnings if needed
{
	if(!nGetDetParams(DetParams,
                   Img, DetAttr, sShapeFile, sDataDir,
