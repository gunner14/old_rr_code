...
inline int iGetX (double X, int iOffset, int iOrthOffset, // all in
                  double DeltaX, double DeltaY)
{
    return iround(X + (iOffset * DeltaX) + (iOrthOffset * DeltaY));
}

inline int iGetY (double Y, int iOffset, int iOrthOffset, // all in
                  double DeltaX, double DeltaY)
{
...
//-----------------------------------------------------------------------------
int nGetProfWidth (int nCols, unsigned ProfSpec)    // in: all
{
...
Vec GetBisector (const Vec &Prev, const Vec &This, const Vec &Next) // all in
{
	Vec p(2, ROWVEC), q(2, ROWVEC);  // static to reduce number of mallocs

	ASSERT(!(Prev(VX) == 0 && Prev(VY) == 0));
	ASSERT(!(This(VX) == 0 && This(VY) == 0));
	ASSERT(!(Next(VX) == 0 && Next(VY) == 0));

	Vec d0; d0.assign(This); d0 -= Prev; d0.normalizeMe();

	p(VX) = d0(VY);                                 // rotate 90 degrees
	p(VY) = -d0(VX);

	Vec d1; d1.assign(Next); d1 -=This; d1.normalizeMe();
...
int iGetPixel (const Image &Img, int ix, int iy)    // in: all
{
...
//-----------------------------------------------------------------------------
void GetPrevNextLandmarks (int &iPrev, int &iNext,   // out
...
void GetProfStepSize (double &DeltaX, double &DeltaY,    // out
                      ...
                      const bool fExplicitPrevNext)      // in
{
	...
	Vec PrevRow; PrevRow.dim(Shape.ncols(), ROWVEC); PrevRow = Shape.row(iPrev);
	Vec NextRow; NextRow.dim(Shape.ncols(), ROWVEC); NextRow = Shape.row(iNext);
...
extern int MAX_PROF_WIDTH_1D;
extern Vec gProf;
int          ngProfWidth;
int          igProfPoint;    // for sanity checking
const byte   *pgProfImage;   // ditto
unsigned     gSubProfSpec;   // ditto
               ...
               unsigned SubProfSpec, const Image &Img, int iPoint, int iOffset)   // in
{
	...
	Prof = gProf.view(0, ngProfWidth/2 + iOffset - nProfWidth/2, 1, nProfWidth);
    ...
    int nPoints)                        // in:
{
	...
	if(IS_2D(ProfSpec)) InitGrads(Grads, Img, ProfSpec);
}
