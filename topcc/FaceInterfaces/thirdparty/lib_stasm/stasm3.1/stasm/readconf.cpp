            ...
            const char sFile[], FILE *pFile)
{
	...
	if(strchr(s, '}')) return false; // reached end of table
	...
	if(2!=sscanf(s, "%s %s", sName, sTemp))
		Err("bad config format in line %d of %s: %s", nGetLineNbr(pFile), sFile, s);

	// look for keyword (stored in sName) in ConfTab
	int i;
	for(i=0; i<nConfTab; i++)
	{
		if(0==strcmp(ConfTab[i].sName, sName)) break;  // found entry in ConfTab
	}
...
// 			char *p1 = ++p;
// 			while(*p1 && *p1 != '"') p1++; // skip to end of string
// 			if(*p1 != '"')
// 				Err("bad config format in line %d of %s: %s"
// 					"       String must be enclosed in \"quotes\"",
// 					nGetLineNbr(pFile), sFile, s);
...
{
	if(VERBOSE_ASM_SEARCH) lprintf("Reading %s\n", sFile);
