...
void parallel_post(size_t seconds, int post_thread) {
  ...
  boost::asio::deadline_timer timer(arch::AsyncRunner::GetService());
...
TEST(AsyncallTest, post) {
  ...
  arch::AsyncRunner::GetService().reset();
  arch::AsyncRunner::GetService().run();
...
TEST(AsyncallTest, post_counted) {
  work_.reset(new boost::asio::io_service::work(arch::AsyncRunner::GetService()));
...
TEST(AsyncallTest, timed_parallel_post) {
  boost::asio::io_service::work work(arch::AsyncRunner::GetService());
  arch::AsyncRunner::instance().Start();
