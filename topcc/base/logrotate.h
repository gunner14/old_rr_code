#ifndef ARCH_BASE_LOG_ROTATE_H__
#define ARCH_BASE_LOG_ROTATE_H__
...
#define getcwd _getcwd
    ...
    int ret = sigaction(ARCH_SIGNAL_LOG_OPEN, &sa, NULL);
    ASSERT(ret == 0);
    ret = sigaction(ARCH_SIGNAL_LOG_ROTATE, &sa, NULL);
    ASSERT(ret == 0);
#endif
  ...
  void Rotate() {
    ...
    /path/to/oce_log {
      rotate 3
        daily
        postrotate
        kill -ARCH_SIGNAL_LOG_OPEN getpid()
        endscript
    }
#endif
...
#define tempnam _tempnam // ugly
  ...
  static void SignalHandler(int sig, siginfo_t* sinfo, void* ucontext) {
    ...
    if (sig == ARCH_SIGNAL_LOG_OPEN)
      r.Open();
    else if(ARCH_SIGNAL_LOG_ROTATE == sig)
      r.Rotate();
  }
#endif
...
#endif // ARCH_BASE_LOG_ROTATE_H__
