...
struct Request {
  Request() :
      user_(0), method_(HV_GET) {
  }

  // !! 必须保证只有一个线程读
  bool Init(FCGX_Stream* in, FCGX_ParamArray envp, std::string &data);
  ...
  static DispositionArrayType ParseMultipartForm(
...
public:
  void Add(HttpHeader hh, std::string const& value) {
    list_.push_back(std::make_pair(hh, value));
  }
  ...
  void Write(std::ostream& ostem) const {
    ...
    ostem << "HTTP/" << ToString(HVER_X) << " " << hc_ << " " << message_
        << CRLF;
