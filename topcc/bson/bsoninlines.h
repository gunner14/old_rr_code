...
namespace bson {
    ...
    inline BSONObj BSONElement::embeddedObjectUserCheck() const {
        ...
        DCHECK(isABSONObj()) << "invalid parameter: expected an object";
    ...
    inline BSONElement BSONObj::getField(const arch::StringPiece& name) const {
    ...
    inline void BSONElement::validate() const {
        ...
        case bson::String: {
            ...
            LOG(ERROR) <<  "Invalid dbref/code/string/symbol size: " << x << " strnlen:" << bson::strnlen( valuestr() , x );
            
            break;
        }
        case CodeWScope: {
            int totalSize_ = *( int * )( value() );
            massert( 10322 ,  "Invalid CodeWScope size", totalSize_ >= 8 );
            int strSizeWNull = *( int * )( value() + 4 );
            massert( 10323 ,  "Invalid CodeWScope std::string size", totalSize_ >= strSizeWNull + 4 + 4 );
            massert( 10324 ,  "Invalid CodeWScope std::string size",
                     strSizeWNull > 0 &&
                     (strSizeWNull - 1) == bson::strnlen( codeWScopeCode(), strSizeWNull ) );
            massert( 10325 ,  "Invalid CodeWScope size", totalSize_ >= strSizeWNull + 4 + 4 + 4 );
            int objSize = *( int * )( value() + 4 + 4 + strSizeWNull );
            massert( 10326 ,  "Invalid CodeWScope object size", totalSize_ == 4 + 4 + strSizeWNull + objSize );
    ...
    inline int BSONElement::size( int maxLen ) const {
        ...
        {
            const char *p = value();
            size_t len1 = ( maxLen == -1 ) ? strlen( p ) : bson::strnlen( p, remain );
            //massert( 10318 ,  "Invalid regex std::string", len1 != -1 ); // ERH - 4/28/10 - don't think this does anything
            p = p + len1 + 1;
            size_t len2 = ( maxLen == -1 ) ? strlen( p ) : bson::strnlen( p, remain - len1 - 1 );
            //massert( 10319 ,  "Invalid regex options std::string", len2 != -1 ); // ERH - 4/28/10 - don't think this does anything
            x = (int) (len1 + 1 + len2 + 1);
        }
        break;
        default: {
        ...
        totalSize_ =  x + fieldNameSize() + 1; // BSONType
