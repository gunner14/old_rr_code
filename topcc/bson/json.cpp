    ...
    struct fieldNameEnd {
        ...
        void operator() ( const char *start, const char *end ) const {
            ...
            DCHECK(
                     ...
                     name != "$date" &&
                     name != "$regex" &&
                     name != "$options" ) << "Invalid use of reserved field name";
#endif
...
// NOTE s must be 24 characters.
    ...
    struct binDataBinary {
        ...
        void operator() ( const char *start, const char *end ) const {
          ...
          DCHECK(( end - start ) % 4 == 0) << "Badly formatted bindata";
    ...
    BSONObj fromjson( const char *str , int* len) {
        ...
        if (len) {
            *len = result.stop - str;
            ...
            if (limit == -1) limit = 10;
            // msgasserted(10340, "Failure parsing JSON std::string near: " + std::string( result.stop, limit ));
            LOG(ERROR) << "Failure parsing JSON std::string near: " << std::string( result.stop, limit );
        }
        BSONObj ret = b.pop();
        assert( b.empty() );
        return ret;
