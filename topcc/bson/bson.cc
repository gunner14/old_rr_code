...
namespace bson {
static BSONElement nullElement;
...
std::string BSONElement::jsonString( JsonStringFormat format, bool includeFieldNames, int pretty ) const {
  BSONType t = type();
  if ( t == Undefined )
    return "";
  ...
  switch ( type() ) {
        ...
        default:
          StringBuilder ss;
          ss << "Cannot create a properly formatted JSON std::string with "
...
std::string BSONObj::md5() const {
  ...
  MD5Context ctx;
  MD5Init(&ctx);
  MD5Update(&ctx, _objdata , objsize());
...
int BSONObj::woSortOrder(const BSONObj& other, const BSONObj& sortKey , bool useDotted ) const{
  ...
  DCHECK(! sortKey.isEmpty()) << "woSortOrder needs a non-empty sortKey";
...
void BSONObj::getFieldsDotted(const arch::StringPiece& name, BSONElementSet &ret ) const {
    ...
    if ( e.mayEncapsulate() ){
      switch ( e.type() ){
                ...
                default:
                  ...
                  LOG(ERROR) << "unhandled cases in BSONObj okForStorage";
      }
...
unsigned OID::_machine = 0; // TODO: (unsigned) security.getNonceInitSafe();
void OID::newState(){
  unsigned before = _machine;
  // using fresh Security object to avoid buffered devrandom
  _machine = 0; // TODO: (unsigned)security.getNonce();
...
void BSONObjBuilder::appendMinForType( const arch::StringPiece& fieldName , int t ){
  switch ( t ){
  ...
  LOG(ERROR) << "type not supported for appendMinElementForType";
}

void BSONObjBuilder::appendMaxForType( const arch::StringPiece& fieldName , int t ){
  switch ( t ){
        ...
        case Timestamp:
          appendTimestamp( fieldName , std::numeric_limits<unsigned long long>::max() ); break;
        default:
...
bool BSONObjBuilder::appendAsNumber( const arch::StringPiece& fieldName , const std::string& data ){
  if ( data.size() == 0 || data == "-")
    return false;
...
static Ordering Ordering::make(const BSONObj& obj) {
  unsigned b = 0;
