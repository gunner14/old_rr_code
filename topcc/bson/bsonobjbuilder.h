...
#ifndef BSON_BSONOBJ_BUILDER_H__
#define BSON_BSONOBJ_BUILDER_H__
...
#if defined(_WIN32)
// warning: 'this' : used in base member initializer std::list
...
public:
  ...
  BSONObjBuilder&  appendAs(const BSONElement& e, const arch::StringPiece& fieldName) {
    assert( !e.eoo() ); // do not append eoo, that would corrupt us. the builder auto appends when done() is called.
  ...
  BSONObjBuilder& append(const arch::StringPiece& fieldName, BSONObj subObj) {
  ...
  /** add a subobject as a member */
  BSONObjBuilder& appendObject(const arch::StringPiece& fieldName, const char * objdata , int size = 0 ){
    assert( objdata );
    if ( size == 0 ){
      size = *((int*)objdata);
    }
  ...
  BufBuilder &subobjStart(const arch::StringPiece& fieldName) {
    _b.appendNum((char) Object);
    _b.appendStr(fieldName);
    return _b;
  }
  ...
  style fields in it.
  */
  BSONObjBuilder& appendArray(const arch::StringPiece& fieldName, const BSONObj &subObj) {
    _b.appendNum((char) Array);
    _b.appendStr(fieldName);
    _b.appendBuf((void *) subObj.objdata(), subObj.objsize());
    return *this;
  }
  BSONObjBuilder& append(const arch::StringPiece& fieldName, BSONArray arr) { 
    return appendArray(fieldName, arr); 
  }    

  /** add header for a new subarray and return bufbuilder for writing to
  the subarray's body */
  BufBuilder &subarrayStart(const arch::StringPiece& fieldName) {
    _b.appendNum((char) Array);
    _b.appendStr(fieldName);
    return _b;
  }
  ...
  BSONObjBuilder& appendBool(const arch::StringPiece& fieldName, int val) {
  ...
  /** Append a boolean element */
  BSONObjBuilder& append(const arch::StringPiece& fieldName, bool val) {
    _b.appendNum((char) Bool);
    _b.appendStr(fieldName);
    _b.appendNum((char) (val?1:0));            
    return *this;
  ...
  BSONObjBuilder& append(const arch::StringPiece& fieldName, int n) {
  ...
  BSONObjBuilder& append(const arch::StringPiece& fieldName, unsigned n) { 
    return append(fieldName, (int) n); 
  }

  /** Append a NumberLong */
  BSONObjBuilder& append(const arch::StringPiece& fieldName, long long n) { 
  ...
  BSONObjBuilder& appendIntOrLL( const arch::StringPiece& fieldName , long long n ){
    long long x = n;
    if ( x < 0 )
      x = x * -1;
  ...
  * mostly for JS
  */
  BSONObjBuilder& appendNumber( const arch::StringPiece& fieldName , int n ){
    return append( fieldName , n );
  }

  BSONObjBuilder& appendNumber( const arch::StringPiece& fieldName , double d ){
    return append( fieldName , d );
  }

  BSONObjBuilder& appendNumber( const arch::StringPiece& fieldName , long long l ){
  ...
  BSONObjBuilder& append(const arch::StringPiece& fieldName, double n) {
  ...
  */
  bool appendAsNumber( const arch::StringPiece& fieldName , const std::string& data );

  /** Append a BSON Object ID (OID type). 
  @deprecated Generally, it is preferred to use the append append(name, oid) 
  method for this.
  */
  BSONObjBuilder& appendOID(const arch::StringPiece& fieldName, OID *oid = 0 , bool generateIfBlank = false ) {
  ...
  */
  BSONObjBuilder& append( const arch::StringPiece& fieldName, OID oid ) {
    _b.appendNum((char) jstOID);
    _b.appendStr(fieldName);
    _b.appendBuf( (void *) &oid, 12 );
    return *this;
  ...
  */
  BSONObjBuilder& appendTimeT(const arch::StringPiece& fieldName, time_t dt) {
  ...
  */
  BSONObjBuilder& appendDate(const arch::StringPiece& fieldName, Date_t dt) {
  ...
  BSONObjBuilder& append(const arch::StringPiece& fieldName, Date_t dt) {
  ...
  */
  BSONObjBuilder& appendRegex(const arch::StringPiece& fieldName, const char *regex, const char *options = "") {
  ...
  @param regex options such as "i" or "g"
  */
  BSONObjBuilder& appendRegex(const arch::StringPiece& fieldName, std::string regex, std::string options = "") {
    return appendRegex(fieldName, regex.c_str(), options.c_str());
  }
  BSONObjBuilder& appendCode(const arch::StringPiece& fieldName, const char *code) {
  ...
  /** Append a std::string element. len DOES include terminating nul */
  BSONObjBuilder& append(const arch::StringPiece& fieldName, const char *str, int len) {
  ...
  BSONObjBuilder& append(const arch::StringPiece& fieldName, const char *str) {
    return append(fieldName, str, (int) strlen(str)+1);
  }
  /** Append a std::string element */
  BSONObjBuilder& append(const arch::StringPiece& fieldName, std::string str) {
    return append(fieldName, str.c_str(), (int) str.size()+1);
  }
  BSONObjBuilder& appendSymbol(const arch::StringPiece& fieldName, const char *symbol) {
  ...
  BSONObjBuilder& appendNull( const arch::StringPiece& fieldName ) {
    _b.appendNum( (char) jstNULL );
    _b.appendStr( fieldName );
    return *this; }

  // Append an element that is less than all other keys.
  BSONObjBuilder& appendMinKey( const arch::StringPiece& fieldName ) {
    _b.appendNum( (char) MinKey );
    _b.appendStr( fieldName );
    return *this; 
  }
  // Append an element that is greater than all other keys.
  BSONObjBuilder& appendMaxKey( const arch::StringPiece& fieldName ) {
  ...
  BSONObjBuilder& appendTimestamp( const arch::StringPiece& fieldName ) {
  ...
  BSONObjBuilder& appendTimestamp( const arch::StringPiece& fieldName , unsigned long long val ) {
    _b.appendNum( (char) Timestamp );
    _b.appendStr( fieldName );
    _b.appendNum( val );
    return *this; 
  ...
  */
  BSONObjBuilder& appendTimestamp( const arch::StringPiece& fieldName , unsigned long long time , unsigned int inc );

  /*
  Append an element of the deprecated DBRef type.
  @deprecated 
  */
  BSONObjBuilder& appendDBRef( const arch::StringPiece& fieldName, const char *ns, const OID &oid ) {
  ...
  */
  BSONObjBuilder& appendBinData( const arch::StringPiece& fieldName, int len, BinDataType type, const char *data ) {
  ...
  BSONObjBuilder& appendBinData( const arch::StringPiece& fieldName, int len, BinDataType type, const unsigned char *data ) {
  ...
  */
  BSONObjBuilder& appendCodeWScope( const arch::StringPiece& fieldName, const char *code, const BSONObj &scope ) {
  ...
  void appendUndefined( const arch::StringPiece& fieldName ) {
  ...
  */
  void appendMinForType( const arch::StringPiece& fieldName , int type );
  void appendMaxForType( const arch::StringPiece& fieldName , int type );

  /** Append an array of values. */
  template < class T >
  BSONObjBuilder& append( const arch::StringPiece& fieldName, const std::vector< T >& vals );

  template < class T >
  BSONObjBuilder& append( const arch::StringPiece& fieldName, const std::list< T >& vals );

  /** The returned BSONObj will free the buffer when it is finished. */
  BSONObj obj() {
    bool own = owned();
    // massert( 10335 , "builder does not own memory", own );
    DCHECK(own) << "builder does not own memory";
    int l;
  ...
  */
  ...
  // prevent implicit std::string conversions which would allow bad things like BSON( BSON( "foo" << 1 ) << 2 )
  ...
  Labeler operator<<( const Labeler::Label &l ) {
    ...
    DCHECK(_s.subobjStarted()) << "No subobject started";
    return _s << l;
  }
...
public:
  ...
  template <typename T>
  BSONArrayBuilder& append(const arch::StringPiece& name, const T& x){
    fill( name );
    append( x );
    return *this;
  }
  ...
  void appendArray( const arch::StringPiece& name, BSONObj subObj ) {
    fill( name );
...
private:
  void fill( const arch::StringPiece& name ) {
    char *r;
    int n = strtol( name.data(), &r, 10 );
    LOG_IF(WARNING, *r) << "can't append to array using std::string field name [" << name.data() << "]";
    // if ( *r )
    //  uasserted( 13048, (string)"can't append to array using std::string field name [" + name.data() + "]" );
    while( _i < n )
      append( nullElt() );
  }
...
template < class T >
inline BSONObjBuilder& BSONObjBuilder::append( const arch::StringPiece& fieldName, const std::vector< T >& vals ) {
...
template < class T >
inline BSONObjBuilder& BSONObjBuilder::append( const arch::StringPiece& fieldName, const std::list< T >& vals ) {
  BSONObjBuilder arrBuilder;
  int n = 0;
  for( typename std::list< T >::const_iterator i = vals.begin(); i != vals.end(); i++ )
