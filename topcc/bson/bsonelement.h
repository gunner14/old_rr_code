...
#ifndef BSON_BSONELMENT_H__
#define BSON_BSONELMENT_H__
...
The BSONElement object points into the BSONObj's data_.  Thus the BSONObj must stay in scope
...
public:
  ...
  std::string foo = obj["foo"].String(); // exception if not a std::string type or DNE
  */
  ...
  */
  ...
  BSONType type() const { return (BSONType) *data_; }
  ...
  */
  ...
  /** raw data_ of the element's value (so be careful). */
  const char * value() const {
    return (data_ + fieldNameSize() + 1);
  }
  ...
  */
  ...
  /** True if number, std::string, bool, date, OID */
  ...
  Note: casts to double, data_ loss may occur with large (>52 bit) NumberLong values.
  */
  ...
  */
  ...
  /** Get a std::string's value.  Also gives you start of the real data_ for an embedded object.
  You must assure data_ is of an appropriate type first -- see also valuestrsafe().
  */
  ...
  /** Get javascript code of a CodeWScope data_ element. */
  const char * codeWScopeCode() const {
    return value() + 8;
  }
  /** Get the scope SavedContext of a CodeWScope data_ element. */
  ...
  /** Get raw binary data_.  Element must be of type BinData. Doesn't handle type 2 specially */
  const char *binData(int& len) const {
    // BinData: <int len> <byte subtype> <byte[len] data_>
    assert( type() == BinData );
    len = valuestrsize();
    return value() + 5;
  }
  /** Get binary data_.  Element must be of type BinData. Handles type 2 */
  ...
  */
  ...
  /** Check that data_ is internally consistent. */
  void validate() const;
  ...
  explicit BSONElement(const char *d, int maxLen = -1) : data_(d) {
    fieldNameSize_ = -1;
    if ( eoo() )
      fieldNameSize_ = 0;
    else {
...
private:
  ...
  mutable int totalSize_; /* caches the computed size */

  friend class BSONObjIterator;
  friend class BSONObj;

  const BSONElement& chk(BSONType t) const {
    LOG_IF(ERROR, t != type()) << "wrong type for BSONElement (" << fieldName() << ") " << type() << " != " << t;
    return *this;
  }
