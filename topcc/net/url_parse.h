...
bool ParseVariant(const std::string &pp, std::vector<std::string> *v, std::map<std::string, std::string> *m) {
  if (!v)
    return false; 
  std::string path;
  std::string var;
...
// a=1&b=eee
bool ParseParam(const std::string &pa, std::map<std::string, std::string> *param) {
  if (!param)
    return false;
  std::string pp;
  if (pa[pa.size() -1] != '&')
...
// host need exist
bool ParseUrl(const std::string &url, std::string *host, int *port, std::string *path, std::map<std::string, std::string> *param) {
  if (param)
    param->clear();
  if (port)
    *port = 80;
