...
size_t SpdyFramer::BytesSafeToRead() const {
  switch (state_) {
    ...
    case SPDY_AUTO_RESET:
    case SPDY_RESET:
      return SpdyFrame::size();
    case SPDY_READING_COMMON_HEADER:
      if (SpdyFrame::size() == current_frame_len_) { //  正好8个字节时
    ...
    case SPDY_INTERPRET_CONTROL_FRAME_COMMON_HEADER:
      /*if (current_control_frame.type() == SYN_STREAM)
        return 4 + 4 + 4;//streamid+a_streamid+Unused+header block
      else if (current_control_frame.type() == SYN_REPLY)
        return 4 + 4; //streamid+Unused+header block
      else if (current_control_frame.type() == RST_STREAM) 
        return 4 + 4;
      else
        return 4;*/
...
void SpdyFramer::ProcessControlFrameHeader() {
  ...
  switch (current_control_frame.type()) {
    ...
    case WINDOW_UPDATE:
      ...
      break;
    default:
      LOG(LS_WARNING) << "Valid spdy control frame with unknown type: "
  ...
  do {
    ...
    if ( control_frame.flags() & CONTROL_FLAG_FIN) {
      if (control_frame.type() == SYN_REPLY)
        ...
        visitor_->OnStreamFrameData(reinterpret_cast<SpdySynStreamControlFrame*>(
                                      &control_frame)->stream_id(),
                                    NULL, 0);
  ...
  if (remaining_payload_) {
    ...
    if (amount_to_forward && state_ != SPDY_IGNORE_REMAINING_PAYLOAD) {
      if (current_data_frame.flags() & DATA_FLAG_COMPRESSED) {
        ...
        if (rv != Z_OK) {
          LOG(LS_WARNING) << "inflate failure: " << rv;
...
bool SpdyFramer::ParseHeaderBlock(const SpdyFrame* frame,
  ...
  // TODO: Java端暂时无法支持压缩 scoped_ptr<SpdyFrame> decompressed_frame(DecompressFrame(*frame));
  scoped_ptr<SpdyFrame> decompressed_frame(new SpdyControlFrame(frame->data(), false));
...
z_stream* SpdyFramer::GetHeaderCompressor() {
  ...
  if (success == Z_OK)
    success = deflateSetDictionary(header_compressor_.get(),
                                   ...
                                   kDictionarySize);
  if (success != Z_OK) {
    LOG(LS_WARNING) << "deflateSetDictionary failure: " << success;
...
z_stream* SpdyFramer::GetHeaderDecompressor() {
  ...
  if (success != Z_OK) {
    LOG(LS_WARNING) << "inflateInit failure: " << success;
...
z_stream* SpdyFramer::GetStreamCompressor(SpdyStreamId stream_id) {
  ...
  int success = deflateInit2(compressor.get(),
                             ...
                             Z_DEFAULT_STRATEGY);
  if (success != Z_OK) {
    LOG(LS_WARNING) << "deflateInit failure: " << success;
    return NULL;
  }
...
SpdyFrame* SpdyFramer::CompressFrameWithZStream(const SpdyFrame& frame,
  ...
  if (rv != Z_OK) {  // How can we know that it compressed everything?
    ...
    LOG(LS_WARNING) << "deflate failure: " << rv;
    return NULL;
  }
