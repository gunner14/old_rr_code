...
public:
  ...
  static bool GetSize(const string& data, int& nWidth, int& nHeight,
                      string& magick);
  ...
  static bool ReadImage(const string& data, Image& image, int nWidth = 0,
                        int nHeight = 0);
  ...
  static bool ImageData(const Image &source, int quality, std::string *buf,
  ...
  static bool SharpenImage(Image& image);
  static bool SharpenImage(const Image &source, Image &image);

  static bool RotateImage(Image& image, double deg, int *width, int *height);
  //reflect each scanline in the vertical direction
  static bool FlipImage(Image& image);
  static bool CropImage(Image& image, int x, int y, int width, int height);
  static bool CropImage(const Image &source, int x, int y, int width,
                        int height, Image &image);
  //cmyk->rgb
  static bool TransferCMYKImage(Image& image);

  static bool ResizeImage(Image &source, int width, int height);
  static bool ResizeImage(const Image &source, int width, int height,
                          Image &image);

  static bool ResizeGifData(const std::string &data, int width, int height,
                            std::string *outdata);
...
private:
  static bool GetFirstImage(const string& data, int width, int height,
                            Image &image);
