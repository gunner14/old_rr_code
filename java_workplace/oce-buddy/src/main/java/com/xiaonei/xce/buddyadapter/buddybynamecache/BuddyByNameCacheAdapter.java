package com.xiaonei.xce.buddyadapter.buddybynamecache;
...
public class BuddyByNameCacheAdapter extends ReplicatedClusterAdapter {
	 ...
	 * @return
	 */
	protected BuddyByNameCacheManagerPrx getBuddyByNameCacheManagerTwoway(
			int userId) {
		Ice.ObjectPrx prx0 = getValidProxy(userId, 300);
		if (prx0 != null) {
			BuddyByNameCacheManagerPrx prx = BuddyByNameCacheManagerPrxHelper
	 ...
	 * @return
	 */
	protected BuddyByNameCacheManagerPrx getBuddyByNameCacheManagerOneway(
			int userId) {
		Ice.ObjectPrx prx0 = getValidProxy(userId, 300);
		if (prx0 != null) {
			BuddyByNameCacheManagerPrx prx = BuddyByNameCacheManagerPrxHelper
	...
	public static void main(String[] args) {
		...
		batch.add(userId1);
		batch.add(userId2);
		List<Integer> listRes1 = BuddyByNameCacheAdapter.getInstance()
				.getFriendList(userId1, -1);
		List<Integer> listRes2 = BuddyByNameCacheAdapter.getInstance()
				.getFriendList(userId2, -1);
		Map<Integer, List<Integer>> listResBatch = BuddyByNameCacheAdapter
				.getInstance().getFriendLists(batch);
		int countRes1 = BuddyByNameCacheAdapter.getInstance().getFriendCount(
				userId1);
		int countRes2 = BuddyByNameCacheAdapter.getInstance().getFriendCount(
				userId2);
		Map<Integer, Integer> countResBatch = BuddyByNameCacheAdapter.getInstance()
...
class BuddyByNameLoaderAdapter extends ReplicatedClusterAdapter {
	...
	protected BuddyByNameLoaderPrx getBuddyByNameLoaderOneway(int userId) {
		Ice.ObjectPrx prx0 = getValidProxy(userId, 300);
		if (prx0 != null) {
			BuddyByNameLoaderPrx prx = BuddyByNameLoaderPrxHelper.uncheckedCast(prx0
