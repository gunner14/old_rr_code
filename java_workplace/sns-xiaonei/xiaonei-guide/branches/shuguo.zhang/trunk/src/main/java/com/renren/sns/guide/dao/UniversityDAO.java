...
public class UniversityDAO extends XnDAO {
     ...
     * 存储univId于群组ID的对应关系
     */
    public Map<Integer, Integer> univTribeIdsCache = new HashMap<Integer, Integer>();

    private static final String sqlUnivTribeIds = "select id, tribe from university ";
    ...
    public void initUnivTribeIds() {
        OpList op = new OpList(sqlUnivTribeIds, DataAccessMgr.BIZ_COMMON) {

            public Object parse(ResultSet rs) throws SQLException {
                Integer[] ids = new Integer[2];
                ids[0] = rs.getInt("id");
                ids[1] = rs.getInt("tribe");
                return ids;
            }
        };

        if (univTribeIdsCache.size() > 0) {
        	univTribeIdsCache.clear();
        }

        try {
            List<Integer[]> ids = DataAccessMgr.getInstance().queryList(op);
            if (ids != null && ids.size() > 0) {
                for (Integer[] id : ids) {
                	univTribeIdsCache.put(id[0], id[1]);
                }
            }
    ...
    public University get(final int id) throws SQLException {
        ...
        if (uni == null) uni = new University();
        return uni;
    }
    ...
    public University getSearch(final int id) throws SQLException {
        OpUniq op = new OpUniq(sqlGetSearch, DataAccessMgr.BIZ_SEARCH) {
    ...
    public University getByUrl(final String url) throws SQLException {
        ...
        return (University) DataAccessMgr.getInstance().queryUnique(op);
        // Query qr =
        // ss.createQuery("select univ from University univ where univ.url = ?").setString(0,
        // url);
    ...
    public List<String> getEmailList(final int univ) throws SQLException {
        ...
        return (ArrayList<String>) DataAccessMgr.getInstance().queryList(op);
    }
