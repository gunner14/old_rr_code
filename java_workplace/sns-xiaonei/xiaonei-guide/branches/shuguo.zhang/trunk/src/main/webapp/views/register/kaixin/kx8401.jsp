...
<script type="text/javascript" src="${applicationScope.urlStatic}/kaixin/jspro/renren.login.reg.js?ver=$revxxx$"></script>
...
.notinv_login{background-image:url(${applicationScope.urlStatic}/kaixin/imgpro/bg/regbtn_invite.gif)!important}
.kaixin_friends{ border:1px solid #e9e9e9; padding-top:11px;}
...
#navigation .nav-right {padding-right:20px;padding-top:3px;}
#navigation .nav-right li a{text-decoration:none;color:#005eac;display:block;padding:5px 10px;line-height:15px;}
#navigation .nav-right li a:hover{color:#005eac;}
#navigation .nav-right {margin-right:0}
.reg_dogbox{_margin-left:10px;}
</style>
</head>
...
<div id="container">
	<div class="mainbox">
    <div id="header">
		<div id="logo"><a href="#" title="开心网"><img src="${applicationScope.urlStatic}/kaixin/imgpro/logo/v2_logo.png" alt="开心网(Logo)" /></a></div>
		<div id="navigation">
            <ul class="nav-right">
            </ul>
        </div>
        ...
        <div class="kaixin_loginarea">
       		<div class="kaixin_login">
				<form method="post" id="loginForm" action="${applicationScope.urlPassport}/PLogin.do" >
					<input type="hidden" id="loginregFrom" name="loginregFrom"  value="fill_info" />
                ...
                <label>帐号:</label>
						<input type="text" name="email" rule="blank#loginName" class="input-text" value="" id="email" tabindex="1" style="width:105px;"/>
					</div>
					<div class="login_textinput">
						<label>密码:</label>
						<input type="password" id="password" name="password" rule="blank" error="请输入密码" class="input-text" tabindex="2" style="width:105px;" />
					...
					<p class="logbtn">
						...
						<button type="submit" class="input-button low-padding" id="login" tabindex="4" />登录</button>&nbsp;&nbsp;<span class="dis-findpwd findpwd"><a href='${applicationScope.urlMain}/GetPasswd.do'>找回密码</a></span>
					</p>
				</form>
				</div>
         </div>
         <div class="kaixin_reg_info">
                ...
                <div class="top_info">
					<div class="low_message">
						<h2>完善个人信息，加入开心网</h2>
                        <img src="${applicationScope.urlStatic}/kaixin/imgpro/reg/add_message_low.png" />
                    	<p>由于你的个人信息不够完全，还不能直接登录，请完善下面个人信息后加入开心网:</p>
					</div>
                </div>
                
                <div class="kaixin_reg_infoclass" style="border-bottom:none">
                <form action="/fireg.do" class="reg_layout" id="regform" name="regForm" method="post">
				<input type="hidden" name="i" value="${requestScope.inviterId}" />
				<input type="hidden" name="e" value="${requestScope.inviteeEmail}" />
				<input type="hidden" name="fromUrl" id="fromUrl"  value="${requestScope.fromUrl}" />
				<input type="hidden" name="appId" id="appId" value="${requestScope.appId}" />
				<input type="hidden" name="inviterId" id="inviterId" value="${requestScope.inviterId}" />
				<input type="hidden" name="inviteType" id="inviteType" value="${requestScope.inviteType}" />
				<input type="hidden" name="code" id="code" value="${requestScope.code}" />
				<input type="hidden" name="from" id="from"  value="${requestScope.from}" />
				<input type="hidden" name="inviteeEmail" id="inviteeEmail" value="${requestScope.inviteeEmail}" />
				<input type="hidden" name="appToken" id="appToken" value="${requestScope.appToken}" />
				<input type="hidden" name="groupName" id="groupName" value="${requestScope.groupName}" />
				<input type="hidden" name="ss" id="ss" value="${requestScope.ss}"/>
				<input type="hidden" name="iid" id="iid"  value="${requestScope.iid}" />
				<input type="hidden" name="action_id" id="action_id" value="208401" />
				<input type="hidden" name="g" id="g" value="${requestScope.g}" />
				<input type="hidden" name="b" id="b" value="${requestScope.b}" />
				<input type="hidden" name="f" id="f" value="${requestScope.fpara}" />
						...
						<table>
							<tr>
								...
								<td class="inputholder">
									...
									<input type="text" name="regEmail" class="input-text" id="regEmail" tabindex="5" value="${requestScope.email}"/>
								</td>
								<td>
							...
							<tr>
								...
								<td class="inputholder">
									<span class="required">*</span>
									<input type="password" name="pwd" class="input-text" id="pwd" tabindex="6" value="${requestScope.password}"/>
								</td>
								<td>
							...
							<tr>
								...
								<td class="inputholder">
									<span class="required">*</span>
									<input type="password" name="repwd" class="input-text" id="repwd" tabindex="7" value="${requestScope.password}"/>
								</td>
								<td>
					...
					<fieldset>
						...
						<table>
							<tr>
								...
								<td class="inputholder">
									<span class="required">*</span>
									<input type="text" name="name" class="input-text" id="name" tabindex="8" />
								</td>
								<td>
							...
							<tr>
								...
								<td class="clearfix">
									<span class="required">*</span>
									<label for="male" class="reg_radio"><input type="radio" name="gender" value="男生"  tabindex="9"/>男</label>
									<label for="woman" class="reg_radio"><input type="radio" name="gender" value="女生"  tabindex="10"/>女</label>
								</td>
								<td>
							...
							<tr>
								...
								<td class="inputholder">
									<span class="required">*</span>
									<select name="birth_year" id="birth_year" style="width:55px;" tabindex="11">
									  <option value=""></option>
									...
									<select name="birth_month" id="birth_month" style="width:55px;" tabindex="12">
										<option value=""></option>
									...
									<select name="birth_day" id="birth_day" style="width:55px;" tabindex="13">
									    <option value=""></option>
										<option value="01">01</option>
										<option value="02">02</option>
										<option value="03">03</option>
							...
							<tr>
								...
								<td class="inputholder">
									...
									<select name="homeprovince"
										id="homeprovince" class="select" tabindex="14"
						...
						<table>
							<tr>
								...
								<td class="inputholder coder">
									<p class="float-left coder_input">
										<span class="required">*</span>
										<input type="text" name="icode" id="icode" size="5" maxlength="5" tabindex="21"/>&nbsp;&nbsp;
									</p>
									...
									%>
										...
										<a href="javascript:refreshCode();" onclick="javascript:void(0);">看不清楚 换一张</a>
									</p>
								</td>
								<td>
							...
							<tr>
								<td colspan="3" class="colspan" style="height:53px">
									...
									<button id="reg_sub" type="button" onclick="ismsnok();$('regform').submit();" style="cursor:pointer;float:left" class="notinv_login">注册帐号</button>
									<a id="reg_sub2" class="microReg" style="color: rgb(128, 128, 128);line-height:34px" onclick="$('msnValid2').value='1';check1.submit();return false;" href="#">使用微软认证</a>
								</td>
							</tr>
							<tr style="display:none">
							...
							<tr>
								<td colspan="3" class="colspan">
									<input type="checkbox" checked="checked" name="sharemsg" id="sharemsg" value="ok"/>
										你可以将这个消息分享给更多的msn好友<br/>点击以上按钮，我们将认可您已阅读并同意遵守 <a href="${applicationScope.urlMain}/info/agreement.jsp" target="_blank">服务条款</a>
									<br>并可能出现您的 Windows Live Id 在此自动登陆的情况。
									</td>
							</tr>
						</table>
					</fieldset>
					<input type="hidden" name="regFlag" id="regFlag" value="fillinfo_reg" />
				</form>
                </div>
            </div>
