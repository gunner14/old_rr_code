...
@LoginRequired
@Path( { "ajaxGuideGetFriendWithinTime" })
public class AjaxGuideGetFriendWithinTimeController {

	@Get
    @ServiceCutter(timeout = 700)
	public String index(Invocation inv, Portal portal, CacheManager cm)
		...
		try {
			...
			addWindow(portal, "window_guideisfriendwithtargetuser", "guideIsFriendWithTargetUser");
			addWindow(portal, "window_guideshowhtargetuserprofile", "guideShowhTargetUserProfile");		
			LogTimeUtils.logEnd(begin, "AjaxGuideGetFriendListWithinTime");			
		} catch (Exception e) {
			e.printStackTrace();
		}
		return "/views/guide/ajax/ajax_guide_get_friend_within_time.jsp";
	}
