...
<li id="feed${id[0]}">
	...
	<h3>
		...
		分享了 <a href="http://www.connect.renren.com/share/redirect.do?loc=1&app_id=${appId[0]}&type=image" target="_blank">${whereName[0]}</a>
		图片 <a href="${pUrl[0]}" target="_blank">${pName[0]}</a>
	</h3>
	<div id="moreThumbnails${id[0]}" class="content">
		<a href="${pUrl[0]}&ref=${vType}" target="_blank"><img width="200" lala="${pSrc[0]}" src="http://s.xnimg.cn/a.gif" onload="feed_img_delay_load(this);" class="photo"/></a>
		<#if comm?exists && comm[0] != ""><p class="comment"><q>${comm[0]}</q></p></#if>
	</div>
	<div class="details">
		<div class="legend">
			<img alt="图片" lala="${appIcon[0]}" src="http://s.xnimg.cn/a.gif" onload="feed_img_delay_load(this);" class="iPhoto"/>
			...
			<span class="seperator">|</span>
			<a href="http://www.connect.renren.com/share/redirect.do?loc=2&app_id=${appId[0]}&type=image" target="_blank">连接：${whereName[0]}</a>
			 <span class="seperator">|</span>
