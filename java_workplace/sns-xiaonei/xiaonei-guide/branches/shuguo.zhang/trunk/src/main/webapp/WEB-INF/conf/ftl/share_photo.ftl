...
<li id="feed${id[0]}">
	<#include "1,4">
	<h3>
		<a href="http://renren.com/profile.do?id=${fID[0]}&ref=share${vType}" target="_blank">${fName[0]}</a> 分享 <a target="_blank" href="http://renren.com/profile.do?id=${uID[0]}&ref=share${vType}">${uName[0]}</a> 的照片 <a href="${(aUrl[0])?replace('xiaonei.com','renren.com')}&ref=share${vType}" target="_blank">${aName[0]}</a>
	...
	<div class="content">
		<div class="figure"><a href="${(pUrl[0])?replace('xiaonei.com','renren.com')}&ref=share${vType}" target="_blank"><img class="photo" lala="${ (pSrc[0])!"about:blank"}" src="http://s.xnimg.cn/a.gif" onload="feed_img_delay_load(this);"/></a></div>
		<p>${title[0]}</p>
		<p>相册: <a href="${(aUrl[0])?replace('xiaonei.com','renren.com')}&ref=share${vType}" target="_blank">${aName[0]}</a></p>
