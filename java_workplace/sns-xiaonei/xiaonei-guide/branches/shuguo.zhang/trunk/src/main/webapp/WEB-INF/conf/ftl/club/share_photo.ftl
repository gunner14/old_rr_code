...
<li id="feed${id[0]}">
	...
	<h3>
		...
		分享 <a target="_blank" href="http://org.renren.com/${uID[0]}?ref=${vType}">${uName[0]}</a> 的照片 <a href="${aUrl[0]}&ref=${vType}" target="_blank">${aName[0]}</a>
	</h3>
	<div class="content">
		<div class="figure"><a href="${pUrl[0]}&ref=${vType}" target="_blank"><img class="photo" lala="${ (pSrc[0])!"about:blank"}" src="http://s.xnimg.cn/a.gif" onload="feed_img_delay_load(this);"/></a></div>
		<p>${title[0]}</p>
		<p>相册: <a href="${aUrl[0]}&ref=${vType}" target="_blank">${aName[0]}</a></p>
