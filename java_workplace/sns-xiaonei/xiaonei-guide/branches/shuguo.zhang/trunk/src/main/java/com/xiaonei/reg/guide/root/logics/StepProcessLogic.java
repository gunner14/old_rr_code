...
//		}
		...
		//2010-8-30 18:36:16 01新生引导
			...
			GuideLogger.printLog(" host:" + host.getId()+" -> OldMobile(AAO)");
	...
	public User getWillMergeToUser(User host){
		...
		if("nuomi.com".equals(thedomain)){
			...
			if(renren_account != null){
				need = true;
				rruser = SnsAdapterFactory.getUserAdapter().get(renren_account.getId());
				GuideLogger.printLog(" host:" + host.getId()+" need:"+need  +" account:"+renren_account.getId()+" rruser:"+(rruser!=null));
			}
		}
		else{ //not nuomi user
			GuideLogger.printLog(" host:" + host.getId()+" need:"+need +" not nuomi.com");
		}
	...
	public boolean isNeedMergeUser(User host) {
		...
		if("nuomi.com".equals(thedomain)){
			//用户使用b登陆人人，人人检测出是A类用户，进入人人引导流程，用户需要输入a的密码，或者通过邮箱找回方式，强制用户输入新密码，新密码生效后，b将不存在 wenzhe.dai
			Account renren_account = getNeedMergeAccount(host);
			if(renren_account != null){
				need = true;
				GuideLogger.printLog(" host:" + host.getId()+" need:"+need  +" account:"+renren_account.getId());
			}
		}
		else{ //not nuomi user
			GuideLogger.printLog(" host:" + host.getId()+" need:"+need +" not nuomi.com");
	...
	private Account getNeedMergeAccount(User host) {
		...
		final List<Account> accounts = PassportAdapterFactory.getWebLogin().get(host.getAccount(),Type.Account);
		if(accounts!=null && accounts.size()>=2){
			...
			for (Account account : accounts) {
				...
				if("kaixin.com".equals(curdomain)){
					...
					GuideLogger.printLog(" host:" + host.getId()+" scaned_account:"+good_account.getId() +" "+good_account.getDomain());
					break;
				}
			}
		...
		//GuideLogger.printLog(" host:" + host.getId()+" scaned_account:"+good_account.getId() +" done:"+good_account.getDomain());
		return good_account;
	}
		...
		if(isNeedMergeUser(host)){
			...
			switch(t){
				...
				case IGuideUserLeadType.OldMobile:
					...
					break;
				default:
					GuideLogger.printLog("spldefault host:" + host.getId(),GuideLogger.WARN);
					toUrl = LeadCoreLogic.getInstance().getRequestUrl(host, fromsource, step);
					break;
			}
			if(toUrl == null){
				GuideLogger.printLog(" host:" + host.getId()+" toUrl null",GuideLogger.WARN);
