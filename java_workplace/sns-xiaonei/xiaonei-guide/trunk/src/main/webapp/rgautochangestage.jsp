...
<%out.println("请输入想要修改Stage的用户的Id们【<font color=blue>是用户的数字Id，不是邮箱或账号</font>】。以换行分隔。" +" <br />"); %>
<%out.println("<font color=red>此举直接改变线上用户属性，操作请谨慎！" +"</font><br />"); %>
...
<%@page import="com.xiaonei.reg.guide.logic.FillInfoLogic"%><style>
    input {
...
%>
<%
String userstage = request.getParameter("userstage");
if(MyStringUtils.isNullOrEmpty(userstage)) {
	out.println("请选择");
	return;
}
Integer i_stage = (Integer)MyStringUtils.stringToObject(userstage,Integer.class);
for(String i : idlist){
    //out.println(i+" :<br />");
    Thread.sleep(10);
    log(ip + " process : [" + i+"]");
    ...
    if(nid != 0){
        try{
            ...
            boolean isadmin = true;//AdminService.getInstance().isAdmin(user.getId());
            if(isadmin){
            	int thestage = user.getStage();
            	
                if(thestage == i_stage){
                    out.println("<font color=blue>[WARN!]</font>"+user.getId()+":["+user.getName()+"] is stage="+i_stage+" <br />");
                }
                else{
                	FillInfoLogic.getInstance().saveStage(user, i_stage);
                    out.println("<font color=green>[DONE!]</font>"+user.getId()+":["+user.getName()+"] is now stage="+i_stage+". <br />");
                }
            }
            else{
