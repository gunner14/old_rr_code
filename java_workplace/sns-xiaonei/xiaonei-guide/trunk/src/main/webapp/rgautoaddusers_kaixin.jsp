...
<%@page import="com.kaixin.register.pojo.RegAccountPojo"%>
<%@page import="com.xiaonei.platform.core.model.User"%>
<%@page import="com.kaixin.register.logic.RegAccountLogic"%>
<%@page import="com.kaixin.register.logic.ActivateLogic"%>
<%@page import="com.kaixin.register.pojo.RegUserConfigPojo"%>
<%@page import="com.kaixin.register.logic.additionalbz.RegUserConfigLogic"%><style>
	input {
...
start = Integer.parseInt(strstart);
	for (int i = start; i < start+50; i++) {
		...
		try {
	        ...
	        RegUserConfigPojo uc = new RegUserConfigPojo(user,99);
	        RegUserConfigLogic.getInstance().configMajorUserInfo(uc);
	        System.out.println("user "+user.getId()+" uc ok!");
	        if(user.getId() %10 == 0){
	        	//out.println("renren_i" + i +" id is["+user.getId()+"] <br />");
	        } 
	        if(true){
	        	out.println("renren_i" + i +" id is["+user.getId()+"] "+name+" [kaixin]<br />");
	        }
			Thread.sleep(500);
		} catch (Throwable e) {
