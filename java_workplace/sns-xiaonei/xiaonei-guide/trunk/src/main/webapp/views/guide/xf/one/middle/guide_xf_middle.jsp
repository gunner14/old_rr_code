...
<div class="find-friend-box">
	...
	<c:if test="${!empty requestScope.companyAndSchool}">	
		<c:forEach varStatus="status" var="csItem" items="${requestScope.companyAndSchool}">
			<c:if test="${csItem.count>0}">				
				...
				<div id="showLessCon${status.index}" class="show-more" style="display:none">
					<c:if test="${csItem.type!='location'}">
						<a href=${applicationScope.urlBrowse}/searchEx.do?ref=guide_people_more_sr&amp;${csItem.moreLink}>
							查看全部来自${csItem.typeName}(共${csItem.count}人) &raquo;
						</a>
					</c:if>
					<%-- <a id="showLess${status.index}" class="toggle" href="#guids-uses-con${status.index}">收起</a>--%>
				</div>				
	...
	<c:if test="${fn:length(sameIPList)>0}">				
		...
		<div id="showLessCon99" class="show-more" style="display:none">
