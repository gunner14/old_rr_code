        ...
        <div class="kaixin_loginarea">
       		<div class="kaixin_login">
				<form method="post" id="loginForm" action="${applicationScope.urlPassport}/PLogin.do" >
					<input type="hidden" id="loginregFrom" name="loginregFrom"  value="msntab_registerbomb" />
            ...
            <div class="kaixin_reg_info">
				...
				<div class="top_info">
					<div class="message">
						...
						<div class="br">&nbsp;</div>
						<div class="btri">&nbsp;</div>
						<h2>完善你的个人资料，接受${requestScope.inviter.name}的好友请求</h2>
						<p>
						    <c:choose>
							<c:when test="${requestScope.gender == '男'}">
							${requestScope.inviter.name}请求加你为好友，去看看他在MSN上面刚领养的狗狗，请在下面简单完善你的个人信息，接受他的邀请。
							</c:when>
							<c:otherwise>
							${requestScope.inviter.name}请求加你为好友，去看看她在MSN上面刚领养的狗狗，请在下面简单完善你的个人信息，接受她的邀请。
							</c:otherwise>
							</c:choose>	
							<c>
						</p>
				...
				<input type="hidden" name="action_id" id="action_id" value="208106" />
				<input type="hidden" name="g" id="g" value="${requestScope.g}" />
				<input type="hidden" name="b" id="b" value="${requestScope.b}" />
				<%@ include file="inc/jsanti.inc" %>
					...
					<input type="hidden" name="regFlag" id="regFlag" value="invite_reg_bomb" />
				</form>
				<script>
						function displayDiv(){
