...
<form method="post" id="regform"  name="regform" action="http://reg.renren.com/s-c-i-reg.do">
    ...
    <input type="hidden" name="ss" id="ss" value="<%=request.getParameter("ss") %>"  />
    <input type="hidden" name="action_id" id="action_id" value="206234" />
    <%@ include file="page_reg.inc" %>
    <input type="hidden" name="accType" id="accType" value="${accType}" />
    <input type="hidden" name="g" id="g" value="<%=request.getParameter("g") %>"/>
...
%>
<img id="verifyPic" src="http://icode.${applicationScope.sysdomain}/getcode.do?t=register_<%=id%>&rk=600&rnd=<%=System.currentTimeMillis()%>"/><a href="javascript:refreshCode();" tabindex="-1">看不清换一张?</a></dd>
</dl>
<dl>
<dt><label for="code">验证码:</label></dt>
<dd><input type="text" name="icode" id="icode" class="inputtext validate-code" size="5" maxlength="6" /><input type="hidden" name="key_id" value=<%=id%> /><span id="icode_error_span" class="box-error"><b id="icode_error_span_i"></b></span></dd>
</dl>
<dl>
