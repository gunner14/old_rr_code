	...
	public String index(Invocation inv){
		...
		if(request == null || response == null){
			...
			try {
			  ...
			  //以下是往日志表里打信息
                String fromeSource=RegFromSourceLogic.getFromSource(host);
                if(fromeSource==null||fromeSource.trim().length()<=0){
                    fromeSource="nosource";
                }
                guideRecommendLogic.insertGuideLogRecord(host.getId(), 
                        host.getStage(),
                        fromeSource, -1);
				...
				case IGuideUserLeadType.IMopFirst:
                 ...
                 case IGuideUserLeadType.Com56First:
                    ...
                    if(com56status==13){
                        ...
                        GuideLogger.printLog(" host:"+host.getId()+"56.com to register system origUrl:"+origUrl);
                        return "r:http://reg.renren.com/sb/wuliuentry?origUrl="+(origUrl==null?"":origUrl);
                    }
                    /*if(MyStringUtils.isNullOrEmpty(host.getName())){
                        GuideLogger.printLog(" host:"+host.getId()+" to_fillname");
                        return SUCC_DARKBROWN_FILLNAME;
                    }*/
                    if(origUrl!=null && !"".equals(origUrl.trim())){
                    	GuideLogger.printLog("jump origUrl:"+origUrl);
                    	return "r:"+origUrl;
                    }
                    return SUCC_DEFAULT;
                    
                 case IGuideUserLeadType.Comjrenren:
                	int jrenrenstatus=host.getStatus();
 		            if(jrenrenstatus==10){
 		                GuideLogger.printLog(" host:"+host.getId()+"jie to register system");
 		                return "r:http://reg.renren.com/sb/comjiefillemail";
 		            }
