...
public class FillinfoSubmitZhongxueController {
	...
	public String post(Invocation inv,@Param("formZhongxue")FormZhongxue form){
		...
		if(request == null){
			...
			if(isdone){
				...
				try {
					GuideLogger.printLog(" host:"+host.getId()+"Schooltype int is "+fform.getSchooltype()+" "+fform.getHighschoolname()+" "+fform.getJuniorhighschoolname()+" "+fform.getElementaryschoolname()+" "+fform.getTechschoolname());
					GuideLogger.printLog(" host:"+host.getId()+"class "+fform.getHighschoolclass1());
					GuideLogger.printLog(" host:"+host.getId()+"info "+fform.getInfo());
					...
					GuideLogger.printLog(" host:"+host.getId()+"sb() error zhongxue "+e,GuideLogger.ERROR);
				}
				
				try {
					GuideLogger.printLog(" host:"+ host.getId()+"sb() - hostid:"+ host.getId()+ " - savestage - stage:"+Stage.STAGE_HEIGHSCHOOL);
					//阶段
...
//					A090729Util.asyncBeFansofPage(600002351, host);
					...
					GuideLogger.printLog(" host:"+ host.getId()+"sb() saveStage error "+host.getId()+" "+e.toString(),1);
					e.printStackTrace();
				}
