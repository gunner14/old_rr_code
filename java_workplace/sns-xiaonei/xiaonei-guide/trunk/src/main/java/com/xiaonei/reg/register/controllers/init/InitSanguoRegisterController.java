...
/**
 * <action path="/initsanguoreg"
 * type="com.xiaonei.reg.register.action.init.InitSanguoRegisterAction"
 * name="regForm" scope="request" > <forward name="success"
 * path="/pages/register/register_sanguo.jsp" /> </action> 普通注册的初始化 2008-9-24
 * 
 * @author wei.cheng@opi-corp.com
 */
@Path("initsanguoreg.do")
public class InitSanguoRegisterController extends BaseRegController {
	...
	public RegForward executeRt(RegMapping mapping, RegForm form,
		...
		if (regForm.getFrom() != null && regForm.getFrom().equals("reg_sanguo")) {
			actionStr = String.valueOf(RegOprDefine.SANGUO_VISIT_REG);
		}
	...
	protected Map<String, RegForward> setRegMapping(Map<String, RegForward> m) {
		// <forward name="success" path="/pages/register/register_sanguo.jsp" />
		m.put("success", new RegForward("/pages/register/register_sanguo.jsp"));
		return m;
	}
