...
public class GuideUtil {
	...
	/**
	 * 
	 * TODO       根据socialgraph推荐的好友列表构造前端需要的json串
	 *	
	 * @author	Kobin (bin.huang1@renren-inc.com)
	 * @since		2012-7-11 下午6:40:16
	 * @param    offerList
	 * @return
	 */
	public static String getJsonForOfferFriend(List<Offer> offerList) {
		try {
			if(offerList == null){
				return "";
			}
			
			List<Integer> userIdList = getOfferFriendIdListOffer(offerList);
			Map<Integer, WUserCache> friendUserCaches = SnsAdapterFactory.getUserCacheAdapter().getUserCacheMap(userIdList);
			
			JSONObject jo = new JSONObject();
			JSONArray retCandidate = new JSONArray();
			for (Offer offer :offerList) {
				WUserCache userCache = friendUserCaches.get(offer.getUserId());
				if(userCache != null){
					if( userCache.getStatus() == 0){ 
						JSONObject joinner = new JSONObject();
						joinner.put("id", userCache.getId());
	...
	/**
	 * 
	 * TODO       根据用户填写资料获取socialgraph最终推荐的好友列表
	 *	
	 * @author	Kobin (bin.huang1@renren-inc.com)
	 * @since		2012-7-18 上午10:29:31
	 * @param    userInfoMap
	 * @param    limit
	 * @return
	 */
	public static  List<Offer> getOfferFriendFinalRecommendByCluster(Map<String, String>userInfoMap, int limit){
		List<Offer> offerList = new ArrayList<Offer>();
		try {
			SortByClusterAdapter adapter = SortByClusterAdapter.getInstance();
			if(adapter != null){
			    offerList = adapter.getFinalRecommendByCluster(userInfoMap, limit);
			}
		} catch (Exception e) {
			GuideLogger.printLog(" userInfoMap:"+userInfoMap.get("p")+" e:"+e.toString());
			return null;
		}
		return offerList;
	}
	...
	/**
	 * 
	 * TODO       根据用户引导阶段获取用户推荐好友列表
	 *	
	 * @author	Kobin (bin.huang1@renren-inc.com)
	 * @since		2012-7-11 下午5:15:04
	 * @param    host
	 * @return
	 */
	public static  List<DefaultOffer> getOfferFriendListByStage(User host){
		if(host == null){
			return null;
		}
		List<DefaultOffer> offerList = new ArrayList<DefaultOffer>();
		try {
			OfferFriendsAdapter<DefaultOffer> adapter = OfferFriendsAdapter.getInstance();
			if(adapter != null){
			    adapter.rebuild(host.getId());   
			    offerList = adapter.getBestOfferByStage(host.getStage(), host.getId(), 50);
			}
		} catch (Exception e) {
	...
	/**
	 * 
	 * TODO       根据推荐好友列表生成推荐好友map
	 *	
	 * @author	Kobin (bin.huang1@renren-inc.com)
	 * @since		2012-7-11 下午5:19:28
	 * @param    offerList
	 * @return
	 */
	public static Map<Integer, DefaultOffer> getOfferFriendMap(List<DefaultOffer> offerList){
		if(offerList == null){
			return null;
		}
		
		Map<Integer, DefaultOffer> offerFriendsMap = new HashMap<Integer, DefaultOffer>();
		
        for (int i = 0; i < offerList.size(); i++) {
        	DefaultOffer  offer = offerList.get(i);
        	offerFriendsMap.put(offer.getUserId(), offer);
        }
		
		return offerFriendsMap;
	}
	
	/**
	 * 
	 * TODO       根据推荐好友列表生成推荐好友idList
	 *	
	 * @author	Kobin (bin.huang1@renren-inc.com)
	 * @since		2012-7-11 下午5:22:26
	 * @param    offerList
	 * @return
	 */
	public static List<Integer> getOfferFriendIdList( List<DefaultOffer> offerList){
		if(offerList == null){
			return null;
		}
		List<Integer> offerFriendIdList = new ArrayList<Integer>();
		for (int i = 0; i < offerList.size(); i++) {
			DefaultOffer  offer = offerList.get(i);
			offerFriendIdList.add(offer.getUserId());
		}
		return offerFriendIdList;
	}
	 ...
	 * @return
	 */
	public static List<Integer> getOfferFriendIdListOffer( List<Offer> offerList){
		if(offerList == null){
			return null;
		}
		List<Integer> offerFriendIdList = new ArrayList<Integer>();
