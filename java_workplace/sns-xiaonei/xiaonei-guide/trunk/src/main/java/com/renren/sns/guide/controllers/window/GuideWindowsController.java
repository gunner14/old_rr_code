...
import com.renren.offerfriends.DefaultOffer;
import com.renren.offerfriends.OfferFriendsAdapter;
...
import com.renren.sns.guide.service.GuideMenuLogic;
...
import com.xiaonei.platform.component.application.home.MenuQualityRecommendHome;
...
import com.xiaonei.reg.guide.util.GuideLoggerCountFeed;
...
import com.xiaonei.xce.feed.FeedViewHaAdapter;
...
//        FeedFactory factory = FeedFactory.getInstance();
        try {
        	List<String> lists = FeedViewHaAdapter.getInstance().getGuideFeed(host.getId(), host.getName(), 0, 0, IFeedRetriever.HOME_LIMIT);
        	if(lists != null && lists.size() > 0) {
        		String feedString = combineString(lists);
        		int feedCount = lists.size();
        		
        		inv.addModel("userFeed", feedString);
                inv.addModel("feedStart", "1");
                inv.addModel("feedEnd", Integer.valueOf(feedCount));
                guideRecommendLogic.insertGuideUserFeedCount(host.getId(), feedCount, "guide_home");
        	} else {
        		guideRecommendLogic.insertGuideUserFeedCount(host.getId(), 0, "guide_home");
        	}
...
//            }
     ...
     * @return 返回拼接后的字符串
     */
    public String combineString(List<String> listStr) {
		StringBuffer combineStr = new StringBuffer();
		if (listStr == null || listStr.size() == 0) {
			return "";
		} else {
			for (String str : listStr) {
				combineStr.append(str);
			}
		}
    ...
    public void guideGetFeedView(Invocation inv, Window window, CacheManager cm) {
        ...
        try {
            ...
            try {
                ...
                if (value == null) {  
                    // 没有cookie信息
                	list = FeedViewHaAdapter.getInstance().getGuideFeed(host.getId(), host.getName(), 0, 0,  IFeedRetriever.HOME_LIMIT);
...
//              list = FeedViewAdapter.getInstance().getHot(host.getId(), host.getName(), 0, 0, IFeedRetriever.HOME_LIMIT);
    ...
    public void guideRecentMenuHome(Invocation inv, Window window, CacheManager cm) {
    	int userId = UserProvider.getInstance().get(cm).getId(); 
    	inv.addModel("recentMenuHome", GuideMenuLogic.getInstance().getMenuRecommendedApp(userId, 
        ...
        inv.addModel("recent_online_app_count", MenuQualityRecommendHome.getInstance().getQualityCount());
