...
<li id="feed${id[0]}">
	<#include "1,4">
	<h3>
		<a href="http://renren.com/profile.do?id=${fID[0]}&ref=share${vType}" target="_blank">${fName[0]}</a> 分享 <a target="_blank" href="http://renren.com/profile.do?id=${uID[0]}&ref=share${vType}">${uName[0]}</a> 的日志 <a href="${(url[0])?replace('xiaonei.com','renren.com')}&ref=share${vType}" target="_blank">${title[0]}</a>
