...
try{
	...
	type_o = request.getAttribute("user_type_string");
	StatLogger.info(host.getId(), ""+"type"+":"+type_o+"",StatLogger.S_ARV_ENTERINTOO_PO_,""+type_o);
}
catch(Throwable e){
...
<head><!-- _pv_darkcoral_ -->
	...
	<title>${domainIdentified["siteName_fillinfo"]} - 上传头像</title>
	<style type="text/css">
		...
		#submit-btn{background:url(${applicationScope.urlStatic}/imgpro/reg/new/add-done-btn.png) no-repeat;width:178px;height:36px;border:none;text-indent:-9999em}
		...
		.upload-bg {position:absolute;background-color:none;}
	</style>
</head>
...
<script type="text/javascript" src="${applicationScope.urlSmallStatic}/jspro/guide/xn.page.guide.common.temp-v1.1.js?ver=$revxxx$" ></script>
<script type="text/javascript" src="${applicationScope.urlSmallStatic}/jspro/guide/xn.guide.newModify.temp-v1.1.js?ver=$revxxx$" ></script>
<script type="text/javascript" >
<%
%>
</script>
    ...
    var mm = ${requestScope.lead_more_friend_confirmlist} || {};
    var wpiMenuInfo = {user:{id:81120}};
        XN.dom.ready(function(){
        	var hasAddFriendList = ${requestScope.lead_more_friend_confirmlist} || {};
			var fl = [],rl = [],cl = [],nl = [];
			fl = hasAddFriendList.candidate || [];
			uploadPic(fl);
			XN.event.addEvent('tasteClosed', 'click', function(){
				sendGuideTrace('ga2'); //
			});
			XN.event.addEvent('uploadBtn', 'click', function(){
				sendGuideTrace('ga1'); //
			});
		});
   </script>
...
//System.out.println("po_aaq:"+rand);
if(type_o!=null && "AAQ".equals((String)type_o) && rand>15){
%>
	<img src="http://count.chanet.com.cn/action.cgi?t=5826&i=${host.id}" width=1 height=1>
<%
}
%>
</body>
