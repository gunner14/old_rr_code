...
<div id="content">
    ...
    <c:choose>
        <c:when test="${requestScope.lead_user_type == user_type_brown0}">
            <div class="nav-register third clearfix">
                <p>1.寻找朋友</p>
                <p>2.完善资料，找到熟人</p>
                <p class="online">3.上传头像，进入首页</p>
            </div>
        </c:when>
        <c:otherwise>
            <div class="nav-register third clearfix">
    ...
    <div class="user-data">
        <div class="title">
            <h1>使用真实头像，朋友能够认出你，还能享受星级用户特权！</h1>
        </div><!--title end-->
...
rvice.fcgi?pagetype=addHeadProfile">
        ...
        <!-- <input type="button" value="保存头像-new" class="input-submit" />　<a href="#" class="skip">跳过-new</a> -->
            <c:choose>
                 <c:when test="${not empty hasHead}">
                     <input id="saveHead" type="button" value="保存头像，进入首页" class="input-submit" onclick="saveImage();"/>
                     <!-- <a href="#nogo" id="jumpHead" onclick="jump();" style="display:none">跳过</a> -->
                     <a href="portraits" id="jumpHead" style="display:none">跳过</a>
                 </c:when>
                 <c:otherwise>
                     <input id="saveHead" type="button" value="保存头像，进入首页" class="input-submit" onclick="saveImage();" style="display:none"/>　
                     <!-- <a href="#nogo" id="jumpHead" onclick="jump();" style="float:right;padding-right:12px;">跳过</a> -->
                     <a href="portraits" id="jumpHead" style="float:right;padding-right:12px;">跳过，进入首页</a>
                 </c:otherwise>
            </c:choose>
        </div>
        </div>
...
<img width="0" height="0" src="http://guide.${domainIdentified["sys.domain"]}/xtrs?nur=rggp3v3a0<%= (new java.util.Random()).nextFloat()%>"/>
<script src="${applicationScope.urlSmallStatic}/utm/urchin.js" type="text/javascript">

</script> 
<script type="text/javascript">var page_id = "p41";</script>
