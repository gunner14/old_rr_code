...
public class PreviewInitAddFriendCombineController {
	@Autowired
    ...
    private static final String INIT_LEAD_AAF = "/views/guide/flowxfivedarkcoral/preview_darkcoral_statis.jsp"; //口香糖
            ...
            if(MyStringUtils.equals(skip, "1")){
                ...
                if(requestlist.size() >= 1 && !"true".equals(inv.getParameter("stay"))){ //加过好友的
                   ...
                   // StatLogger.info(host.getId(), ""+"friends:"+friendlist.size()+" requests:"+requestlist.size(),StatLogger.S_ACT_ADDEDFRIEN_FI_,user_type);
                    ...
                    if(ids==null || size<50){ //再推可能认识的人
                        
                        tempIds = getOfferFriendList(inv,host);                 
                        if(tempIds.size()>=50-size){
                            for(int i=0;i<50-size;i++){
                                ids.add(tempIds.get(i));
                            }
                        }
                        else{
                    ...
                    ret=INIT_LEAD_AAF;
                    GuideLogger.printLog("*************************************** ids:"+ids.size()+
                            " friendlist"+friendlist.size()+" stay:"+inv.getParameter("stay")+" requestlist:"+requestlist.size()+" tempIds:"+tempIds);
                }
                else if(ids.size() > 0){ 
                    ret = INIT_LEAD_AAF;
                }
                    ...
                    ids = getOfferFriendList(inv,host);                 
                    cost.endStep();
                ...
                GuideStatLogUtil.log("User:" + host.getId() + "|Action:recommonedFriend|count:" + ids.size() + "|real_count:" + cmap.size() + "|stage:" + host.getStage() + "|from_flag:" + friend_srouce_flag);
    ...
    private int proccessCacheMap(User host, Invocation inv, Map<Integer, WUserCache> cmap, String logparam) {
    ...
    private int processMapWithoutActSort(Invocation inv, Map<Integer, WUserCache> cmap, String logparam, int hostid) {
        String resultstring = "";
    ...
    private int processMapWithActSort(Invocation inv,
            Map<Integer, WUserCache> cmap, String logparam, int hostid) {
        String resultstring = "";
        JSONObject jo = new JSONObject();
        JSONArray retCandidate = new JSONArray();
