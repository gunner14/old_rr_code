...
public class GuideXFiveFriendNewsLogic {
    static class SingletonHolder {
        static GuideXFiveFriendNewsLogic instance = new GuideXFiveFriendNewsLogic();
    }
    public static GuideXFiveFriendNewsLogic getInstance() {
    ...
    public void tellAll(final User host){
		...
		if ( work == -1) {
			...
			sum += work; 
			GuideLogger.writeLine(this, host.getId()+": tellAll() - work:"+work+" - host:" + host.getId());
		}
		cost.endFinally();
		GuideLogger.writeLine(this,host.getId()+": tellAll() - end - host:"+host.getId()+" [inshion]for_stat:"+sum);
    }
    
    private int tell(User host,SearchResult result,String comment,String network,String url){
    	MyTimeCost cost = MyTimeCost.begin();
    	IGuideRecommend publish = GuideRecommend.getInstance();
		int c = 0;
		List<Integer> idlist = new ArrayList<Integer>();
		/*idlist.add(81120);
		idlist.add(223500512);*/
...
//		model.setUrl("url");// 点我认识TA要去的链接
    ...
    private void seeResult(User host,SearchResult result) {
    	/*
    	int c = 0;
    	GuideLogger.printLog(" Result Count:"+result.getTotalCount());
		for(SearchView sv : result.getContents()){
			c++;
			if(c>=30) break;
			User user = SnsAdapterFactory.getUserAdapter().get(sv.getId());
			//GuideLogger.writeLine(this,"Result "+c+" :"+user.getId()+user.getName()+user.getAccount());
			GuideLogger.printLog(" host"+host.getId()+" ->"+c+":"+user.getId()+user.getName()+user.getAccount());
		}
		*/
	}
				...
				try {
					...
					if(adapter == null){
						...
						if(result != null){
							...
							ret = tell(host,result,comment, network, url);
							//ret = result.getTotalCount();
							seeResult(host,result);
						}
					}
				} catch (Exception e) {
