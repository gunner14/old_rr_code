...
public class RecommendController {
	...
	public Object index(@Param("uid")int userId	,@Param("offset")int offset, @Param("len")int len, @Param("from")int[] froms, @Param("type")int[] types){
		try {
			...
			List<String> contentIdList = recommendServiceX.getRecomendContentsX(userId, offset, len, typeList, fromList, null);
			logger.debug("fromList:"+fromList+"\ttypeList:"+typeList+"\tcontentIdList:"+contentIdList);
			...
			logger.error("get recommend contents error", e);
			return new XoaBizErrorBean("get recommend contents error");
		}
	}
