...
public class InviteDAO extends XnDAO {/*
    *//**
     * 这个是邀请DAO，与之相关联的表是invite_list
     * 字段及其含义：
     * 1.owner_id:发送邀请的人的人人网ID
     * 2.owner_name:发送邀请的人的名称
     * 3.account:被邀请的人的帐户（Email）
     * 4.flag:邀请的类型
     * <p>
     * 0：仅保存的邀请链接
     * 1：已经成功发送的邀请链接
     * 2：成功发送并且已经点击邀请链接返回的邀请链接
     * 3: 用户在页面将邀请记录设置为不可见以后将不在页面上显示//add by 康伟
     * </p>
     * 5.tinyurl：
     * 6.from_account:发送邀请的人的帐户（目前是Email帐户）
     * 7.headurl：头像连接
     * 8.action_type  具体参照 com.dodoyo.invite.logic.importer.ImportStrategyFactory
     *//*
//    private static InviteDAO instance = new InviteDAO();

    private static InviteDAO getInstance() {
        if (instance == null) {
    ...
    *//**
    ...
    public int save(final int owner_id, final String owner_name, final String account, final int flag,
                    final String tinyurl, final String from_account, final String headurl) throws SQLException {
    ...
    public int saveWithActionType(final int owner_id, final String owner_name, final String account, final int flag,
                                  final String tinyurl, final String from_account, final String headurl, final int action_type, final int appId, final String nickname) throws SQLException {
        OpUpdate op = new OpUpdate(sqlInsertWithActionType, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
                ps.setInt(1, owner_id);
                ps.setString(2, owner_name);
    ...
    public int UpdateInviteStatusWithNickname(final int id, final int status, final String nickname) throws SQLException {

        OpUpdate op = new OpUpdate(sqlUpdateStatusWithNickName, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
                ps.setInt(1, status);
    ...
    public int UpdateInviteStatus(final int id, final int status) throws SQLException {
        OpUpdate op = new OpUpdate(sqlUpdateStatus, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
    ...
    public int updateInviteStatusForUserId(final int id, final int invitee_user_id, final int status) throws SQLException {
        OpUpdate op = new OpUpdate(sqlUpdateStatusForUserId, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
    ...
    *//**
     ...
     * @throws SQLException
     *//*
    public int UpdateInviteStatus(final int id, final int hostid, final int status) throws SQLException {
        OpUpdate op = new OpUpdate(sqlUpdateStatus, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
    ...
    *//**
    ...
    public InviteInfo checkSelect(final int owner_id, final String account) throws SQLException {
        OpUniq op = new OpUniq(sqlCheckSelect, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
                ps.setInt(1, owner_id);
    ...
    public InviteInfo getInviteInfo(final int inviteId, final String invitedEmail) throws SQLException {
        OpUniq op = new OpUniq(sqlGetOneNewlyInviteInfo, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
    ...
    *//**
    ...
    public List<InviteInfo> getInviteInfoByOwnerId(final int owner_id, final int flag, final int offset, final int limit) throws SQLException {
        OpList op = new OpList(sqlGetInviteInfoByOwnerId, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
                ps.setInt(1, owner_id);
                ps.setInt(2, flag);
    ...
    public InviteInfo getInviteInfoById(final int id) throws SQLException {
        OpUniq op = new OpUniq(sqlGetInviteInfoById, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
                ps.setInt(1, id);
            }
    ...
    public int getInviteCount(final String email) throws SQLException {
        OpUniq op = new OpUniq(sqlGetInviteCount, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
    ...
    *//**
    ...
    @SuppressWarnings("unchecked")
    public List<InviteInfo> getInviteUsers(final String email) throws SQLException {

        OpList op = new OpList(sqlGetInviteUserByEmail, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
                ps.setString(1, email);
            }
        ...
        return (ArrayList<InviteInfo>) DataAccessMgr.getInstance().queryList(op);
    }

    private final static String sqlGetNotInviteUserByEmail = "SELECT * FROM `invite_list` WHERE `account`=? AND `flag`=0";

    @SuppressWarnings("unchecked")
    public List<InviteInfo> getNotInviteUsers(final String email) throws SQLException {

        OpList op = new OpList(sqlGetNotInviteUserByEmail, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
                ps.setString(1, email);
            }
    ...
    public List<InviteInfo> getMSNFriends(final int userId) throws SQLException {
        ...
        OpList op = new OpList(sqlGetMSNFriendsByEmail, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
                ps.setInt(1, userId);
            }
    ...
    *//**
     * 下面的描述将以"寻找好友"这个名称统称"寻找好友"以及"邀请好友"两个功能，就是说，当提及"寻找好友"的时候，其实是在说这两个功能.因为它们只是在概念上有区别
     * <p/>
    ...
    @SuppressWarnings("unchecked")
    public List<InviteInfo> invitedGetInvitoer(final String email) throws SQLException {

        OpList op = new OpList(sqlInvitedUserGetInvitoer, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
                ps.setString(1, email);
            }
    ...
    //private final static String sqlInviteduserGetInvitoer = "select * from invite_list where account = ? and flag = ?";
    *//**
     * 根据邀请标志位返回邀请者信息
     *
     * @param email 邀请者的email
     * @param flag  标志位 @see invite_list 表结构中flag意义
     * @return InviteInfo对象,发信者的信息
     * @throws SQLException 查询异常
     *//*
    @SuppressWarnings("unchecked")
    public List<InviteInfo> invitedGetInvitoer(final String email, final int flag) throws SQLException {
    ...
    *//**
     ...
     *//*
    private static final String compute_count = "select count(1) from invite_list_2 where flag = 1";

    public int getCount() throws SQLException {
        Connection conn = null;
        conn = getConn(false);
        PreparedStatement ps = conn.prepareStatement(compute_count);
    ...
    *//**
     ...
     * @throws SQLException
     *//*
    public int saveFriendGroup(final int hostId, final int groupid, final String friendMail, final String groupname) throws SQLException {
        OpUpdate op = new OpUpdate(INSERT_INTO_FRIEND_GROUP, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
                ps.setInt(1, hostId);
                ps.setInt(2, groupid);
    ...
    *//**
    ...
    public List<InviteInfo> getMail() throws SQLException {
        OpList op = new OpList(sql_1, InviteDatasourceMgr.getInstance().getBizStat(this.getClass())) {
            @Override
            public void setParam(PreparedStatement ps) throws SQLException {
            }
        ...
        System.out.println("------------------------>批量选取成功");
        return (ArrayList<InviteInfo>) DataAccessMgr.getInstance().queryList(op);
    }

    private static final String sql_2 = "update invite_list_2 set flag = 2 limit 1000";
    ...
    *//**
    ...
    public int[] update() throws SQLException {
        Statement stat = null;
        stat = getConn(true).createStatement();
        stat.addBatch(sql_2);
        System.out.println("------------------------>批量更新成功");
        return stat.executeBatch();
    }

    private static final String invite_time = "select invite_time from invite_list_2 where owner_id = ?";
    ...
    *//**
     ...
     * @throws SQLException
     *//*
    public Date getInvite_time(int id) throws SQLException {
        Connection conn = getConn(false);
        PreparedStatement ps = conn.prepareStatement(invite_time);
        ResultSet rs = ps.executeQuery();
        return rs.getDate("invite_time");
    }
