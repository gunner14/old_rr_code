...
//    private static InviteMailLogic iml = new InviteMailLogic();
    ...
    public InviteMailLogic() {
    }

   /* private static InviteMailLogic getInstance() {
        return iml;
    }*/
...
//        boolean isIpValid = InviteParamsUtils.getInstance().checkInviteParams(ip);
        boolean isIpValid = inviteParamsUtils.checkInviteParams(ip);
        if (!isIpValid)
            return "";

        //update InviteParams by template content
        templateUtils.updateInviteParamsByTemplateId(ip);
...
//                MailUtils.getInstance().sendMail(email);
                LogUtils.logInfo("___email_to_be_sent from:" + email.getEmailFrom() + " to:" + email.getEmailTo() + " disc:" + email.getDisc());
                mailUtils.sendMail(email);

                //add invite mail statistic here
//                StatUtils.getInstance().recordInviteMail(ip, email);
                statUtils.recordInviteMail(ip, email);
            }
        };
...
//		InviteMailLogic.getInstance().sendInviteMail(ip);
        ...
        //String url = inviteFacade.getURLForLinkInvite(400000458, "123324", "grouptest", "", "");
