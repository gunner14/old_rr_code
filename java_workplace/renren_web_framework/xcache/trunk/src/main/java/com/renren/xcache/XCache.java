...
public abstract class XCache<T> extends XCacheBase {
    ...
    /**
	 * Retrieve multiple objects from the memcache. And return as {@code T[]}.
	 * 
	 * This is recommended over repeated calls to {@link #get(String) get()},
	 * since it<br/>
	 * is more efficient.<br/>
	 ...
	 * @param ids
	 *            String array of ids to retrieve
	 * @return a array of type T with entries for each id is found by the server, 
	 * for each ids[i], it's value is ret[i], while ret is the returned variable 
	 * for example. If ids[i] not found in cache, ret[i] will be null.
	 */
    public abstract T[] getMultiAsArray(String[] ids);
    
    public abstract StatsManager getStatsManager();
