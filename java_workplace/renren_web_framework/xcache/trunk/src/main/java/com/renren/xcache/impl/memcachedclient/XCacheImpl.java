...
public class XCacheImpl<T> extends XCache<T> {
    ...
    public boolean add(String id, T value, int expire) {
        ...
        try {
			...
			throw new RuntimeException("Error while operating on key-prefix: " + keyPrefix, e);
		}
    }
	...
	public T[] getMultiAsArray(String[] ids) {
		Map<String, T> map = getMulti(ids);
		T[] ret = (T[])Array.newInstance(this.valueClass, ids.length);
		for (int i = 0; i < ids.length; i++) {
			T value = map.get(ids[i]);
			ret[i] = value;
		}
		return (T[])ret;
	}
     ...
     * @return
     */
    private Date calcExpireDate(long expire) {
        ...
        return new Date(expire * 1000);	//这里的expire必须要是long型的，否则乘以1000后可能会越界
    }
	...
	public StatsManager getStatsManager() {
		return new StatsManagerImpl(this);
	}
}
