...
public interface TagDAO {
    ...
    @SQL("SELECT id, namespace, value, tag_count FROM tag WHERE namespace = :1 AND value = :2")
    public Tag getByValue(String namespace, String value);

    @SQL("SELECT id, namespace, value, tag_count FROM tag WHERE id = :1")
    public Tag getById(int id);
    
    @SQL("UPDATE tag SET tag_count = tag_count + :2 WHERE id = :1")
    public void incTagCount(int id, int inc);
