...
public interface EntryService {
    ...
    /**
     * 保存一个Entry信息，如果由entryId代表的Entry已经存在了，则完全更新其 所有的Field（等价于先删除后新增）
     * 
     * @param entryId Entry的gid
     * @param fields Entry的字段
     ...
     * @param cursor 如果按照时间倒序来排列检索结果，那么查询是cursor表示所要检索的最大的sequence id，
    ...
    @XoaGet("/entry/find")
    public ServiceFuture<EntryPage> find(@XoaParam("query") String query,
            @XoaParam("forward") boolean forward, @XoaParam("cursor") long cursor,
            @XoaParam("limit") int limit);
     ...
     * @param gtype 指定获取哪个 @ GType} 的entry id，具体的常量定义请参考 @ GType} 类
     */
