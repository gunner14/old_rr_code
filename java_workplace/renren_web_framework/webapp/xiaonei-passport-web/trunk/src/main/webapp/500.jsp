...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body id="error500Page" class="error">
		<script>
			window.onload=function(){
				(new Image()).src="http://log.xiaonei.com/rl.do?f=500&m=" + document.getElementById('mach').value + "&ref=" + document.getElementById('ref').value + "&t=" + Math.random();
			}
		</script>
		<div id="container">
			<%@ include file="/inc/header-noad.inc" %>
			<div id="opi">
				<div class="http500 error msg">
					<h2>500</h2>
					<p>抱歉，内部错误。<a href="${applicationScope.urlWww}">返回首页</a>或<a href="${applicationScope.urlSupport}/GetGuestbookList.do">报告给管理员</a></p>
				</div>
...
<%
try{
org.apache.log4j.Logger logger = org.apache.log4j.Logger.getLogger("xnlog.500");
java.lang.Throwable t = (java.lang.Throwable)request.getAttribute("javax.servlet.error.exception");
String p = "host:"+application.getAttribute("hostName")+" - ref:" + request.getHeader("Referer")+" - errUri:http://"+request.getServerName()+(request.getAttribute("javax.servlet.error.request_uri")==null?request.getRequestURI():request.getAttribute("javax.servlet.error.request_uri"))+(request.getQueryString()==null?"":("?"+request.getQueryString())) + "- UserAgent:" + request.getHeader("User-Agent");
logger.error(p,t);
