...
<head>
<link href='${applicationScope.urlSmallStatic}/n/apps/login/compatible/login-widget-all-min.css?ver=$revxxx$' rel="stylesheet" type="text/css" media="all"/>
<c:choose>
	<c:when test="${ callBackCssUrl != null }"><link href='${callBackCssUrl}' rel="stylesheet" type="text/css" media="all"/></c:when>
</c:choose>
<script src="${applicationScope.urlSmallStatic}/n/core/base-all.js?ver=$revxxx$" type="text/javascript"></script>
<script src="${applicationScope.urlSmallStatic}/n/apps/login/login-widget.js?ver=$revxxx$" type="text/javascript" ></script>
<c:choose>
	<c:when test="${ callBackJsUrl != null }"><script src="${callBackJsUrl}" type="text/javascript" ></script></c:when>
</c:choose>
<script type="text/javascript">XN.config.parentDomain = true;</script>
</head>
<body>
...
<input type="hidden" id="useHttps" name="useHttps" value="${useHttps}" />	
...
<form method="post" id="loginForm" class="login-form" action="login-iframe.jsp">
	...
	<dl class="bottom">
		...
		<input type="hidden" name="captchaType" id="captcha_type" value="login" />
		<input type="hidden" name="loginRegistrationId" id="loginRegistrationId" value="${loginRegistrationId}" />
		<input type="submit" id="login" class="input-submit login-btn" value="登录人人网" tabindex="5"/>
	</dl>
	<div class="registDiv">还没有人人账号？<a target="_blank" href="http://reg.renren.com/xn6245.do?ss=10113&rt=27">立即注册</a></div>
</form>
