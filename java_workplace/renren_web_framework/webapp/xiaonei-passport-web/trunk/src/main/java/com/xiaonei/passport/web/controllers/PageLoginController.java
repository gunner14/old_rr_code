...
public class PageLoginController {
	 ...
	 * 这个一个get,做什么用的呢?<br>
	 * 换票,主要工作
	 * 这里主要是为了针对page.renren.com下的所有页面的登陆
	 * 小站的入口也是走的这里
	 * @param inv
	 * @param domain
	 * @param origURL
	 ...
	 */
	@RequestMapping(value = {"/PLogin.do"}, method=RequestMethod.GET)
	public String get(@RequestParam(value = "domain", required = false)String domain,
		...
		try {
			...
			if (LoginStatusCode.LOGIN_OK != code) {
				String redirectUrl=EventProcess.getInstance().fireErrorLoginEvent(request, response, pForm, code.getCode(), LoginResult.getDefualLoginResultWrapper(),LoginConfigureKey.GET_PAGE_LOGIN_FAIL_LOGIN_EVENT);
			...
			if(logger.isWarnEnabled()) {
				logger.warn("Before PLogin.do transfer method exception " + e);
			}
		}
		...
		if (result==null||LoginStatusCode.LOGIN_OK.getCode()!= result.getLoginStatusCode()) {
			...
			String redirectUrl= EventProcess.getInstance().fireErrorLoginEvent(request, response, pForm, LoginStatusCode.FAILNOPOST.getCode(), null, LoginConfigureKey.GET_PAGE_LOGIN_FAIL_LOGIN_EVENT);
			return  UrlReturnUtil.redirectUrl(response,redirectUrl);
		}
	 ...
	 * post<br>
	 * 负责登录的方法
	 * 负责处理www.renren.com/PLogin.do的POST方法
	 * 
	 * @param inv
	 * @param pForm
	 * @return
	 * 
	 * @author zhe.long      2012.7.30
	 */
	@RequestMapping(value = { "/PLogin.do"}, method=RequestMethod.POST)
	public String post(@RequestParam(value = "domain", required = false)String domain,
		...
		//String domain = LoginConfigureImpl.getLocalInstance().getUniqLoginConfigure(pForm.getLoginRegistrationId(), LoginConfigureKey.DOMAIN);
		//pForm.setDomain(domain);
		LoginStatusCode code = EventProcess.getInstance().fireBeforeLoginEvent(request, response, pForm, LoginConfigureKey.GET_PAGE_LOGIN_BEFORE_LOGIN_EVENT.getKey());
		if (LoginStatusCode.LOGIN_OK != code) {		    
