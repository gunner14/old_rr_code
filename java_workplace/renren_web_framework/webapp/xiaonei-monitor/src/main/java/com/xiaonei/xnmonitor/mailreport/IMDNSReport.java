...
public class IMDNSReport {
	...
	public String getMessage() {
		...
		// String sql =
		// "select distinct url,type,avg(cost_time) as average_cost ,count(id) as total ,"
		// +
		...
		String sql = "select remote_host,url,ip,cost_time,type from im "
				+ "where type='dns' and happen_time between ? and ? and  "
				+ "remote_host !='NULL' order by cost_time desc limit 100";
		List<Object[]> result = Ar.sql(sql, begin, end);
		List<IMInfo> remoteResult = IMDNSPackage
				.getIMRemoteDNSReportList(result);

		msgBuilder.append("<html><head>IM DNS解析最长耗费时间TOP50 " + begin + "~"
				+ end + "</head><body>");
		msgBuilder
				.append("<table><tr><td>用户IP</td><td>域名</td><td>IP</td><td>消耗的时间</td><td>Type</td></tr>");
		for (IMInfo info : remoteResult) {
			msgBuilder.append("<tr><td>" + info.getRemoteHost() + "</td><td>"
					+ info.getUrlOrHost() + "</td><td>" + info.getIP()
					+ "</td><td>" + info.getCostTime() + "</td><td>"
					+ info.getType() + "</td>");
	...
	public void setRemoteLocation(List<IMInfo> imInfoList) {
		for (IMInfo iminfo : imInfoList) {
			iminfo.setLocation(IpSeekAdapter.getInstance().getAddress(
