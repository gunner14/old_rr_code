...
public abstract class AbstractStructAction extends LoginAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			request.setAttribute(LoginAction.ACCOUNT, account);
			...
			// 弱密码判断
			if (PwdUtil.isWeakPassWordCipher(passwordEncode ? password
					: Encoder.encode(password))) {
				LoginLogicFactory.getValidation()
						.setWarningTagOfWeakPasswordInAttribut(request);
			}

			// 后继任务
			Runnable logTask = new LogTaskSet(account.getId(), ip, request
