...
<c:if test="${hasZhan||hasPage||hasLoverSpace||hasGroup}">
	<article class="site-menu-apps">
		...
		<section class="site-menu-apps-admins">
			<c:if test="${hasPage}">
				<c:forEach var="item" items="${managePageList}" begin="0" end="1">
					<div class="apps-item">
						<div class="item-title">
							<span class="item-main"> <a title="${item.name}" href="http://page.renren.com/${item.id}?origin=50303" target="_blank"><img class="icon" src="${applicationScope.urlStatic}/imgpro/icons/follow.gif" /> ${fn:substring(item.name, 0, 7)}</a>
							</span>
						</div>
					</div>
				</c:forEach>
			...
			<c:if test="${hasZhan}">
				<c:forEach var="item" items="${homeLeftSites}" begin="0" end="1">
					<div class="apps-item">
						<div class="item-title">
							<span class="item-main"> <a title="${item.name}" href="http://zhan.renren.com/zhan/${item.id}?origin=50303" target="_blank"><img class="icon" src="${applicationScope.urlStatic}/imgpro/icons/zhan.png" /> ${fn:substring(item.name, 0, 7)}</a>
							</span>
						</div>
					</div>
				</c:forEach>
			...
			<c:if test="${hasLoverSpace}">
				<c:forEach var="item" items="${manageLoverPageList}" begin="0" end="1">
					<div class="apps-item">
						<div class="item-title">
							<span class="item-main"> <a title="${item.name}" href="${applicationScope.urlLover}/${item.id}?origin=50303" target="_blank"><img class="icon" src="${applicationScope.urlStatic}/imgpro/page/love/lover-heart.png" /> ${fn:substring(item.name, 0, 7)}</a>
							</span>
						</div>
					</div>
				</c:forEach>
