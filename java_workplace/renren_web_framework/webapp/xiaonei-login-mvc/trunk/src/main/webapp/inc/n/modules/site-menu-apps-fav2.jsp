...
<c:forEach var="item" items="${favList}" begin="0" end="5">
	...
	<div class="apps-item" id="appsItem_${item.appId}">
		<div class="item-title">
				<span class="item-main"> <a stats="${stats}" ${target} href="${itemUrl}${sp}origin=${favorOrig}" <c:if test="${item.appId==45||item.appId==60||item.appId==95003||item.appId==120019||item.appId==141879||item.appId==166607||item.appId==166613||item.appId==166614||item.appId==166615||item.appId==212540}"> ui-async="async"</c:if>><img src="${item.appIcon}" class="icon"> <span class="app-name">${item.appName}</span>
				<c:if test="${item.appId==8&&presentNum!=0}"><span class="apps-item-count" id="allAppsInvite">${presentNum}</span></c:if>
			</a>
			</span>
		</div>
		<div class="item-editor">
			<span class="editor-operations"> <a href="#nogo" data-optype="order" onclick="this.blur();return false;" class="an-operation an-operation-order" title="排序"><img src="http://a.xnimg.cn/imgpro/v6/drag2.png" /></a> <a href="#nogo" data-appid="${item.appId}" data-optype="remove" onclick="this.blur();return false;" class="an-operation an-operation-remove" title="移出"><img src="http://a.xnimg.cn/imgpro/v6/remove.png" /></a>
			</span>
		</div>
	</div>
</c:forEach>
