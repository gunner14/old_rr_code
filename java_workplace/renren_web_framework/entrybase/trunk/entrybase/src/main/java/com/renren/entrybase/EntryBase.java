...
/**
 * {@link EntryBase}
 * 表示一个可以提供数据存储的数据库，封装了有关存储的底层细节以及存储逻辑（包括存储数据结构、数据可扩展性等等的功能或非功能特性），
 * 完整提供了对数据的增删改查功能。
 * 
 * @see Entry
 * @see Field
...
public interface EntryBase {
     ...
     * 注册一个关联库。
     * 
     * @param name 关联库的名字，要保证一个base下的关联库的名字要唯一，只能包含字母、数字、下滑线、连接线（亦称横线、减号等）。
     * @param referbase 被关联的库，一定不能是自己本身
     */
    void addReferBase(String name, EntryBase referbase);
     ...
     * @return 非空、不可更改的一个集合
     */
    Set<String> getReferBases();

    /**
     * 获取已注册的一个关联库。如果给定的名字未有关联库，则将抛出 {@link NullPointerException}
     * 
     * @param name 被注册时侯所使用的名字
     * @return
     */
    EntryBase getReferBase(String name);
     ...
     * </ul>
     * 
     * @param query 参见 {@link Query#toQuery(String)}
     */
     ...
     * @param referBase
     */
	void moveEntry(long id, EntryBase referBase);
