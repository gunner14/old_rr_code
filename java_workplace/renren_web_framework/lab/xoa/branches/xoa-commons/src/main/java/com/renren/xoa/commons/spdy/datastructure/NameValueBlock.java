...
public class NameValueBlock implements Map<String, String>, Serializable {
     ...
     * @deprecated 这种构造方式使用了大量的内存copy，不够高效， 请使用NameValueBlock(byte[] data,
     *             int offset, int length) 来替代
     */
    public NameValueBlock(byte[] fromByteStructure) {

        for (int index = 0; index < fromByteStructure.length;) {
            //get name length 2bytes
            int nameLength = Utils.lengthOfItem(fromByteStructure, index);
            index += LENGTH_OF_LENGTH_IN_BYTE;//skip length of length

            //get name String
            String nameString = Utils.string(fromByteStructure, index, nameLength);
            index += nameLength;//skip name length

            //get value length
            int valueLength = Utils.lengthOfItem(fromByteStructure, index);
            index += LENGTH_OF_LENGTH_IN_BYTE;//skip length of length

            //get value String
            String valueString = Utils.string(fromByteStructure, index, nameLength);
    ...
    public NameValueBlock(byte[] data, int offset, int length) {
        ...
        while (offset < end) {
            //get name length 2bytes
            int nameLength = Utils.lengthOfItem(data, offset);
            offset += LENGTH_OF_LENGTH_IN_BYTE;

            //get name String
            String name = Utils.string(data, offset, nameLength);
            offset += nameLength;

            //get value length
            int valueLength = Utils.lengthOfItem(data, offset);
            offset += LENGTH_OF_LENGTH_IN_BYTE;

            //get value String
            String value = Utils.string(data, offset, valueLength);
            offset += valueLength;
    ...
    /**
     * {@link NameValueBlock}按照SPDY协议的格式转化为 byte数组后的长度
     * 
     * @return
     */
     ...
     * 
     * 第一版的实现在toByteStructure0里，本实现是对其的优化， 减少了构造和copy
     * byte[]数组的次数，经过测试，性能大概能提高 10%左右
     * 
