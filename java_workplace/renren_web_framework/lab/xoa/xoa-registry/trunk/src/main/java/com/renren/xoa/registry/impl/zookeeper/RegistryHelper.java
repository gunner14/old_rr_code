...
public class RegistryHelper {
	 ...
	 * @param desc
	 */
	public boolean registerServiceNode(XoaServiceDescriptor desc)  {
		try {
	 ...
	 * @return
	 */
	public boolean updateServiceNode(XoaServiceDescriptor desc)  {
		try {
			String path = toConfigFilePath(desc);
	 ...
	 * @return
	 */
	public boolean deleteServiceNode(XoaServiceDescriptor desc) {
		try {
			String path = toConfigFilePath(desc);
			if (zookeeper.exists(path, watcher) == null) {
	...
	public boolean exists(XoaServiceDescriptor desc) {
	    try {
            String path = toConfigFilePath(desc);
            if (zookeeper.exists(path, watcher) != null) {
                return true;
	...
	public static String toConfigFilePath(XoaServiceDescriptor desc) {
	 ...
	 * @return
	 */
	public static byte[] toBytes(XoaServiceDescriptor desc) {
	 ...
	 * @return
	 */
	public XoaServiceDescriptor getNodeConfig(String serviceId, String ip, int port) {
	 ...
	 * @return
	 */
	public static XoaServiceDescriptor fromBytes(String serviceId, String ip, int port, byte[] data) {
	  //获取其他属性
        Properties properties = new Properties();
        try {
            properties.load(new ByteArrayInputStream(data));
            String status = (String) properties.getProperty("status");
            boolean disabled = XoaServiceDescriptor.STATUS_DISABLED.equals(status);
