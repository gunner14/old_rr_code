...
public class ZookeeperBasedRegistry extends AbstractXoaRegistry {
			...
			for (String child : children) {
				try {
					if(isValidIpPort(child)) {	//child是IP:Port
						...
						XoaServiceDescriptor desc = RegistryHelper.fromBytes(serviceId, ip, port, data);
						descs.add(desc);
					} else {
	...
	public XoaService getService(String serviceId) {
		XoaService svc = new XoaService();
		svc.setServiceId(serviceId);
		List<XoaServiceDescriptor> nodes = this.getServiceNodes(serviceId);
		svc.setNodes(nodes);
		return svc;
	}
	...
	public List<XoaService> getServices() {
		List<XoaService> list = new ArrayList<XoaService>();
		//从根据经开始深度搜索
		dfs("/", list);
		return list;
	}
	 ...
	 * @param list
	 */
	private void dfs(String nodePath, List<XoaService> list) {
		try {
			Stat nodeStat = zookeeper.exists(nodePath, false);
			if (nodeStat == null) {
				return;
