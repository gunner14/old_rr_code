...
public class TestProtoBuffResponse {
...
/*XoaClient client = new XoaClient();
		...
		try {
			...
			if (status == 200) {
				...
				RizhiList list = res.getContentAs(RizhiList.class);
				for (Rizhi rizhi : list.getRizhisList()) {
					System.out.println(rizhi.getContent());
				}
			} else {
