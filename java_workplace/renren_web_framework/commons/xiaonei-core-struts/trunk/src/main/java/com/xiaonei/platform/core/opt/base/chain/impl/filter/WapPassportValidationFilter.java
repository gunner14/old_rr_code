 ...
 */
public class WapPassportValidationFilter extends AbstractBizFilter {
	...
	private Logger logger  = BizFilterUtil.getLogger();
	...
	public boolean executeMe(WebContext webContext) throws BizFilterException {
		...
		Integer hostid = null;
		if (host != null) { //如果当前请求中已经有host对象了,就不走通常的取票验票过程了
			hostid = host.getId();
		} else { //取票验票操作，虽然方法名叫getHostIdFromCookie，但是实际上会和中间层交互
			String ticket = null;
			//从cookie取票
			hostid = PassportManager.getInstance().getHostIdFromCookieForWap(request, response, ticketCookieName);
			if (hostid != null && hostid != -1) {	//验票成功，得到这个票是什么
				ticket = CookieManager.getInstance().getCookie(request, ticketCookieName);
				if (logger.isDebugEnabled()) {
					logger.debug("passport validation succ from cookie:" + ticketCookieName + "=" + ticket);
				}
			} else {
				if (logger.isDebugEnabled()) {
			...
			if ((hostid == null || hostid == -1 ) && ticketFromParam) {
				//从request参数中取票
				hostid = PassportManager.getInstance().getHostIdFromRequestParamForWap(request, ticketParamName);
				if (hostid != null && hostid != -1) {	//验票成功，得到这个票是什么
					ticket = request.getParameter(ticketParamName);
					if (logger.isDebugEnabled()) {
						logger.debug("passport validation succ from request parameter:" + ticketParamName + "=" + ticket);
					}
				} else {
					if (logger.isDebugEnabled()) {
						logger.debug("passport validation failed from request parameter:" + ticketParamName + "=" + 
								request.getParameter(ticketParamName));
					}
				}
			}
