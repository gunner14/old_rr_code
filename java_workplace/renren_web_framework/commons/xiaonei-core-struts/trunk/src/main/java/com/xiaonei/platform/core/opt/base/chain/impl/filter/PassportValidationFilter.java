 ...
 */
public class PassportValidationFilter extends AbstractBizFilter {
	/**
	 * 是否从request参数中取票
	 */
	...
	public boolean executeMe(WebContext webContext) throws BizFilterException {
		...
		if (host != null) { //如果当前请求中已经有host对象了,就不走通常的取票验票过程了,这种情况极为特殊,目前只发现apps.xiaonei.com有此类用途
			...
			if (hostid == null && ticketFromParam) {
				try {
                    //从request参数中取票
                    hostid = PassportManager.getInstance().getHostIdFromRequestParam(request,
                            ticketParamName);
                } catch (Ice.TimeoutException e) {
                    logger.error("", e);
                }
		...
		if (host != null && (host.getStatus() % 10) >= User.statusActivateVerify) { //属于被封禁的用户
