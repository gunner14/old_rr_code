...
public class Buffer {
	...
	public void resize(int n, boolean reading) {
		...
		//
		// When used for reading, we want to set the buffer's limit to the new
		// size.
		//
		if (reading) {
			b.limit(_size);
	...
	private void reserve(int n) {
		if (n > _capacity) {
			_capacity = java.lang.Math.max(n, java.lang.Math.min(2 * _capacity,
					_maxCapacity));
		...
		try {
			...
			LocalException e = new LocalException("OutOfMemoryError occurred while allocating a ByteBuffer");
			e.initCause(ex);
			throw e;
		}
	}
	...
	private java.nio.ByteBuffer _emptyBuffer = java.nio.ByteBuffer.allocate(0);
	...
	//
	// This flag indicates whether we should use direct byte buffers. It is set
	// to false
	// because direct byte buffers have been known to cause problems in the past
	// (for
