...
public class ServiceGroupWatcher extends ServiceGroupImpl implements Watcher {
	...
	ServiceGroupWatcher(ServiceWatcher serviceWatcher, ZooKeeper zookeeper, String parentPath, String groupName) {
  ...
  int propGetShardFactor(Properties prop) {
	...
	boolean load() {
		int groupId = 0;
		try {
          groupId = Integer.parseInt(groupName.substring(ConfigConstants.SERVICE_GROUP_PREFIX.length()));
		} catch (NumberFormatException e) {
			return false;
		}
        setId(groupId);
		...
		for (String child : children) {
			if (child.startsWith(ConfigConstants.SHARD_PREFIX)) {
				try {
					...
					logger.error("Invalid shard name:" + fullPath
							+ "/" + child);
				}
			}
		}
	...
	public void process(WatchedEvent event) {
        ...
        if (shardFactor != getShardFactor()) {
        	logger.warn("serviceGroup " + fullPath + " shardFactor changed : "
        			+ getShardFactor() + "->" + shardFactor);
