...
public class XoaProcessor implements TProcessor {
	...
	public boolean process(TProtocol in, TProtocol out) throws TException {
		...
		StringBuffer message =  new StringBuffer("XoaProcessor ");
		long interval = endTime - startTime;
		if (interval < 200) {
			message.append(msg.name).append(" called. time(ms)=").append(interval);
			logger.debug(message.toString());
		} else {
			message.append(msg.name).append(" called SLOW. time(ms)=").append(interval);
