...
public class CommonServiceRouter implements ServiceRouter {
    ...
    @Override
    public XoaTransport routeService(String serviceId, String version, String shardBy)
            throws Exception {
        ...
        while (true) {
            ...
            if (shardBy == null || shardBy.equals("")) {
                node = loadBalancer.getNode(serviceId, version);
            } else {
                node = loadBalancer.getNode(serviceId, version,
            ...
            // 没有可用的node，这是严重的错误
            if (node == null) {
                logger.warn("No service available : " + serviceId);
                return null;
            }
            ...
            try {
                ...
                logger.warn("Get service error : " + getNodeKey(node)
                        + ' ' + serviceId);
